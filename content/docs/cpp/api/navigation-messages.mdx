...
`NavPath` cannot be published or subscribed from C++: it has no
`Publisher`/`Subscriber` specialization, no `HORUS_TOPIC_IMPL` line and no
declaration in `horus_c.h`, so `advertise<NavPath>` and `subscribe<NavPath>` do not
resolve. Send its waypoints as `Waypoint` or `PathPlan` instead.
...
One dynamic obstacle, modelled as a disc travelling at constant velocity. Like
`Waypoint`, and unlike `NavGoal`, `GoalResult` and `PathPlan`, it has no
`timestamp_ns` field.
