...
| Debug | 3 | Verbose diagnostics, matching Rust's `hlog!(debug, ...)` and Python's `node.log_debug` |

The log level is passed internally as an integer to the C FFI layer (`horus_log(level, node, msg)`). Info = 0, Warn = 1, Error = 2, Debug = 3.

`horus::log` wraps levels 0-2 only, and `horus_c.h` defines constants for those three. The C ABI does accept level 3; until a `horus::log::debug` wrapper exists, emit it by calling the C function directly. Reaching for `info()` instead mixes verbose diagnostics into the normal log stream, where nothing can filter them back out:

```cpp
horus_log(3, "controller", "loop iteration 4200, dt=1.02ms");
```
...
own recorder with `sched.blackbox(size_mb)`, which writes
