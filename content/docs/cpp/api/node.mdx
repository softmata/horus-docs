...
4. **Tick Loop** -- Each cycle: call `tick()`, measure timing, check budget/deadline, then feed the watchdog -- only if the tick returned `Ok`, so a panic or a hang lets the watchdog expire
