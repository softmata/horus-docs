...
HORUS provides zero-copy shared memory pools for large data: camera frames, lidar scans, and neural network tensors. Data lives in a pre-allocated SHM region and is shared by reference between the processes that open the same pool -- no serialization, no copies.
...
| Class | Purpose |
|-------|---------|
| `TensorPool` | Manages a shared memory region divided into a fixed number of allocation slots |
...
`Image` and `PointCloud` are convenience wrappers over pool-backed memory: the pixel and point data live in the pool's SHM region rather than inside the object. All three release their slot automatically when destroyed; `Tensor` additionally exposes `release()` to return the slot early -- see `release()` below.
...
|------|------|-------------|
| `pool_id` | `uint32_t` | Identifier for this pool. Constructing a pool with an id that already exists opens that SHM region instead of creating a new one -- that is how two processes share a pool -- so distinct pools must use distinct ids. |
...
| `release()` | `void` | Returns the slot to the pool **early**. Optional -- `~Tensor()` already releases the slot before destroying the handle. After `release()` the Tensor is empty and `data()` returns `nullptr`, and the destructor will not release the same slot twice. |
...
A camera node that reserves frame buffers in pool-backed shared memory.
...
if (frame) {
    // Frame memory is now reserved in the pool's SHM region.
    // data_size() = 1280 * 720 * 3 = 2,764,800 bytes
}
```

There is no publish step here: `Publisher<T>` / `Subscriber<T>` are specialized only for the fixed-size `horus::msg::` POD types, so `sched.advertise<horus::Image>(...)` does not compile. From C++, pool-backed types are allocated inside the processes that open the pool, and only `Tensor` exposes a data pointer (`data()`) -- `Image` and `PointCloud` expose metadata only. See [Topic](/cpp/api/topic) for the message types that can be published.
...
if (input) {
    ...
    input.release();  // optional -- returns the slot early; ~Tensor() also releases it
}
```
...
Overallocating pool size is not free. At construction HORUS pre-faults the pool's data region with `madvise(MADV_POPULATE_WRITE)`, up to `HORUS_SHM_PREFAULT_MAX_BYTES` (64 MiB by default), which commits those pages to physical memory. A pool of 64 MB or less -- every size in the table above -- is therefore fully resident the moment it is created; only the part of a larger pool beyond the cap stays uncommitted. Set `HORUS_SHM_PREFAULT=0` to disable pre-faulting, or lower `HORUS_SHM_PREFAULT_MAX_BYTES`, if you would rather pay the page faults in the control loop. Underallocating max_slots causes allocation failures when the pipeline is full.
