...
For complex nodes with state. Subclass `horus::Node` and override lifecycle methods:

```cpp
class ArmController : public horus::Node {
public:
    ArmController() : Node("arm_ctrl") {
        joint_sub_ = subscribe<horus::msg::JointState>("joints.state");
        cmd_pub_   = advertise<horus::msg::JointCommand>("joints.cmd");
    }

    void tick() override {
        auto state = joint_sub_->recv();
        if (!state) return;
        // compute and publish command...
    }

private:
    horus::Subscriber<horus::msg::JointState>* joint_sub_;
    horus::Publisher<horus::msg::JointCommand>* cmd_pub_;
};
...
std::signal(SIGINT, [](int) { /* handled by scheduler */ });

// Or stop programmatically after 10 seconds:
std::thread timer([&] {
...
sched.add("producer")
    .rate(100_hz)
    .tick([&] {
        pub.send(horus::msg::CmdVel{0, 1.0f, 0.0f});
        tick_count++;
    })
    .build();
...
assert(msg->linear == 1.0f);
```

### Network-Enabled Scheduler

Enable LAN replication so topics are shared across machines:
...
// Topics automatically replicate to other schedulers
