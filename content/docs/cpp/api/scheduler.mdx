...
These flags configure the scheduler's own thread: `mlockall` plus `SCHED_FIFO` at priority 50. They do not gate the node threads.
...
| (neither) | The scheduler thread stays SCHED_OTHER and memory is not locked. RT-class nodes are unaffected -- each still runs on its own thread and still attempts `SCHED_FIFO` at its `.priority(n)`, default 80 |
...
Deterministic mode replaces the wall clock with a virtual `SimClock` and forces every node onto the main thread in `.order()` tiers, so tick order is reproducible. It does not touch RNG state -- HORUS's per-tick RNG is seeded from the tick number and node name unconditionally, and is not exposed to C++. Use for reproducible tests and replay:
...
Topic names use **dots** (not slashes) as separators. This is the HORUS convention, not a platform constraint -- see [Use Dots, Not Slashes](/concepts/core-concepts-topic#use-dots-not-slashes).
...
`tick_once()` executes exactly one tick of the registered nodes. It starts no executor threads — RT, compute, event and async-I/O nodes all tick in-line and sequentially on the calling thread, whatever class they declared; the execution-class partition only runs under `spin()`. Useful for testing and stepped simulation:
