...
`catch_unwind` catches Rust panics raised inside the closure; the node is then marked failed and later ticks are skipped. It does **not** protect the `extern "C"` boundary itself — anything that unwinds out of your C++ lambda aborts the process.
