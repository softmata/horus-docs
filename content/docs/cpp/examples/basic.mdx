...
Complete, runnable C++ projects. Each includes `horus.toml` and `src/main.cpp` — `horus.toml` is the single source of truth and the CLI generates `.horus/CMakeLists.txt` for you. Do not add a root `CMakeLists.txt`: `horus run` prefers it over the generated one and the build loses the horus include/link wiring.
...
```bash
horus new hello_node --cpp
cd hello_node
# paste the code above into src/main.cpp
horus run
```
...
Unlike the examples above, this one drives the scheduler by hand with `tick_once()` instead of `spin()`. `tick_rate()` paces the loop that `spin()` runs; `tick_once()` executes one cycle on the calling thread and returns immediately — it starts no executor threads, and per-node rate limiting still applies, so it is not a guarantee that every node ran, so the manual loop below sleeps to hold the declared 10 Hz itself.
...
int main() {
    ...
    sched.add("driver")
        .tick([&] {
            ...
            static int t = 0;
            if (++t % 10 == 0) {  // once per second at 10 Hz
    ...
    // Step the graph manually and simulate parameter changes.
    // tick_once() does not sleep, so the loop paces itself at ~10 Hz.
    for (int i = 0; i < 100; i++) {
        sched.tick_once();
        if (i == 30) {
...
- [Quick Start](/getting-started/quick-start) — step-by-step first project
