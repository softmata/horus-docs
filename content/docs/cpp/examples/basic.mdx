...
Unlike the examples above, this one drives the scheduler by hand with `tick_once()` instead of `spin()`. `tick_rate()` paces the loop that `spin()` runs; `tick_once()` executes one cycle of every node and returns immediately, so the manual loop below sleeps to hold the declared 10 Hz itself.
