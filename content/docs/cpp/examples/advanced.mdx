...
Two separate binaries communicating over SHM.
...
horus.run(
    # Typed specs, not plain strings: the C++ neighbours publish/subscribe the
    # POD horus::msg::CmdVel. Plain strings would open GenericMessage
    # (MessagePack) topics, which cannot read or write that wire format.
    horus.Node(name="ai", subs={"camera.detections": CmdVel},
               pubs={"ai.decision": CmdVel}, tick=ai_tick, rate=10)
)
```
...
```bash
LD_LIBRARY_PATH=target/debug ./cpp_motor &   # C++ motor (subscriber — start first)
python3 python_ai.py &                       # Python AI
LD_LIBRARY_PATH=target/debug ./cpp_sensor &  # C++ sensor (publisher)

horus topic list     # 3 topics across the 2 languages
...
int main() {
    ...
    server.set_handler([](const uint8_t* req, size_t len,
        ...
        size_t cap = *res_len;   // IN: real capacity of res, in bytes
        int n = std::snprintf(reinterpret_cast<char*>(res), cap,
                              R"({"sum":%d})", a + b);
        if (n < 0 || static_cast<size_t>(n) >= cap) return false;  // failed or truncated
        *res_len = static_cast<size_t>(n);   // OUT: bytes written
        return true;
    });

    // call() blocks polling for its response, so run it on its own thread and
    // drive the server from this one — nothing else pumps it.
    std::thread caller([] {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        server.process();   // answers pending requests
    ...
    std::printf("Goal %lu submitted\n", goal.id());
    goal.cancel();

    // goal.status() is a local field fixed at Pending when the handle is created,
    // so it never reports the outcome and is_active() is always true. The terminal
    // status arrives on the result topic — poll for it.
    ...
    while (std::chrono::steady_clock::now() < goal_deadline) {
        if (nav.poll_result(goal.id(), status, result_json)) break;
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    std::printf("Canceled: %s\n",
        status == horus::GoalStatus::Canceled ? "yes" : "no");
}
```
...
- [Multi-Language Support](/concepts/multi-language) — C++ + Rust + Python in one system
