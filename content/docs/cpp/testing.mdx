...
table. The slot is claimed before the node is registered and never returned:
destroying the `Scheduler` or the `NodeBuilder` does not give it back, and a
`build()` that throws `horus::Error("Failed to build node")` — a non-finite or
non-positive tick rate, or a panic caught at the FFI boundary — burns its slot
too. The overflow check is the only path that refunds one. The counter
