...
The whole Rust runtime is statically linked into your executable, so a one-node
hello-world is not small — and the debug and release numbers are 40x apart, which is a
surprise worth having before deploy day rather than on it:
...
Debug keeps its symbols on purpose, which is why it stays at 82 MB. Flash the release
build. See [Binary Size and Flash Budget](/advanced/deployment#binary-size-and-flash-budget)
for the Rust comparison and what to do when 2.1 MB per executable is still too much.
...
void tick() override {
    horus::Publisher<horus::msg::CmdVel> pub("cmd");  // DON'T
}

// GOOD — create once in constructor
class MyNode : public horus::Node {
public:
    MyNode() : Node("x") { pub_ = advertise<horus::msg::CmdVel>("cmd"); }
