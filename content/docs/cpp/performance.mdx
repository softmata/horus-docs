...
// SHM-backed) — constant regardless of message size (LaserScan is 1480 bytes).
...
scan.ranges[0] = 1.5f;
pub.send(scan);  // ~15ns — copies 1480 bytes to SHM
```
...
| LaserScan send (1480 bytes) | ~20 ns | Zero-copy loan |
