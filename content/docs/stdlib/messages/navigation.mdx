...
> **Rust**: `NavGoal`, `NavPath`, and `PathPlan` live in the `horus_robotics` crate, re-exported through `horus::prelude` — `use horus::prelude::*;`
...
topic = horus.Topic(horus.NavGoal)
topic.send(goal)
```

See [Python Navigation Messages](/python/messages/navigation#navgoal) for all helpers (`is_reached`, `is_position_reached`, `is_orientation_reached`, `with_timeout`).
...
path_sub = horus.Topic(horus.NavPath)

path = path_sub.recv()
if path is not None:
    for i in range(len(path)):
        wp = path.waypoints[i]
        # wp.x, wp.y, wp.theta, wp.velocity
```
...
plan_sub = horus.Topic(horus.PathPlan)

plan = plan_sub.recv()
if plan is not None:
    n = plan.waypoint_count
    for i in range(n):
        x = plan.waypoint_data[i * 3]
        y = plan.waypoint_data[i * 3 + 1]
        theta = plan.waypoint_data[i * 3 + 2]
```
