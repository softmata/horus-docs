...
> **Rust**: `SegmentationMask` lives in the `horus_robotics` crate — `use horus_robotics::prelude::*;`
...
// simplified
use horus::prelude::*;
use horus_robotics::messages::segmentation::classes; // Note: not re-exported in prelude
...
<MermaidDiagram
  chart={`flowchart LR
    ...
    C --> D["SegmentationMask"]
    C --> E["overlay on Image"]
`}
...
- [Detection](/stdlib/messages/detection) — Related perception type
- [Image](/stdlib/messages/image) — Camera frames that feed segmentation models
- [OccupancyGrid](/stdlib/messages/occupancy-grid) — Grid-based maps from segmentation output
