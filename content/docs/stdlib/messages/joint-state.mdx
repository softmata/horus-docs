...
> **Rust**: `JointState` and `JointCommand` live in the `horus_robotics` crate, re-exported through `horus::prelude` — `use horus::prelude::*;`
...
# Construct from parallel lists
state = horus.JointState(
    names=["shoulder", "elbow", "wrist"],
    positions=[0.5, -1.2, 0.0],
)

# Access parallel arrays
print(len(state), state.names, state.positions)

topic = horus.Topic(horus.JointState)
topic.send(state)
```

See [Python Sensor Messages](/python/messages/sensor) for the full Python API.
...
cmd = horus.JointCommand(
    names=["shoulder", "elbow", "wrist"],
    positions=[1.0, -0.5, 0.3],
    velocities=[0.5, 0.5, 1.0],
)

# Or build incrementally
cmd = horus.JointCommand()
cmd.add_position("shoulder", 1.0)
cmd.add_position("elbow", -0.5)
cmd.add_velocity("wrist", 1.0)

assert cmd.is_valid()
topic = horus.Topic(horus.JointCommand)
topic.send(cmd)
```

See [Python Control Messages](/python/messages/control) for the full Python API.
