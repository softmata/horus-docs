...
# From ML frameworks (copies into shared memory pool, bridge via NumPy)
img = Image.from_numpy(tensor.numpy(), "rgb8")
```
...
# Convert to ML frameworks (zero-copy NumPy view, then bridge)
arr = img.to_numpy()                 # numpy array
t = torch.from_numpy(img.to_numpy()) # PyTorch tensor
j = jnp.asarray(img.to_numpy())      # JAX array
...
| `to_numpy()` | Python: -> ndarray | Zero-copy to numpy (bridge into PyTorch/JAX with `torch.from_numpy(...)` / `jnp.asarray(...)`) |
