...
**C++:** C++17 bindings (`horus_cpp`) — `Node`, `Scheduler`, and `Publisher`/`Subscriber` over the same shared-memory topics. Scaffold with `horus new --cpp my_project`.
...
**Wide-area / internet-scale distribution** - HORUS ships transparent LAN replication via the opt-in `net` feature (`horus_net`): topics are mirrored across machines over UDP with the same `Topic<T>` API, configured through the `[network]` section of `horus.toml`. It is designed for a local network — peer discovery uses UDP multicast on the same subnet — not for WAN/internet transport. For that, use gRPC, HTTP, or message queues.
