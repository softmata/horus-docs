...
`.order()`. When no node registered a topic during `init()` — the usual case — `.order()` tiers decide the sequence instead:
...
flowchart TB
    subgraph App["Your Application"]
        direction LR
        SN["Sensor Node<br/>publishes"]
        PN["Process Node<br/>subscribes, publishes"]
        DN["Display Node<br/>subscribes"]
    end

    SN -->|"sensor.temp"| PN
    PN -->|"filtered.temp"| DN

    SCHED["Scheduler<br/>calls tick() on each node"]
    TOPIC["Topic<br/>pub/sub, one SHM ring per topic"]
    LOG["hlog!<br/>shared-memory log buffer"]
    SHM["Platform shared memory<br/>Linux / macOS / Windows"]
    MON["horus monitor<br/>optional plugin process"]
    ...
    SHM -.->|"reads"| MON
`}
  caption="Everything a node sends or logs lands in shared memory; the monitor is a separate process that reads it"
/>
