...
HORUS achieves sub-microsecond message latency through shared memory. The system auto-selects from 10 backend modes based on topology:

| Backend | Scenario | Latency |
|---------|----------|---------|
| DirectChannel | Same thread | ~3 ns |
| SpscIntra | Same process, 1 producer/1 consumer | ~18 ns |
| MpmcIntra | Same process, many-to-many | ~36 ns |
| PodShm | Cross-process, POD types | ~50 ns |
| MpmcShm | Cross-process, many-to-many | ~167 ns |

This performance enables tight control loops and high-frequency data processing without introducing significant delay.

**How it works:** Shared memory instead of network sockets. POD types use zero-copy memory-mapped I/O. Non-POD types use fast bincode serialization.
...
**Multi-machine distributed systems** - HORUS currently operates as a local shared-memory IPC framework. Network transport for multi-robot communication is planned but not yet implemented. For cross-machine communication, use gRPC, HTTP, or message queues.
...
| Aspect | HORUS | ROS |
|--------|-------|-----|
| Typical latency | ~0.003-0.167 µs | 50-100 µs (intra-machine DDS) |
...
|--------|-------|----------------|
| Latency | ~0.003-0.167 µs | 1-10 ms |
...
**IPC Latency (per backend mode):**
- Same-thread: ~3ns (DirectChannel)
- Same-process: ~18-36ns (Spsc/Mpmc Intra)
- Cross-process: ~50-167ns (Shm backends)
...
- Per topic: Auto-sized based on message type (small POD types ~73KB, large non-POD types up to ~8MB)
...
**Advanced:**
9. [Multi-Language](/concepts/multi-language) - Python integration
