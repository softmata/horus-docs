...
- All send-only (one direction), on an Intel i7-10750H; the raw shared-memory
  floor on the same machine is 79 ns, so cross-process 1:1 pays 72 ns of
  framework overhead — see [Benchmarks](/performance/benchmarks)
