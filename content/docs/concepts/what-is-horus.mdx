...
HORUS achieves **~40ns to ~85ns** backend transport latency through shared memory, and ~125ns one-way cross-process end to end — roughly **30x lower** than ROS 2's published REP 2014 reference for default DDS (~5µs). HORUS's figures are measured here by `all_paths_latency`; the ROS 2 figure is quoted, not measured. The system automatically selects the optimal shared-memory backend based on your topology (how many publishers and subscribers a topic has, and whether the message type is plain data). No configuration needed.
...
**Execution order:** `.order()` is a fallback, not an absolute rule. As soon as any node in
the program has pub/sub topic metadata, the scheduler builds a dependency graph from it and
dispatches from the graph — a subscriber runs after its publisher even when it has the lower
`.order()`. Only when no node has topic metadata do `.order()` tiers decide the sequence:
all order 0 nodes, then all order 1, then all order 2, repeat. See
