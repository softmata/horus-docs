...
- **Latency**: ~85ns-437ns (Topic) for production messages
- **Use case**: All robotics applications - control, sensing, perception
- **Message Types**: CmdVel (~500ns MPMC / ~85ns SPSC), IMU (~940ns MPMC), LaserScan (~2.2µs Topic MPMC), Odometry (~1.1µs Topic MPMC)

```rust
use horus::prelude::*;

// Create publisher/subscriber for any message type
let cmd_pub: Topic<CmdVel> = Topic::new("motor.cmd_vel")?;
let cmd_sub: Topic<CmdVel> = Topic::new("motor.cmd_vel")?;

// Multiple publishers can send to same topic
let pub1: Topic<SensorData> = Topic::new("sensors.imu")?;
let pub2: Topic<SensorData> = Topic::new("sensors.imu")?;

// Multiple subscribers receive all messages
let sub1: Topic<SensorData> = Topic::new("sensors.imu")?;
let sub2: Topic<SensorData> = Topic::new("sensors.imu")?;
```
...
| Message Type | Size | MPMC Latency | SPSC Latency | vs ROS2 |
|--------------|------|-------------|--------------|---------|
| **CmdVel** | 16 B | ~500 ns | ~85 ns | **160-403x faster** |
...
fn tick(&mut self) {
    ...
    let cmd = CmdVel::new(1.5, 0.8);  // ~500ns (Topic MPMC) / ~85ns (Topic SPSC)
