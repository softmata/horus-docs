...
pub trait Node: Send {
    ...
    // Name (has default, but you should always set it)
...
> **Note:** See the [API Reference](/rust/api/core) for complete method documentation including checkpoint/restore support.
...
**name()**: Returns a string identifying the node. Has a default implementation, but you should always set it.
...
When using the `node!` macro, the name is auto-generated from the struct name (e.g., `SensorNode` → `"sensor_node"`).
...
**rate_hz()**: Optional tick rate for this node (default: None = use scheduler's global rate)
...
When set, the scheduler will tick this node at the specified rate instead of the global rate. Can be overridden at runtime via `Scheduler::set_node_rate()`.
...
pub struct NodeInfo {
    ...
    //   .config() -> &NodeConfig
    //   .metrics() -> &NodeMetrics
    //   .params -> RuntimeParams (public field)
}
```

Create with `NodeInfo::new(name)` or `NodeInfo::new_with_config(name, config)`.
...
Access metrics via the scheduler or `init()`/`shutdown()` methods:
...
fn init(&mut self) -> Result<()> {
    ...
    self.serial_port = serialport::new("/dev/ttyUSB0", 115200)
        .open()
        .map_err(|e| HorusError::driver(format!("Failed to open serial: {}", e)))?;
    ...
    fn init(&mut self) -> Result<()> {
        ...
        self.serial_port = Some(
            serialport::new("/dev/ttyUSB0", 115200)
                ...
                .open()
                .map_err(|e| HorusError::driver(format!("Serial open failed: {}", e)))?
        );
...
fn init(&mut self) -> Result<()> {
    if !self.sensor.is_available() {
        return Err(HorusError::internal("Sensor not found"));
    }
    Ok(())
}
```
...
fn init(&mut self) -> Result<()> {
    self.device = Device::open().map_err(|e| {
        HorusError::internal(format!("Failed to open device: {}", e))
    })?;
...
### Python Hardware Node Shutdown Behavior

HORUS provides pre-built Python hardware nodes that handle shutdown automatically:

| Node | Shutdown Action |
|------|----------------|
| **ImuNode** | Stops IMU reading thread |
| **CameraNode** | Releases camera capture device |
| **LidarNode** | Stops LiDAR scanning |
| **GpsNode** | Closes serial connection |
| **SerialNode** | Closes serial port |
| **JoystickNode** | Releases joystick device |
| **KeyboardNode** | Restores terminal mode |

See the [Python Hardware Nodes](/python/api/python-bindings) documentation for details.
