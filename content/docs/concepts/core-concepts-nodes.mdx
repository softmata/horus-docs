...
If no rate is set, the node ticks at the scheduler's global rate. The scheduler raises its own tick period automatically to satisfy the fastest registered node.
...
The metadata methods are inherent `publishers()` and `subscribers()` methods on the generated struct, each returning `Vec<TopicMetadata>` — they are not `Node` trait methods, so hand-written nodes cannot override them. Hand-written nodes do not declare topics at all: the scheduler auto-detects them via `TopicNodeRegistry` on a topic's first `send()` or `recv()`, and only for topics created inside `init()` or `tick()`. A topic built in the node's constructor is not attributed to it, so it will not appear in the dependency graph or in `horus node info`.
