...
fn init(&mut self) -> Result<()> {
    ...
    self.serial_port = serialport::new("/dev/ttyUSB0", 115200)
        .open()
        .map_err(|e| Error::driver(format!("Failed to open serial: {}", e)))?;
    ...
    fn init(&mut self) -> Result<()> {
        ...
        self.serial_port = Some(
            serialport::new("/dev/ttyUSB0", 115200)
                ...
                .open()
                .map_err(|e| Error::driver(format!("Serial open failed: {}", e)))?
        );
...
fn init(&mut self) -> Result<()> {
    if !self.sensor.is_available() {
        return Err(Error::internal("Sensor not found"));
    }
    Ok(())
}
```
...
fn init(&mut self) -> Result<()> {
    self.device = Device::open().map_err(|e| {
        Error::internal(format!("Failed to open device: {}", e))
    })?;
