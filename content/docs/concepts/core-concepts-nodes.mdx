...
With the zero-overhead IPC, `send()` and `recv()` no longer take `ctx`. For introspection, use external tools or `send_logged()`:

```rust
fn tick(&mut self) {
    // Zero-overhead IPC (no logging)
    self.velocity_pub.send(1.5).ok();

    // With logging (for debugging) - requires LogSummary trait
    self.velocity_pub.send_logged(1.5).ok();
    // Output: [12:34:56.789] MyNode --PUB--> 'cmd_vel' = 1.5

    // Subscribe (zero-overhead)
...
impl Node for Filter {
    fn tick(&mut self) {
        if let Some(input) = self.input_sub.recv() {
            ...
            self.output_pub.send(output).ok();
        }
    }
}
```
...
### Use Standard Logging in tick()

Since `tick()` no longer receives `ctx`, use the `log` crate for logging:

```rust
fn tick(&mut self) {
    log::info!("Processing data");
    log::debug!("Detailed debug info: {:?}", self.state);
}
```
...
impl Node for MyMotorController {
    ...
    fn tick(&mut self) {
        ...
        self.motor_pub.send(self.velocity).ok();
    }
