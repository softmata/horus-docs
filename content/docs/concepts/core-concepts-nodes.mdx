...
When using the `node!` macro, the name is auto-generated from the struct name (e.g., `SensorNode` → `"sensor_node"`). You can override this with an explicit name:
...
This explicit name is used everywhere: scheduler logs, monitor display, and log messages.
...
```rust
fn init(&mut self) -> Result<()> {
    // Metrics are available in init/shutdown via ctx
    hlog!(info, "Starting with priority {}", ctx.priority);
    Ok(())
}

fn tick(&mut self) {
    // tick() doesn't have ctx - use internal state tracking if needed
    self.tick_count += 1;
}
```
