...
**Per-node rate**: set the tick rate at registration time, not on the trait:
...
scheduler.add(control_node).order(10).done();

// Sensors run third (order 50)
scheduler.add(sensor_node).order(50).done();

// Logging runs LAST (order 100)
scheduler.add(logger_node).order(100).done();

// Fine-grained priorities for complex systems
scheduler.add(emergency_stop).order(0).done();      // Highest
scheduler.add(motor_control).order(15).done();      // Between HIGH and NORMAL
scheduler.add(vision_processing).order(55).done();  // Slightly lower than normal
scheduler.add(telemetry).order(90).done();          // Between LOW and BACKGROUND
```
