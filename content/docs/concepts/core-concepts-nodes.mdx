...
When you pass `ctx` to `Topic::send()` or `Topic::recv()`, HORUS automatically logs IPC operations:
...
**You don't need to call any logging methods manually** - Topic handles it automatically when you pass `ctx`.
