...
- How the scheduler tracks per-node metrics and timing, and how `hlog!` handles logging
...
**Ordered Execution**: main-loop nodes run in dependency order, falling back to `order` tiers

**Zero Boilerplate**: The `node!` macro generates all necessary boilerplate code

**Type-Safe Communication**: Compile-time guarantees for message passing — within one program. Whether a publisher and subscriber in different nodes, processes or languages agree is checked at runtime, when the topic is opened
...
pub trait Node: Send {
    ...
    fn on_error(&mut self, error: &str) { /* no-op by default - the scheduler already logged the failure */ }

    // Safety hooks
    fn is_safe_state(&self) -> bool { true }
    fn enter_safe_state(&mut self) { /* no-op */ }

    // Optional: runtime parameter hook (NOTE: not yet wired by the scheduler)
...
> **Note:** See the [API Reference](/rust/api/core) for complete method documentation.
...
When using the `node!` macro, the name is auto-generated as the **snake_case** form of the struct name (`SensorNode` → `"sensor_node"`). Override it with a `name: "MyName"` section in the macro body.
...
### Setting a Node's Tick Rate

There is no rate method on the `Node` trait. A node's tick rate is set at registration time:

```rust
scheduler.add(sensor_node).order(50).rate(100_u64.hz()).build()?;  // this node runs at 100 Hz
```

If no rate is set, the node ticks at the scheduler's global rate. If any registered node declares a rate faster than the scheduler's current tick rate, the scheduler automatically shortens its own tick period — raising its tick rate — so it is fast enough for the fastest registered node.
...
**Uninitialized**: The node exists but has not ticked yet. Your `init()` runs
while the node is still in this state — the scheduler calls it before the first
tick, and the state does not change until that tick begins.

**Running**: The node has ticked at least once. `start_tick()` promotes a node
from `Uninitialized` on its first tick.

**Error**: A tick returned an error or panicked. What happens next is decided by
the node's [`FailurePolicy`](/advanced/circuit-breaker), not by this state.

**Crashed**: `init()` or a re-initialisation failed with `Severity::Fatal`. This
is reached only from initialisation — a failing `tick()` produces `Error`, never
`Crashed`.

**Stopped**: The scheduler shut the node down.

Two more variants exist on the `NodeState` enum but are not currently produced by
the runtime, so no transition below leads to them: **Initializing** is set and
overwritten within a single call and is never externally observable, and
**Stopping** is never assigned at all. Treat them as reserved.
...
impl Node for LifecycleDemo {
    ...
    fn tick(&mut self) {
        // Called REPEATEDLY in main loop (100 Hz default)
        self.counter += 1;
...
With the zero-overhead IPC, `send()` and `recv()` no longer take a context parameter, and topics do not log their traffic:

```rust
let velocity_pub: Topic<f32> = Topic::new("cmd_vel")?;
...
For introspection, use the CLI: `horus topic echo`, `horus topic hz`, `horus monitor`. Use `hlog!` inside `tick()` when you need application-level logging.

### Performance Metrics

The scheduler records detailed performance metrics for every node:
...
pub struct NodeMetrics {
    ...
    messages_sent: u64,      // always 0 — nothing writes these
    messages_received: u64,  // always 0 — use `horus topic hz`
    errors_count: u64,
    warnings_count: u64,
    uptime_seconds: f64,
}
...
Fields are private — read them via accessors: `m.name()`, `m.order()`, `m.total_ticks()`, `m.avg_tick_duration_ms()`, etc. Get a snapshot of all nodes with `Scheduler::metrics() -> Vec<NodeMetrics>`.

For example, from your application after the scheduler has run:

```rust
for m in scheduler.metrics() {
    hlog!(info, "{}: {} ticks, avg {:.3} ms",
        m.name(), m.total_ticks(), m.avg_tick_duration_ms());
}
```
...
Main-loop nodes with no topic relationship execute in `order` sequence:
...
const HIGH: u32        = 10;
const NORMAL: u32      = 50;  // Recommended for ordinary nodes
const LOW: u32         = 80;
const BACKGROUND: u32  = 100; // Lowest priority
```

These are conventions, not framework constants. If you omit `.order()`, the builder leaves the node at order `0`, which means "no preference" rather than "critical".
...
// Safety monitor runs first among these main-loop nodes (order 0)
...
`build()` validates the configuration and returns a `HorusResult`, so it must be handled — don't discard it. You may also see `.done()` in older code; it is a compatibility alias that calls `build()`.
...
The metadata methods are inherent `publishers()` and `subscribers()` methods on the generated struct, each returning `Vec<TopicMetadata>` — they are not `Node` trait methods, so hand-written nodes cannot override them. Hand-written nodes do not declare topics at all: the scheduler auto-detects them via `TopicNodeRegistry` on a topic's first use, and every access point registers — `send()`, `try_send()`, `send_blocking()`, `recv()`, and `read_latest()` alike. Ownership is resolved at that first call (`Topic::resolve_owner`), not at `Topic::new()`, so it does not matter whether the topic was built in the node's constructor, in `init()`, or in `tick()`: as long as that first call happens inside a node callback, where the node context is set, the topic is attributed to that node and appears in the dependency graph and in `horus node info`. A topic used only outside any node context — from a test, a CLI tool, or a bare helper thread — retries the lookup on each use and gives up permanently after a bounded number of failed attempts, so it stays unattributed.
...
The `shutdown()` method runs **once** when your application exits (Ctrl+C, SIGINT, SIGTERM), for every node whose `init()` returned `Ok`. Use it to safely stop hardware and release resources.

A node whose `init()` returned an error or panicked is skipped entirely: the scheduler marks a node initialised only on the `Ok` arm, and shutdown is gated on that flag. In the `init()` above, a failure at step 3 leaves `/dev/ttyUSB0` open and the motor driver half-configured, and no `shutdown()` will run to undo it. Release what you have already acquired on the failure path inside `init()` itself.
...
The `shutdown()` column covers nodes that got through `init()`. For the safety-critical rows, the failure path inside `init()` needs the same cleanup, because a node that failed to initialise never gets its `shutdown()` called.
...
`tick()` takes no logging context — use the `hlog!` macro instead:
...
flowchart TB
    CTRL["Ctrl+C (SIGINT)"] --> H1["ctrlc handler<br/>running = false"]
    TERM["kill / timeout (SIGTERM)"] --> H2["SIGTERM handler<br/>sets SIGTERM_RECEIVED"]
    H1 --> LOOP["Tick loop exits"]
    H2 --> LOOP
    LOOP --> STOPEX["Stop the four executors,<br/>reclaim their nodes (3s each)"]
    STOPEX --> SHUT["shutdown() every node<br/>that was initialized"]
    SHUT --> FIN["Flush blackbox, export telemetry,<br/>clean session"]
    FIN --> EXIT["run() returns"]
    H1 -.->|"if shutdown outlasts the 6s watchdog"| FORCE["Force exit<br/>shutdown() skipped"]
`}
...
def shutdown(node):
    node.send("cmd_vel", (0.0, 0.0))  # Stop motors
...
Verify in the terminal, where `hlog!` prints `[LEVEL] [node] message`:
```
[INFO] [MyMotorController] MyMotorController shutting down
[INFO] [MyMotorController] All motors stopped safely
```

`horus log` renders the same entries with a timestamp instead:
```
12:34:56.789 INFO [MyMotorController] MyMotorController shutting down
```
