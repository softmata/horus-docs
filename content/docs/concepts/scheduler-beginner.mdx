...
**Lower order number = runs first.** This is how you ensure data flows correctly through your system:

| Order | Node | Why This Order |
|-------|------|----------------|
| 0 | SensorNode | Reads data from hardware first |
| 1 | ControlNode | Processes sensor data and computes commands |
| 2 | LoggerNode | Records everything that happened this tick |

If two nodes have the same order, they run in the order they were added to the scheduler. The scheduler provides order-range guidelines:
...
The scheduler assigns each node an **execution class** that determines how it runs:

| Class | How it runs | Auto-detected when you set | Use for |
|-------|------------|---------------------------|---------|
| **BestEffort** | Sequential in main loop | Default (no special config) | Logging, telemetry, display |
| **Rt** | Dedicated thread | `.rate()`, `.budget()`, or `.deadline()` | Motor control, safety, sensor fusion |
| **Compute** | Thread pool | `.compute()` | Path planning, SLAM, image processing |
| **Event** | Wakes on topic message | `.on("topic_name")` | Emergency stop, command handlers |
| **AsyncIo** | Tokio runtime | `.async_io()` | Cloud upload, HTTP, database |

You don't pick a class directly — the scheduler infers it from how you configure the node:

```rust
// BestEffort (default) — no special config
scheduler.add(logger).order(2).build()?;

// Rt — auto-detected from .rate()
scheduler.add(motor).order(1).rate(1000_u64.hz()).build()?;

// Compute — explicitly marked
scheduler.add(planner).order(1).compute().build()?;

// Event — triggered by a topic message
scheduler.add(estop).order(0).on("emergency.stop").build()?;
```

<Callout type="tip">
You don't need to understand execution classes to get started. Just use `.order()` and `.rate()` and the scheduler picks the right class. Execution classes become important when you have nodes with very different workload patterns and need to optimize resource usage.
</Callout>
