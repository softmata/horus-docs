...
`order` sequences nodes that run on the **main tick loop** and have no topic
...
`order` tiers are the fallback used when there is no topic metadata to build a graph
