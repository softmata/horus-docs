...
There is no `.rt()` method — a node becomes real-time by calling `.rate(N.hz())` (or `.budget()` / `.deadline()`) *without* `.compute()`, `.on()` or `.async_io()`. Omit all of these and the node is best-effort, ticking in the main loop.
...
- Other nodes continue normally — unless the error's `Severity` is `Fatal`, in which case the scheduler stops immediately
- A panic inside `init()` is caught and converted to `NodeError::InitPanic`
...
let mut replay_scheduler = Scheduler::replay_from(
    "~/.horus/recordings/my_session/scheduler@abc123.horus".into()
)?;
...
If `init()` fails, the node enters Error state and won't tick. Other nodes continue — unless the error's `Severity` is `Fatal`, in which case the scheduler stops immediately. Panics inside `init()` are caught and converted to `NodeError::InitPanic`.
...
scheduler.add(sensor).order(50).compute().rate(1000_u64.hz()).done();
...
| `Miss::SafeMode` | Call `enter_safe_state()` on the node, then keep ticking | Nodes with a safe fallback state |
| `Miss::Stop` | Stop the entire scheduler (last resort) | Hard real-time safety-critical nodes |

`Miss::SafeMode` fires on *every* deadline miss — the node is not isolated, and the scheduler does not poll `is_safe_state()` for automatic recovery.
