...
let mut scheduler = Scheduler::new()
    ...
    .rt_scheduling(true);      // Attempt RT priority, mlockall, CPU affinity
...
RT features are configured per-node with `.rt()` and globally through builder methods:

```rust
let mut scheduler = Scheduler::new()
    .rt_scheduling(true); // Enable RT scheduling globally
...
When `.rt_scheduling(true)` is set, the scheduler attempts to apply (in order):
...
Enable safety monitoring through the builder API, then use per-node builder methods for WCET and deadlines:

```rust
let mut scheduler = Scheduler::new()
    .safety_monitor(true);  // Enable watchdog / WCET enforcement
...
let mut scheduler = Scheduler::new()
    .tick_hz(1000.0)           // 1 kHz control loop
    .rt_scheduling(true)       // Enable RT priority, mlockall, CPU affinity
    .with_blackbox(16)         // Flight recorder
    .safety_monitor(true);     // Watchdog / WCET enforcement
...
**Use builder methods for production** — enable RT features, recording, and safety monitoring:
```rust
let mut scheduler = Scheduler::new()
    .rt_scheduling(true)
    .with_blackbox(16)
...
When a real-time node exceeds its time budget, the `DeadlineMissPolicy` determines what happens:

```rust
scheduler.add(safety_node)
    .order(0)
    .rt()
    .deadline_miss_policy(DeadlineMissPolicy::EmergencyStop)
    .done();
```

| Policy | Behavior | Use For |
|--------|----------|---------|
| `Warn` | Log a warning and continue normally | Soft real-time nodes (logging, UI) |
| `Skip` | Skip the node for this tick | Firm real-time nodes (non-critical processing) |
| `Degrade` | Downgrade priority for future ticks | Adaptive workloads |
| `Fallback` | Switch to a fallback node | Nodes with safe-mode alternatives |
| `EmergencyStop` | Trigger safety shutdown | Hard real-time safety-critical nodes |

The default policy depends on the RT class:
- **Hard RT** → `EmergencyStop`
- **Firm RT** → `Skip`
- **Soft RT** → `Warn`
