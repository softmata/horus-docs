...
Applies RT features (best-effort) + 16MB BlackBox flight recorder. Equivalent to `Scheduler::new().realtime().with_blackbox(16)`.
...
The `.realtime()` method attempts to apply in order:
...
```rust
let scheduler = Scheduler::new()
    .with_blackbox(16); // 16MB ring buffer for post-mortem analysis
```
...
let mut scheduler = Scheduler::new()
    .with_safety_monitor(10); // Emergency stop after 10 deadline misses
...
| `.with_safety_monitor(max_misses)` | Enable safety monitoring |
...
fn init(&mut self) -> Result<()> {
    Err("Sensor not connected".into()) // Node won't run, others unaffected
}
```
