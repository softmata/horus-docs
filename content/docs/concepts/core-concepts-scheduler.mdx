...
globally for the whole program rather than pair by pair: if *no* node has registered a
topic, the graph falls back to tiers and every main-loop node is ordered by them.

That decision is re-taken after the first tick, not settled at `init()`. Registration is
lazy — it happens on the first `send()`/`recv()`, not in `Topic::new` — so a program whose
nodes exchange messages inside `tick()` runs tick 1 on tiers, and the scheduler then
rebuilds the graph from the topic metadata at the start of tick 2 and keeps it. From that
point the pub/sub graph decides the sequence and `order` no longer applies.
...
`order` tiers are the fallback used only while no topic metadata exists. Registration is
lazy — it happens on the first `send()`/`recv()`, not in `Topic::new` — so the trace above
is tick 1. At the start of tick 2 the scheduler rebuilds the dependency graph from the
topics those sends registered, and Safety, which subscribes to Sensor's topic, is ordered
after Sensor by the graph from then on regardless of its tier. If you need a sequence that
holds for the whole run, use `.deterministic(true)`.
...
3. **CPU affinity** — RT executor threads are pinned whether or not you call `.cores()`. With
   `.cores(&[2, 3])` those are the cores; without it the scheduler derives them from the machine
   (`recommended_rt_cpus`, else the highest-numbered CPU), and `HORUS_RT_CORES="2,3"` sets them
   from the environment when `.cores()` was not called
...
| `.cores(&[usize])` | Choose which CPU cores the RT threads pin to. Without it they still pin, to cores derived from the machine; `HORUS_RT_CORES` sets them from the environment |
...
let mut scheduler = Scheduler::new()
    .tick_rate(1000_u64.hz())  // 1 kHz control loop
    .prefer_rt()               // Attempt RT priority + mlockall (degrade if unavailable)
    .cores(&[2, 3])            // Choose the cores; RT threads are pinned either way
    .blackbox(16)              // Flight recorder
    .watchdog(500_u64.ms());   // Frozen-node watchdog
