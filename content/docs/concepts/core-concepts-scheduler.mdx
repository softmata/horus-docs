...
**Separate Lines (Recommended for Beginners):**
...
**Chaining (Advanced - More Compact):**
...
// Chain multiple nodes in a single expression
scheduler
    .add(safety_node).order(0).done()      // Critical
    .add(controller).order(10).done()      // High priority
    .add(sensor_node).order(50).done()     // Normal
    .add(logger).order(100).done();        // Background
```

Both styles work identically. Use whichever you prefer! Separate lines are clearer for beginners, while chaining is more compact for experienced users.
...
Track your node metrics using internal state or external tools:

```rust
fn tick(&mut self) {
    self.tick_count += 1;

    // Use horus monitor CLI for metrics
    // Or track internally for custom warnings
    if self.tick_count % 1000 == 0 {
        log::debug!("Processed {} ticks", self.tick_count);
    }
}
```
...
Profile your nodes using internal timing:
...
| `.enable_determinism()` | Enable deterministic (reproducible) execution |
| `.with_config(SchedulerConfig::safety_critical())` | Configure for real-time with RT priority |
...
// This is a module-level function in horus::scheduling::runtime
horus::scheduling::runtime::set_realtime_priority(80)?;
