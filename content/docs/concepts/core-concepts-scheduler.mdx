...
Applies RT features (best-effort) + BlackBox flight recorder. Equivalent to `Scheduler::new().with_config(SchedulerConfig::deploy())`.
...
RT features are applied through `SchedulerConfig` presets. Use `deploy()`, `safety_critical()`, or `hard_realtime()` for pre-configured RT settings:

```rust
// Use a preset with RT features
let scheduler = Scheduler::deploy();

// Or apply a custom config
let scheduler = Scheduler::new()
...
BlackBox recording is enabled through `SchedulerConfig` presets (e.g., `deploy()` includes it by default):
...
Safety monitoring is enabled through `SchedulerConfig` (presets like `safety_critical()` include it):
...
Recording is enabled through `SchedulerConfig`:

```rust
use horus_core::scheduling::config::{SchedulerConfig, RecordingConfigYaml};
...
| Method | Description |
|--------|-------------|
| `metrics()` | Get `Vec<SchedulerNodeMetrics>` for all nodes |
