...
- Composable scheduler builders: `.monitoring(true)`, `.require_rt()`, `.with_blackbox(n)`, `.with_profiling()`
...
scheduler.add(motor_ctrl).order(0).budget(500.us()).build()?;
scheduler.run()?;
```
...
|--------|----------------|-------------|
| `.monitoring(true)` | Safety monitor, budget enforcement, deadline monitoring, watchdog (500ms) | Production robots |
| `.require_rt()` | All RT features + memory locking + RT scheduling class. **Panics** without RT capabilities | Hard real-time systems |
| `.with_blackbox(n)` | BlackBox flight recorder with n MB buffer | Post-mortem debugging |
| `.with_profiling()` | Runtime profiling | Performance analysis |
| `.deterministic(true)` | Deterministic execution mode | Certification, replay |
| `.prefer_rt()` | Request RT features (degrades gracefully if unavailable) | Best-effort RT |
| `.max_deadline_misses(n)` | Emergency stop after n deadline misses | Safety-critical |
| `.cores(&[0,1])` | Pin to specific CPU cores | RT isolation |
...
scheduler.add(motor_ctrl)
    .order(0)
    .budget(500.us())     // 500μs max execution time → auto RT
...
| `.budget(dur)` | Set tick budget as a `Duration`, e.g. `200.us()` — auto-marks as RT |
| `.deadline(dur)` | Set deadline as a `Duration`, e.g. `1.ms()` — auto-marks as RT |
...
// Per-node: budget / deadline auto-mark as RT
scheduler.add(motor_ctrl)
    .order(0)
    .budget(500.us())         // 500μs tick budget → auto RT
    .deadline(1.ms())         // 1ms deadline
...
`.monitoring(true)` enables safety monitoring (budget enforcement, deadline monitoring, watchdog) automatically:

```rust
let mut scheduler = Scheduler::new()
    .monitoring(true);           // enables safety monitoring, watchdog, budget enforcement

// RT node with budget and deadline enforcement
scheduler.add(motor_ctrl)
    .order(0)
    .budget(500.us())            // 500μs tick budget
    .deadline(100.ms())          // 100ms deadline
    .on_miss(Miss::SafeMode)     // Enter safe mode on miss
    .build()?;
```
...
scheduler.add(motor_ctrl).order(5).budget(500.us()).build()?;
...
**Use composable builders for production** — enable RT features, recording, and safety monitoring:
```rust
...
| `.with_name(name)` | `Self` | Set scheduler name for logging |
| `.tick_rate(freq)` | `Self` | Set global tick rate (default: 60 Hz) |
| `.monitoring(bool)` | `Self` | Enable safety monitor, budget enforcement, watchdog |
| `.prefer_rt()` | `Self` | Try RT features, degrade gracefully if unavailable |
| `.require_rt()` | `Self` | Enable RT features, panic if unavailable |
| `.deterministic(bool)` | `Self` | Sequential execution on main thread |
| `.verbose(bool)` | `Self` | Enable/disable verbose executor logging |
| `.cores(&[usize])` | `Self` | Pin scheduler and RT threads to CPU cores |
| `.max_deadline_misses(n)` | `Self` | Max deadline misses before emergency stop |
| `.with_profiling()` | `Self` | Enable runtime profiling |
| `.with_blackbox(size_mb)` | `Self` | Enable flight recorder with buffer size |
| `.with_recording()` | `Self` | Enable recording with sensible defaults |
| `.with_telemetry(endpoint)` | `Self` | Enable telemetry export (`"udp://host:port"` or `"file:///path"`) |
