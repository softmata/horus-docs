...
- Per-node configuration: `.order()`, `.rate()`, `.budget_us()`, `.deadline_ms()`, `.on_miss()`
...
// Production robot — safety monitor, watchdog, blackbox
let mut scheduler = Scheduler::deploy()
    .tick_rate(1000.0.hz())           // 1 kHz control loop
...
| `Scheduler::deploy()` | Safety monitor, budget enforcement, deadline monitoring, watchdog (500ms), blackbox (64MB) | Production robots |
...
| `Scheduler::safety_critical()` | `hard_rt()` + blackbox + profiling | Medical, aerospace |
...
scheduler.add(fast_sensor).order(0).rate(1000.0.hz()).build()?;
...
| `.rate(hz.hz())` | Set node-specific tick rate in Hz |
...
let scheduler = Scheduler::new()
    .tick_rate(1000.0.hz()); // 1kHz control loop
```
...
```rust
let mut scheduler = Scheduler::new();
scheduler.add(fast_sensor).order(0).rate(1000.0.hz()).build()?;  // 1kHz
scheduler.add(slow_logger).order(200).rate(10.0.hz()).build()?;  // 10Hz
...
// Duration helpers — works on u64, f64, i32
...
scheduler.add(sensor).order(50).compute().rate(1000.0.hz()).build()?;
