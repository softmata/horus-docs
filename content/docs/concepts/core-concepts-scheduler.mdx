...
**Key details**:
- Nodes execute **sequentially in order** within the BestEffort class. RT, Compute, Event, and AsyncIo nodes run on their own threads and are synchronized at tick boundaries.
...
2. `init()` is called on every node, in order
3. If a node's `init()` returns `Err`, that node enters Error state and is excluded from ticking. Other nodes continue.
