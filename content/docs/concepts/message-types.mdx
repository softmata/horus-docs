...
Strongly-typed Rust structs — all messages in `horus_library::messages`:
...
When logging is active, `log_summary()` is called once per send/recv. Logs appear in the console, in the shared memory ring buffer at `/dev/shm/horus_logs`, and in `horus monitor`.
...
- Messages: `CmdVel`, `Image`, `CompressedImage`, `CameraInfo`, `RegionOfInterest`, `StereoInfo`, `NavSatFix`, `PointCloud`, `DepthImage`, `PwmCommand`, `StepperCommand`, `GenericMessage`
...
### PwmCommand / StepperCommand

Low-level actuator commands for PWM channels and stepper motors with fields for duty cycle, frequency, microsteps, current limits, and enable flags.
...
Raw image data (variable-size, not POD — uses serialization):
...
| `encoding` | `ImageEncoding` | Pixel format (Mono8, Rgb8, Bgr8, Rgba8, Depth16, etc.) |
...
Variable-size point cloud (not POD — uses serialization):

```rust
use horus::prelude::*;

let cloud = PointCloud::xyz(1000);      // 1000 XYZ points
let cloud = PointCloud::xyzrgb(1000);   // 1000 XYZRGB points
```

| Field | Type | Description |
|-------|------|-------------|
| `width` / `height` | `u32` | Point cloud dimensions |
| `fields` | `[PointField; 16]` | Field descriptors |
| `field_count` | `u8` | Number of fields |
| `is_dense` | `bool` | Whether all points are finite |
| `point_step` / `row_step` | `u32` | Byte strides |
...
| `PointCloudHeader` | `num_points`, `point_type`, `point_stride`, `timestamp_ns`, `frame_id` | Fixed-size cloud descriptor |

### DepthImage

Depth image data (variable-size, not POD):

| Field | Type | Description |
|-------|------|-------------|
| `width` / `height` | `u32` | Image dimensions |
| `depths` | `Vec<u16>` | Depth values |
| `min_depth` / `max_depth` | `u16` | Depth range |
| `depth_scale` | `f32` | Scale factor |
| `frame_id` | `[u8; 32]` | Reference frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |

Methods: `get_depth()`, `set_depth()`, `is_valid_depth()`, `to_point_cloud()`, `depth_statistics()`
...
let goal = Goal::new(Pose2D::new(5.0, 3.0, 0.0));
...
```rust
let grid = OccupancyGrid::new(0.05, 200, 200);  // 5cm resolution, 200x200
let (gx, gy) = grid.world_to_grid(1.0, 2.0);
grid.set_occupancy(gx, gy, 100);  // Mark occupied

if grid.is_free(gx, gy) { /* navigable */ }
if grid.is_occupied(gx, gy) { /* blocked */ }
```
...
```rust
let costmap = CostMap::from_occupancy_grid(&grid);
costmap.inflate_obstacles();
let cost = costmap.get_cost(gx, gy);
```
...
| `target_rate_hz` / `actual_rate_hz` | `f32` | Expected vs actual tick rate |
...
| `TactileArray` | Tactile sensor array (up to 64 sensors) |
...
Multi-robot coordination. All are POD types.

| Message | Description |
|---------|-------------|
| `RobotState` | Individual robot state (pose, velocity, status, battery, capabilities) |
| `FleetStatus` | Fleet-wide status (up to 64 robots) |
| `TaskAssignment` | Task allocation with priority, deadline, and required capabilities |
| `FormationControl` | Formation control parameters (type, spacing, leader) |
| `AuctionBid` | Task auction bid with bid value, estimated time, and capability score |
...
Time synchronization and scheduling. All are POD types.

| Message | Description |
|---------|-------------|
| `TimeSync` | Clock synchronization (offset, drift, accuracy) |
| `ScheduledEvent` | Timed event with priority, repeat interval, and status |
| `Timeline` | Event timeline (up to 64 scheduled events) |
| `ClockStats` | Clock statistics (offset, drift, sync quality) |
