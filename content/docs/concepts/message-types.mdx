...
`GoalResult::status` holds the navigation status code: Pending (0), Active (1), Succeeded (2), Aborted (3), Cancelled (4), Preempted (5), TimedOut (6).
...
topic.send(RobotStatus {
    ...
    temperature: 42.0,
    error_code: 0,
    timestamp_ns: horus::now().as_nanos(),
});
```
...
`Topic<T>` detects POD layouts automatically — there is no marker trait to implement in the prelude.
