...
Strongly-typed Rust structs. Core types and robotics messages are both available via `use horus::prelude::*` — the robotics messages live in the `horus_robotics` crate but are re-exported through `horus::prelude`.
