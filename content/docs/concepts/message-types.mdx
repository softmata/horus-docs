...
`Topic<T>` detects POD layouts automatically (no destructor, non-zero size) — there is no marker trait to implement in the prelude. The `bytemuck` impls above are optional: they are only needed if you also implement `horus_core::communication::PodMessage` for its `as_bytes()` / `from_bytes()` helpers.
