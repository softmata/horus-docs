...
**Technical requirement**: The `Topic::send()` and `Topic::recv()` methods have a trait bound requiring `LogSummary`, so you must implement it for all message types.

**Standard operation**:
- LogSummary implementation required but typically **only called during debugging**
- Production code has minimal overhead
