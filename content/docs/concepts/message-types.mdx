...
## HorusTensor (Raw Zero-Copy)

For low-level tensor transport (ML inference, custom pipelines), `Topic<HorusTensor>` provides direct access to the zero-copy shared memory path. Only the 168-byte `HorusTensor` descriptor flows through the ring buffer while the actual data stays in a shared-memory `TensorPool`.
