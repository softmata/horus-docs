...
**Technical requirement**: The `Topic::send_logged()` and `Topic::recv_logged()` methods have a trait bound requiring `LogSummary`. The standard `send()` and `recv()` methods do **not** require it.

**Standard operation**:
- LogSummary is only needed when using `send_logged()`/`recv_logged()` for introspection
- Production code using `send()`/`recv()` has zero logging overhead
- Implementing LogSummary is recommended for debugging and monitoring purposes
...
status_pub.send(RobotStatus::new(75.0, 42.0)).ok();
```
