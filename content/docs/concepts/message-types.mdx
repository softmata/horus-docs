...
- Messages: `CmdVel`, `CompressedImage`, `CameraInfo`, `RegionOfInterest`, `StereoInfo`, `NavSatFix`, `PwmCommand`, `StepperCommand`, `GenericMessage`
- Descriptors: `ImageDescriptor`, `PointCloudDescriptor`, `DepthImageDescriptor`, `HorusTensor`
...
RAII image type with zero-copy shared memory backing. Pixel data lives in a `TensorPool` — only a 288-byte descriptor flows through the ring buffer.
...
| **IPC** | ~50ns (zero-copy Pod descriptor) | ~50ns (zero-copy Pod descriptor) |
| **Data location** | Shared memory pool | Shared memory pool |
| **Best for** | Raw tensor access, ML inference | Rich API with pixel/point methods |
| **API** | Shape-based accessors (`from_tensor()`) | Field-level (`pixel()`, `get_depth()`, `point_at()`) |

Both approaches use zero-copy shared memory. The difference is the API surface: tensor types provide raw shape/dtype access, while standard types provide domain-specific convenience methods.
