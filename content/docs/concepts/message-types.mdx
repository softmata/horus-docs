...
**Best practices**:
- Consider fixed-size arrays for known-size data (more efficient)
- Include timestamp fields for all time-sensitive data
- Use `#[repr(C)]` if you need C interop
...
```rust
// Rust - Typed message
use horus::prelude::*; // Provides {Topic, Pose2D}
...
**Benefits:**
- **Ultra-fast**: ~85-167 nanoseconds IPC latency (zero-copy shared memory)
...
- **IPC latency:** ~0.085-0.167µs (message passing)
- **Logging overhead:** ~50-100ns (string formatting)
- **Total with logging:** ~200-300ns
...
```python
# Python - Generic topic (string topic name)
...
```rust
// Rust - GenericMessage
use horus::prelude::*; // Provides {Topic, GenericMessage}
...
|---------|---------------|------------------|
| **IPC Latency** | ~0.085-0.167µs | 4-10µs |
...
**With typed messages (SPSC):**
- 1000 sends/sec × 0.085µs = 0.085ms total overhead
- **99.99% of CPU available** for your code
...
- Typed SPSC: 0.85ms overhead → **99.9% CPU available**
...
```python
# Phase 1: Prototyping with generic topic
...
```python
# Phase 2: Production with typed topic
from horus import Node, Pose2D
...
The `LogSummary` trait is **required for all message types** used with Topic communication. It provides a human-readable summary for logging and monitor monitoring.
...
**Technical requirement**: The `Topic::with_logging()` builder method requires `T: LogSummary` at construction time. The standard `Topic::new("name")?` does **not** require it. There is only one `send()` method and one `recv()` method — logging behavior is determined when the topic is created.
...
### Custom: Override for Large Types

For types where `Debug` output would be too large (images, point clouds, scans), implement `LogSummary` manually instead of deriving:
...
**Small messages** use `#[derive(LogSummary)]` — the `Debug` output is already compact:
```rust
...
**Large messages** use manual impls to avoid logging massive data:
```rust
...
**LogSummary overhead**:
- Benchmarks can disable logging for accurate timing
- Production code has minimal logging overhead
...
// Use with Topic
use horus::prelude::*;
let status_pub: Topic<RobotStatus> = Topic::new("robot_status")?;
status_pub.send(RobotStatus::new(75.0, 42.0));
```
...
```rust
// For known-size data: Fixed-size arrays are efficient
pub struct LaserData {
    pub ranges: [f32; 360],  // Fixed size, good for shared memory
}

// For variable data: Vec also works
pub struct DynamicLaserData {
    pub ranges: Vec<f32>,  // Works fine - HORUS handles serialization
}
```
...
| Message Type | Size | Latency (Topic MPMC) | Latency (Topic SPSC) |
|--------------|------|---------------|----------------|
| CmdVel | 16B | ~167ns | ~85ns |
| Pose2D | 32B | ~180ns | ~90ns |
| IMU | 304B | ~250ns | ~120ns |
| LaserScan | 1.5KB | ~600ns | ~300ns |
...
```rust
use horus::prelude::*; // Provides {Topic, Node, NodeInfo, LaserScan}
...
- **[Advanced Examples](/rust/examples/advanced-examples)** - Complex patterns like multi-process systems and Python integration
...
```rust
use horus::prelude::*; // Provides {Topic, Node, NodeInfo, CmdVel}
