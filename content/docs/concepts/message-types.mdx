...
A dash means that language has no separate page for the category yet — with one
exception. The Vision/C++ dash means the types are genuinely absent: `CameraInfo`,
`RegionOfInterest` and `StereoInfo` are `repr(Rust)` upstream and were withdrawn from
the C ABI, so `horus::msg::CameraInfo` does not compile. Nor does every standard
message cross the language boundary. The `Vec`-carrying payloads — `CompressedImage`,
`OccupancyGrid`, `CostMap`, `TactileArray` — are not POD and have no C++ declaration
either. [Not available as C++ topics](/cpp/api/topic#not-available-as-c-topics) is the
full accounting. [Multi-language](/concepts/multi-language) explains how the rest
cross, and [`rust/api/messages`](/rust/api/messages) is the fullest single list.

### Two families, and why the difference is visible

Nearly every message in the catalogue is one of two kinds, and which one it is
...
* **Serialized** — a variable-size value (anything holding a `Vec`, a `String`
  or a map). It is encoded on publish and decoded on receive. `TactileArray`
  (`forces: Vec<f32>`) and `CompressedImage` (`data: Vec<u8>`) are the common
  examples.

There is a third arrangement the two families do not cover. The pool-backed types —
`Image`, `PointCloud`, `DepthImage` — keep their payload in the tensor pool and put
only a fixed-size POD descriptor on the ring, so the pixels and points are neither
encoded nor copied through the topic.
...
- Large messages (>256 bytes): ~4.4 µs (overflow buffer)
  ...
  Both figures are quoted from `GenericMessage`'s own documentation in
  `horus_types/src/generic.rs`; no benchmark in the repository measures
  `GenericMessage`, so unlike the typed-topic numbers on
  [Benchmarks](/performance/benchmarks) they are not a measured result.
