...
- **Execution Classes** - Nodes are annotated with an execution class (Rt, Compute, Event, AsyncIo, BestEffort) via the builder methods `.rate()`, `.compute()`, `.on("topic")`, `.async_io()`
