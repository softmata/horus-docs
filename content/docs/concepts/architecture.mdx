...
flowchart TB
    ...
    subgraph SPSC["Topic SPSC (Point-to-Point)"]
        direction LR
        PR["Producer"] --> LK["Topic"] --> CO["Consumer"]
    end
`}
...
|---------|----------|---------|
| **Topic** | Multiple readers need the same data | Camera images → multiple vision algorithms |
| **Topic** | Strict ordering between two nodes | Sensor → filter → controller pipeline |

### Topic (MPMC): Broadcast to Many

Topic implements **publish-subscribe** messaging. Any node can publish to a topic, and any number of nodes can subscribe.

```rust
// Camera node publishes images
let image_pub: Topic<Image> = Topic::new("camera.image")?;
image_pub.send(frame);

// Multiple subscribers receive the same image
let image_sub: Topic<Image> = Topic::new("camera.image")?;
if let Some(frame) = image_sub.recv() {
    // Process frame
}
```

**Topic MPMC characteristics:**
- Many-to-many communication
- Latest-value semantics (subscribers get most recent)
- ~~167ns round-trip latency
- Automatic network transport for distributed systems

### Topic (SPSC): Fast Pipeline

Topic SPSC implements **point-to-point** channels. One producer, one consumer, guaranteed ordering.

```rust
// Producer side
let link: Topic<SensorData> = Topic::new("sensor_pipeline")?;
link.send(data);
...
- ~~85ns round-trip latency
- Ideal for processing pipelines

### Cross-Process Communication

Topic work transparently across process boundaries using shared memory:
...
flowchart LR
    ...
    DET -->|"Detections<br/>(Topic MPMC)"| NAV
    NAV -->|"Velocity<br/>(Topic MPMC)"| CTRL
    CTRL -->|"PWM<br/>(Topic SPSC)"| MOT
`}
...
| Controller → Motors | Topic | Strict ordering, lowest latency |
...
| Metric | Value |
|--------|-------|
| Topic MPMC round-trip | ~~167ns |
| Topic SPSC round-trip | ~~85ns |
