...
| **Error(msg)** | Something went wrong, but recoverable |
| **Crashed(msg)** | Unrecoverable failure |
...
- **Runtime Profiler** - Tracks how long each node takes (avg, min, max tick duration)
- **NodeMetrics** - Per-node metrics: total ticks, successful ticks, failed ticks, messages sent/received
...
Large data types like `Image`, `PointCloud`, and `DepthImage` are backed by shared memory automatically:
...
|------------|-----------|-----|
| Camera → Detector | Image (zero-copy) | Large image, shared memory |
...
| Shared memory allocation | ~100ns |
