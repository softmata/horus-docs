...
|---------|---------|---------------|
| Same-thread | ~3 ns | Publisher and subscriber on same thread |
| Same-process | ~18–36 ns | Same process, different threads |
| Cross-process | ~50–167 ns | Different processes, shared memory |
...
Python shares the same memory pool via zero-copy NumPy views:
