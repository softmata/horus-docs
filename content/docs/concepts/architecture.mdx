...
flowchart LR
    subgraph POOL["Tensor Pool (Shared Memory)"]
        T["Image Data<br/>1080 x 1920 x 3 (H x W x C)<br/>refcounted slot"]
    end

    CAM["Camera"] -->|"write once"| T
    T -->|"read, own reference"| V1["Vision 1"]
    T -->|"read, own reference"| V2["Vision 2"]
    T -->|"read, own reference"| V3["Vision 3"]
    T -.->|"freed when the last reference drops"| FREE["Pool free list"]
`}
...
flowchart LR
    ...
    CAM -->|"Image (Topic)<br/>pool-backed, zero-copy"| DET
    DET -->|"Detections (Topic)"| NAV
    NAV -->|"Velocity (Topic)"| CTRL
    CTRL -->|"Motor command (Topic)"| MOT
`}
