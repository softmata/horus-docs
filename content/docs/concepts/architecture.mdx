...
You always use the same `Topic::new()` call. HORUS detects the topology and picks the fastest backend for it. Measured end-to-end latency ranges from **~3ns** (same-thread) to **~167ns** (cross-process, many-to-many):
...
**Total pipeline latency:** Under 1 microsecond for message passing (same-process backends). Tensor allocation is a separate cost that scales with tensor size.
