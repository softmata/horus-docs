...
- You need the server to report success or failure back — the call returns the server's error, or times out if no response arrives
...
// simplified
  ...
  <LangTab language="Python">
    <Callout type="warn">
    Services are currently available in Rust only. Python support is planned.
    </Callout>
  </LangTab>
</LanguageTabs>
...
match client.call(request, Duration::from_secs(1)) {
    Ok(response) => { /* success */ }
    Err(ServiceError::Timeout) => { /* no response in time — also how a missing server manifests */ }
    Err(ServiceError::ServiceFailed(msg)) => { /* handler returned Err */ }
    Err(ServiceError::NoServer) => { /* not produced by the current transport — a missing server surfaces as Timeout */ }
    Err(ServiceError::Transport(msg)) => { /* not produced by the current shared-memory transport */ }
}
```

| Error | Cause | Retried by `call_resilient`? |
|-------|-------|------------------------------|
| `Timeout` | Server didn't respond within the timeout (also how a missing server manifests) | Yes |
| `ServiceFailed(msg)` | Server handler returned `Err(msg)` | No (permanent) |
| `NoServer` | Not produced by the current transport — a missing server surfaces as `Timeout` | No (permanent) |
| `Transport(msg)` | Not produced by the current shared-memory transport | Yes |

In practice, `call()` returns only `Timeout` or `ServiceFailed`. `NoServer` and `Transport` are defined in the `ServiceError` enum but are not produced by the current shared-memory transport — a missing server surfaces as `Timeout` (use `call_optional` to treat that as "no server yet").
