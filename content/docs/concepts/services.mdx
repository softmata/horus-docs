...
```rust
// Auto-retry with default settings (3 attempts, exponential backoff)
...
use horus::services::RetryConfig;
...
`call_resilient` retries on `Timeout` and `NoServer` errors. `ServiceFailed` errors are not retried since they indicate a server-side logic failure.
