...
- Each client also opens its own response topic `{name}.response.{client_id}` and sends the name in the request's `response_topic` field; the server publishes there to avoid contention between concurrent clients
