...
- Each client also opens its own response topic `{name}.response.{client_id}` and sends that name in the request's `response_topic` field; the server replies there rather than on the shared `{name}.response` topic in the diagram, so concurrent clients never contend for the same response channel
...
match client.call(request, Duration::from_secs(1)) {
    ...
    // `NoServer` is defined but never returned — a missing server times out
    Err(ServiceError::Timeout) => { /* no response within the timeout */ }
