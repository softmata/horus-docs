...
HORUS supports two transport mechanisms: **local shared memory** and **network communication**. Topic work transparently with either transport.

## Transport vs Pattern

**Important distinction**:
- **Pattern** (MPMC vs Topic SPSC): How nodes communicate (MPMC vs SPSC)
...
|---------|----------------|-------------------|
| **Topic (MPMC)** | `Topic::new("topic")` | `Topic::new("topic@192.168.1.5")` |
| **Topic (SPSC)** | `Topic::new("topic")` | `Topic::new("topic@192.168.1.5:9000")` |
...
**Latency**: Ultra-fast (~85ns SPSC, ~167ns MPMC round-trip)

```rust
// Automatically uses shared memory
let topic: Topic<SensorData> = Topic::new("sensors")?;
let link: Topic<ImuData> = Topic::new("imu")?;
```
...
**Latency**: Very fast (5-15µs SPSC, &lt;50µs Topic MPMC)

```rust
// Automatically uses network backend
let topic: Topic<SensorData> = Topic::new("sensors@192.168.1.5")?;
let link: Topic<ImuData> = Topic::new("imu@192.168.1.5:9000")?;
```

**How it works**:
- TCP (Topic SPSC): Reliable, ordered, connection-oriented
- UDP (Topic MPMC): Connectionless, efficient for pub/sub
...
| Transport | SPSC (round-trip) | MPMC (round-trip) | Use Case |
|-----------|-------------|------------|----------|
| **Local SHM** | ~85ns | ~167ns | Same machine, ultra-low latency |
...
**Local Shared Memory**:
- Topic SPSC: 2.5M msgs/sec (small messages)
- Topic MPMC: 1.5M msgs/sec (small messages)
- Limited by memory bandwidth, not synchronization

**Network**:
- Topic SPSC: 100K-500K msgs/sec (depending on message size)
- Topic MPMC: 50K-200K msgs/sec (depending on message size)
...
- **Fastest possible**: ~85ns-~167ns latency
...
let position_pub: Topic<Position> = Topic::new("fleet.positions@router")?;

// Robot 2 (192.168.1.20) subscribes
let positions: Topic<Position> = Topic::new("fleet.positions@router")?;
```
...
let camera: Topic<Image> = Topic::new("camera@192.168.1.100:9000")?;

// Server (192.168.1.100) processes images
let images: Topic<Image> = Topic::new("camera@0.0.0.0:9000")?;
```
...
let sensor1: Topic<Temperature> = Topic::new("sensors@192.168.1.5")?;
let sensor2: Topic<Temperature> = Topic::new("sensors@192.168.1.6")?;

// Central aggregator
let temps: Topic<Temperature> = Topic::new("sensors@router")?;
```
...
let status: Topic<RobotStatus> = Topic::new("status@192.168.1.100")?;

// Monitor on laptop subscribes
let monitor: Topic<RobotStatus> = Topic::new("status@0.0.0.0:8080")?;
```
...
- **Higher latency**: 5-50µs vs ~85-167ns (20-100x slower)
...
struct RobotSystem {
    ...
    imu: Topic<ImuData>,           // ~85ns latency
    motors: Topic<MotorCmd>,       // ~85ns latency

    // Network: Communication with other robots
    fleet_position: Topic<Position>, // &lt;50µs latency (acceptable)
    camera_stream: Topic<Image>,     // Network to processing server
}
...
let imu_local: Topic<ImuData> = Topic::new("imu")?;              // ~85ns
let motors_local: Topic<MotorCmd> = Topic::new("motors")?;      // ~85ns

// Network: Telemetry to ground station
let telemetry: Topic<Telemetry> = Topic::new("telem@192.168.1.100")?;  // &lt;50µs

// === ON GROUND STATION (192.168.1.100) ===
// Network: Receive telemetry from drone
let telem_sub: Topic<Telemetry> = Topic::new("telem@0.0.0.0:8000")?;

// Network: Send commands to drone
let cmd_pub: Topic<Command> = Topic::new("cmd@192.168.1.50")?;
```
...
Topic::new("sensors")?                          // → Shared memory

// Network (contains '@')
Topic::new("sensors@192.168.1.5")?              // → UDP to 192.168.1.5
Topic::new("sensors@192.168.1.5:9000")?         // → UDP with custom port
Topic::new("imu@192.168.1.5:9000")?       // → TCP to 192.168.1.5:9000
Topic::new("imu@0.0.0.0:9000")?           // → TCP listen on port 9000
```
...
let telemetry: Topic<Status> = Topic::new("status@gcs")?;
if let Err(_) = telemetry.send(status) {
...
let motors: Topic<MotorCmd> = Topic::from_config("motors")?;
```
...
- [Topic (SPSC)](/concepts/core-concepts-topic) - Topic SPSC pattern details
