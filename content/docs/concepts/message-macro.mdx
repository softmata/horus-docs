...
// Manual definition — derive handles LogSummary
...
> **Note:** `#[derive(LogSummary)]` eliminates the need to manually implement the `LogSummary` trait. See the [macros reference](/rust/api/macros#derivellogsummary) for details.

For simple types, that's still more than necessary.
...
A key advantage of `message!` is that it **doesn't require serde in your `horus.yaml`**.

The macro generates code using `::horus::serde::Serialize` which uses HORUS's re-exported serde. This means your project only needs HORUS as a dependency.

**Manual approach** (requires serde in horus.yaml):
```yaml
# horus.yaml - serde required!
dependencies:
  - name: serde
    features: ["derive"]
```
...
```rust
// Requires serde as a direct dependency
#[derive(Serialize, Deserialize)]
struct MyMessage { x: f32, y: f32 }
```

**With message! macro** (no extra dependencies):
```yaml
# horus.yaml - no serde needed!
dependencies: []
```

```rust
// Just works - no serde dependency needed
message!(MyMessage { x: f32, y: f32 });
```

> **Tip:** Built-in message types like `Twist`, `Odometry`, and `Image` also work without adding serde - they're pre-compiled in the HORUS library.
...
### 3. Full Topic Compatibility

Your message immediately works with all HORUS communication:

```rust
let topic: Topic<YourMessage> = Topic::new("topic")?;  //  Works!
```
...
impl TempSensor {
    fn new() -> Result<Self> {
        Ok(Self {
            publisher: Topic::new("sensors.temperature")?,
            reading: 20.0,
        })
    }
}
...
// Alert node
struct AlertMonitor {
    subscriber: Topic<TempReading>,
    alert_pub: Topic<Alert>,
    threshold: f32,
}
...
impl AlertMonitor {
    fn new() -> Result<Self> {
        Ok(Self {
            subscriber: Topic::new("sensors.temperature")?,
            alert_pub: Topic::new("alerts.temperature")?,
            threshold: 30.0,
        })
    }
}
...
impl Node for AlertMonitor {
    ...
    fn tick(&mut self) {
        if let Some(TempReading(temp, _)) = self.subscriber.recv() {
...
fn main() -> Result<()> {
    ...
    scheduler.add(AlertMonitor::new()?).order(1).done();
...
// use horus::prelude::*; // Image from horus library
...
### When You Need serde as a Dependency

You only need to add serde to your `horus.yaml` if you:

1. **Define types manually** with `#[derive(Serialize, Deserialize)]`
2. **Use serde directly** (e.g., `serde_json::to_string()`)

If you use `message!` for all custom types, you don't need serde.

```yaml
# Only needed for manual derives or direct serde usage
dependencies:
  - name: serde
    features: ["derive"]
```
...
> **Tip:** Use `#[derive(LogSummary)]` instead of manual `impl LogSummary`. Only implement manually for large types (images, point clouds) where `Debug` output would be too verbose. See the [macros reference](/rust/api/macros#derivellogsummary) for details.
 ...
 **Topic compatibility** - works immediately
...
- Learn about [Topic](/concepts/core-concepts-topic) for pub/sub communication
