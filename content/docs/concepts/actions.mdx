...
For single-field actions:

```rust
simple_action!(Spin, angular_velocity: f64, current_angle: f64, total_rotations: u32);
...
# Send a goal (JSON)
horus action send_goal navigate '{"target_x": 5.0, "target_y": 3.0, "max_speed": 1.0}'

# Send and wait for result
horus action send_goal navigate '{"target_x": 5.0}' --wait --timeout 30

# Cancel a goal
horus action cancel_goal navigate --goal-id <uuid>
```
