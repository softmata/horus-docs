...
Every HORUS topic is shared-memory-backed from creation, so a subscriber in the same thread, a different thread, or a different process all connect to the same ring buffer. What actually moves latency is **core locality** — not the process boundary:

| Route | Latency (one-way) | Why |
|-------|-------------------|-----|
| Same thread, self-loop (one handle, fast path) | ~12ns | Producer and consumer share one core — L1-cache-hot |
| Same thread, real pub + sub (one executor thread) | ~19ns | Still one core, two ring handles |
| Cross-thread **or** cross-process | ~220ns | Producer and consumer on different cores — cache-coherence traffic |

The jump to ~220ns comes from crossing cores, not from crossing the process boundary — two nodes on separate threads inside one process pay the same ~220ns as two processes. No backend or layout choice avoids it; it is cache-coherence physics. The fastest realistic route is to co-locate a producer and consumer on one executor thread.

You don't configure any of this. HORUS picks one of four shared-memory backends automatically from the topology — `SpscShm` (1 producer : 1 consumer), `MpscShm` (N producers : 1 consumer), `PodShm` (1 producer : N consumers, POD broadcast), or `FanoutShm` (N : M / non-POD broadcast) — as participants join or leave.
...
The `WheelEncoder` messages flow through shared memory at ~220ns one-way latency (the two processes run on different cores), with zero configuration.
...
|--------|----------------|---------------|
| **Latency** | ~12-19ns one-way if co-located on one executor thread | ~220ns one-way (cross-core) |
...
### Why the Same Transport Works Same-Process and Cross-Process

Every HORUS topic is shared-memory-backed from creation — there is no separate in-process transport that has to be swapped for a cross-process one when a topic spans processes. A publisher and subscriber in the same process talk through the same ring buffer they would use across processes; the only thing that changes with placement is core locality (~12-19ns one-way when co-located on one executor thread, ~220ns one-way once producer and consumer sit on different cores — whether that is two threads or two processes). Users write the same `Topic::new("name")` call regardless. This means code that works in a single-process prototype deploys to a multi-process production system with zero changes: a cross-process consumer joins an existing topic with no migration. Splitting a monolith into separate processes does not require code changes.
...
| **Process isolation** | One crash does not take down the system; independent restart and upgrade | Higher baseline latency (~220ns one-way cross-core vs ~12-19ns same-thread); shared memory files persist after exit and need cleanup |
