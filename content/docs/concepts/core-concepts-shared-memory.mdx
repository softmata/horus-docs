...
HORUS achieves **~85ns-2.8µs latency** through a zero-copy shared memory IPC architecture. This page explains how HORUS uses platform-specific shared memory for ultra-low latency inter-process communication.
...
Topic::new("cmd_vel")  → /dev/shm/horus/topics/horus_cmd_vel
...
```rust
// Project A
Topic::new("robot_arm.cmd_vel")?;

// Project B
Topic::new("mobile_robot.cmd_vel")?;
```

This creates separate topics:
- `/dev/shm/horus/topics/horus_robot_arm.cmd_vel`
- `/dev/shm/horus/topics/horus_mobile_robot.cmd_vel`
...
`ShmTopic<T>` is the internal implementation used by Topic for lock-free ring buffer communication in shared memory.
...
`Topic<T>` uses ShmTopic internally to provide the user-facing pub/sub API. When you call `topic.send()`, it internally uses the loan pattern to write directly to shared memory and automatically publishes the message to all subscribers.
...
The recommended approach using Topic:

```rust
// Specify number of messages to buffer
Topic::with_capacity("topic", 1000)?;  // 1000 messages
```

**Actual memory usage** = `capacity × sizeof(T) + overhead`

**Note:** Topic SPSC uses a single-slot design (no capacity parameter) - it always stores exactly one message (the latest value) for minimal latency.
...
```rust
use horus::prelude::*; // Provides ShmRegion
...
// Example: Topic with 1000 messages of type LaserScan
let capacity = 1000;
let message_size = size_of::<LaserScan>();  // ~1536 bytes
let total_memory = capacity * message_size;  // ~1.5 MB

Topic::<LaserScan>::with_capacity("scan", capacity)?;
```
...
Topic::<PointCloud>::with_capacity("points", capacity)?;
```
...
Topic::<CmdVel>::with_capacity("cmd_vel", 1000)?;  // 16 KB

// Medium messages, moderate frequency
Topic::<Imu>::with_capacity("imu", 500)?;  // 152 KB

// Large messages, lower frequency
Topic::<LaserScan>::with_capacity("scan", 100)?;  // 150 KB
```
...
`send()` is infallible — when the buffer is full, the oldest message is overwritten ("keep last" semantics):
...
fn tick(&mut self) {
    ...
    self.publisher.send(data);  // Topic::new("sensors")
}

// GUI (Terminal 2) - inside Node::tick()
fn tick(&mut self) {
    if let Some(data) = self.subscriber.recv() {  // Topic::new("sensors")
...
```rust
// Project A
Topic::new("project_a.sensors")?;

// Project B
Topic::new("project_b.sensors")?;
```
