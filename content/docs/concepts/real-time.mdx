...
|---|---|---|
| Motor control at 100+ Hz | `.rate(100.hz())` | Auto-derives budget and deadline, gets dedicated thread |
...
| Emergency stop handler | `.on("emergency.stop")` | Runs only when the event fires, zero polling overhead |
| Background logging | default (no config) | BestEffort is fine |
| Visualization / UI | `.rate(30.hz())` or default | Low rate, no deadline needed |
