...
// simplified
  ...
  <LangTab language="Python">
    ...
    display = horus.Node(
        ...
        tick=display_tick,
        order=100,
    )  # In Python, rate defaults to 30 Hz → runs as Rt @ 30 Hz (see note below)
    ```
  </LangTab>
</LanguageTabs>

<Callout type="warning">
**Python note:** `horus.Node` always carries a tick `rate` (default **30 Hz**), so a Python node with no `compute=`, `on=`, or `async def` tick is scheduled as an **Rt** node at that rate — *not* BestEffort. At startup you'll see `Added RT node '<name>' ... at 30.0Hz`. True BestEffort (main-loop, no timing enforcement) is reachable only from **Rust**, by omitting `.rate()`, `.budget()`, and `.deadline()`. In Python, set `rate=` to control the frequency; there is no way to opt out of a rate.
</Callout>
...
<Callout type="info">
This table describes the **Rust** builder, where the class follows the methods you call. In **Python**, `horus.Node` always sets a `rate` (default 30 Hz), so the `(nothing special) → BestEffort` row never applies — a Python node with no `compute=`/`on=`/`async` tick is always **Rt**. See the Python note under [BestEffort](#besteffort-default).
</Callout>
...
// simplified
  ...
  <LangTab language="Python">
    ...
    # Rt @ 30 Hz — display node (Python has no BestEffort; rate defaults to 30)
