...
// simplified
  ...
  <LangTab language="Python">
    ...
    # node = horus.Node(name="node", tick=fn, compute=True, async_io=True)  # ERROR

    # DO THIS — pick one
    node = horus.Node(name="node", tick=fn, async_io=True)
    ```
  </LangTab>
</LanguageTabs>
...
// simplified
  ...
  <LangTab language="Python">
    ...
    telemetry = horus.Node(
        ...
        order=100, async_io=True, rate=1, failure_policy="ignore",
    )

    # Dashboard — runs in main loop, no special needs
    dashboard = horus.Node(name="dashboard", tick=update_ui, order=200)

    sched = horus.Scheduler(tick_rate=500, watchdog_ms=500, blackbox=64, rt=True)
