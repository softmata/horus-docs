...
flowchart TD
    ...
    Q -->|"Reactive to external events?"| SM["<b>STATE MACHINE</b><br/><i>E-stop handling</i><br/><i>Mode switching</i><br/><i>UI flows</i>"]

    Q -->|"Sequence of steps<br/>to achieve a goal?"| MP["<b>MISSION PLANNER</b><br/><i>Pick-and-place</i><br/><i>Navigation sequences</i><br/><i>Calibration</i>"]

    Q -->|"Both event-driven<br/>AND goal-oriented?"| BOTH["<b>COMBINE THEM</b><br/><i>See Integration below</i><br/><i>Autonomous robot</i><br/><i>with mode management</i>"]

    Q -->|"Simple node<br/>coordination?"| SCHED["<b>USE SCHEDULER</b><br/><i>Sensor fusion</i><br/><i>Control loops</i>"]

    style SM fill:#4a9eff,color:#fff
    style MP fill:#10b981,color:#fff
    style BOTH fill:#f59e0b,color:#fff
...
impl Node for AMROrchestrator {
    fn tick(&mut self) -> Result<()> {
        ...
        if let Some(cmd) = self.command_sub.recv() {
            match cmd.as_str() {
                "deliver:A:B" => {
