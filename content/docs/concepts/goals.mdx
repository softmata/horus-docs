...
- `horus lock` records what resolution picked, so a version change shows up in a diff

**Impact**: Onboard new developers quickly, and pin the framework version across a fleet.

**Not yet**: `horus.lock` is a record, not a constraint. No install path reads its
pins back, so it does not by itself make two machines resolve the same dependency
versions — write exact versions in `horus.toml` for anything that must not move.
Enforcing the lockfile is planned; until then this page does not claim it.
