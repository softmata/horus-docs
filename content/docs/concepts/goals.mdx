...
**Zero Compromise Performance**: Deliver sub-200ns message passing — 151 ns one-way cross-process, roughly 33x lower latency than ROS 2's published REP 2014 reference for default DDS — without sacrificing safety or ease of use
...
**Dependency Isolation**: Solve dependency hell with one manifest for Rust, Python and system dependencies, isolated per project, and a framework version you can pin across a fleet
...
- Sub-microsecond latency for typical robotics messages, measured from 16 B to 4 KB by value; larger frames ride the pool-backed path, where only a descriptor crosses the ring
...
**HORUS Solution**:
- One manifest, `horus.toml`, for Rust crates, PyPI packages and system libraries alike — no `Cargo.toml` plus `requirements.txt` plus a README paragraph about apt
- `HORUS_VERSION` pins the framework itself, binary and source from one release tag, so every machine in a fleet runs one known tree
- Isolated dependency resolution prevents version conflicts
- Package registry with automatic dependency installation
- `horus lock` records what resolution picked, and every installer reads those pins back before choosing a version, so two machines resolve the same dependencies

**Impact**: Onboard new developers quickly, and pin both the framework and its dependencies across a fleet.

**Not yet**: the `[toolchain]` and `[[system]]` sections of `horus.lock` are
recorded and reported, but nothing installs or switches to them — a machine with
a different Rust or Python is described accurately and not corrected. Package
pins are enforced; toolchain pins are not.
...
- **Use Case**: Pick-and-place robots, assembly line automation, high-rate trajectory following
...
- **Performance**: sub-200ns message passing, measured — 151 ns one-way cross-process, roughly 33x lower than ROS 2's published DDS reference. HORUS's own numbers are measured by `all_paths_latency` and are one-way, not round-trip; the ROS 2 figure is quoted from REP 2014, not measured here.
...
- **Performance**: Sub-microsecond latency for typical robotics messages, measured from 16 B to 4 KB by value
