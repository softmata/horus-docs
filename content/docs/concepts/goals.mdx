...
Instead of writing yet another IMU driver, just use the built-in Python hardware nodes:
```python
from horus import Scheduler, ImuNode, LidarNode, CameraNode

scheduler = Scheduler()
scheduler.add(ImuNode(), order=0)
scheduler.add(LidarNode(), order=0)
scheduler.add(CameraNode(device_id=0), order=1)
scheduler.run()
```
