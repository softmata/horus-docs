...
```rust
use horus::prelude::*; // Provides {Scheduler, ImuNode}
...
```rust
use horus::prelude::*; // Provides {Scheduler, PidControllerNode}

let mut scheduler = Scheduler::new();
// Create PID controller with default topics
let pid = PidControllerNode::new()?;
scheduler.add(pid).order(1).done();
```

Build a robot in hours by composing existing nodes:
```rust
use horus::prelude::*; // Provides {Scheduler, ImuNode, LidarNode, PidControllerNode, DifferentialDriveNode}

let mut scheduler = Scheduler::new();
scheduler.add(ImuNode::new()?).order(0).done();
scheduler.add(LidarNode::new()?).order(0).done();
scheduler.add(PidControllerNode::new()?).order(1).done();
scheduler.add(DifferentialDriveNode::new()?).order(2).done();
scheduler.run()?;
```
