...
// simplified
  ...
  <LangTab language="Python">
    ...
    telemetry = horus.Node(name="telemetry", tick=upload,
                           order=100, async_io=True, rate=1)  # won't block anything
    ```
  </LangTab>
</LanguageTabs>
...
// simplified
  ...
  <LangTab language="Python">
    ...
    logger = horus.Node(
        name="logger", tick=log_data,
        order=100, async_io=True, failure_policy="ignore",
    )
...
// simplified
  ...
  <LangTab language="Python">
    ...
    sched = horus.Scheduler(
        tick_rate=500, watchdog_ms=500, blackbox=64,
