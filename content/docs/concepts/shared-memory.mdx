...
Your robot's camera publishes 30 frames/sec at 2MB each. Copying through kernel pipes would consume 60MB/s of bandwidth and add milliseconds of latency. HORUS puts the data in shared memory — zero copies, sub-microsecond latency, no kernel involvement.
...
This separation is the single most important optimization for achieving sub-20ns same-thread (same-core) latency. Without it, every `send()` would invalidate the consumer's cache line and vice versa.
...
// simplified
  ...
  <LangTab language="Python">
    ...
    # Fixed-size fields are auto-detected — zero-copy transport
    register_message("WheelSpeed", "wheels.speed", [
        ("left", "f32"),
        ("right", "f32"),
        ("timestamp_ns", "u64"),
    ])
...
// simplified
  ...
  <LangTab language="Python">
    ...
    register_message("LogEntry", "logs.entry", [
        ("message", "string"),
        ("tags", "string"),
        ("timestamp_ns", "u64"),
    ])
...
|-----------|----------------------|------------------------|-------------|
| POD zero-copy | ~12-36ns | ~50-167ns | Fixed-size sensor data, commands |
...
| Backend | Topology | When selected |
|---------|----------|---------------|
| `SpscShm` | 1 producer : 1 consumer | ≤1 publisher and ≤1 subscriber |
| `MpscShm` | N producers : 1 consumer | More than one publisher, a single subscriber |
| `PodShm` | 1 producer : N consumers | POD type broadcast to many subscribers — every subscriber gets every message |
| `FanoutShm` | N : M | Non-POD broadcast or many-to-many, via a contention-free per-pair SHM SPSC matrix |

**All four backends are SHM-backed** and share the same lock-free ring; the backend is chosen purely from the publisher/subscriber counts and whether the message type is POD. Latency is *not* a property of the backend — it is set by **core locality**:

| Route | One-way latency |
|-------|-----------------|
| Same thread, self-loop (single handle, `Both` fast path) | ~12 ns (~24 ns per send + recv) |
| Same thread, real publisher + subscriber (one executor thread) | ~19 ns (~39 ns per send + recv) |
| Cross-thread **or** cross-process | ~220 ns |

The jump from ~19 ns to ~220 ns is cache-coherence physics, not a backend choice: once the producer and consumer sit on different cores, the ring's cache line bounces between them — whether the peer is another thread in the same process or a separate process makes no difference. An in-process heap ring would *not* beat SHM here. The fastest realistic route is to co-locate producer and consumer on one executor thread.

**No migration when a process joins.** Every topic is SHM-backed from creation — the creator's PID is stamped into the header at init — so a second *process* can join an existing topic with no data-plane migration and no producer/consumer relocation; it simply mmaps the same region. Migration happens only when the *topology* changes (for example, a second subscriber turns a 1:1 topic into a broadcast, switching `SpscShm` → `PodShm` or `FanoutShm`). That transition takes a CAS-based lock, drains in-flight messages, increments an epoch counter, and updates the backend mode atomically; existing publishers and subscribers adapt within 32 messages (~1-10μs).
...
**macOS topic naming**: `shm_open()` does not allow `/` characters in SHM object names (beyond the leading `/`). Use **dots** instead of slashes: `sensor.imu` not `sensor/imu`. Dots are strongly recommended, not validator-enforced — `/` passes topic-name validation but fails at runtime on macOS `shm_open`, so use dots for portability.
...
| Choice | Benefit | Cost |
|--------|---------|------|
| SHM ring buffers | Zero-copy, sub-microsecond, no kernel | Complex synchronization, platform-specific code |
...
`shm_open()` on macOS does not allow `/` in SHM object names. Always use dots: `sensor.imu` not `sensor/imu`. Dots are strongly recommended, not validator-enforced — `/` passes topic-name validation but fails at runtime on macOS `shm_open`, so use dots for portability.
