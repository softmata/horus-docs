...
flowchart TB
    M["<b>Mission</b><br/>Deliver Package to Room 101"]

    M --> G1["<b>Goal</b><br/>Pick up package"]
    M --> G2["<b>Goal</b><br/>Navigate to destination"]
    M --> G3["<b>Goal</b><br/>Deliver package"]

    G1 --> T1["Task: Navigate to<br/>pickup location"]
    G1 --> T2["Task: Detect package"]
    G1 --> T3["Task: Grasp package"]

    G2 --> T4["Task: Plan path"]
    G2 --> T5["Task: Execute trajectory"]

    G3 --> T6["Task: Navigate to<br/>drop zone"]
    G3 --> T7["Task: Release package"]
...
```rust
use horus::prelude::*; // Provides {Node, Topic, Subscriber, Publisher, SharedMissionPlanner, MissionPlannerBuilder, MissionSpec, MissionStatus}
use std::sync::Arc;
...
impl MissionExecutor {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            mission_sub: Topic::new("missions")?,
            status_pub: Topic::new("mission_status")?,
        })
    }
}
...
impl Node for MissionExecutor {
    fn tick(&mut self) -> Result<()> {
        // Accept new missions
        if let Some(mission) = self.mission_sub.recv() {
...
- Explore [Async Nodes](/python/api/async-nodes) for non-blocking operations
