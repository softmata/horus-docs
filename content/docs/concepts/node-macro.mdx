    ...
    MotorController {
        ...
        tick() {
            if let Some(target) = self.target_velocity.recv() {
                ...
                self.pwm_output.send(output).ok();
            }
        }
    }
}
...
3. **[Master the Topic ](/concepts/core-concepts-topic)** - Understand communication
