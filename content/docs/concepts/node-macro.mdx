...
Tick rate is not a section — it is set on the scheduler when you register the node. See [Tick Rate](#tick-rate-set-on-the-scheduler) below.
...
Three rules matter enough to repeat here; the worked examples are in the Rust
macro reference under
[Best Practices](/rust/api/macros#best-practices), which is where they are
maintained.

- **Name topics for what they carry**, not for where they came from:
  `motors.speed`, not `data`.
- **Keep `tick` fast.** It runs on the scheduler's thread; a `sleep` or a
  blocking read in `tick` stalls every node sharing that executor.
- **Allocate in `init`, not in `tick`.** Reserve buffers once, then reuse them.
...
Compiler errors from `node!` — "cannot find type in scope", "expected `,`, found
`{`", the CamelCase warning — are listed with their fixes in
[Troubleshooting the macros](/rust/api/macros#troubleshooting).

One correction worth stating here, because this page used to say the opposite: a
node name does **not** have to be CamelCase. The macro accepts a snake_case name
and emits the struct verbatim, so what you get is rustc's
`non_camel_case_types` warning and an awkward type name — not a rejection.

```rust,ignore
// Compiles. Warns, and gives you a type called `my_node`.
node! { my_node { tick { } } }
```
