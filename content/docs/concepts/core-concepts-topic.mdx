...
**Live Migration**: paths upgrade immediately when a participant joins, and a clean departure is noticed immediately too — a dropped handle gives its registration back, so `pub_count()` and `sub_count()` reflect who is actually there. The path itself only ever **widens**: a topic that has reached a broadcast backend stays there for the life of its shared-memory segment, because narrowing mid-stream would drop the messages already sitting in the wider ring for whoever is still reading
...
|----------|--------------------|-----------------------|
| 1 publisher, 1 subscriber | point-to-point | newest rejected while a live subscriber drains; oldest retired when nothing is |
| Many publishers, 1 subscriber | multi-producer point-to-point | newest rejected while a live subscriber drains; oldest retired when nothing is |
...
If topology changes (e.g. a second subscriber joins), HORUS automatically migrates to the
optimal path. **A message in flight across that switch can be lost.** The switch happens
under a lock, but nothing counts in-flight operations — there is no such field in the
topic header — so there is nothing to drain against, and the runtime does not claim
otherwise.

Pre-warm a topic to its final topology before it carries traffic that matters: one
`send()` and one `recv()` per participant is enough to resolve the backend, after which
no further migration happens. HORUS's own test suite does exactly this.
