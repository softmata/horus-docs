...
**For introspection**, enable logging at construction time with `.with_logging()` (requires `T: LogSummary`):
```rust
let topic: Topic<MyMsg> = Topic::new("name")?.with_logging();
topic.send(msg)?;  // Automatically records timing and logs to introspection buffer
```
...
```rust
let topic: Topic<MyMsg> = Topic::new("name")?.with_logging();
if let Some(data) = topic.recv() { ... }  // Automatically records timing and logs
```

See [Zero-Overhead IPC Architecture](/concepts/zero-overhead-ipc) for details on `Topic::new()` vs `Topic::new()?.with_logging()`.
