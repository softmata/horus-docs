...
Latency scales linearly with message size, with one exception: `Image`, `PointCloud`, `DepthImage`, `OccupancyGrid` and `CostMap` are pool-backed — only a small fixed-size descriptor (224-440 bytes, depending on the type) crosses the ring, so their latency is independent of payload size.
