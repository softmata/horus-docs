...
unresolved backend answers permissively — to find out which kind of topic you
actually have.
</Callout>
...
jumps forward, usually to the current head, and the skipped messages are lost. The loss is silent —
`dropped_count()` counts send-side failures, and on a broadcast ring the publisher never
fails to send, so it stays at `0`. Keep subscribers fast enough to drain the ring, or use
`send_blocking()` where a gap is unacceptable.
