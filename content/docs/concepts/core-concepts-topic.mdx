...
Attempts to send a message, returning it on failure (buffer full). Useful when you need explicit retry control instead of the automatic retry in `send()`.
...
Enables automatic introspection logging on every `send()` and `recv()`. Records timing and message summaries to the introspection buffer. Only adds overhead when enabled.

```rust
let topic: Topic<MyMsg> = Topic::new("name")?.with_logging();
topic.send(msg);  // Automatically records timing and logs
```
...
let pub_topic: Topic<CmdVel> = Topic::publish(CMD_VEL)?;
pub_topic.send(CmdVel::new(1.0, 0.5));

// Subscriber — same type enforced
let sub_topic: Topic<CmdVel> = Topic::subscribe(CMD_VEL)?;
