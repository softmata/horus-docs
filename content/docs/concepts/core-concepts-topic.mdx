...
```bash
# View HORUS shared memory segments (Linux)
...
**"No space left on device"** — Shared memory directory is full:

```bash
df -h /dev/shm              # Check usage (Linux)
horus clean --shm            # Clean up
sudo mount -o remount,size=4G /dev/shm   # Increase if needed
```

**"Permission denied"**:

```bash
ls -la /dev/shm/horus/       # Check permissions
chmod 755 /dev/shm/horus/    # Fix if needed
```
