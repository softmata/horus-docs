...
Publishes a message to the topic. Fire-and-forget — it returns nothing, so there is no `Result` to handle, but delivery is not guaranteed. On point-to-point topologies (1 publisher / 1 subscriber, or many publishers / 1 subscriber) a full ring **rejects the new message**: `send()` retries briefly (spin, then yield) and then drops it; queued messages are preserved in FIFO order. On broadcast topologies (1 publisher, multiple subscribers) the ring is latest-wins and the **oldest** slot is overwritten instead. Use `try_send()` to detect the drop, or `send_blocking(msg, timeout)` for guaranteed delivery on critical topics.
...
Attempts to send a message, returning it on failure. Unlike `send()`, which silently drops the message after a brief retry, `try_send()` gives you explicit control over failure cases.
...
Low-level receive that bypasses the logging and recording hooks. Marked `#[doc(hidden)]` — intended for internal and test use. Prefer `recv()`, which also takes the same-instance fast path and participates in runtime debug logging.
...
- Inconsistent topic names between publisher and subscriber
...
Auto capacity is always clamped to `[16, 1024]` and rounded up to the next power of two — so it is never an odd number like 73. Slot size tracks the message size for simple fixed-size types; only non-POD (heap-owning) types fall back to the 8 KB default slot.
...
The only per-message-type figure HORUS publishes is `CmdVel` (16B): **~20ns median / ~29ns p99** intra-process. Cross-process latency is measured per backend rather than per message type — SpmcShm is ~198ns p50 (1 publisher / 2 subscribers) rising to ~276ns p50 (1 publisher / 8 subscribers), against a raw shared-memory atomic floor of ~167ns.

Latency scales linearly with message size, with one exception: `Image`, `PointCloud`, `DepthImage`, `OccupancyGrid` and `CostMap` are pool-backed — only a small descriptor (~288-336 bytes) crosses the ring, so their latency is independent of payload size.
