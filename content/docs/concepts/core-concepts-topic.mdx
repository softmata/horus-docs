...
impl Node for Publisher {
    ...
    fn tick(&mut self) {
        ...
        // Send message (zero-overhead, infallible)
...
- Non-blocking, infallible operation - returns immediately
- Uses ring buffer "keep last" semantics - oldest message overwritten when buffer is full
...
`send()` always succeeds using "keep last" semantics — no error handling needed:

```rust
// send() is infallible — oldest message overwritten when buffer is full
