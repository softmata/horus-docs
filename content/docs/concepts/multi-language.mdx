...
Python and Rust nodes communicate through HORUS's shared memory system. Both sides must agree on the topic name and the message type; **typed messages are the recommended choice** for crossing the language boundary, because they take the zero-copy path.
...
For message age or staleness, compare a message's `timestamp_ns` field against `horus.get_timestamp_ns()` — there is no `Node` method for this.
...
3. All processes communicate via shared memory at `/dev/shm/horus_<namespace>/` (default: `/dev/shm/horus_default/`)
...
| **Rate control** | `.rate(100_u64.hz())` per node + `Scheduler::tick_rate()` global | `rate=Hz` per node |
| **Backend hints** | Automatic (topology-based) | Automatic (topology-based) |
| **Message types** | Full `horus_types` + `horus-robotics` (both via `horus::prelude`) | 75 typed message classes (CmdVel, Pose2D, Imu, Odometry, LaserScan, Twist, JointState, …) |
| **Transform Frame** | `TransformFrame::new()` | `TransformFrame()` |
