...
if let Some(temp) = topic.recv(&mut None) {
...
if let Some(pose) = topic.recv(&mut None) {
...
fn main() -> Result<()> {
    let scan_topic = Topic::LaserScan>::new("lidar")?;
    let cmd_topic = Topic::CmdVel>::new("cmd_vel")?;

    loop {
        if let Some(scan) = scan_topic.recv(&mut None) {
            let cmd = plan_path(&scan);  // Your planning logic
            cmd_topic.send(cmd, &mut None).ok();
        }
    }
}
```
