...
- **Ring buffer**: The shared memory is organized as a fixed-size circular buffer (imagine slots arranged in a circle). When all slots are full and a new message arrives, a point-to-point (1:1) topic rejects the new message (keeping the queued ones, in order); a broadcast (multi-subscriber) topic overwrites the oldest. This guarantees the system never runs out of memory and the publisher never blocks — but slow subscribers may miss messages.
...
`send()` **always succeeds**. It never blocks. It never returns an error. If the ring buffer is full (the subscriber hasn't read fast enough), a message is silently dropped: a point-to-point (1:1) topic drops the new message (keeping the queued ones, in order); a broadcast topic overwrites the oldest.
...
<Callout type="tip">
Use `send()` for almost everything: sensor data, velocity commands, state updates, telemetry. It's the fastest option (~12 ns one-way when publisher and subscriber share a thread) and the simplest to reason about.
</Callout>
...
If a publisher sends faster than a subscriber reads, the ring buffer fills up. On a point-to-point (1:1) topic the next `send()` is then dropped (the oldest queued messages are kept, in order); on a broadcast topic the oldest unread message is overwritten. You can detect this:
...
| Scenario | Latency (one-way) | % of 1 ms budget |
|----------|-------------------|-------------------|
| Same thread, self-loop (a node reading its own topic) | ~12 ns | 0.0012% |
| Same thread, publisher + subscriber on one executor | ~19 ns | 0.0019% |
| Different core — cross-thread **or** cross-process | ~220 ns | 0.022% |

Notice the jump from ~19 ns to ~220 ns. It appears the moment the publisher and subscriber sit on **different CPU cores** — whether that's two threads or two separate processes — because the data now has to travel through the cache-coherence fabric between cores. That's physics, not a HORUS overhead, and no backend or layout choice avoids it. The fastest arrangement is to keep a producer and its consumer on the same executor thread (same core, L1-cache-hot). And because every topic is shared-memory backed from the moment it's created, moving a subscriber into a separate process for fault isolation costs the same ~220 ns — the topic is never re-created or migrated.
...
<Callout type="info">
**What's a nanosecond?** Light travels about 30 centimeters (1 foot) in one nanosecond. HORUS can deliver a message between two nodes on the same core in the time it takes light to travel a few meters. This matters because robots have strict timing budgets — every nanosecond spent on communication is a nanosecond not spent on control computation. A motor controller that misses its deadline can cause a robot arm to overshoot its target.
</Callout>
...
During development, all your nodes typically run in one process — often on one executor thread, the fastest path (same core, L1-cache-hot). In production, you might split the camera and controller into separate processes for fault isolation. Every HORUS topic is shared-memory backed from the moment it's created, so splitting nodes across processes needs no reconfiguration — a subscriber in another process simply joins the same shared-memory region, no migration required. On top of that, HORUS picks the right delivery backend automatically from the communication pattern — one-to-one, many-to-one, or one-to-many broadcast — so your code stays identical across every deployment configuration.
