...
// simplified
    ...
    // Publish each edge of the chain (parent → child transform)
    hf.update_transform("base_link", &Transform::from_translation([0.0, 0.0, 0.1]), timestamp_now())?;
  ...
  <LangTab language="Python">
    ...
    # Publish each edge of the chain (parent → child transform)
    tf.update_transform("base_link", Transform(translation=[0.0, 0.0, 0.1]))
    tf.update_transform("camera", Transform(translation=[0.1, 0.0, 0.3]))
