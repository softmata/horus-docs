...
Transform frames live in their own crate, `horus-tf`, but you usually do not have to
name it: `horus` depends on it, and `horus::prelude` re-exports `TransformFrame`,
`TransformFrameConfig`, `Transform` and `FrameBuilder`. To build a frame tree and
query it, a dependency on `horus` alone is enough.

Depend on `horus-tf` directly for the rest of its surface — `FrameId`,
`timestamp_now`, `TransformFramePublisher`, `TransformQuery` — none of which the
`horus` prelude re-exports. The listing below calls `timestamp_now()`, so it takes
the direct dependency. `horus-tf` declares `horus_core` and `horus_macros` as relative path dependencies that do not exist inside its git checkout, so a plain-cargo project has to redirect them itself, in its own `Cargo.toml`:

```toml
# Cargo.toml — a plain-cargo project, not horus.toml
[dependencies]
...
A `horus.toml` project needs none of that: `horus-tf` is one of the build's fixed patch
targets, so the generated `.horus/Cargo.toml` carries that same table on every build. Do
not copy the block into `horus.toml` — `patch` is not a top-level key there, `horus check`
rejects it as unknown, and the table never reaches the build. Extra patch entries of your
own go under `[rust.patch."…"]`.
...
320 ms. To have an out-of-window query reported instead of clamped, use `tf_at_strict`,
which returns `Err(HorusError::Transform(TransformError::Extrapolation { .. }))` when any
hop in the chain would have to extrapolate, or `tf_at_with_tolerance`, which clamps only
while the nearest stored sample is within `tolerance_ns` of the timestamp you asked for.
Both have `_by_id` forms. Raising `history_len` widens the window; it does not tell you
...
```rust
// Default / Small (256 frames, ~2MB)
let hf = TransformFrame::new();       // Same as TransformFrame::small()
let hf = TransformFrame::small();

// Medium (1024 frames, ~8MB)
let hf = TransformFrame::medium();

// Large (4096 frames, ~32MB)
let hf = TransformFrame::large();
```

| Preset | Frames | Static | History | Cache | Memory |
|--------|--------|--------|---------|-------|--------|
| `small()` | 256 | 128 | 32 | 64 | ~2MB |
| `medium()` | 1024 | 512 | 32 | 128 | ~8MB |
| `large()` | 4096 | 2048 | 32 | 256 | ~32MB |

The Memory column is what the constructor allocates, not what the frames in use cost.
Every preset sets `enable_overflow`, and the constructor pushes all `4 × max_frames`
slots up front — each one a `history_len`-long array of 64-byte entries — so `small()`
claims 1024 × 32 × 64 B before you register a single frame.
Divide by four if you turn overflow off.

Additional presets for simulation:

```rust
// Massive (16384 frames, ~128MB) - 100+ robot simulations
let hf = TransformFrame::with_config(TransformFrameConfig::massive());

// Grow-on-demand: max_frames 4096 pre-allocated 4x, so the limit doubles
// into that space and stops at 16384 frames
let hf = TransformFrame::with_config(TransformFrameConfig::unlimited());
```

65536 (`MAX_SUPPORTED_FRAMES`) is the cap applied to that 4x pre-allocation, not
`unlimited()`'s own ceiling — only `massive()` reaches it. Register more frames than
the pre-allocation holds and the call returns `Maximum frame limit (16384) reached
(physical capacity exhausted)`. Every preset on this page sets `enable_overflow`, so
`unlimited()` differs from `large()` only in its chain cache (512 entries against 256).
...
let hf = TransformFrame::with_config(TransformFrameConfig {
    ...
    enable_overflow: false,     // false: hard ceiling at max_frames (2048 here).
                                // true: pre-allocate 4x and let the limit double into it
    chain_cache_size: 256,      // LRU cache for chain lookups
});
```

Or use the builder. `build()` validates the settings and reports a rejection as a
plain `String` — its signature is `Result<TransformFrameConfig, String>`, not a
`HorusError` — so `?` alone will not lift it into a function returning horus's
`Result`. Convert it with `HorusError::config`, or `.expect()` it at startup:
...
At startup, where a hard-coded config that fails validation is a programmer error
rather than a runtime condition, `expect` says the same thing in one expression:
