...
A single `Topic` instance that both sends and receives (`role = Both`) takes an inlined fast path: it moves the value with a plain typed write into the ring, skipping the backend's function-pointer dispatch and the per-slot sequence flags, so nothing is serialized for either kind of type. The ring itself is still shared memory — every HORUS topic is SHM-backed. Communication between two separate `Topic` instances — even inside the same process — goes through the full backend path, so the POD/non-POD difference applies there too.
