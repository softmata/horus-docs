...
description: How HORUS automatically optimizes message transfer for maximum speed
order: 25
---

# Message Performance

HORUS automatically optimizes how messages are transferred between nodes and processes. Standard fixed-size types get **~50ns zero-copy** transfer, while variable-size types get **~167ns serialized** transfer. No configuration is needed — the framework picks the fastest path for each type.
