...
**Latency**: real same-thread pub+sub on one executor thread runs ~39 ns per send+recv (~19 ns one-way), with a single-handle self-loop faster at ~24 ns per send+recv (~12 ns one-way); a cross-thread or cross-process hop is ~220 ns one-way. Every topic is SHM-backed from creation, so a cross-process consumer can join later with no migration. The latency cliff is core locality, not intra-vs-shared-memory: a producer and consumer on the same core stay L1-hot (~12–19 ns one-way), while a different core — another thread or another process — costs ~220 ns of cache-coherence physics that no backend choice can avoid. See [Topics — Full Reference](/concepts/core-concepts-topic) for the four shared-memory backends (SpscShm, MpscShm, PodShm, FanoutShm) and how HORUS picks between them.
...
Services use two internal topics (`{name}.request` and `{name}.response`). Actions use topics for goal submission, feedback, and results. This is not an implementation shortcut — it is a deliberate design choice. Topics already handle all the hard problems: cross-process shared memory (every topic is SHM-backed from creation, so a cross-process consumer can join with no migration), automatic backend selection, zero-copy for large data. Building services and actions on top means they inherit all of that infrastructure for free. It also means a single debugging tool (`horus topic list`) shows all communication in the system, including service and action traffic.
