...
Topics and Actions work locally (shared memory) and across machines (network transport via configuration).
...
- **Non-blocking send**: `send()` returns `()` — it never returns an error. On a full ring it retries briefly (spin + yield) and then drops the **newest** message, preserving the queued oldest ones in FIFO order (drop-newest). Broadcast paths (`PodShm`/fanout) instead overwrite the oldest slot. Use `try_send()` to get the message back on failure, or `send_blocking(msg, timeout)` when loss is unacceptable.
