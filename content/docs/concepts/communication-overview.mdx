...
**Latency**: tens of nanoseconds (same-process shared memory) to ~167 ns (cross-process), depending on topology. Real topics are SHM-backed even same-process (so a cross-process consumer can join later); the ~3 ns in-process heap path applies only to in-memory-only topics. See [Topics — Full Reference](/concepts/core-concepts-topic) for the 10 automatic backend paths.
