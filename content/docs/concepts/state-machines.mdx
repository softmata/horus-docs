...
**For many users: Yes, eventually.** State machines are great for managing robot modes.

**When this page IS useful:**
- Robot has distinct modes (idle, moving, charging, emergency)
- Need clean transitions between behaviors
- Want structured behavior management

**When to skip:**
- Your robot just runs one continuous loop
- You're just getting started with HORUS

**Quick answer:** Read this once your robot grows beyond a simple read-sensor-act loop. It's a powerful pattern but not required for simple robots.
...
fn main() -> Result<(), StateMachineError> {
    ...
    fsm.process_event(&Event::new("start"))?;
...
fsm.process_event(&Event::new("toggle"))?;
...
fsm.process_event(&Event::new("next"))?;  // -> zero_sensors
fsm.process_event(&Event::new("next"))?;  // -> test_motors
fsm.process_event(&Event::new("next"))?;  // -> verify
fsm.process_event(&Event::new("next"))?;  // -> complete
```

### Mode Selector Machine

Topic-and-spoke pattern for mode selection:
...
```rust
use horus::prelude::*; // Provides {Node, Topic, Subscriber, StateMachine, StateMachineBuilder, Event}
...
impl ModeController {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            cmd_sub: Topic::new("mode_commands")?,
        })
    }
}
