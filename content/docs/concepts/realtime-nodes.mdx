...
<Callout type="info" title="Do I Need Real-Time Nodes?">

**Most projects don't need this.** Real-time nodes are for specialized applications where:
- Missing a deadline could cause physical harm (surgical robots, industrial machinery)
- You need guaranteed response times (not just "fast" response times)
- You're building safety-critical systems that require certification

**If you're:**
- Learning HORUS → Use regular nodes
- Building a hobby robot → Use regular nodes
- Prototyping → Use regular nodes
- Building a product that controls dangerous machinery → Read this page

Regular nodes already achieve sub-microsecond latency. Real-time nodes add **guarantees**, not speed.
...
```rust
use horus::prelude::*; // Provides {Scheduler, RtNode, RtPriority, RtClass, SchedulerConfig, Node, NodeInfo, Result}
use std::time::Duration;
