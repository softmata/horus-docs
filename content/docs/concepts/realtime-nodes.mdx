...
```rust
use horus::prelude::*; // Provides {Scheduler, Node, Miss, RtStats, DurationExt, RtConfig, RtScheduler}
...
The `Node` trait has two safety hooks. They are ordinary runtime methods — there are
no `debug_assert!`-gated pre/post-condition contracts:
...
two real knobs: a failure policy for nodes that error out, and `Miss::SafeMode`
