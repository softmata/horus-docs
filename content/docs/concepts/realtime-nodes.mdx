...
description: Build time-critical robotics applications with per-node timing constraints and deadline monitoring
---
...
Real-time nodes give a node an explicit timing contract — a period, a tick budget and a
deadline — plus a declared response when it is missed. Reach for them when:
- A loop has to run at a fixed rate, and a late tick is a fault rather than a delay
- A hung node has to be *detected* rather than waited on
- A deadline miss needs a defined response (warn, skip, safe state, stop) instead of silent drift

The contract is enforced, not guaranteed: HORUS measures overruns and reacts to them, but nothing
in the repository bounds worst-case latency. See
...
scheduler.add(YourControlNode::new())
    .order(0)                            // No effect here: .rate() moves this node
                                         // off the main tick loop to the RT executor
...
`Scheduler::watchdog(Duration)`, shown under [Watchdog and Safe-Mode Configuration](#watchdog-and-safe-mode-configuration).
...
### Watchdog and Safe-Mode Configuration

For a fixed-rate loop that must fail into a safe state rather than drift:

```rust
use horus::prelude::*;
use std::time::Duration;

// 1 kHz tick, RT where the OS allows it, watchdog armed
let mut scheduler = Scheduler::new()
    .tick_rate(1000_u64.hz())
    .prefer_rt()        // mlockall + SCHED_FIFO where available
    .watchdog(Duration::from_millis(100));

scheduler.add(ControlLoopNode::new())
    .order(0)
    .rate(1000_u64.hz())
...
run only when one node misses its deadline `max_deadline_misses` times **consecutively**
(default 100). The count is per node, and any tick that meets its deadline resets it, so a
node that misses intermittently and recovers never reaches the ceiling. A process-wide
lifetime total still exists, but it only feeds the timing report.
...
| `max_deadline_misses` (default 100) | **Emergency stop**, and the node is killed — `shutdown()` called and it is removed from execution permanently |

The last rung is the `.max_deadline_misses(n)` ceiling, so the ladder's end and the
emergency stop are the same event. They have to be: killing a node removes it from
execution for good, so a rung that killed it *below* the ceiling would mean the node could
never miss again and the ceiling could never be reached. Setting `n` moves this rung.

A node that recovers is restored to its original rate after 100 consecutive ticks that
**meet their deadline** at the reduced rate; a single miss puts that count back to zero.
A node that reaches the ceiling does not come back for the life of the process.
...
|---------|-----------|-------|
| `Scheduler::watchdog()` | 1ms | Timeouts are rounded **up** to a whole millisecond, and the adjusted value is logged. Rounding up on purpose: a looser watchdog is a safety feature, a tighter one halts a healthy robot |
...
fn main() -> Result<()> {
    // 1 kHz tick rate; per-node budget and deadline are enforced below
    let mut scheduler = Scheduler::new().tick_rate(1000_u64.hz());
