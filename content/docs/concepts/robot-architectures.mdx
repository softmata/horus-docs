...
```rust
use horus::prelude::*; // Provides {Scheduler, JoystickNode, DifferentialDriveNode, DcMotorNode, Result}

fn main() -> Result<()> {
    let mut scheduler = Scheduler::new();

    // Input: Joystick (order 0 - runs first)
    let joystick = JoystickNode::new()?;
    scheduler.add(joystick).order(0).done();

    // Control: Differential drive (order 1)
...
```rust
use horus::prelude::*; // Provides {Scheduler, EmergencyStopNode, LidarNode, CollisionDetectorNode, PathPlannerNode, DifferentialDriveNode, BldcMotorNode, EscProtocol, Result}

fn main() -> Result<()> {
    let mut scheduler = Scheduler::new();

    // SAFETY LAYER (order 0) - Always runs first!
    let estop = EmergencyStopNode::new("cmd_vel")?;
    scheduler.add(estop).order(0).done();

    // SENSOR LAYER (order 1)
    let mut lidar = LidarNode::new()?;
    lidar.configure_serial("/dev/ttyUSB0", 115200);
    scheduler.add(lidar).order(1).done();

    // PERCEPTION LAYER (order 2)
...
```rust
use horus::prelude::*; // Provides {Scheduler, LidarNode, ImuNode, OdometryNode, LocalizationNode, Result}

fn main() -> Result<()> {
    let mut scheduler = Scheduler::new();

    // SENSORS (order 1 - parallel execution)
    let mut lidar = LidarNode::new()?;
    ...
    scheduler.add(localization).order(2).done();
...
```rust
use horus::prelude::*; // Provides {Scheduler, SafetyMonitorNode, CameraNode, YOLOv8DetectorNode, DifferentialDriveNode, Result}
...
fn main() -> Result<()> {
    ...
    scheduler.add(camera).order(1).done();
...
- **[Topic Communication](/concepts/core-concepts-topic)** - How pub/sub works
- **[API Reference](/rust/api)** - Core API documentation
