...
<Callout type="warning" title="The public registry is not running yet">
There is no public HORUS package registry at the moment, so `horus search`,
`horus install <name>` and `horus publish` have nothing to talk to. `horus search`
returns local results only and says so; the registry commands fail with an
explanation rather than a network error.

**What still works, unchanged:** `path` and `git` dependencies in `horus.toml`,
and installing from crates.io and PyPI. Those cover everything on this page
except the registry-hosted flows.

**Self-hosting works too.** Set `HORUS_REGISTRY_URL` to your own registry and
every command below behaves exactly as documented. Examples on this page use
`https://registry.example.com` to stand in for that URL.

Progress: [softmata/horus#173](https://github.com/softmata/horus/issues/173).
</Callout>
...
|--------|-------------|---------|
| **HORUS Registry** | Curated robotics packages — *not yet available, see above* | `horus install pid-controller` |
...
# Fix: Install working version into this workspace
rm .horus/packages/serde  # Remove symlink to broken global
horus install serde@1.0.150 -t my_robot_project

# Result:
.horus/packages/serde/  # Real directory — only with -t
# horus run will use this, ignoring broken global!
```

`-t` is what makes the copy local. Without it the install goes to the global
cache as `cratesio_serde@1.0.150` and `.horus/packages/serde` is recreated as a
symlink to it.
...
> **Note**: Detailed package information can be viewed on the registry web interface, when one is configured.
...
Installing needs no login, but it is not unconditional. Once you have trusted a
publisher key for a package (`horus auth trust-publisher`, see the [CLI
reference](/development/cli-reference#horus-auth-login-to-registry)), that
package must verify against it: an unsigned download or a signature that does
not match is refused outright, with no override flag. Trusting the key is the
statement that the package must be signed, so an unsigned download is a
downgrade rather than a fallback. With no key on file, a signed package installs
behind a warning that the signature could not be checked.
...
Visit: https://registry.example.com/dashboard/keys
...
# Visit https://registry.example.com
...
Published: my-awesome-package@1.0.0
  View at: https://registry.example.com/packages/my-awesome-package
```
...
Published: my-sensor-package@1.0.0
   View at: https://registry.example.com/packages/my-sensor-package
```
...
`horus publish` sends each link only when you supplied one, as separate form
fields: `source_url`, and `docs_url` paired with a `docs_type` of either
`external` or `local`. `source_url` falls back to `repository` in your
`horus.toml` when you skip the prompt; the documentation fields have no such
fallback.

How a registry renders those fields is up to that registry — the CLI's
responsibility ends at uploading them:

- **Documentation** — sent as `docs_url` + `docs_type`. `external` is a URL you
  own; `local` means the `/docs` markdown was bundled into the tarball itself,
  so it travels with the package rather than being fetched.
- **Source** — sent as `source_url`. Any repository host works; GitHub, GitLab
  and a plain public URL are all accepted verbatim.
...
**Search packages:**
```bash
curl 'https://registry.example.com/api/packages/search?q=sensor'
```

**Get package info:**
```bash
curl https://registry.example.com/api/packages/pid-controller
```

**Download package:**
```bash
curl -o pkg.tar.gz https://registry.example.com/api/packages/pid-controller/1.2.0/download
```
