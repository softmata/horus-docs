...
# Install globally (run outside a workspace, then pick
# "Global (~/.horus/cache)" in the interactive target selector)
...
**System-wide** (chosen from the interactive target selector, which appears when you install outside a workspace):
```
~/.horus/
── cache/
    ...
    ── git_abc123/                   # Git dependencies (via horus.toml)
```
...
#    Create symlink: .horus/packages/serde -> ~/.horus/cache/cratesio_serde@1.0.228/
#    Disk efficient!
...
~/.horus/cache/cratesio_serde@1.0.228/  # Corrupted
...
Location: ~/.horus/cache/cratesio_ripgrep@14.0.0/
```
...
The metadata prompts are skipped when stdin is not a TTY or when you pass
`--dry-run`; in those cases the values recorded in `horus.toml` are used as-is.
...
- **[Environment Management](/package-management/environment-management)** - Pin dependencies with `horus.lock` and share the global cache
