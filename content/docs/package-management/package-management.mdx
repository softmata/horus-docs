...
List installed packages. To search the registry, use
[`horus search <QUERY>`](#horus-search) instead — `horus list` takes no query
argument.
...
2. **Complete `horus.yaml`** metadata:
   ```yaml
...
**Options:**
- `--freeze` - Also generate a `horus-freeze.yaml` file after publishing
...
```bash
# 1. Update version in horus.yaml
# version: "1.1.0"
...
dependencies:
  ...
  - name: motion-planner
    version: "^2.0"             # Compatible (2.x.x, not 3.0.0)
  - name: sensor-drivers
    version: "~1.5.0"           # Patch updates (1.5.x)
```
