...
horus install serde -v 1.0.200

# Install globally (share across all projects)
horus install sensor-drivers -g
```
...
horus install serde -v 1.0.150  # Install working version locally
...
# From HORUS registry
horus install pid-controller
horus install motion-planner -v 2.0.1
...
horus install numpy
horus install opencv-python -v 4.8.0
horus install torch

# Global installation
horus install serde -g
...
> **Note**: Detailed package information can be viewed on the registry web interface at https://registry.horusrobotics.dev
...
3. Token saved to `~/.horus/auth.json`
4. Ready to publish!
...
Token saved to ~/.horus/auth.json
...
Credentials file: `~/.horus/auth.json`

```json
{
  "api_key": "horus_key_abc123def456...",
  "registry_url": "https://registry.horusrobotics.dev",
  "cloud_url": "https://cloud.horus.dev",
...
Generated API key:
  horus_key_abc123def456ghi789jkl012mno345pqr678stu901
...
```bash
# Set for session
export HORUS_API_KEY=horus_key_abc123...
...
echo '{"api_key":"horus_key_abc123..."}' > ~/.horus/auth.json
```
...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                sh 'horus run --build-only --release'
            }
        }
...
```bash
# Verify permissions
ls -la ~/.horus/auth.json
# Should show: -rw------- (600)

# Fix if needed
chmod 600 ~/.horus/auth.json
```

Backup:
```bash
# Backup credentials (encrypted)
gpg -c ~/.horus/auth.json
# Creates ~/.horus/auth.json.gpg

# Restore
gpg -d ~/.horus/auth.json.gpg > ~/.horus/auth.json
chmod 600 ~/.horus/auth.json
```
...
# 3. Test new key
export HORUS_API_KEY=horus_key_new_key...
horus auth whoami
...
# Visit https://registry.horusrobotics.dev
...
mv ~/.horus/auth.json ~/.horus/auth.json.account1

# Login with second account
horus auth login

# Switch back
mv ~/.horus/auth.json ~/.horus/auth.json.account2
mv ~/.horus/auth.json.account1 ~/.horus/auth.json
```
...
**Options:**
- `--freeze` - Also generate a `horus-freeze.toml` file after publishing
- `--dry-run` - Validate package without actually uploading
...
Published: my-awesome-package@1.0.0
  View at: https://registry.horusrobotics.dev/packages/my-awesome-package
```

After uploading, HORUS interactively prompts for optional metadata (categories, package type, documentation, source repository).
...
Published: my-sensor-package@1.0.0
   View at: https://registry.horusrobotics.dev/packages/my-sensor-package
...
**Version constraints when installing:**
```bash
...
Dependencies live in your native build files. For Rust projects, add to `Cargo.toml`:
...
For HORUS registry packages, use `horus add`:
```bash
horus add pid-controller
horus add motion-planner
```

See [Configuration Reference](/package-management/configuration) for details on project configuration.
...
horus install pid-controller -v 1.3.0

# Check available versions on registry
horus list pid-controller
```
...
```bash
# Check spelling
horus list nonexistent

# Search registry for correct package name
horus list correct-package
```
...
horus install sensor-fusion -v 2.0.0  # Install compatible version

# Option 2: Pin version in your native build file (Cargo.toml or pyproject.toml)
```
...
ping registry.horusrobotics.dev
...
**Search packages:**
```bash
curl https://registry.horusrobotics.dev/api/packages?q=sensor
```

**Get package info:**
```bash
curl https://registry.horusrobotics.dev/api/packages/pid-controller
```

**Download package:**
```bash
curl -o pkg.tar.gz https://registry.horusrobotics.dev/api/packages/pid-controller/1.2.0/download
```
