...
**System-wide** (chosen via the interactive target selector, or when installing outside a workspace):
```
~/.horus/
── cache/
...
> **Note**: There is deliberately **no `HORUS_API_KEY` environment variable** — nothing in HORUS reads one. Credentials always come from `auth.json`. For non-interactive environments, write that file from your secret store (see [API Keys](#api-keys-for-cicd)).
