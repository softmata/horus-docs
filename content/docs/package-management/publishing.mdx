...
# Add a co-owner
horus owner add my-lidar-driver teammate

# Remove an owner
horus owner remove my-lidar-driver former-teammate

# Transfer ownership to another user
horus owner transfer my-lidar-driver new-maintainer
  ...
  publish:
    ...
    steps:
      ...
      - name: Authenticate
        run: |
          mkdir -p ~/.horus
          printf '{"api_key":"%s"}\n' "$HORUS_REGISTRY_KEY" > ~/.horus/auth.json
        env:
          HORUS_REGISTRY_KEY: ${{ secrets.HORUS_REGISTRY_KEY }}
      - name: Publish
        run: horus publish
```

`horus publish` reads the key from `~/.horus/auth.json` — there is no API-key
environment variable, so CI has to write that file from a secret.

Generate the API key:
```bash
horus auth api-key --name github-actions --environment ci-cd
# Save the key as a GitHub secret: HORUS_REGISTRY_KEY
```
