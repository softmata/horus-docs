...
<Callout type="warning" title="The registry commands on this page need a registry">
...
`horus.toml` (see [Configuration](/package-management/configuration)), or point
...
<Callout type="info" title="What the other robot reproduces">
...
flowchart LR
    S["<b>Sensor</b><br/>.order(2)"] -->|"raw"| F["<b>Filter</b><br/>.order(3)"]
    F -->|"filtered"| C["<b>Controller</b><br/>.order(4)"]
    C -->|"cmd"| A["<b>Actuator</b><br/>.order(5)"]
`}
  caption="Prebuilt nodes compose over topics — each stage only needs the message type, not the node that produced it"
/>
