...
<Callout type="warning" title="`horus` is not on crates.io">
...
<Callout type="warning" title="`horus.lock` does not yet make this reproducible">

The lockfile records what resolution picked; no install path reads it back, so
the other robot resolves its own versions from `horus.toml` and can land on
different ones. Until enforcement lands, write exact versions in `horus.toml`
for anything that must not move — that is the constraint installs actually obey
— and pin the HORUS release itself with `HORUS_VERSION` when you install on the
robot. See [Environment
Management](/package-management/environment-management#quick-start).
