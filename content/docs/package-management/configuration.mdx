...
### `repository`, `categories`, `package-type` (Optional)

Registry metadata. `horus publish` prompts for all three, and these keys are the
defaults it offers — on a non-interactive publish (CI) there are no prompts, so
what the manifest records is what gets sent. Recording them here is how a CI
publish keeps the same metadata as one you ran by hand.

**Examples:**
```toml
repository = "https://github.com/acme/warehouse-navigation"
categories = ["navigation", "slam"]
package-type = "node"
```

`package-type` is one of `node`, `driver`, `tool`, `algorithm`, `model`,
`message`, `app`.
...
| Key | Type | Purpose |
|-----|------|---------|
| `type` | string | Crate target type — `bin` (default), `lib`, or `both` |
| `standard` | string | C++ language standard for the generated CMake build — `"c++17"` (the default), `"c++20"`, `"c++23"` |
| `rust_edition` | string | Rust edition; `[rust].edition` overrides it |
| `edition` | string | Manifest schema edition, for forward compatibility. Currently only `"1"` |
...
|----------|-------|----------------|
| `"*"` | Any version — the newest available | Everywhere. This is what `horus add` writes |
...
`horus new` writes no `[dependencies]` table at all — empty sections are omitted
from the generated manifest. The first `horus add` creates it.
...
**`workspace`** (optional):
- Boolean; inherit version, source, and features from `[workspace.dependencies]`
- Only meaningful in a workspace member. See [Workspaces](#workspace-optional)

**`lang`** (optional):
- Which language's build the dependency belongs to; `"cpp"` is the value that
  does anything today
- `lang = "cpp"` forces a dependency into the generated CMake build. HORUS
  otherwise guesses from `source = "system"`, a `cmake_package` override, or its
  table of known C++ libraries

**Resolution Order:**
1. HORUS registry (set `HORUS_REGISTRY_URL`; there is no public default yet — see [#173](https://github.com/softmata/horus/issues/173))
...
[dependencies]
horus-robotics = { version = ">=0.4.0", source = "pypi" }
numpy = { source = "pypi" }
...
<Callout type="warning" title="`>=0.4.0` does not resolve yet — and that is the point">

The newest `horus-robotics` on PyPI is **0.1.9**. It speaks an older
shared-memory format than a 0.4.x CLI, so a looser floor — `>=0.2`, or no
constraint at all — installs a package that imports cleanly, runs cleanly, and
whose nodes publish topics the CLI cannot see. Nothing reports it;
`horus topic list` just shows nothing.

The floor turns that silence into a resolver error you can read. Until 0.4.x
reaches PyPI, install the bindings from the tree the installer already cached,
which is the same tag as your CLI by construction:

```bash
pip install ~/.horus/cache/horus@0.4.0/horus_py
```

Substitute your own version — `horus --version` prints it. See
[Python Support](/getting-started/installation#python-support).
...
| `terra` | string | Legacy source key: driver shortname — prefer `use` |
| `source` | string | Where to resolve the package from |
| `simulated` | bool | Alias of `sim` — substitute the simulated stub under `horus run --sim`. This is the key `horus add --driver --source sim` writes. An explicit `sim` wins if both are present |
...
name to a shell command, run with `horus scripts <name>` or with `horus run <name>`. The bare form `horus <name>` does **not** work — an unknown first argument is resolved as a plugin, never as a script, so it fails with clap's usage message.
...
horus run calibrate           # the same script, via `run`
horus scripts                 # list what is available
```

<Callout type="note" title="A script name shadows a file of the same name">

Before it looks for anything to build, `horus run` takes a single bare argument — one
containing no `.` and no `/` — and checks it against the `[scripts]` table of
`horus.toml`. On a hit it runs the script and stops there. So with `calibrate` defined
above, `horus run calibrate` runs the Python tool even if a directory named `calibrate`
sits next to the manifest; the directory is never reached. Arguments carrying a dot or a
path separator — `horus run nodes/main.py`, `horus run main.rs` — skip the check.
...
**Every `post_` hook runs on both the success and the failure path.** That is
deliberate: the moment you most need an arm parked, a bus released, or a motor
controller de-energized is the moment the run died unexpectedly — or the test aborted
half way through. A teardown that only ran after clean exits would be missing from
...
<Callout type="warning" title="`horus launch` does not read `[network]`">

HORUS applies this table by translating it into `HORUS_NET_*` variables on the child
process. Every `horus run` form does this — a bare `horus run`, a `horus.toml` run, a
directory run, a `[workspace]` run and a multi-file run alike.

`horus launch` does not: it sets no `HORUS_NET_*` variable, so a launch file's processes
get the defaults no matter what `[network]` says. Set the variables in the environment
there:

```bash
HORUS_NET_ENABLED=1 HORUS_NET_IMPORT=auto horus launch robot.launch.yaml
```

That is the reliable path in every case: the translation deliberately does not overwrite
a variable you set yourself, so an explicit `HORUS_NET_*` always wins over the manifest.
...
[network.safety]
heartbeat_ms     = 100             # safety heartbeat interval (link-loss detection), not discovery
...
`optimize` names **optimizers**, not topics. Four names are recognised, but only three are installed — `delta` is refused:

| Optimizer | What it does |
|-----------|--------------|
| `fusion` | Coalesces multiple small updates into one datagram |
| `delta` | **Not installed.** Recognised but refused — it encodes on the send side and has no decoder, so enabling it would silently drop or corrupt replicated data. Naming it prints `[horus_net] Optimizer 'delta' is not implemented on the receive side and would silently drop or corrupt replicated data. Ignoring.` to stderr and installs nothing |
...
The two thresholds are **not configurable today**, and not from `horus.toml` at all.
`spatial_radius` and `predict_threshold` are not manifest keys: they are fields of an
in-memory `TopicNetConfig` that has no `Deserialize` impl, held in a per-topic override
map that is created empty at every construction site and filled nowhere outside the test
suites. There is no `[network.topic.*]` table either — `[network]` accepts exactly six
keys, so writing `[network.topic.lidar]` produces the unknown key `network.topic`, which
`horus check` counts as an error and exits non-zero on. The optimizers are always built
with `SpatialOptimizer::new()` (15 m) and `PredictOptimizer::new()` (0.01); the
`with_radius()` / `with_threshold()` constructors that would honour an override are
...
same message crate. A member opts in with `workspace = true`:

```toml
# perception/horus.toml
[dependencies]
nalgebra = { workspace = true }
horus-robotics = { workspace = true, features = ["telemetry"] }
```

`workspace = true` is what performs the inheritance — a member entry without it is
used as written, which is how two members end up on different versions. HORUS emits
it as `<name>.workspace = true` into the generated member `Cargo.toml`, so cargo
resolves it: a name absent from `[workspace.dependencies]` is a cargo error, and any
`features` the member lists are added to the workspace entry's rather than replacing
them.
...
Simulation asset dependencies. The table is parsed and its version strings are
validated like any other dependency's, but nothing in the CLI installs it — it is
reserved for simulator tooling that does not ship with HORUS. There is no
`horus sim` subcommand; `horus run --sim` launches the `horus-sim3d` plugin,
which resolves its own assets.
...
| `rustflags` | array | Flags handed to every `rustc` invocation. Cargo.toml has no rustflags section, so this one is written to a generated `.cargo/config.toml` at the project root rather than into `.horus/Cargo.toml`. An exported `RUSTFLAGS` still wins |
