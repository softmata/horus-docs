...
Declares the hardware nodes your robot has. Each entry names a driver; `horus run`
starts them alongside your own nodes, so a driver you list here does not need to be
constructed in your code.

```toml
[hardware]
camera = "opencv"        # backend name
lidar  = true            # enable with the default backend
imu    = false           # declared, but not started
```

The three forms above are the shorthand. For anything that needs parameters, use a
sub-table:

```toml
[hardware.arm]
package = "ur5e-driver"        # registry package providing the driver
node    = "ArmDriver"          # node inside that package
topic_command = "arm/cmd"      # topic the driver subscribes to
topic_state   = "arm/state"    # topic the driver publishes on
sim     = true                 # use the simulated backend under `horus run --sim`

[hardware.arm.params]          # free-form; passed through to the driver
port     = "/dev/ttyUSB0"
baudrate = 115200
```
...
| Field | Type | Purpose |
|-------|------|---------|
| `package` | string | Registry package that provides the driver |
| `node` | string | Node within that package |
| `crate_name` | string | Rust crate name, when it differs from `package` |
| `pip` | string | PyPI distribution, for Python drivers |
| `source` | string | Where to resolve the package from |
| `exec` | string | Run an external executable as the driver |
| `args` | array | Arguments passed to `exec` |
| `topic` | string | Single topic, for drivers that only publish or only subscribe |
| `topic_state` | string | Topic the driver publishes state on |
| `topic_command` | string | Topic the driver takes commands from |
| `use_name` | string | Node name to register under, if not the table key |
| `sim` | bool | Substitute the simulated backend under `--sim` |
| `params` | table | Driver-specific parameters, passed through untouched |

### `[drivers]` and `[sim-drivers]` (Deprecated)

`[drivers]` is the former name of `[hardware]`, and `[sim-drivers]` the former way to
override a driver for simulation. Both are still read, so existing projects keep
working, but new projects should use `[hardware]` with `sim = true` on the entries that
have a simulated backend. Keeping the real and simulated definition in one entry is
what makes `horus run --sim` a flag rather than a second copy of the manifest to keep
in step.
...
name to a shell command, runnable as `horus <name>` or `horus scripts <name>`.
...
optimize = ["lidar/points"]        # topics to prioritize for bandwidth
...
on_link_lost     = "safe"          # what to do then: "safe", "warn", or "ignore"
```
...
`on_link_lost = "safe"`, nodes that depend on a remote topic enter their safe state
...
Local safety mechanisms — `SafetyMonitor`, watchdogs, `on_link_lost = "safe"` — are
...
[cpp]
compiler   = "clang++"
toolchain  = "arm-linux-gnueabihf"           # cross-compilation target
...
| `toolchain` | string | Cross-compilation toolchain prefix |
