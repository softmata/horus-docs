...
Project metadata lives under the `[package]` table. These keys are never written at the top level of the file.
...
Both `language` and `languages` are rejected as unknown keys — remove them and let detection do the work.
...
The Python distribution is published on PyPI as `horus-robotics` and imported as `horus`. Give every Python dependency `source = "pypi"` — without it they default to the HORUS registry and are skipped when the generated `pyproject.toml` is written.
