...
<Callout type="warning" title="`[network]` only reaches multi-file `horus run` invocations today">

HORUS applies this table by translating it into `HORUS_NET_*` variables on the child
process — and it does that only when **several file paths** reach `horus run` on the
command line (`horus run a.rs b.rs`, or a glob matching more than one file). A
`horus.toml` run, a `[workspace]` run, a directory run and a bare `horus run` all resolve
to a single executable and never perform the translation, so `[network]` is parsed and
then has no effect.
