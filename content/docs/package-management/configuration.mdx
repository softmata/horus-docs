...
— `horus run` warns when it finds one in that situation.
</Callout>
...
```toml
[[schema]]
url = "https://docs.horus-registry.dev/schema/horus.toml.schema.json"
include = ["**/horus.toml"]
```
...
The schema is generated from the same Rust structs `horus check` validates against and is
regenerated on every release, so the editor and the CLI cannot disagree about which keys
exist. A vendored copy does go stale — its `$id` names the URL of the current one.
