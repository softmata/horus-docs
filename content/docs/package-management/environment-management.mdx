...
<Callout type="warning" title="`horus.lock` is recorded, not yet enforced">

`horus lock` writes the versions resolution picked, and `horus lock --check`
verifies the file is current without rewriting it. What it does **not** do yet is
make an install obey it: no install path reads the `[[package]]` entries back.
The pip, crates.io and registry installers still take their versions from
`horus.toml` and fall back to `latest`, so checking out a committed `horus.lock`
on a clean machine can install different versions than the ones it records — and
the next `horus lock` there overwrites the committed pin with whatever `latest`
resolved to on that machine.

`checksum` is likewise written (always empty today) and never compared against a
download. The one field that is consulted is `config_hash`, which decides whether
resolution can be skipped.

So: treat `horus.lock` as a record of what **was** resolved, not a guarantee of
what **will** be. For a dependency that must not move, write an exact version in
`horus.toml` — that is the constraint installs actually read.
