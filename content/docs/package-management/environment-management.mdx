...
There is a second root. `horus install` writes the XDG cache above, but `horus run`'s
own dependency installer writes `~/.horus/cache` for both PyPI and crates.io packages.
Every reader searches both roots, so a package installed by either command is found by
the other — but if you are clearing a cache by hand, or looking for where something
landed, check both.
...
# Option 3: Clear the cache `horus run` fills and let it reinstall
rm -rf ~/.horus/cache/cratesio_serde@1.0.228/
horus run  # Auto-reinstalls into ~/.horus/cache
```
...
**Always commit the lockfile.** It is the record of what resolved.
`horus lock --check` in CI compares the pins against the packages actually
installed under `.horus/packages` and exits non-zero on drift, naming each
package. It does not read `horus.toml`, so it will not catch a manifest edit on
its own — run `horus lock` after changing dependencies:
```bash
# Add to git
