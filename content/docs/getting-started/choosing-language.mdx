...
def experimental_tick(node):
    ...
    input_val = node.get("sensor") or 0.0
    strategy = "aggressive"
...
def ml_tick(node):
    sensor_data = node.get("sensor_data")
