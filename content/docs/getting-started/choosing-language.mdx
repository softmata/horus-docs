...
| Control loop (1kHz) | Above the measured ~530 Hz tick ceiling | Easy | — |
| Control loop (10kHz) | No | Achievable | — |

Python figures come from `horus_py/benchmarks/README.md` (Python 3.12, Linux x86_64, WSL2); Rust figures from the performance table in the project `README.md` (Intel i9-14900K). Intra-process ring-buffer sends measure 3-23ns p50 on a third rig (`benchmarks/README.md`, Intel i7-10750H). These are not all the same machine, so treat the ratio as indicative rather than exact.
