...
```bash
horus new temperature_pipeline --rust
cd temperature_pipeline
```

`--rust` picks plain Rust without macros, which is what the listing below uses. Leave the flag off
...
impl Node for SensorNode {
    ...
    fn init(&mut self) -> Result<()> {
        // Either place works: HORUS resolves the owning node lazily, on the
        // first send()/recv() inside tick(), so a topic built in new() is still
        // attributed to this node in `horus monitor`. This walkthrough builds
        // them in init() to keep the steps separate; `horus new` and the
        // examples/ tree build their topics directly in new().
...
fn main() -> Result<()> {
    ...
    // Run the scheduler: it calls init() on every node, then ticks them in
    // dependency order worked out from the topics they publish and subscribe
    // to (blocks until Ctrl+C)
    scheduler.run()?;
...
**Dependency Ordering**: FilterNode subscribes to what SensorNode publishes, so the scheduler runs it after — the `.order()` numbers are not what produces this
