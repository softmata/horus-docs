...
The simulation is deterministic, so these values are exactly what you'll see. Only
`println!` output is shown above - `hlog!` writes to stderr, so the terminal also
interleaves `[INFO] [SensorNode] Published raw temp: ...` and
`[INFO] [FilterNode] Filtered: ...` lines, plus the scheduler's own startup output.
...
### Metrics Tab
- **IPC Latency**: ~10ns p50 for these 1-pub/1-sub in-process topics (SpscIntra backend; sub-microsecond!)
...
// 2. In main(), add two sensors with distinct names
