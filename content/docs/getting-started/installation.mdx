...
- Builds core libraries and caches them in `~/.horus/cache/`:
  ...
  - `horus_library` - Standard message types and hardware nodes
...
- `cuda` - CUDA IPC for cross-process GPU memory sharing (requires NVIDIA GPU)

> **Note:** Most users should use `horus run --enable <capability>` or configure `enable:` in `horus.yaml` rather than setting cargo features manually.
...
### Available Python Hardware Nodes

With hardware features enabled, HORUS provides ready-to-use Python nodes:

- `SerialNode` - Generic serial port communication
- `CameraNode` - Camera capture (OpenCV)
- `LidarNode` - LiDAR sensor integration
- `ImuNode` - IMU sensor reading
- `GpsNode` - GPS receiver
- `JoystickNode` - Gamepad/joystick input
- `KeyboardNode` - Keyboard input

Additional hardware drivers (CAN bus, SPI, I2C, Dynamixel servos, stepper motors, etc.) are available as **registry packages**. Use `horus pkg search` to find drivers for your hardware.

See the [Hardware Discovery](/development/hardware-discovery) documentation for detailed usage examples.
