...
- Builds only the `horus_manager` package, which produces the `horus` binary
...
This script:
1. Downloads a pre-built `horus` binary from GitHub Releases (checksum-verified), or compiles `horus_manager` from source if no binary matches your platform (~3-5 min)
2. Clones and caches the HORUS source at `~/.horus/cache/horus@<version>/`
...
None of these ship with the core install. Find and install them with the `horus` CLI (`horus search <hardware>`, then `horus install <package>`), and wire them up under `[hardware]` in `horus.toml`. The "System Packages" column lists the host libraries the driver needs to be present.
...
Or set them once in `horus.toml`:
```toml
enable = ["cuda", "editor"]
```
...
- `sim` (alias `simulation`) - Simulation mode; enables no hardware features
