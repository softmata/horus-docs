...
2. Builds and caches core libraries in `~/.horus/cache/`:
   ...
   - `horus_types`, `horus-robotics`, and `horus-tf` — Standard message types and transforms
   - `horus_manager` — CLI tool and project management
3. Installs Python bindings (`horus`) if Python 3.9+ is detected
...
# Install HORUS
RUN curl -fsSL https://github.com/softmata/horus/raw/release/install.sh | bash
...
curl -fsSL https://github.com/softmata/horus/raw/release/install.sh -o install.sh
