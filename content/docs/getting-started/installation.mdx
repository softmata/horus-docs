...
| **Raspberry Pi** | Supported | `/dev/shm/horus_<namespace>/` | ARM64 and ARMv7 built in CI |
...
- Clones the HORUS source into `~/.cache/horus/horus@<version>/` (kept because `horus run`/`horus build` compile projects against horus as *path* dependencies)
...
This script:
1. Clones and caches the HORUS source at `~/.cache/horus/horus@<version>/`
...
**GPU acceleration:**
- HORUS can leverage CUDA for vision processing nodes (`horus run --enable cuda`)
...
Capabilities such as GPU acceleration or the editor *are* command-line flags:
```bash
horus run --enable cuda --release
```

Or set them once in `horus.toml` (top level, above any `[section]` header):
```toml
# horus.toml
enable = ["cuda", "editor"]
```

**Available capabilities** (managed by the `horus` CLI via `--enable`):
- `cuda` (alias `gpu`) - CUDA/GPU acceleration
- `editor` - Editor UI
- `headless` - Headless rendering
- `visual` - Visualization
- `python` (alias `py`) - Python bindings
- `opencv` - OpenCV backend
- `io-uring` - io_uring networking backend
- `ultra-low-latency` - Ultra-low-latency tuning
- `full` - Bundle enabling the full feature set
- `sim` (alias `simulation`) - Simulation mode; turns on no extra build features by itself
...
> **Note:** Most users should use `horus run --enable <capability>` or set `enable = ["cuda", "editor"]` in `horus.toml` rather than setting cargo features manually.
...
2. Remove all cached libraries (`~/.cache/horus/`)
3. Ask if you want to remove `~/.horus/` (blackbox recordings, shell env files)
...
rm -rf ~/.horus/                 # blackbox recordings, env.sh / env.fish
rm -rf ~/.cache/horus/           # package cache and cached source
