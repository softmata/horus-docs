...
def controller_tick(node):
    ...
    if imu_data:
        # A typed topic takes an instance of its message class, not a dict
...
Declaring a topic with `{"type": CmdVel}` makes it a typed topic, and the typed send path
requires an actual instance of that class — passing a dict raises `TypeError: 'dict'
object is not an instance of 'CmdVel'`. (`CmdVel`'s fields are `linear` and `angular`.)
Dicts are for generic topics, declared with a bare name and no `type`.
