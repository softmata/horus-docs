...
```python
from horus import Node, Topic, Scheduler, Imu, CmdVel
...
class Controller(Node):
    ...
    def tick(self):
        imu_data = self.imu_sub.recv()
        if imu_data:
            self.cmd_pub.send(CmdVel(linear_x=0.5, angular_z=0.0))

scheduler = Scheduler()
scheduler.node(Controller()).rate(50).build()
scheduler.run()
```
