...
| `HORUS_PREFIX=/opt/horus` | Install root override: `$HORUS_PREFIX/bin` for the binary, `$HORUS_PREFIX` in place of `~/.horus` for state and cache. Also how a root install says where it means to go. Set `HORUS_SOURCE=$HORUS_PREFIX/cache/horus@<version>` alongside it — `horus run` searches `~/.cache/horus` and `~/.horus/cache` for the source tree and does not consult `HORUS_PREFIX` |
...
`horus pkg`. Remove it by hand if you want it gone.
