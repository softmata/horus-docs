...
```bash
horus install horus-sim3d   # the simulator plugin `--sim` launches — see the caveat below
...
> **`horus-sim3d` is not publicly installable.** `softmata/horus-sim3d` is a private
> repository, so that first command reports "not found in registry" outside the org, and
> the package page 404s. (The public registry also is not currently serving — see
> [horus#173](https://github.com/softmata/horus/issues/173) — which is a second,
> independent reason it fails.) Since `horus run --sim` refuses to start without the
> plugin, `--sim` is unavailable until the simulator is published. Substituting a stub for
> a `[hardware]` entry without a simulator is what `sim = true` plus your own stub node
> does.
