...
- Installs five core libraries to `~/.horus/cache/`:
  ...
  - `horus-robotics` - Python bindings (auto-installed if Python 3.9+ detected)
...
**Available hardware features** (managed by the `horus` CLI):
...
**Cargo feature flags** (for `horus` crate):
