...
For better logging and type safety, use HORUS message types instead of raw values:
...
def motor_tick(node):
    velocity = node.get("cmd_vel")
    if velocity:
        # velocity is a CmdVel message with typed fields
        print(f"Linear: {velocity.linear_x}, Angular: {velocity.angular_z}")

motor = horus.Node(
    name="MotorController",
    subs={"cmd_vel": {"type": CmdVel}},
    tick=motor_tick,
    rate=50
)
...
See [Message Types](/concepts/message-types) for the full list of available message types.
...
Yes. HORUS supports network communication between robots via mDNS peer discovery (requires the `mdns` feature flag).
...
- [Topics](/concepts/core-concepts-topic) - Message passing system
