...
**Why:** Messages can be missed if you don't check every tick. Topic only stores the latest message.
...
fn tick(&mut self) {
    ...
    self.pub.send(data);  // Compiler warning: unused Result
}
```
...
fn tick(&mut self) {
    // Log if send fails
    if let Err(e) = self.pub.send(data) {
        hlog!(warn, "Failed to publish data: {:?}", e);
    }
}
...
fn tick(&mut self) {
    self.topic.send(42.0).ok();  // Reuse existing Topic
}
```
