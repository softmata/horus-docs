...
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    self.topic.send(42.0, &mut ctx).ok();  // Reuse existing Topic
}
```
...
pub_topic.send(42.0, &mut ctx).ok();

// Subscriber expects i32
let sub_topic: Topic<i32> = Topic::new("data")?;  // WRONG TYPE!
let value = sub_topic.recv(&mut ctx);  // Will get garbage data
```
...
impl Node for MySensor {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        self.pub_topic.send(data, &mut ctx).ok();
    }
}
```
