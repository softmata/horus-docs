...
**Why:** For nodes on the main loop, execution order comes from the **topic dependency
graph**, not from `.order()`. The scheduler reads `.order()` tiers only while it has no
topic metadata at all — the first tick, and programs whose nodes never publish or
subscribe. Once any node has published or subscribed, the graph is rebuilt from the
topics and the order numbers are not read again.

So two independent BestEffort nodes run concurrently on worker threads whatever numbers
you gave them, and a node with a low number does not "go first" unless something makes it
a dependency.
