...
`tick()` — the common shape — there is no metadata at startup, the graph falls back to
`.order()` tiers, and it **stays** there for the life of the process. The post-first-tick
rebuild is gated on a counter seeded before tick 1, so for these programs it never fires.
`.order()` really does sequence them.

The graph is built from real publisher → subscriber edges only when some node already
sent or received during `init()`. For those programs `.order()` stops deciding the order,
and independent nodes run concurrently whatever their numbers.
