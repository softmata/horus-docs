...
<Callout type="info" title="Two ways to build — and why plain `cargo build` is not one of them">
`horus new` creates a **managed** project: `horus.toml` is the manifest and
`.horus/Cargo.toml` is generated, which is why a bare `cargo build` here fails
with `could not find Cargo.toml`. Build with `horus run`, `horus build` or
`horus test`. If the installer set up shell integration (`horus env --init`),
plain `cargo` commands inside projects route through HORUS and work too.

Want a manifest cargo owns? `horus eject` writes a root `Cargo.toml` from the
generated one, and `horus new my_robot --cargo` starts that way. From then on
`cargo build`/`run`/`test` work directly, and `[rust]` in `horus.toml` no longer
applies. See [Depending on HORUS](/development/depending-on-horus).
</Callout>
