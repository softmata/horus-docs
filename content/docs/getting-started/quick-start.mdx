...
<Callout type="note" title="`.order()` is a fallback, not the ordering rule">
For nodes on the main loop, execution order comes from the topic dependency graph.
`.order()` tiers are used only while the scheduler has **no** topic metadata at all —
the very first tick, and programs whose nodes never publish or subscribe. After that the
graph is rebuilt from the topics and `.order()` is not read.

It still matters on the RT, compute and async I/O executors, which start their pools
sorted by it. On the main loop, express "this runs first" by what a node subscribes to.
</Callout>
