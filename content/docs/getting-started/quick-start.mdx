...
> **Note**: `.horus/` is automatically managed. For Rust projects, HORUS generates `Cargo.toml` from your `horus.toml` using path references (no source copying). See [Environment Management](/package-management/environment-management#solving-dependency-hell) for details.
