...
> **Note**: `.horus/` is automatically managed. For Rust projects, HORUS generates `Cargo.toml` from your `horus.yaml` using path references (no source copying). See [Environment Management](/package-management/environment-management#solving-dependency-hell) for details.
...
impl Node for TemperatureSensor {
    ...
    fn tick(&mut self) {
        ...
        // Send the reading
        self.publisher.send(self.temperature).ok();
...
- `.rate_hz(n)` - Set node-specific tick rate
- `.rt()` - Mark as real-time node
- `.done()` - Finish and register the node
...
No! The new fluent API handles boxing automatically:

```rust
// New API (recommended) - no Box needed
scheduler.add(MyNode::new())
    .order(0)
    .done();

// Also works with Box if you prefer
scheduler.add_dyn(Box::new(MyNode::new()))
    .order(0)
    .done();
```
