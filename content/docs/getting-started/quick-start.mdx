...
- `.rate(freq)` - Set node-specific tick rate, e.g. `.rate(100_u64.hz())` (auto-derives budget = 80% of period and deadline = 95% of period)
...
Usually an invalid topic name (allowed characters: `a-z`, `0-9`, `_`, `-`, `.`, `/`), a shared-memory permission problem, or `/dev/shm` being full. Check the topic name, then clear stale shared memory:
