...
The skip is silent. It raises no error and **increments no counter** —
`dropped_count()` reports send-side failures only, so it stays at `0` while a
slow subscriber is losing frames.
