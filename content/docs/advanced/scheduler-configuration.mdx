...
The Python API uses the unified Node + run() pattern. All scheduling config goes on the Node:

```python
from horus import Node, run, us

# All config on Node — run() handles the scheduler
...
Or with explicit Scheduler for dynamic control:

```python
from horus import Node, Scheduler

scheduler = Scheduler(tick_rate=1000, rt=True, cores=[0, 1], verbose=True)
scheduler.add(Node(tick=sensor_fn, rate=100, order=0))
scheduler.add(Node(tick=ctrl_fn, rate=100, order=1, on_miss="safe_mode"))
scheduler.add(Node(tick=logger_fn, rate=10, order=2, failure_policy="skip"))
scheduler.run()
```
...
scheduler = Scheduler(tick_rate=1000, watchdog_ms=500, blackbox_mb=64, verbose=True)
```
