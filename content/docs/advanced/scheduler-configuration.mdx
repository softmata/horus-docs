...
Create custom configurations using builder methods for specific needs:
...
let config = SchedulerConfig::standard()
    .with_execution_mode(ExecutionMode::Sequential)
    .with_tick_rate(500.0)                        // 500 Hz
    .with_max_jitter(50)                          // 50μs max jitter
    .with_circuit_breaker(true)
    .with_wcet_enforcement(true)
    .with_watchdog(true, 50)                      // 50ms watchdog
    .with_cpu_cores(vec![2, 3])                   // Isolate to cores 2-3
    .with_profiling(false)
    .with_black_box(true, 256);                   // 256MB black box

let mut scheduler = Scheduler::new().with_config(config);
```

For advanced customization requiring direct struct access, start from a preset and modify fields:

```rust
use horus::prelude::*;

// Get mutable config from preset
let mut config = SchedulerConfig::safety_critical();

// Modify specific fields
config.timing.global_rate_hz = 500.0;
config.timing.max_jitter_us = 50;
...
config.monitoring.black_box_size_mb = 256;
