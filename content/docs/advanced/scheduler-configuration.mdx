...
These are the two knobs used most often, not the whole surface: `Scheduler` has fourteen chainable builder methods. `.prefer_rt()`, `.require_rt()` and `.blackbox(mb)` are covered under [Global Configuration with Builder Methods](#global-configuration-with-builder-methods) below, and `.deterministic(bool)` under [Sequential Mode](#sequential-mode-deterministic-true). The rest — `.cores()`, `.watchdog()`, `.max_deadline_misses()`, `.verbose()`, `.with_params()`, `.with_recording()`, `.telemetry()` and `.network()` — are listed in [Core API](/rust/api/core#builder-methods).
...
**`Scheduler::new()` runs your nodes in parallel**, driven by the dependency graph it builds from the topics they publish and subscribe. That is the right starting point for most robots. Add `.deterministic(true)` when you need a reproducible execution order — for replay and regression testing.
...
Deterministic run order. This is **not** the default; you opt into it. A reproducible order is not a
certification claim — see [Certification Status](/advanced/deterministic-execution#certification-status).

| Metric | Value |
|--------|-------|
| Deterministic | **Yes** |
| Multi-core | No (single thread) |
| Best For | Replay, regression tests, fixed-order control loops |

```rust,ignore
use horus::prelude::*;

// Fixed-order robot controller — 1 kHz tick, everything on the main loop
let mut scheduler = Scheduler::new()
    .tick_rate(1000_u64.hz())
...
Concurrency is opt-in per node, through the execution class — not through `order`. A node leaves the main tick loop as soon as it has an execution class: RT (`.rate()` / `.budget()` / `.deadline()`) moves to a dedicated RT thread, `.compute()` to the parallel thread pool, `.on(topic)` to a per-node watcher thread, and `.async_io()` to the tokio blocking pool. `.order()` tiers decide the main-loop sequence whenever no node registered a topic during `init()` — registration is lazy, so a node that first calls `send()`/`recv()` inside `tick()` never enters the graph and its tier keeps ordering it for the life of the process. The graph is built from real pub/sub edges only for programs that send or receive during `init()`, and only there does `.order()` stop deciding. Nodes sharing an `order` value form **one parallel step**; different values run one after another. Only `.deterministic(true)` makes the main loop strictly sequential.
...
The last two rows are deliberate: no measurement or analysis in this repository bounds either property.
[Certification Status](/advanced/deterministic-execution#certification-status) explains what the status
values mean and what evidence does not exist.
