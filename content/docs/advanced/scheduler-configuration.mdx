...
Nodes with no execution class stay BestEffort, so they run in `order` sequence on the main thread. Adding `.deterministic(true)` to `Scheduler::new()` goes further and keeps *every* node on the main thread — no executor threads are spawned at all, even for nodes that declare an RT or compute class.
