...
// Use a preset that includes checkpointing
let config = SchedulerConfig::space();  // Includes 5-second checkpoint interval

let mut scheduler = Scheduler::new().with_config(config);

// Or configure checkpoints manually
let config = SchedulerConfig::standard();
// config.fault.checkpoint_interval_ms = 60000;  // 60 second checkpoint interval
