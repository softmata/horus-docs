...
Use `SchedulerConfig` with a `blackbox_size_mb` value to enable the BlackBox:
...
let config = SchedulerConfig {
    blackbox_size_mb: 1024,
    safety_monitor: true,
    ..Default::default()
};
let mut scheduler = Scheduler::from_config(config);

// 100MB black box for hard real-time or deterministic modes
let config = SchedulerConfig {
...
let config = SchedulerConfig { blackbox_size_mb: 16, ..Default::default() };
let mut scheduler = Scheduler::from_config(config);
...
|----------|---------------|-------------|
| Development | `SchedulerConfig { blackbox_size_mb: 16, .. }` | 16 MB |
| Long-running production | `SchedulerConfig { blackbox_size_mb: 100, .. }` | 100 MB |
| Safety-critical | `SchedulerConfig { blackbox_size_mb: 1024, .. }` | 1 GB |
