...
| Emergency stops | Safety-system activations (e.g. the safety monitor tripping) |
| Custom events | User-defined markers and internal safety notes |

> **Note**: The BlackBox records anomalies and lifecycle events — **not** every node tick. Individual per-tick node execution and node error details are not captured here. Use [Record & Replay](/advanced/record-replay) for full per-tick node state, or `horus log` for node error messages.
...
[03:17:01.504] BudgetViolation { name: "collision_checker", budget_us: 1500, actual_us: 5100 }
[03:17:01.504] EmergencyStop { reason: "deadline miss threshold exceeded" }
```

**Step 3: Diagnose**
The collision checker started missing its 1.9ms deadline (taking 4-5ms instead) and blew its execution budget. Once the repeated misses crossed the safety threshold, the scheduler triggered an emergency stop — halting the robot before the late trajectory could be acted on. (Per-tick node execution is not recorded, so the timeline shows the anomalies that led to the stop, not every tick.)
...
The scheduler knows when every node ticks, when deadlines are missed, and when failures occur. Requiring developers to manually add recording calls would lead to incomplete data. The BlackBox records the anomalies and lifecycle events the scheduler observes, with zero code changes.
