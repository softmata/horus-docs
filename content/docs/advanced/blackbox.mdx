...
The scheduler can be configured with BlackBox monitoring:

```rust,ignore
use horus::prelude::*;

// Note: Configure blackbox through MonitoringConfig
let mut config = SchedulerConfig::default();
// config.monitoring.blackbox = Some(BlackBoxConfig { ... });

let scheduler = Scheduler::with_config(config);
// Scheduler events can be recorded to blackbox
```
