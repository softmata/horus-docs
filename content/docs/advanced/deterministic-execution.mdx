...
| **+ Performance** | `.tier(NodeTier::*)` | Basic + explicit optimization tiers | When you need speed + determinism |
...
scheduler.add(pid_controller).order(0).tier(NodeTier::UltraFast).done();  // Highest priority inline
...
| Average Latency | 87-313ns | Variable |
| Multi-core | No | Yes |
...
The default sequential mode is best for most use cases. Use Parallel mode only when throughput matters more than determinism.
...
| Racing / competition (max throughput) | + Speed | `SchedulerConfig::high_performance()` |
