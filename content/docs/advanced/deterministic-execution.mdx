...
For randomness you manage yourself — HORUS's own per-tick `horus::rng` is already deterministic, see Level 4:
...
When you do need a timestamp, use `horus::now()` — it reads the scheduler's clock, which is the virtual `SimClock` under `.deterministic(true)`, so it replays identically.
