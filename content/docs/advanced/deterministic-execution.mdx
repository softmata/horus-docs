...
**For most users: No.** HORUS is already deterministic by default for node execution order.

**When this page IS useful:**
- Building safety-critical systems (medical, automotive)
- Debugging hard-to-reproduce timing bugs
- Need bit-for-bit identical runs for certification
- Writing tests that must behave identically every time

**Quick answer:** If you're learning HORUS or building hobby robots, skip this page. Come back when you need formal safety certification.
...
| **+ Performance** | `.tier(NodeTier::*)` | Basic + explicit JIT/optimization tiers | When you need speed + determinism |
...
scheduler.add(pid_controller).order(0).tier(NodeTier::Jit).done();       // JIT compiled
scheduler.add(sensor_reader).order(1).tier(NodeTier::Fast).done();       // Inline
scheduler.add(data_logger).order(5).tier(NodeTier::Background).done();   // Low priority
...
scheduler.add(ImuNode::new()).order(0).done();
scheduler.add(ControllerNode::new()).order(1).done();
scheduler.add(MotorNode::new()).order(2).done();
...
For deterministic execution, deadline enforcement is handled via `RealTimeConfig` settings (`deadline_monitoring`, `wcet_enforcement`). The `safety_critical()` preset enables these automatically.
...
| Performance-critical + deterministic | + Performance | `.tier(NodeTier::*)` |
