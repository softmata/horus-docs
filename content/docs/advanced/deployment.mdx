...
Every HORUS executable statically links the whole runtime — there is no shared
`libhorus.so` to install next to it. That is what makes a deployed binary a single file
you can `scp` onto a robot, and it is also why the debug build is enormous. Plan flash
...
- **Share the runtime across processes.** This is the one thing HORUS does not currently
  offer: `horus_cpp` builds a `cdylib`, but the generated CMake always links the static
  `libhorus_cpp.a`, and there is no `horus.toml` key to select the dynamic one. A
  multi-executable robot therefore pays the runtime's size once per executable. If that is
  what is squeezing you, say so on the issue tracker — the artifact exists, only the
  build-file switch does not.
