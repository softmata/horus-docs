...
misses `n` times in a row, and any tick that **meets its deadline** resets its count to
zero. A tick that stayed inside its budget does not: a node woken late blows its deadline
having executed in a fraction of its budget. Below the ceiling, sustained misses go
through the graduated ladder: warn at 3 consecutive, half rate at 5, isolate at 10. The
ceiling is the ladder's last rung — reaching it kills the node and latches the emergency
stop together.
...
- **Drop capabilities you do not run.** `horus run --enable net` links in the networking
  stack; every other `--enable` name is forwarded to cargo as a feature of your own crate,
  so it links in only what you declared under `[rust.features]`. Build the deployment
  target with only what the robot uses.
...
The safety monitor runs two separate graduated ladders, and they escalate on different
evidence.

The **watchdog** ladder answers "is this node still alive?", counted in multiples of the
watchdog timeout: at 1x the node is marked Warning, at 2x Unhealthy, and past 3x it is
isolated and a system-wide emergency stop is latched. Isolating queues
`enter_safe_state()` on the node — but that runs on the node's own executor thread, so a
node hung inside `tick()` may never execute it. The emergency stop does not depend on it.

The **deadline-miss** ladder answers "is this node keeping up?", counted in consecutive
missed deadlines: warn at 3, halve the node's rate at 5, isolate it at 10, and at the
`.max_deadline_misses(n)` ceiling kill it and latch an emergency stop. No rung below the
ceiling halts the robot.
