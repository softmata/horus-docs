...
fn tick(&mut self) {
    ...
    if let Some(cmd) = self.velocity_sub.recv() {
    ...
    self.state_pub.send(motor_state);
...
while !replayer.is_finished() {
    if let Some(snapshot) = replayer.current_snapshot() {
        ...
        // Get specific output
        if let Some(motor_data) = replayer.output("motor_state") {
...
Replay is configured through the `Scheduler` builder methods:
...
let mut scheduler = Scheduler::replay_from(
    PathBuf::from("~/.horus/recordings/mission/scheduler@abc123.horus")
)?;

// Mixed replay - some nodes from recording, others live
let mut scheduler = Scheduler::new();
scheduler.add_replay(PathBuf::from("recordings/Lidar@001.horus"), 0)?;
scheduler.add_replay(PathBuf::from("recordings/Camera@002.horus"), 1)?;
scheduler.add(live_controller).order(2).done();
...
let bp3 = debugger.add_named_breakpoint("after_warmup",
...
> **Planned Feature**: The auto-recording API (`AutoRecordConfig`, `AutoRecorder`, `AutoRecordTrigger`) described below is planned but not yet implemented.
...
for session in manager.list_sessions()? {
    ...
    for path in manager.session_recordings(&session)? {
...
> **Planned Feature**: The standalone compression utility functions described below are planned but not yet implemented. The `RecordingConfig.compress` field controls compression at the recording level.
...
The Scheduler has built-in recording and replay support:
...
// Enable recording via SchedulerConfig
...
// Recording via CLI: horus run --record <session_name> my_project
```

When recording is enabled, the scheduler automatically creates `NodeRecorder` instances for each node and a `SchedulerRecording` that tracks execution order.
...
// Time travel: start at a specific tick
let mut scheduler = Scheduler::replay_from(path)?
    .start_at_tick(1500);
...
// Override specific outputs for what-if testing
let mut scheduler = Scheduler::replay_from(path)?
    .with_override("sensor_node", "temperature", 25.0f32.to_le_bytes().to_vec());
```

### Mixed Replay (Inject Recorded Nodes)

```rust,ignore
use horus::prelude::*;

// Create a scheduler with some live nodes and some replayed
let mut scheduler = Scheduler::new();
...
horus record inject my_session --nodes sensor --script process.rs
