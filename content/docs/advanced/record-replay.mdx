...
**For most users: Not at first.** Record/replay is a debugging and testing tool for when things go wrong.

**When this page IS useful:**
- Debugging intermittent bugs that are hard to reproduce
- Testing: replay the same sensor data to verify fixes
- Demonstrating behavior to teammates
- Post-mortem analysis of robot failures

**Quick answer:** Start building your robot first. Come back to record/replay when you need to debug something tricky.
...
fn tick(&mut self) {
    ...
    if let Some(cmd) = self.velocity_sub.try_recv() {
    ...
    self.state_pub.send(motor_state).ok();
