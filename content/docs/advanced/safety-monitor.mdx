...
The Scheduler manages the safety monitor internally — you enable it via `SchedulerConfig` and the scheduler automatically feeds watchdogs, checks WCET, and triggers emergency stops.
...
### SchedulerConfig Safety Fields

| Field | Description | Example |
|-------|-------------|---------|
| `safety_monitor` | Enable safety monitoring | `true` |
| `wcet_enforcement` | Enable WCET enforcement | `true` |
| `watchdog_timeout_ms` | Watchdog timeout in ms | `100` |
| `max_deadline_misses` | Misses before emergency stop | `0` (zero tolerance) |
| `memory_locked` | Enable `mlockall()` | `true` |
| `rt_scheduling` | Enable RT scheduling (SCHED_FIFO) | `true` |
...
|----------|---------------|
| Medical / surgical robots | `SchedulerConfig { safety_monitor: true, max_deadline_misses: 0, .. }` |
| Industrial control | `SchedulerConfig { safety_monitor: true, max_deadline_misses: 0, .. }` |
| CNC / aerospace | `SchedulerConfig { safety_monitor: true, max_deadline_misses: 3, .. }` |
