...
The Scheduler manages the safety monitor internally — you enable it via a preset and the scheduler automatically feeds watchdogs, checks WCET, and triggers emergency stops.

## Enabling Safety Monitoring

Use a Scheduler preset that includes safety monitoring:

```rust
use horus::prelude::*;

// safety_critical() — all safety features, zero tolerance for deadline misses
let scheduler = Scheduler::safety_critical();

// hard_realtime() — safety monitor + 10ms watchdog, tolerates 3 deadline misses
let scheduler = Scheduler::hard_realtime();
```

### What Each Preset Enables

| Feature | `safety_critical()` | `hard_realtime()` |
|---------|---------------------|-------------------|
| Safety monitor | Yes | Yes |
| WCET enforcement | Yes | Yes |
| Watchdog | Yes (100ms) | Yes (10ms) |
| Max deadline misses | 0 (zero tolerance) | 3 |
| Memory locking | Yes | Yes |
| RT scheduling (SCHED_FIFO) | Yes | Yes |

## Configuring Critical Nodes

After choosing a preset, register critical nodes with timing constraints using the node builder:
...
| Industrial control | `Scheduler::safety_critical()` |
| CNC / aerospace | `Scheduler::hard_realtime()` |
