...
| `.max_deadline_misses(n)` | Emergency stop after `n` deadline misses, counted cumulatively across **every** RT node — critical or not. A critical node does **not** e-stop on its first miss; per-node escalation is the `Miss` policy's job, and this ceiling is the only deadline-driven e-stop | `.max_deadline_misses(10)` |
