...
disables the watchdog entirely. A non-zero timeout below 1 ms is raised to 1 ms and logged as a
warning, because truncating it to 0 would silently turn the watchdog off. Anything else truncates
toward zero with no diagnostic, so `.watchdog(1500_u64.us())` runs a 1 ms watchdog and e-stops at
