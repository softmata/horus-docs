...
Failure events are recorded in the [BlackBox](/docs/advanced/blackbox) flight recorder:
...
if let Some(bb) = scheduler.get_blackbox() {
    for record in bb.lock().unwrap().anomalies() {
...
- [Safety Monitor](/docs/advanced/safety-monitor) — Watchdog and graduated degradation
- [BlackBox Recorder](/docs/advanced/blackbox) — Crash forensics
- [Scheduler Configuration](/docs/advanced/scheduler-configuration) — Per-node builder API
