...
**For most users: No.** TMR and voting are for aerospace/medical-grade systems.

**When this page IS useful:**
- Building systems where failures can't be tolerated (surgical robots, drones)
- Need to detect sensor/hardware failures automatically
- Regulatory requirements demand redundancy

**Quick answer:** If one sensor failing would just mean a degraded experience (not a safety issue), skip this. Use it when failures have serious consequences.
...
For redundancy/TMR, use the `RedundancyManager` directly in your application code (shown above).
...
impl Node for FaultTolerantController {
    ...
    fn tick(&mut self) {
        ...
        let sensor_result = self.redundancy_mgr.execute_redundant(|replica| {
            if let Some(val) = self.sensor_sub.try_recv() {
        ...
        self.motor_pub.send(command).ok();
    ...
    fn init(&mut self) -> Result<()> {
        ...
        if let Ok(Some(checkpoint)) = self.checkpoint_mgr.load_latest_checkpoint() {
            hlog!(info, "Recovering from checkpoint {}", checkpoint.id);
