...
|----------------|-----------------|---------|---------------|
| `.rate(freq)` | `Rt` | Fixed-rate control loops | Dedicated high-priority thread, absolute-deadline sleep with a bounded guard spin |
...
The RT thread sleeps to an absolute `CLOCK_MONOTONIC` deadline for the bulk of each period and spins only the final guard window — 20 µs by default, never more than a sixteenth of the period. `HORUS_RT_WAIT=spin` opts into a pure busy-wait instead; under a real RT policy that one warns on every boot, because Linux RT bandwidth control then dequeues the thread for ~50 ms once its share is spent.
...
```rust,ignore
// Fixed-rate control loop: dedicated RT thread, absolute-deadline sleep with a guard spin
