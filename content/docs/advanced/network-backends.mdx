...
**For most users: No.** The default transport works great for single-machine robots.

**When this page IS useful:**
- Robot communicates across multiple machines (distributed system)
- Need WAN/internet communication (cloud, remote operation)
- Integrating with ROS2 systems (use Zenoh)
- Optimizing network latency for competition

**Quick answer:** Start with default settings. Add network backends when you expand to multiple machines.
...
| **Zenoh** | ~100µs | High | ROS2 interop, mesh |
...
```rust
use horus::communication::network::{QuicConfig, QuicTransport};
...
```rust
use horus::communication::network::{QuicConfig, QuicTransport, get_or_create_cert};
...
```rust
use horus::communication::network::{RealIoUringConfig, RealIoUringBackend};
...
```rust
use horus::communication::network::{RealIoUringBackend, RealIoUringConfig, is_real_io_uring_available};
...
```rust
use horus::communication::network::{CompressionConfig, CompressionAlgo, Compressor};
...
```rust
use horus::communication::network::CompressedPacket;
...
```rust
use horus::communication::network::{MessageBatcher, BatchConfig};

// Create batcher with configuration
let config = BatchConfig::default();  // 100 messages, 60KB, 5ms timeout
let mut batcher = MessageBatcher::new("sensor_data", config);

// Low latency config (smaller batches, shorter timeout)
let config = BatchConfig::low_latency();  // 10 messages, 500µs timeout
let mut batcher = MessageBatcher::new("control_cmd", config);

// High throughput config (larger batches, longer timeout)
let config = BatchConfig::high_throughput();  // 500 messages, 20ms timeout
let mut batcher = MessageBatcher::new("telemetry", config);

// Add messages - returns Some(batch) when batch is full
let payload = bincode::serialize(&sensor_data)?;
...
// Check timeout - call periodically to flush incomplete batches
...
```rust
use horus::communication::network::{CongestionController, CongestionConfig, CongestionResult, DropPolicy};

// Default config (error on buffer full)
let config = CongestionConfig::default();
let mut controller = CongestionController::new(config);

// Drop oldest messages when congested
let config = CongestionConfig::drop_oldest();
let mut controller = CongestionController::new(config);

// Rate limiting (messages per second)
let config = CongestionConfig::rate_limited(1000);  // 1000 msg/sec
let mut controller = CongestionController::new(config);

// Blocking mode (waits for space)
let config = CongestionConfig::blocking();
let mut controller = CongestionController::new(config);

// Try to send a message
let payload = bincode::serialize(&sensor_data)?;
let priority = 5;  // Higher = more important
match controller.try_send(payload, priority) {
    CongestionResult::Accepted => {
        // Message queued for sending
    }
    CongestionResult::Dropped(reason) => {
        // Message was dropped due to congestion
        eprintln!("Dropped: {:?}", reason);
    }
    CongestionResult::RetryAfter(duration) => {
        // Rate limited, retry after duration
        std::thread::sleep(duration);
    }
}

// Pop next message to send
if let Some(payload) = controller.pop() {
    send_udp(&payload)?;
}

// Check buffer utilization
println!("Buffer {}% full", controller.utilization());
```
...
let config = BatchConfig {
    max_messages: 100,           // Up to 100 messages
    max_bytes: 60000,            // ~MTU minus headers
    flush_timeout: Duration::from_millis(1),  // 1ms max latency
    enabled: true,
};
let mut batcher = MessageBatcher::new("sensor_topic", config);
```
...
```rust
use horus::communication::network::ZenohConfig;
...
```rust
use horus::communication::network::{ZenohQos, Reliability, HistoryPolicy, Durability};
...
```rust
use horus::communication::network::{Ros2ServiceConfig, Ros2ServiceClient, Ros2ServiceServer};
...
```rust
use horus::communication::network::{Ros2ActionClient, Ros2ActionServer, Ros2ActionConfig, GoalStatus};
