...
**For most users: Not initially.** Telemetry shines when you need to monitor robots remotely.

**When this page IS useful:**
- Monitoring robots in the field remotely
- Integrating with Grafana, Prometheus, or other dashboards
- Tracking long-term performance and health
- Fleet management (multiple robots)

**Quick answer:** Build your robot first. Add telemetry when you deploy to production or need remote monitoring.
...
match state {
    NodeState::Uninitialized => println!("Node created but not started"),
    ...
    NodeState::Stopping => println!("Shutting down"),
    NodeState::Stopped => println!("Cleanly shut down"),
    NodeState::Error(msg) => println!("Error: {}", msg),
    NodeState::Crashed(msg) => println!("Crashed: {}", msg),
}

// NodeState implements Display
println!("State: {}", state);
```
...
struct MonitoredNode {
    ...
    heartbeat_pub: Topic<NodeHeartbeat>,
    status_pub: Topic<Status>,
    resources_pub: Topic<ResourceUsage>,
...
impl Node for MonitoredNode {
    ...
    fn tick(&mut self) {
        ...
        self.heartbeat_pub.send(heartbeat).ok();

        // Publish resource usage
        let resources = self.get_resource_usage();
        self.resources_pub.send(resources).ok();
