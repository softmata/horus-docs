...
| `scheduler` | RT scheduling class (Fifo/RoundRobin/Deadline) | `Normal` |
...
let link: Topic<CmdVel> = Topic::new("cmd_vel")?;  // <100ns latency
```
...
For applications requiring good latency with round-robin scheduling:

```rust
let config = RtConfig::soft_realtime();
config.apply()?;  // Requires CAP_SYS_NICE for SCHED_RR
```

This enables:
- `SCHED_RR` (round-robin RT scheduling) at priority 50
- No memory locking (can run without `CAP_IPC_LOCK`)
...
| `prefault_stack(bytes)` | Stack size in bytes to prefault |
| `scheduler(RtScheduler)` | Scheduling class |
| `priority(i32)` | RT priority (1-99 for FIFO/RoundRobin) |
| `cpu_affinity(&[usize])` | CPU cores to pin to |
| `warn_on_degradation(bool)` | Log warnings when features degrade |
| `fail_on_degradation(bool)` | Fail instead of degrading |
...
| `RoundRobin` | Round-robin among same priority | Multiple RT tasks |
| `Deadline` | Explicit deadlines | Periodic tasks with guaranteed timing |
...
| `scheduler(Fifo/RoundRobin/Deadline)` | `CAP_SYS_NICE` |
...
scheduler.add(MotorControl::new())
    ...
    .wcet_us(50)        // 50µs WCET budget
    .deadline_us(500)   // 500µs deadline (2kHz)
    .done();
...
|--------|------------|----------------|-------------|
| Topic SPSC send latency | ~150ns | ~87ns | 1.7x |
...
match config.apply() {
    ...
    Ok(RtApplyResult::Failed(msg)) => {
        eprintln!("RT config failed: {}", msg);
    }
    Err(e) => eprintln!("IO error: {}", e),
}
```

Use `warn_on_degradation(true)` to automatically log warnings, or `fail_on_degradation(true)` to return an error instead of degrading.
...
```rust
// Get current thread's scheduler and priority
let (scheduler, priority) = RtConfig::get_current_scheduler()?;
println!("Scheduler: {:?}, Priority: {}", scheduler, priority);

// Get current thread's CPU affinity
let cpus = RtConfig::get_current_affinity()?;
...
fn main() -> Result<()> {
    ...
    match rt_config.apply()? {
        ...
        RtApplyResult::Failed(msg) => {
            return Err(HorusError::config(format!("RT config failed: {}", msg)));
        }
    }
