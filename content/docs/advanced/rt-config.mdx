...
`.prefer_rt()` and `.require_rt()` request OS-level RT when `run()` starts:
`mlockall` for the process, and `SCHED_FIFO` at priority 50 for the scheduler's
own thread. `.prefer_rt()` warns and continues when the kernel refuses;
`.require_rt()` panics when the system offers neither `SCHED_FIFO` nor
`mlockall`.

The node threads are separate and are not gated on either flag. Every RT-class
node — `MotorControl` above earns that class from its `.budget()` /
`.deadline()` — runs on its own executor thread, and that thread attempts
`SCHED_FIFO` at the node's `.priority(n)`, defaulting to 80, and honours its
`.core(cpu)`. If the kernel refuses, the thread logs `Could not set SCHED_FIFO
… continuing with normal priority` and keeps running on the normal class. The
`.priority(80)` in the `RtConfig::builder()` above is the calling-thread knob,
...
single process. Baseline latency per route - **without** any RT configuration
applied - is tabulated in [Benchmarks](/performance/benchmarks), measured with
`all_paths_latency` on an Intel Core i7-10750H (6C/12T, `powersave` governor).
Take them from there rather than from `benchmarks/README.md` in the HORUS
repository, which has withdrawn its own result tables and tells you to
regenerate them.

The backend is chosen automatically from the topic's topology and message type;
the scenario names on that page label what the benchmark exercised, not
