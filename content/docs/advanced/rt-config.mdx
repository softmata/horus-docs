...
**For most users: No.** RtConfig is for demanding real-time applications on Linux production systems.

**When this page IS useful:**
- Deploying to production Linux robots with strict timing needs
- Building industrial or medical devices with sub-millisecond guarantees
- Eliminating timing jitter for control loops

**Quick answer:** Develop on any OS without RtConfig. Add it later when deploying to Linux if you need hard real-time guarantees.
...
| `priority(u8)` | RT priority (1-99 for FIFO/RoundRobin) |
...
pub enum RtScheduler {
    ...
    Deadline,    // SCHED_DEADLINE - For periodic tasks (Linux 3.14+)
}
```
...
// Logging with hlog!
    if ctx.metrics.max_tick_duration_ms > 1.0 {
        hlog!(warn, "RT constraint violated!");
    }
}
```
