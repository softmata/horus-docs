...
description: Configure system-level real-time settings for deterministic execution with sub-100ns intra-process IPC latency
order: 15
category: "advanced"
---
...
HORUS provides `RtConfig`, a builder API for configuring system-level real-time settings. It gives hard real-time robotics applications deterministic execution on top of HORUS IPC, which measures in the tens of nanoseconds intra-process (see [Performance Impact](#performance-impact)).
...
Measured HORUS IPC latency on an Intel Core i7-10750H @ 2.60 GHz (6C/12T),
`powersave` governor, 100K iterations per scenario - **without** any RT
configuration applied (`benchmarks/README.md`):

| Path | Backend | p50 | p99 |
|------|---------|-----|-----|
| Intra-process, same thread | `DirectChannel` | 23 ns | 31 ns |
| Intra-process, 1 pub / 1 sub | `SpscIntra` | 10 ns | 19 ns |
| Cross-process, 1 pub / 2 sub | `SpmcShm` | 198 ns | 298 ns |
...
fn main() -> Result<()> {
    ...
    // Both endpoints live in this process, so this is the intra-process
    // path (tens of ns) - RT config is what keeps the tail flat
