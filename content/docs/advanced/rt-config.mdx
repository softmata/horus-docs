...
before installing a kernel — the limits file is written without a prompt — and
`horus setup-rt --undo` removes that limits file only. A kernel package or an
`isolcpus` boot parameter has to be undone by hand. The
manual steps that follow are the same thing done by hand.
...
|------|---------|-----|-----|
| 1 pub / 2 sub | `PodShm` | 198 ns | 298 ns |
| 1 pub / 4 sub | `PodShm` | 236 ns | 417 ns |
