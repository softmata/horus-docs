...
description: "The 6 Python execution classes — Rt, Compute, Event, AsyncIo, Gpu, BestEffort — and when to use each"
order: 1
---
...
HORUS solves this with **execution classes**: six different executors, each optimized for a specific workload type. The scheduler automatically selects the right class based on how you configure the `Node()` constructor — you describe what your node needs, and the scheduler figures out how to run it.
...
flowchart TD
    ...
    A -->|"gpu=True"| G["Gpu<br/>CUDA stream per node"]
...
The default class. The scheduler **automatically parallelizes** independent BestEffort nodes using a dependency graph built from topic `send()`/`recv()` calls. Nodes that share no topics run simultaneously. Nodes with topic dependencies execute in causal order (publisher before subscriber). When no topic metadata exists, nodes fall back to `order=` tiers.
...
**How it works**: At startup, the scheduler builds a dependency graph from topic metadata. Independent nodes are dispatched to a thread pool via the **ready-dispatch executor** — each node starts the instant its last dependency finishes. Dependent nodes execute in causal order.
...
def preprocess_tick(node):
    img = node.recv("camera.rgb")
    if img is None:
        return
    # Zero-copy GPU transfer via DLPack
    tensor = torch.from_dlpack(img)
    # CUDA kernel launches non-blocking on the scheduler-managed stream
    out = torch.nn.functional.interpolate(tensor.unsqueeze(0), size=(224, 224))
    node.send("camera.preprocessed", out)

preprocess = horus.Node(
    name="preprocess",
    subs=["camera.rgb"],
    pubs=["camera.preprocessed"],
    tick=preprocess_tick,
    rate=30,
    gpu=True,            # Run on the GPU executor
    order=2,
)
```

**How it works**: The GpuExecutor runs on a dedicated thread with a CUDA context. Each GPU node gets its own CUDA stream. Per tick cycle: (1) all ready nodes call `tick()` which launches GPU kernels non-blocking, (2) all streams synchronize, (3) GPU-side timing is recorded via CUDA events.

**Use for**: Image preprocessing (resize, normalize, color convert), neural network inference, point cloud filtering, any CUDA kernel work. `gpu=True` takes precedence over `rate=` for execution class — a GPU node with `rate=30, gpu=True` runs on the GPU executor at 30 Hz, not the RT executor.
...
| `gpu=True` | Gpu |
| `gpu=True, rate=30` | Gpu (rate-limited, CUDA stream managed) |

**Key rule**: `rate=` only auto-enables RT when no explicit execution class (`compute=True`, `on=`, `async def`, `gpu=True`) is set. When combined with an explicit class, `rate=` just limits tick frequency.
...
| Image preprocessing on GPU | **Gpu** | `gpu=True` |
| Neural-network inference (CUDA) | **Gpu** | `gpu=True, rate=30` |
| Point-cloud filtering on GPU | **Gpu** | `gpu=True` |
