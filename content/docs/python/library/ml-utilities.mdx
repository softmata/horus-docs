...
# Run
scheduler = Scheduler()
scheduler.add(VisionNode(), order=0)
scheduler.run()
```
...
def main():
    scheduler = Scheduler()
    scheduler.add(CameraNode(), order=0)
    scheduler.add(DetectorNode(), order=1)
    scheduler.run()
