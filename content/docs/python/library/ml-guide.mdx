...
# Image → PyTorch (via NumPy bridge — shares memory, zero-copy for CPU tensors)
frame = img.to_numpy()
...
pt = torch.from_numpy(t.numpy())  # zero-copy PyTorch tensor (CPU)
...
def jax_tick(node):
    ...
    # HORUS → JAX via the zero-copy NumPy view
    frame = jnp.asarray(img.to_numpy())
...
# A Tensor must travel on a TYPED topic — a bare-string ("jacobian") topic is a
# generic dict channel and cannot serialize a Tensor. Pass the type and route it
# with endpoint=; both nodes using the same endpoint share the "jacobian" channel.
topic = horus.Topic(horus.Tensor, endpoint="jacobian")
topic.send(J)
...
# Arithmetic
scaled = t * 0.5
# Tensor has .mean() but no .std() — take std via the NumPy view
normalized = (t - t.mean()) / float(t.numpy().std())
```
...
# Tensors need typed topics — route each with a distinct endpoint.
env_topics = [horus.Topic(horus.Tensor, endpoint=f"env.{i}.obs") for i in range(16)]
...
def batch_tick(node):
    ...
    # Send actions back to each environment (typed Tensor topics)
    for i, action in enumerate(actions):
        action_topic = horus.Topic(horus.Tensor, endpoint=f"env.{i}.action")
...
- [Image API](/python/api/image) — Camera frame handling with NumPy zero-copy
