...
> **Planned** — The `horus.nodes` module described on this page is not yet implemented. The node classes and data types below represent the planned API for a future release.

HORUS will provide **ready-to-use Python nodes** for common robotics hardware. These nodes will handle the low-level hardware communication, letting you focus on your robot's logic.
...
scheduler.add(imu, order=1)
scheduler.add(camera, order=2)
scheduler.run()
```
...
#   keyboard.pressed - Currently pressed keys (list of strings)
```
...
class ImuData:
    ...
    gyro_y: float
    gyro_z: float
    mag_x: float        # μT (magnetometer, optional)
    mag_y: float
    mag_z: float
    temperature: float  # °C
    frame_id: str
...
class GpsData:
    ...
    fix_type: int       # 0=no fix, 1=GPS, 2=DGPS
...
| `frame_id` | str | "camera" | TF frame ID |
| `simulation` | bool | False | Enable simulation mode |
...
- [Hardware Discovery](/development/hardware-discovery) - Hardware setup
- [API Reference](/rust/api) - Rust API reference
