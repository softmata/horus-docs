...
- **Nanosecond timestamps** - Most message types carry a `timestamp_ns` field. These do not, and reading `timestamp_ns` on one raises `AttributeError`: `Point3`, `Vector3`, `Quaternion`, `Transform`, `TransformFrame`, `BoundingBox2D`, `BoundingBox3D`, `CostMap`, `Waypoint`, `Detection`, `Detection3D`, `DetectionList`, `COCOPose`, `TrackedObject`, `Landmark`, `Landmark3D`, `PointField`, `PointXYZ`, `PointXYZRGB`, `TrajectoryPoint`, `VelocityObstacle`, `RegionOfInterest`, `StereoInfo`, `DiagnosticValue`, `NodeHeartbeat`. `JoystickInput` and `KeyboardInput` carry `timestamp_ms` (milliseconds) instead
