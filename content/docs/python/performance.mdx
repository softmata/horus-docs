...
description: "Optimization techniques for Python HORUS nodes — topic types, GIL management, zero-copy patterns, ML framework interop, and profiling"
order: 14
---

# Performance Guide (Python)

Your Python node works. Now you want it faster. This page covers every optimization available to Python HORUS nodes — from topic type selection to ML framework interop — with concrete latency numbers so you can decide what matters for your application.
...
| `torch.from_numpy(img.to_numpy())` | ~3 us | Wraps the SHM view — zero-copy for CPU tensors |
...
Python Node
    ...
    ├── torch.from_numpy(img.to_numpy())  ~3 μs  (wraps the view, no copy)
    ├── jnp.asarray(img.to_numpy())       ~3 μs  (via NumPy view)
    │
...
**Key insight:** `to_numpy()` is zero-copy. `from_numpy()` copies once into the pool. Design your pipeline to minimize `from_*()` calls.
...
| `torch.from_numpy(img.to_numpy())` | No | ~3 us | Wraps the SHM view (CPU tensors) |
...
HORUS shares tensor data with PyTorch, JAX, and other frameworks through the zero-copy NumPy view — `torch.from_numpy()` / `jnp.asarray()` wrap it without copying (zero-copy for CPU tensors).
...
def ml_tick(node):
    ...
    # Zero-copy: SHM → NumPy view → PyTorch CPU tensor
    cpu_tensor = torch.from_numpy(img.to_numpy())  # ~3 μs, no copy
...
def jax_tick(node):
    ...
    # SHM → NumPy view → JAX array
    jax_array = jnp.asarray(img.to_numpy())  # ~3 μs
...
Use `.as_tensor()` to get a general-purpose Tensor from any pool-backed type, then bridge into any framework through its NumPy view:
...
pt = torch.from_numpy(t.numpy())       # zero-copy to PyTorch (CPU tensors)
...
```
torch.from_numpy(img.to_numpy())  ~3 μs     (SHM → CPU tensor, zero-copy)
...
| NumPy bridge to ML frameworks | Works with PyTorch, JAX, OpenCV, ONNX | Requires framework-specific import |
