...
description: "Virtual environments, dependency pinning, services, logging, monitoring, and production hardening for Python HORUS nodes"
order: 13
---
...
The `horus log` CLI command reads the structured log stream:

```bash
# Follow logs from all running nodes
horus log -f

# Filter by node name (positional)
horus log -f camera

# Filter by level
horus log -f --level warning
```
...
These go through the scheduler's logging pipeline, tagged with the node name and timestamp. They appear in `horus log` and, when running under systemd, in the journal.
