...
```python
# simplified
from horus import TransformFrame, HorusTimeoutError

tf = TransformFrame()

try:
    # A blocking wait with a deadline — raises HorusTimeoutError if it expires.
    # (node.recv() is non-blocking: it returns None when no message is ready,
    # so it never raises a timeout.)
    transform = tf.wait_for_transform("base_link", "lidar_link", timeout_sec=0.5)
except HorusTimeoutError:
    node.log_warning("Transform base_link->lidar_link not available within 500ms")
```
...
**Fatal** stops everything. The scheduler safes the faulted node (`enter_safe_state`), calls `shutdown()` on all nodes, and exits. Use this for motor controllers or safety monitors where an error means the robot is in an unknown state.
...
def safety_tick(node):
    check_limits()  # if this raises, failure_policy="fatal" stops everything
```
