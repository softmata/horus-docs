...
`horus run` resolves your Python dependencies automatically. It does **not** create a virtualenv — instead it generates `.horus/pyproject.toml` from your `horus.toml` and installs any declared PyPI dependencies with `pip`:

```bash
# Generates .horus/pyproject.toml and resolves declared PyPI deps via pip,
# then launches your node. No .horus/venv is created.
horus run src/main.py
```

```toml
# horus.toml — declare Python deps under [dependencies] with source = "pypi"
[dependencies]
numpy    = { version = ">=1.24", source = "pypi" }
pyserial = { version = ">=3.5",  source = "pypi" }
```

> `.horus/` holds generated build artifacts (including `pyproject.toml`) — never edit it by hand.

If you want an isolated interpreter, manage a virtualenv yourself and run the entry file directly:
...
- [Deployment Guide](/operations/deploy-to-robot) — General deployment (Rust + Python)
