...
### PyTorch — bridge via NumPy

```python
# simplified
import torch
tensor = torch.from_numpy(img.to_numpy())  # shares memory (zero-copy for CPU tensors)
```

`img.to_numpy()` returns a zero-copy `(H, W, C)` view into shared memory, and `torch.from_numpy()` wraps it without copying. Permute to CHW (`tensor.permute(2, 0, 1)`) and scale to `[0.0, 1.0]` for most PyTorch vision models.
...
```python
# simplified
from horus import CameraInfo, StereoInfo

# Both cameras share intrinsics here; the LEFT camera's fx drives the depth math.
cam = CameraInfo(width=640, height=480, fx=525.0, fy=525.0, cx=320.0, cy=240.0)
stereo = StereoInfo(left_camera=cam, right_camera=cam, baseline=0.12)  # 12cm baseline
```
...
| Field | Type | Description |
|-------|------|-------------|
| `left_camera` | `CameraInfo` | Left camera intrinsics — its `fx` is used for depth/disparity conversion |
| `right_camera` | `CameraInfo` | Right camera intrinsics |
| `baseline` | `float` | Distance between camera centers in meters |
| `depth_scale` | `float` | Scale factor applied to computed depth (default `1.0`) |
...
# simplified
cam = CameraInfo(width=640, height=480, fx=525.0, fy=525.0, cx=320.0, cy=240.0)
stereo = StereoInfo(left_camera=cam, right_camera=cam, baseline=0.12)  # 12cm between cameras
depth = stereo.depth_from_disparity(disparity=30.0)  # 0.12 * 525 / 30 = 2.1 m
# depth = baseline * fx / disparity  (fx taken from the left camera)
```
