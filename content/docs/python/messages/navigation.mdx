...
### `.from_waypoints(waypoints, goal_x, goal_y, goal_theta)` — Build from Array

```python
# simplified
plan = PathPlan.from_waypoints(
    waypoints=[[0.0, 0.0, 0.0], [3.0, 0.0, 0.0], [3.0, 4.0, 1.57]],
    goal_x=3.0, goal_y=4.0, goal_theta=1.57,
)
```

The goal pose is passed as three scalars (`goal_x`, `goal_y`, `goal_theta`), not a list.
