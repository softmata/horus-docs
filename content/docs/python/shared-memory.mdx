...
- **Overflow never blocks**: If a 1:1 subscriber cannot keep up, a full ring **rejects new sends** — `send()` retries briefly, then drops the new message; the queued messages are kept in publish order. Broadcast (multi-subscriber) topics instead overwrite the oldest (newest-wins). To always read the freshest 1:1 sample, drain with `recv_all()[-1]` or use a small capacity.
...
def my_tick(node):
    ...
    cmd = CmdVel(linear=0.5, angular=0.1)
...
This works because typed messages (`CmdVel`, `Imu`, `Pose2D`, `Pose3D`, `LaserScan`, `Odometry`, and the other 50+ types in the top-level `horus` module) are **POD types** — plain old data with no pointers, no heap allocations, and a fixed binary layout. The Rust and Python representations are byte-for-byte identical.
...
### ML Framework Interop via NumPy

Pool-backed types expose a zero-copy NumPy view (`to_numpy()`), and any framework that accepts NumPy arrays wraps it without copying. `torch.from_numpy()` shares the memory (zero-copy for CPU tensors); move to your own GPU with the framework's own transfer:
...
# Image to PyTorch — zero-copy via the SHM-backed NumPy view
img = node.recv("camera")
tensor = torch.from_numpy(img.to_numpy())   # torch.Tensor on CPU, backed by SHM

# Move to your GPU with the framework's own transfer
gpu_tensor = tensor.cuda()                   # CPU → GPU (PCIe copy)

# Tensor to PyTorch — shares the pool memory
features = node.recv("features")
pt = torch.from_numpy(features.numpy())

# Tensor to JAX — via NumPy view
jax_arr = jnp.asarray(features.numpy())
```

The NumPy view is backed directly by the shared-memory pool, so `torch.from_numpy()` / `jnp.asarray()` add no copy. Any NumPy-compatible framework (PyTorch, JAX, OpenCV, ONNX Runtime, TensorFlow) works through the same view.
...
Rust allocator ──► SHM pool ──► Python receives descriptor
                      ...
                      └──► torch.from_numpy(img.to_numpy())  ──► PyTorch tensor (same memory)
                      └──► jnp.asarray(img.to_numpy())       ──► JAX array (same memory)
```

`from_numpy()` copies once — it places data into the pool so it can be shared. `to_numpy()` does not copy — it creates a view into existing pool memory.

| Direction | Method | Copies | Why |
|-----------|--------|--------|-----|
| Python to SHM | `Image.from_numpy(arr)` | 1 | Data must go into a pool slot at a specific address |
| Python to SHM | `Tensor.from_numpy(t.numpy())` | 1 | Same reason |
| SHM to Python | `img.to_numpy()` | 0 | Returns a view into the pool slot |
| SHM to Python | `torch.from_numpy(img.to_numpy())` | 0 | Wraps the pool pointer (CPU tensors) |
...
**Topic naming**: Always use dots as separators (`sensor.imu`, `camera.rgb`), never slashes. Dots are strongly recommended, not validator-enforced — `/` passes topic-name validation but fails at runtime on macOS, where `shm_open` treats it as a path separator. Dot-separated names work identically on Linux and macOS.
...
**Why ring buffers instead of queues?** Ring buffers have a fixed memory footprint (allocated once at topic creation) and predictable access patterns. A growable queue would require dynamic allocation, which is unpredictable in real-time contexts. The fixed size also means overflow behavior is explicit: a full point-to-point ring rejects new sends (keeping the queued messages in order); a broadcast ring overwrites the oldest.

**Why never block on overflow?** A robot controller that blocks waiting for a full buffer to drain is a robot controller that crashes into a wall. So `send()` never blocks: on a full point-to-point buffer it drops the new message (the queued messages stay, in order); on a broadcast buffer it overwrites the oldest. If you need every message (log collection, recording), increase the capacity or use `send_blocking()`. If you need the freshest 1:1 sample, drain with `recv_all()[-1]`.
...
| Overflow never blocks (1:1 rejects newest; broadcast drops oldest) | Publisher runs at full speed | Subscribers that fall behind lose messages |
