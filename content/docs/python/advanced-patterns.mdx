...
For structured data that crosses to Rust, define a custom message and build it once, then send instances over a typed topic:
...
register_message("RobotDiagnostics", "diagnostics", [
    ("cpu_temp", "f32"),
    ("battery_pct", "f32"),
    ("error_code", "i32"),
    ("uptime_sec", "f64"),
...
from horus import RobotDiagnostics

def diagnostics_tick(node):
    node.send("diagnostics", RobotDiagnostics(
        cpu_temp=65.0,
        battery_pct=85.0,
        error_code=0,
        uptime_sec=horus.elapsed(),
...
# Declare the typed message on the node so the topic is typed end-to-end
node = horus.Node("diag", pubs={"diagnostics": RobotDiagnostics}, tick=diagnostics_tick, rate=1)
```

See [Custom Messages](/python/api/custom-messages) for the full define → build → use workflow.
