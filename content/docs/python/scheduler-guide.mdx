...
| 2x watchdog | Unhealthy | Record and report |
| 3x watchdog (critical node) | Isolated | Call `enter_safe_state()`, latch emergency stop |

<Callout type="info">
**Where the ladder runs.** The transitions are evaluated on the scheduler's
main loop — the one thread a hung node cannot block — and apply to every node
regardless of which executor owns it. Safing an executor-hosted node is a
request its executor honours between ticks, because `enter_safe_state()` needs
exclusive access to the node. A node hung *inside* `tick()` blocks that
executor, so it will not be safed there; the 3x rung latches an emergency stop
independently of that thread, which is what covers this case.

`Unhealthy` does **not** by itself stop a node being ticked on an executor.
The tick is what feeds the watchdog, so suppressing on watchdog health would be
self-reinforcing: a node marked `Unhealthy` would stop feeding, reach 3x, and
halt the robot on one transient overrun. Stopping a node is the job of the
separate degradation ladder (`Kill`), which sets a flag every executor honours.
</Callout>
