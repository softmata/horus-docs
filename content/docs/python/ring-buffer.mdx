...
You have a LiDAR node publishing scans at 40 Hz and a path planner consuming them at 10 Hz. Four scans arrive between each planner tick. Where do the extra three go? They sit in the ring buffer. When the planner calls `node.recv("scan")`, it gets the oldest unread scan -- not the latest. If the planner falls further behind and 1024 scans pile up, the buffer is full. The next scan is **rejected**: `send()` retries briefly, then drops the *new* scan, and the 1024 already queued stay put. No error. No exception. No backpressure. The publisher keeps running at full speed.

This is the fundamental contract of a HORUS **point-to-point** topic (one publisher, one subscriber): **publishers never block, subscribers never stall, and when the buffer fills, the newest message is dropped -- the queue keeps its oldest messages, in publish order.**

<Callout type="warning" title="This is reject-newest, not ROS 2 KEEP_LAST">
Coming from ROS 2, you may expect `KEEP_LAST` semantics, where a full queue evicts the *oldest* sample so the subscriber always sees the freshest data. HORUS point-to-point topics do the opposite: a full queue **rejects the newest** send and preserves the oldest, in order -- this keeps delivery in-order and the shared-memory write safe (see [Design Decisions](#design-decisions)). If you want freshest-wins for a 1:1 stream, drain with `recv_all()[-1]`, use a small capacity, or use a broadcast (multi-subscriber) topic (see [When Drops Are OK](#when-drops-are-ok-vs-when-they-are-not)). Broadcast topics *do* keep-newest / drop-oldest.
</Callout>
...
9th publish — REJECTED. msg9 is dropped; the buffer is unchanged:
  [msg1] [msg2] [msg3] [msg4] [msg5] [msg6] [msg7] [msg8]
   tail                                               head
   ^ recv() still returns msg1 (the oldest); msg9 never entered
```
...
| Overflow policy | Newest message rejected/dropped, oldest kept (no backpressure) |
...
| Publisher behavior on full buffer | Rejects the new message (drop-newest), continues at full speed |
...
If this continues indefinitely, the buffer fills to 1024. After that, every new publish is **rejected** -- the buffer keeps the 1024 images already queued, and newer frames are dropped until the subscriber drains some.

**This is not a bug.** The detection node is processing images slower than they arrive. The ring buffer absorbs the burst; once full it holds the *oldest* 1024 images (in publish order) and rejects newer ones. For perception you usually want the *newest* frame instead -- drain with `recv_all()[-1]` every tick so the queue never saturates (see [When Drops Are OK](#when-drops-are-ok-vs-when-they-are-not)).
...
For these cases, **drain** with `recv_all()` and use only the last message -- this empties the queue every tick so it never saturates and you always process the freshest frame. (A plain `recv()` one-per-tick would instead hand you the *oldest* queued message and reject newer ones once the buffer fills.)
...
**Why reject the newest message instead of dropping the oldest (point-to-point)?** When a 1:1 buffer is full and a new message arrives, HORUS **rejects the new one** and keeps the queued messages, in order. This is deliberate, for two reasons. **Safety:** a point-to-point topic is a lock-free in-order queue; a producer that overwrote the oldest slot could clobber a slot a consumer is mid-read on and stall the in-order consumer forever, so the producer never overwrites an unconsumed slot. **Ordering:** rejecting on full preserves gap-free FIFO delivery of everything already queued (see "Why `recv()` returns the oldest" below). The cost is that a *saturated* queue serves the oldest data -- if you need freshest-wins, drain with `recv_all()[-1]`, use a smaller capacity so it never saturates, or use a **broadcast** (multi-subscriber) topic. Broadcast rings use a latest-wins protocol with supersession detection, so they *can* safely overwrite the oldest (newest-wins) -- the right default for sensor fan-out to several consumers.
