...
- Raw `bytes` / `bytearray` (encode as a list of ints or a base64 string first)
...
### PyTorch and JAX interop (via NumPy)
...
# Image to PyTorch — torch.from_numpy shares memory (zero-copy for CPU tensors)
pt_tensor = torch.from_numpy(img.to_numpy())

# Tensor to PyTorch
pt_tensor = torch.from_numpy(tensor.numpy())

# JAX (via NumPy view)
jax_array = jnp.asarray(img.to_numpy())
```
...
### Without a build step: dict topics

HORUS has no runtime custom-message compiler -- there is no way to mint a new binary Pod type without a build. When you need custom fields and want to skip the build entirely, send a plain dict over a dict topic (see the **Dict Topics (GenericMessage)** section above). No code generation, no registration, instant iteration:

```python
# simplified
# Custom fields as a dict — no build step, no type registration
node.send("robot.status", {
    "battery_level": 85.0,
    "error_code": 0,
    "is_active": True,
    "motor_temp": 42.3,
...
Dict topics accept only MessagePack-serializable values -- primitives, lists, and nested dicts, with a 4KB payload cap. For a fixed schema with zero-copy performance, compile a Pod message instead (below).
...
robot = horus.Node(
    name="robot_brain",
    # Imu auto-derives the "imu" topic; bind Image to "camera.rgb" and the
    # Tensor costmap to "nav.costmap" so the typed topics match the tick's
    # recv/send names. "config" is a bare-string (dict) topic.
    subs=[horus.Imu, horus.Sub("camera.rgb", horus.Image), "config"],
    pubs=[horus.CmdVel, "detections", horus.Pub("nav.costmap", horus.Tensor)],
    tick=robot_tick,
    rate=30,
)
horus.run(robot)
