...
| Method | Signature | Returns | Description |
|--------|-----------|---------|-------------|
| `tick_once` | `tick_once()` | — | Execute one tick cycle across all nodes (lazy init on first call) |
...
- Ticks **all** nodes in order — per-node filtering is not supported for a single tick (only `tick_for()` accepts `node_names`)
...
- Passing `node_names` has no effect — it emits a `RuntimeWarning` and still ticks all nodes
...
| `delete_recording` | `delete_recording(name: str) -> bool` | `bool` | Delete a recorded session (`True` if found and deleted) |
