...
| `HorusBackpressureError` | `send_blocking()` is refused because the topic's backend overwrites unread messages and has no full ring to wait on. Not transient: it lasts as long as the topology that caused it |
