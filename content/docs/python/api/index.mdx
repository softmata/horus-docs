...
- `Scheduler` configuration — `tick_rate`, `rt`, `deterministic`, `watchdog_ms`, `blackbox_mb`, `cores`, `net` keyword arguments
...
| `Scheduler` kwargs | `tick_rate`, `rt`, `deterministic`, `watchdog_ms`, `blackbox_mb`, `recording`, `cores`, `net` |
...
There is no Python router client/server class. Networking is exposed two ways:
