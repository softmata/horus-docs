...
```python
# Register child frame under a parent
...
# Set the transform from parent to child
...
frames = tf_tree.frames()  # ["world", "base_link", "lidar", "camera"]
...
# Get last update timestamp for a frame
ts = tf_tree.get_timestamp("lidar")  # nanoseconds

# Wait for a frame to appear (blocking with timeout)
if tf_tree.wait_for_frame("lidar", timeout_ms=1000):
    print("Lidar frame available")
...
| Method | Returns | Description |
|--------|---------|-------------|
| `register_frame(name, parent)` | `bool` | Register a new frame |
| `unregister_frame(name)` | `bool` | Remove a frame |
| `update_transform(name, tf)` | `bool` | Set transform for a frame |
| `tf(source, target)` | `Transform` | Look up transform (latest) |
| `tf_at(source, target, ts)` | `Transform` | Look up at specific timestamp |
| `frames()` | `list[str]` | All registered frame names |
| `parent(name)` | `str` or `None` | Parent frame name |
| `children(name)` | `list[str]` | Child frame names |
| `get_timestamp(name)` | `int` | Last update timestamp (ns) |
| `wait_for_frame(name, timeout_ms)` | `bool` | Block until frame exists |
