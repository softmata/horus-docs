...
| Method | Returns | Description |
|--------|---------|-------------|
| `register_frame(name, parent)` | `int` | Register a new frame, returns frame ID |
| `unregister_frame(name)` | `None` | Remove a frame (raises on error) |
...
| `wait_for_transform(src, dst, timeout_sec)` | `Transform` | Block until transform is available |
