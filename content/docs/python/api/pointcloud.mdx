...
# From PyTorch tensor (bridge via NumPy)
import torch
tensor = torch.randn(10000, 3)
cloud = PointCloud.from_numpy(tensor.numpy())
```

| Factory | Parameters | Use case |
|---------|-----------|----------|
| `PointCloud(n, f, dtype)` | count, fields, dtype | Pre-allocate for filling |
| `PointCloud.from_numpy(arr)` | ndarray shape (N, F) | LiDAR driver output, ML model output (`model_out.cpu().numpy()`) |
...
torch_points = torch.from_numpy(cloud.to_numpy())

# To JAX — via NumPy view
jax_points = jnp.asarray(cloud.to_numpy())
```
