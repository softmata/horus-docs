...
# From PyTorch tensor — copies into pool (bridge via NumPy)
import torch
tensor = torch.zeros(480, 640, 3, dtype=torch.uint8)
img = Image.from_numpy(tensor.numpy(), encoding="rgb8")
...
| `Image.from_numpy(arr, enc?)` | ndarray, optional encoding | Camera capture, OpenCV output, ML model output (`model_out.cpu().numpy()`) |
...
# To PyTorch — via NumPy (torch.from_numpy shares memory, zero-copy for CPU tensors)
torch_tensor = torch.from_numpy(img.to_numpy())

# To JAX — via NumPy view
jax_array = jnp.asarray(img.to_numpy())
```

The `to_numpy()` method is zero-copy (~3 us). It returns a view into the shared memory pool — no pixel data is copied.
