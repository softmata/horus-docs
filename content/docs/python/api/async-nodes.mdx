...
class MyAsyncNode(horus.AsyncNode):
    async def setup(self):
        ...
        self.topic = horus.AsyncTopic("sensor_data", dict)
...
class AsyncTopic:
    def __init__(self, topic: str, msg_type: type):
        """Create async topic with message type"""
...
topic = horus.AsyncTopic("sensors", dict)
await topic.async_subscribe(handle_sensor_data)
```
...
class AsyncCloudNode(horus.AsyncNode):
    ...
    async def setup(self):
        self.topic = horus.AsyncTopic("sensor/processed", dict)
