...
- **Production Performance**: ~40-85ns cross-process latency (same shared memory as Rust)
...
| `SpmcShm` | ~70ns | Cross-process, single producer, multiple consumers |
...
|-----------|---------|----------|
| Local (`Topic(CmdVel)`) | ~40-85ns | Same-machine communication (auto-detected backend) |
