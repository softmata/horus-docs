...
- **Production Performance**: ~167ns cross-process latency (same shared memory as Rust)
- **Per-Node Rate Control**: Different nodes at different frequencies (100Hz sensor, 10Hz logger)
- **Message Timestamps**: every message type carries a nanosecond `timestamp_ns` field you set on publish
...
Every typed message carries a nanosecond `timestamp_ns` field, but nothing stamps it for you.
The publisher sets it on the message it sends, and the subscriber compares it against
...
HORUS Python supports network communication for distributed multi-machine systems. Topic works transparently over the network.
...
|---------|---------|-------------|
| `DirectChannel` | ~3ns | Same-thread direct channel |
| `SpscIntra` | ~18ns | Same-process, single producer/consumer |
| `MpscIntra` | ~26ns | Same-process, multiple producers |
| `MpmcIntra` | ~36ns | Same-process, multiple producers/consumers |
| `SpscShm` | ~85ns | Cross-process, single producer/consumer |
| `MpmcShm` | ~167ns | Cross-process, multiple producers/consumers |
...
### Router Endpoints (WAN/NAT Traversal)

For communication across networks or through NAT, address the topic through the router by
using the `"topic@router"` endpoint form. There is no Python router client or server class —
the endpoint string is the whole API:

```python
from horus import Topic, CmdVel

# Route this topic through the HORUS router (TCP broker)
topic = Topic(CmdVel, endpoint="cmdvel@router")
```
...
| Local MPMC (auto-selected `MpmcShm`) | ~167ns | Same-machine, multiple producers/consumers |
...
horus run tests/test_rate_control.py         # Per-node rates
horus run tests/test_timestamps.py           # Timestamps
