...
| `topic.backend_type` | Property: the active backend name (e.g., `"PodShm"`, `"SpmcShm"`); `"Unknown"` until the backend resolves |
...
| `topic.send_blocking(msg, timeout_s, node=None)` | Waits up to `timeout_s` seconds for room. Raises `horus.HorusTimeoutError` on timeout, `ValueError` for a negative, NaN or infinite timeout, and `NotImplementedError` on Image, PointCloud, DepthImage and Tensor topics |
...
`"Unknown"`, `"PodShm"`, `"SpscShm"`, `"SpmcShm"`, `"MpscShm"`, or `"FanoutShm"`.
