...
```python
def recv_all(topic: str) -> list
```

Receive ALL available messages as a list. Drains the buffer. Returns empty list if none available.

#### has_msg

```python
def has_msg(topic: str) -> bool
```

Check if at least one unread message is available on the topic.
...
Request the scheduler to shut down gracefully after the current tick completes.

| Method | Returns | Description |
|--------|---------|-------------|
| `node.log_info(msg)` | `None` | Log an informational message |
| `node.log_warning(msg)` | `None` | Log a warning message |
| `node.log_error(msg)` | `None` | Log an error message |
| `node.log_debug(msg)` | `None` | Log a debug message |
...
Framework clock functions — work in both normal and deterministic mode:

```python
import horus

horus.now()               # Current time in seconds (wall clock or SimClock)
horus.dt()                # Timestep for this tick in seconds
horus.elapsed()           # Time since scheduler start in seconds
horus.tick()              # Current tick number (int)
horus.budget_remaining()  # Time left in tick budget (float('inf') if no budget)
horus.rng_float()         # Random float [0, 1) — deterministic if deterministic=True
horus.timestamp_ns()      # Nanosecond timestamp for TF queries (int)
```

In deterministic mode (`run(..., deterministic=True)`), `now()` returns SimClock time, `dt()` is fixed `1/rate`, and `rng_float()` is tick-seeded — identical results across runs.

---

## Runtime Parameters

`horus.Params` — dict-like access to runtime configuration from `.horus/config/params.yaml`:

```python
import horus

p = horus.Params()              # Load from default location
p = horus.Params("path.yaml")  # Or explicit path

# Dict-like access
kp = p["pid_kp"]               # KeyError if missing
kp = p.get("pid_kp", 1.0)     # Default if missing
p["pid_kp"] = 2.5             # Set
p.save()                       # Persist to disk
...
"pid_kp" in p   # True
len(p)           # Number of params
list(p.keys())   # All param names
p.reset()        # Reset to defaults
```
...
`horus.Rate` — drift-compensated rate limiting for background threads (not for nodes — nodes use `rate=` kwarg):

```python
from horus import Rate

rate = Rate(30)  # 30 Hz target

while running:
    do_work()
    rate.sleep()  # Compensates for work time to maintain 30 Hz

rate.actual_hz()  # Measured frequency (smoothed)
rate.target_hz()  # Target frequency (30.0)
rate.period()     # Target period in seconds (0.033...)
rate.is_late()    # True if current cycle exceeded period
rate.reset()      # Reset after a pause
```
...
`horus.drivers` — load hardware connections from `horus.toml` `[drivers]` section:

```python
import horus

hw = horus.drivers.load()               # From horus.toml
hw = horus.drivers.load_from("path")    # Explicit path

# Typed accessors (return DriverParams)
bus = hw.dynamixel("arm")
lidar = hw.rplidar("lidar")
cam = hw.realsense("camera")
port = hw.serial("gps")

# DriverParams — dict-like config access
bus["port"]                # "/dev/ttyUSB0"
bus.get("baudrate", 115200)
bus.has("servo_ids")
list(bus.keys())

# Introspection
hw.list()          # ["arm", "lidar", "camera", ...]
hw.has("arm")      # True
len(hw)            # Number of drivers
"arm" in hw        # True

# Register Python driver class
horus.drivers.register_driver("MyDriver", MyDriverClass)
node = hw.local("my_device")  # Instantiates MyDriverClass(params)
```

All 15 Terra accessors: `dynamixel`, `rplidar`, `realsense`, `i2c`, `serial`, `can`, `gpio`, `pwm`, `usb`, `webcam`, `input`, `bluetooth`, `net`, `ethercat`, `spi`, `adc`, `raw`.
...
- [Memory Types](/python/api/memory-types) — Image, PointCloud, DepthImage (zero-copy)
...
- Enabling monitor to read Python node logs from `shm_base_dir()/logs (namespaced)`
