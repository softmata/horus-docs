...
scheduler.node(motor_ctrl).order(0).budget_us(200).rate_hz(1000.0).build()
scheduler.node(planner).order(5).compute().build()
scheduler.node(telemetry).order(10).async_io().rate_hz(1.0).build()
```
...
scheduler.add(controller, order=1, budget_us=200, on_miss="safe_mode", deadline_ms=5.0)
...
| `budget_us` | `int` | `None` | Tick budget in microseconds (auto-marks as RT) |
...
```python
# Via add()
scheduler.add(motor, order=0, budget_us=200, on_miss="safe_mode")

# Via fluent builder
scheduler.node(motor).order(0).budget_us(200).on_miss("safe_mode").build()
```

### Scheduler Profiles

Use profiles for common deployment patterns:

```python
from horus import Scheduler, SchedulerConfig

# Development — lightweight
scheduler = Scheduler(tick_rate=100.0)

# Production — safety + watchdog + fault tolerance + blackbox
scheduler = Scheduler(tick_rate=1000.0, profile="deploy")

# Hard real-time — panics without RT capabilities
scheduler = Scheduler(tick_rate=1000.0, profile="hard_rt")

# Safety-critical — hard_rt + blackbox + profiling
scheduler = Scheduler(tick_rate=1000.0, profile="safety_critical")
```
...
scheduler.add(controller, order=1, logging=False, rate_hz=50.0, budget_us=500)
...
HORUS defines specific exception types for better error handling:
...
|-----------|-------------|
| `HorusNotFoundError` | Topic, node, or resource not found |
| `HorusTransformError` | Coordinate transform error |
| `HorusTimeoutError` | Operation exceeded time limit |
