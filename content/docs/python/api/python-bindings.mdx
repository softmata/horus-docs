...
# Presets via config (preferred)
scheduler = horus.Scheduler(config=horus.SchedulerConfig.deploy())
scheduler = horus.Scheduler(config=horus.SchedulerConfig.safety_critical())
scheduler = horus.Scheduler(config=horus.SchedulerConfig.high_performance())
scheduler = horus.Scheduler(config=horus.SchedulerConfig.deterministic())
scheduler = horus.Scheduler(config=horus.SchedulerConfig.hard_realtime())

# Shorthand presets
scheduler = horus.Scheduler.deploy()                     # Production with RT priority + BlackBox
scheduler = horus.Scheduler.preset_safety_critical()     # Sequential, 1kHz, WCET enforcement
scheduler = horus.Scheduler.preset_high_performance()    # Parallel, 10kHz
scheduler = horus.Scheduler.preset_deterministic()       # Sequential, reproducible
scheduler = horus.Scheduler.preset_hard_realtime()       # Parallel, 1kHz, <5μs jitter
```
