...
# From configuration
config = horus.SchedulerConfig.builder().rate_hz(1000.0).build()
scheduler = horus.Scheduler(config)
...
| `"spmc"` | varies | SPMC intra-process — multiple consumers |
| `"mpmc"` | ~36ns | MPMC intra-process — multiple producers/consumers |
| `"spsc_shm"` | ~85ns | SPSC shared memory — cross-process |
| `"mpsc_shm"` | varies | MPSC shared memory — cross-process |
| `"spmc_shm"` | varies | SPMC shared memory — cross-process |
