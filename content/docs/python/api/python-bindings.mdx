...
| `topic.provides_backpressure` | Property: whether a full ring can make this topic refuse a send. Check it **after** the first send — the backend is unresolved until a handle moves a message, and an unresolved backend answers `False`. `False` means `send()` loses data silently and `send_blocking()` raises `HorusBackpressureError` |
