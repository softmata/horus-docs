...
Runtime node removal is **not supported**: the scheduler cannot detach a node once it is running (a "removed" actuator would keep commanding). Construct the scheduler without the node, or gate the node's work inside its own `tick()`.
...
#### `scheduler.tick_once() -> None`

Execute exactly one tick cycle — **all** nodes, in order — then return. Essential for deterministic testing: run one tick and verify output. There is no per-node filtering for a single tick; every node runs each call.
...
HORUS Python supports network communication for distributed multi-machine systems. Typed topics work transparently over the network via direct UDP endpoints.
...
| `topic.backend_type` | Property: the active SHM backend name (e.g., `"spsc_shm"`, `"pod_shm"`) |
...
print(f"Backend: {topic.backend_type}") # e.g. "spsc_shm"
...
Every topic is shared-memory-backed from the moment it is created, so a cross-process consumer can join later with no migration. HORUS picks one of four SHM backends automatically from the publisher/subscriber topology — you never configure this manually:

```python
# simplified
from horus import Topic, CmdVel

# Just create a topic — HORUS picks the right SHM backend automatically:
#   SpscShm   — 1 publisher : 1 subscriber
#   MpscShm   — N publishers : 1 subscriber
#   PodShm    — 1 publisher : N subscribers (POD broadcast)
#   FanoutShm — N : M (non-POD broadcast)
topic = Topic("cmd_vel", CmdVel)
```

**SHM backends:**

| Backend | Topology | Use |
|---------|----------|-----|
| `SpscShm` | 1 : 1 | One publisher, one subscriber |
| `MpscShm` | N : 1 | Many publishers, one subscriber |
| `PodShm` | 1 : N | One publisher fanning out to many (POD broadcast) |
| `FanoutShm` | N : M | Many publishers and subscribers (non-POD broadcast) |

**Latency is set by core locality, not by which backend or by the process boundary.** All four backends are shared-memory; what moves the latency cliff is whether the producer and consumer run on the same CPU core:

| Route | Latency (one-way) | Why |
|-------|-------------------|-----|
| Same thread, self-loop (pub+sub in one node) | ~12 ns | L1-cache-hot |
| Same thread, separate pub + sub node (one executor thread) | ~19 ns | L1-cache-hot |
| Cross-thread **or** cross-process | ~220 ns | cache-coherence physics — no backend choice moves it |

An intra-process heap ring would not beat SHM across cores: the ~220 ns is hardware cache-coherence, not transport overhead. The fastest realistic route is co-locating the producer and consumer on one executor thread.

### When to Use What

| Transport | Latency (one-way) | Use Case |
|-----------|-------------------|----------|
| Same-thread (`Topic(CmdVel)`, pub+sub co-located) | ~12-19ns | Pub and sub on one executor thread (fastest, SHM) |
| Cross-thread or cross-process (`Topic(CmdVel)`) | ~220ns | Same machine, different core (automatic, SHM) |
...
imu_topic = Topic(Imu)  # local shared memory (~12ns same-thread, ~220ns cross-core)
...
pytest tests/test_rate_control.py       # Per-node rate control
pytest tests/test_timestamps.py         # Message timestamps
pytest tests/test_topic_resolution.py   # Typed message routing

# With coverage
pytest --cov=horus tests/

# End-to-end multiprocess workflow
...
`horus.drivers` loads hardware node configs from `horus.toml`'s `[hardware]` section.

### Module Functions

| Function | Returns | Description |
|----------|---------|-------------|
| `drivers.load()` | `list[(str, obj)]` | Load hardware entries from `horus.toml` |
| `drivers.load_from(path)` | `list[(str, obj)]` | Load from explicit config path |
| `drivers.register_driver(name, cls)` | — | Register a Python node class |
