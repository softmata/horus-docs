...
Every method in that table except `run`, `stop` and `status` raises
`RuntimeError("... unavailable while scheduler is running")` if you call it from
inside a tick callback. `run()` takes the native scheduler out of the Python
object to drive it, so nothing can read its state until the run returns — the
methods used to answer with a placeholder instead (zero nodes, tick 0, `False`
for `is_recording()`), which is indistinguishable from a real reading. Call them
before `run()` or after it returns. `status()` answers `"running"`, which is the
one honest thing available.
...
Then check the tick rate is not higher than the work needs:

```python
node = horus.Node(tick=fn, rate=30)  # 30 Hz is reasonable
```
