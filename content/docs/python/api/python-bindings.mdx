...
scheduler.node(motor_ctrl).order(0).budget_us(200).rate(1000.0.hz()).build()
scheduler.node(planner).order(5).compute().build()
scheduler.node(telemetry).order(10).async_io().rate(1.0.hz()).build()
```
...
scheduler.add(logger, order=2, rate=10.0, logging=True)
...
scheduler = horus.Scheduler()
scheduler.add(sensor, order=0, logging=True, rate=100.0)
scheduler.add(controller, order=1, logging=False, rate=50.0, budget_us=500)
scheduler.add(logger, order=2, logging=True, rate=10.0)
