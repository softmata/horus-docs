...
| Type | Create | From NumPy | To NumPy | To PyTorch | To JAX |
|------|--------|------------|----------|------------|--------|
| `Image` | `Image(h, w, enc)` | `Image.from_numpy(arr)` | `img.to_numpy()` | `img.to_torch()` | `img.to_jax()` |
| `PointCloud` | `PointCloud(n, f, dtype)` | `PointCloud.from_numpy(arr)` | `pc.to_numpy()` | `pc.to_torch()` | `pc.to_jax()` |
| `DepthImage` | `DepthImage(h, w, dtype)` | `DepthImage.from_numpy(arr)` | `d.to_numpy()` | `d.to_torch()` | `d.to_jax()` |

All `to_*()` methods are zero-copy (~3us). `from_*()` methods copy data into the shared memory pool (one copy at publish time).

## Design Decisions

**Why pool-backed instead of heap-allocated?** Pool-backed memory enables cross-process sharing via shared memory. A heap-allocated NumPy array lives in one process's address space and must be serialized for IPC. Pool-backed types live in shared memory from the start, so `topic.send()` only copies a small descriptor (64-128 bytes), not the megabytes of pixel/point data.

**Why DLPack for PyTorch/JAX instead of direct memory sharing?** DLPack is the standard protocol for zero-copy tensor exchange across ML frameworks (adopted by NumPy 1.25, PyTorch 1.10, JAX 0.4, CuPy, TensorFlow). Using DLPack means HORUS works with any DLPack-compatible framework without framework-specific code. The `__dlpack__()` and `__dlpack_device__()` methods implement the Python DLPack protocol.

**Why `from_numpy()` copies but `to_numpy()` doesn't?** Publishing requires placing data at a specific shared memory pool slot (chosen by the pool allocator). NumPy arrays live at arbitrary heap addresses that can't be shared across processes. So `from_numpy()` copies once into the pool. On the receiving side, `to_numpy()` returns a view into the already-shared pool memory -- zero copies.

**Image vs PointCloud vs DepthImage:** These are pool-backed equivalents of the fixed-size message types. Use `Image` for camera frames, `PointCloud` for LiDAR/3D data, `DepthImage` for depth maps. For other large data shapes, use `GenericMessage` or define a custom message.

**Thread safety:** Pool-backed types are safe to pass between threads. The underlying shared memory pool uses atomic reference counting. However, the NumPy/PyTorch views returned by `to_numpy()` / `to_torch()` should not outlive the source object -- when the HORUS type is dropped, the pool slot may be reclaimed.
