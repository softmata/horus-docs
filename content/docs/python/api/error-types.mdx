...
tf.register_frame("base_link")  # register the root frame first — HORUS has no auto-root

try:
    # "nonexistent_frame" was never registered
    result = tf.tf("base_link", "nonexistent_frame")
...
tf.update_transform("camera", horus.Transform())  # broadcast at 'now'

try:
    # Strict lookup 5s in the past — outside the buffer window, so it extrapolates
    stale_ts = horus.timestamp_ns() - 5_000_000_000
    result = tf.tf_at_strict("base_link", "camera", stale_ts)
...
try:
    # 'gripper' is never broadcast — wait up to 1 second, then time out
    result = tf.wait_for_transform("base_link", "gripper", timeout_sec=1.0)
...
def tick(node):
    try:
        transform = tf.tf("base_link", "camera")
