...
pt = torch.from_numpy(received.numpy())      # PyTorch (zero-copy for CPU tensors)
```
...
t = Tensor.from_numpy(pt.cpu().numpy())       # from PyTorch (via NumPy)
```
...
```python
# simplified
pt = torch.from_numpy(t.numpy())          # zero-copy into PyTorch (CPU tensors)
t = Tensor.from_numpy(pt.cpu().numpy())   # copy into shared memory

# Use directly in models
features = model(torch.from_numpy(t.numpy()))
action = Tensor.from_numpy(policy(obs_tensor).cpu().numpy())
```
...
t.shape                          # [480, 640, 3]
torch.from_numpy(t.numpy())      # works
...
# PyTorch inference
pt_obs = torch.from_numpy(obs.numpy())
with torch.no_grad():
    action = policy(pt_obs)

# Send command
cmd = horus.Tensor.from_numpy(action.cpu().numpy())
cmd_topic.send(cmd)
```
...
# simplified
# Detection node
img_tensor = torch.from_numpy(img.as_tensor().numpy())
features = backbone(img_tensor.unsqueeze(0).float() / 255.0)
feature_map = horus.Tensor.from_numpy(features.squeeze(0).cpu().numpy())
