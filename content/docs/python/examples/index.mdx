...
sensor = horus.Node(name="sensor", pubs=["scan"], tick=sensor_tick, rate=100)
controller = horus.Node(name="ctrl", subs=["scan"], pubs=["cmd"], tick=ctrl_tick, rate=100)
logger = horus.Node(name="logger", subs=["cmd"], tick=log_tick, rate=10)

scheduler = horus.Scheduler(tick_rate=100, watchdog_ms=500)

# Critical — runs first, safe mode on deadline miss
scheduler.add(sensor, order=0, rate=100)

# RT control — runs after sensor
scheduler.add(controller, order=1, rate=100, on_miss="safe_mode")

# Non-critical — skip on repeated failures
scheduler.add(logger, order=10, failure_policy="skip")
...
with horus.Scheduler(tick_rate=100) as sched:
    sched.add(sensor, order=0, rate=100)
    sched.add(controller, order=1, rate=100)
...
For execution classes (compute, async I/O, event-driven):
...
scheduler.node(planner).compute().rate(10).build()

# Async I/O: non-blocking network/file operations
scheduler.node(uploader).async_io().rate(1).failure_policy("ignore").build()
...
def physics_tick(node):
    dt = horus.time_dt()                    # Fixed in deterministic mode
    elapsed = horus.time_elapsed()          # Time since start
    tick = horus.time_tick()                # Tick number
    rng = horus.time_rng_float()            # Deterministic random [0, 1)
    budget = horus.time_budget_remaining()  # Seconds left in budget
...
|----------|---------|-------------|
| `horus.time_now()` | `float` | Current time (seconds) |
| `horus.time_dt()` | `float` | Timestep (seconds) |
| `horus.time_elapsed()` | `float` | Time since start (seconds) |
| `horus.time_tick()` | `int` | Current tick number |
| `horus.time_budget_remaining()` | `float` | Budget left (seconds, `inf` if none) |
| `horus.time_rng_float()` | `float` | Random in [0, 1) |
...
scheduler.node(node).compute().order(5).build()  # Runs on compute pool
scheduler.run()
```
