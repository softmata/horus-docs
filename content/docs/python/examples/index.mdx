...
Use the framework time functions for deterministic-compatible code:

```python
import horus

class PhysicsNode(horus.Node):
    """Physics integration using framework time"""
    ...
    def tick(self, info):
        # horus.time_dt() returns fixed dt in deterministic mode
        dt = horus.time_dt()
        self.position += self.velocity * dt

        # Deterministic random noise
        noise = horus.time_rng_float() * 0.02 - 0.01
        self.velocity += noise

        # Framework elapsed time
        if horus.time_elapsed() > 10.0:
            horus.hlog("info", f"Position after 10s: {self.position:.3f}")

        # Budget-aware computation
        budget = horus.time_budget_remaining()
        if budget > 0.001:  # More than 1ms left
...
|----------|---------|-------------|
| `horus.time_now()` | `float` | Current time in seconds |
| `horus.time_dt()` | `float` | Timestep in seconds |
| `horus.time_elapsed()` | `float` | Time since scheduler start in seconds |
| `horus.time_tick()` | `int` | Current tick number |
| `horus.time_budget_remaining()` | `float` | Seconds left in tick budget (`inf` if no budget) |
| `horus.time_rng_float()` | `float` | Random float in [0, 1) (deterministic in deterministic mode) |

---

## Scheduler Configuration

Python scheduler with execution classes and failure policies:

```python
import horus

# Create scheduler with RT features
scheduler = horus.Scheduler(
    tick_rate=100,
    prefer_rt=True,
    watchdog_ms=500,
...
# Compute-heavy planner on thread pool
scheduler.add(
    PathPlanner(),
    order=5,
...
scheduler.add(
    TelemetryUploader(),
    order=200,
    execution_class="async_io",
    rate=1,
)

# Run with deterministic mode for testing
scheduler.set_deterministic(True)

# Single tick (for simulation loop)
scheduler.tick_once()
...
- [Python Bindings](/docs/python/api/python-bindings) - Core Python API
- [Message Library](/docs/python/library/python-message-library) - Available message types
- [Time API (Rust reference)](/docs/rust/time-api) - Full time API documentation
- [Deterministic Mode](/docs/advanced/deterministic-mode) - Deterministic execution guide
