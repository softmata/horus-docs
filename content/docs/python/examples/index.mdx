...
|---------|-------------|--------------|
| `simple_async.py` | Basic async node usage | AsyncNode, AsyncTopic |
...
| `network_topic_example.py` | Network Topic communication | UDP transport, remote nodes |
| `network_topic_example.py` | Network Topic communication | Point-to-point, low latency |
...
class SensorNode(horus.AsyncNode):
    ...
    async def setup(self):
        self.sensor_pub = horus.AsyncTopic("sensor_data", float)
...
network_topic = Topic(CmdVel, endpoint="cmdvel@192.168.1.100:9000")

# Automatic transport selection
auto_topic = Topic(CmdVel, endpoint="cmdvel@auto")  # Uses optimal transport

node = Node("network_example")

# Send locally
local_topic.send(CmdVel(1.0, 0.0), node)

# Send over network
network_topic.send(CmdVel(1.0, 0.0), node)

# Receive from network
msg = network_topic.recv(node)
if msg:
