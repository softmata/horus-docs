...
**Backend selection is automatic — there is no `backend=` argument.** HORUS picks
DirectChannel (~3ns) for same-thread callbacks, SpscIntra (~18ns) for a single
producer/consumer inside one process, and MpmcShm (~167ns) across processes. The only
tuning knobs are `capacity` and `endpoint`:
...
session = None  # your onnxruntime.InferenceSession, loaded in init
