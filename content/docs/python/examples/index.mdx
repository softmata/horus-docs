...
| [Cross-Language](#cross-language-communication) | Python-Rust communication | Network topics, endpoint |
...
# Network transport (cross-machine)
net_topic = Topic(CmdVel, endpoint="cmd_vel@192.168.1.100:9000")
```
...
print(f"Avg: {stats['avg_ms']:.1f}ms, P95: {stats['p95_ms']:.1f}ms, FPS: {stats['fps']:.0f}")
...
Python and Rust nodes communicate via typed topics over the network:

```python
from horus import Topic, CmdVel
import time

# Create network topic (sends to Rust node on another machine)
topic = Topic(CmdVel, endpoint="cmd_vel@192.168.1.5:9000")
...
while True:
    ...
    topic.send(CmdVel(linear, angular))
    time.sleep(0.1)
```

**Rust subscriber on the other end:**
```rust
use horus::prelude::*;
...
Using the built-in hardware nodes:

```python
from horus import Node, Scheduler
from horus import SerialNode, JoystickNode, CameraNode

# Serial communication with Arduino
serial = SerialNode(port="/dev/ttyUSB0", baudrate=115200, topic_prefix="arduino")

# Joystick input
joy = JoystickNode(device_id=0, topic_prefix="joy")

# Camera capture
cam = CameraNode(device_id=0, width=640, height=480, topic_prefix="cam")

# Process joystick to motor commands
def teleop_tick(node):
    if node.has_msg("joy.state"):
        state = node.get("joy.state")
        # Map joystick axes to motor commands
        linear = state.axes[1] if len(state.axes) > 1 else 0.0
        angular = state.axes[0] if len(state.axes) > 0 else 0.0
        node.send("arduino.tx", f"{linear:.2f},{angular:.2f}\n")
...
scheduler = Scheduler()
scheduler.add(serial, order=0)
scheduler.add(joy, order=1)
scheduler.add(teleop, order=2)
scheduler.add(cam, order=3)
scheduler.run()
```
...
- [Hardware Nodes](/python/library/python-hardware-nodes) - Built-in hardware nodes
