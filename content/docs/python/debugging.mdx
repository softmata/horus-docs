...
A message is "dropped" when the publisher writes to a full ring buffer. On a point-to-point (1:1) topic the *new* message is rejected (the queue keeps its oldest messages, in order); on a broadcast (multi-subscriber) topic the oldest is overwritten by the newest. Either way, a message is lost and the subscriber never sees it.

### What Causes Drops

Drops happen when the publisher sends faster than the subscriber reads. The ring buffer has a fixed capacity (default: 1024 messages). When it fills up, a point-to-point topic rejects new messages (keeping the oldest queued, in order); a broadcast topic overwrites the oldest with the newest.
