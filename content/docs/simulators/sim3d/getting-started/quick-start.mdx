...
fn main() -> Result<()> {
    ...
    let cmd_topic: Topic<Twist> = Topic::new("scout.cmd_vel")?;

    // Send velocity command
    let cmd = Twist::new_2d(0.5, 0.1);  // 0.5 m/s forward, 0.1 rad/s turn
    cmd_topic.send(cmd, &mut None)?;
...
fn main() -> Result<()> {
    ...
    let odom_topic: Topic<Odometry> = Topic::new("scout.odom")?;
    let scan_topic: Topic<LaserScan> = Topic::new("scout.scan")?;
    let imu_hub: Topic<Imu> = Topic::new("scout.imu")?;

    loop {
        // Read odometry
        if let Some(odom) = odom_topic.recv(&mut None) {
