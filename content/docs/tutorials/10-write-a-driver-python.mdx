...
second Python node in the same process can push this one over the line. An
overrun is counted and logged, nothing more: it does not stop the node or the
robot, because the deciding knob is the node's `BudgetPolicy`, whose default is
`Warn` — take no corrective action — and Python cannot change it.
