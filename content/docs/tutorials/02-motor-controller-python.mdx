...
# ── Main ────────────────────────────────────────────────────────────────
# deterministic=True keeps every node on the main tick loop, sequential
...
The encoder runs BEFORE the controller so the controller always has fresh feedback. `deterministic=True` is what turns that into a guarantee: it keeps every node on the main tick loop and executes them sequentially in dependency-graph order. No node registers a topic during startup here, so those steps come from the `order=` tiers and the encoder really does go first — but the rule is the graph, not the tier number: a program that publishes or subscribes at startup gets its order from the topics instead. Without it, every node here is an RT node — `rate=` alone promotes a node to `ExecutionClass::Rt`, and all three declare `rate=100` — so all three leave the main tick loop for the RT executor, which runs each RT node on its own thread. `order=` sequences nodes on the main loop, not across RT threads, so the encoder/controller interleaving is no longer guaranteed.
