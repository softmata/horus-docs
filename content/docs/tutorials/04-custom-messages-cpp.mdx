...
`horus_c.h` is a hand-maintained mirror of `c_api.rs` — nothing generates it (`horus_cpp/build.rs` is an empty stub). The specialization added in Step 5 calls nine `extern "C"` functions (`horus_publisher_weather_data_new/_destroy/_send/_dropped_count` and `horus_subscriber_weather_data_new/_destroy/_recv/_has_msg/_missed_count`), so they must be declared by hand first, or the build fails with `use of undeclared identifier 'horus_publisher_weather_data_new'`.
...
The macro is `#define`d partway down the file and `#undef`'d after the last invocation, so the line must land between them. Placed anywhere else — for example next to the explicit `horus_publisher_temperature_*` prototypes higher up in the file — it will not compile. (Equivalently, you can write all nine prototypes out longhand in the style of those `temperature` declarations. `dropped_count()` and `missed_count()` are ordinary members of the specialization, not templates, so omitting their two prototypes is a compile error even if you never call them.)
...
- The `HORUS_TOPIC_IMPL` macro generates the entire Publisher/Subscriber specialization in one line — once the nine C prototypes from Step 4 exist

## Next Steps

- [Tutorial 5: Hardware & Real-Time (C++)](/tutorials/05-hardware-rt-cpp) — drive real hardware with these messages
- [Tutorial 4: Custom Messages (Python)](/tutorials/04-custom-messages-python) — the same message from the Python side

## See Also

- [Topics & Communication](/cpp/api/topic) — full `message!` and `Topic<T>` reference
- [Standard Messages](/cpp/api) — the 50+ built-in types, before you write your own
