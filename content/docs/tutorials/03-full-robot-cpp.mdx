...
int main() {
    ...
    MotorDriver motors;
    // `on_miss` fires on a *deadline* miss, so the node needs a timing bound for it to
    // mean anything — a budget auto-derives the deadline.
    sched.add(motors).order(20).budget(5_ms).on_miss(horus::Miss::SafeMode).build();  // 100 Hz
