...
def imu_sensor_tick(node):
    """Simulate an IMU sensor publishing acceleration and angular velocity."""
    # Imu(accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, timestamp_ns=0)
    # Fields are flat scalars — there is no zero-argument constructor.
    imu = horus.Imu(0.0, 0.0, 9.81, 0.0, 0.0, 0.0)
    node.send("imu", imu)
...
def controller_tick(node):
    ...
    if imu is not None:
        ...
        # Read acceleration via flat scalars (.accel_x/.accel_y/.accel_z) or
        # the vector accessor imu.linear_acceleration_vec().
        tilt = abs(imu.accel_x)
...
<Callout type="info">`horus.toml` uses `[package]` (not `[project]`), and there is no `entry` field — the file to run is auto-detected (`main.py`) or passed to `horus run` positionally.</Callout>

Run it:

```bash
horus run typed_demo.py
```
...
For Python-only custom messages at high frequency, use `horus.msggen` to register a message schema, then compile it with `build_messages()` (which runs `maturin`) for near-native performance (~3-5us). This approach requires a one-time build step but gives you a typed, binary-serialized message without writing any Rust yourself.
...
First, register the message schemas and compile them. Create `gen_messages.py`:

```python
# gen_messages.py — run ONCE to generate and compile the custom message types.
from horus.msggen import register_message, build_messages

# register_message(name, topic, fields) records a schema. It returns None —
# the usable class is produced by the build step below, not by this call.
...
register_message("WheelSpeed", "wheel.speed", [
    ("left_rpm", "f32"),
    ("right_rpm", "f32"),
    ("timestamp_ns", "u64"),
])

# Generates Rust PyO3 code for each schema and runs `maturin develop --release`.
# Requires maturin (`pip install maturin`). Afterwards the compiled classes are
# importable as `from horus import RobotStatus, WheelSpeed`.
build_messages()
```

Run the generator once — this is the required build step (there is no interpreted fallback):
...
Then use the generated types in your application. Create `compiled_demo.py`:

```python
import horus
from horus import RobotStatus, WheelSpeed  # compiled by gen_messages.py
...
# --- Node definitions ---
# The generated classes carry their own topic name, so register them as TYPED
# topics — a compiled message cannot be sent over a bare-string topic. Send/recv
# still address the topic by name ("robot.status", "wheel.speed").
...
Run the app (after `gen_messages.py` has compiled the types):

```bash
horus run compiled_demo.py
```

The compiled classes use binary serialization instead of Python's `struct` module, giving ~3-5us latency.
...
| **Dict** (plain dict) | ~6-50us | No (Python only) | No | Prototyping, flexible schemas |
...
**Do you need a custom type at high frequency?** Use compiled messages (Approach 4). Register the schema with `register_message()`, compile it with `build_messages()` (which runs `maturin`), and get ~3-5us latency with binary serialization. This is the right choice for production Python nodes running at 50Hz or above with custom data.
...
- [Python API: Custom Messages](/python/api/custom-messages) -- `register_message()`, `build_messages()`, and compiled workflows
