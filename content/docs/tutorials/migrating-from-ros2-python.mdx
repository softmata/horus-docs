...
Python topics are shared-memory only. A Python node reaches another machine only when a Rust process built with the `net` feature replicates the topic for it — `horus run --net` — and which topics cross is decided by the `[network]` section of `horus.toml`, not in Python — though `[network]` is only applied when `horus run` launches several executables; for a single binary set the `HORUS_NET_*` variables in the environment. If your ROS2 system relies on DDS discovery across hosts, that part of the migration is a Rust-side replicator, not a Python change. See [Python Bindings](/python/api/python-bindings#multi-machine-systems).
</Callout>
