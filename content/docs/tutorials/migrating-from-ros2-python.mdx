...
Python topics are shared-memory only. A Python node reaches another machine only when a Rust process built with the `net` feature replicates the topic for it — `horus run --net` — and which topics cross is decided by the `[network]` section of `horus.toml`, not in Python. Every `horus run` form applies that section, including a single binary; only `horus launch` does not, and there you set the `HORUS_NET_*` variables in the environment instead. If your ROS2 system relies on DDS discovery across hosts, that part of the migration is a Rust-side replicator, not a Python change. See [Python Bindings](/python/api/python-bindings#multi-machine-systems).
</Callout>
...
The transport under a Python topic is the same shared-memory ring the Rust and C++ nodes use — around 40-85 ns depending on the backend, and 75 ns measured between two nodes for a 16-byte `CmdVel`. Python does not get those numbers. Every `send()` and `recv()` crosses PyO3 and takes the GIL, and that crossing, not the transport, is what you are actually paying for: **budget microseconds, not nanoseconds**, for Python-to-Python traffic.
