...
horus.run(
    horus.Node(
        name="ai_detector",
        subs=[horus.Sub("sensor.data", horus.CmdVel)],
        pubs=[horus.Pub("ai.decision", horus.CmdVel)],
        tick=detector_tick,
        rate=10
    )
)
...
```bash
# Terminal 1: C++ sensor
horus run cpp_sensor/

# Terminal 2: Python AI
horus run py_detector/
