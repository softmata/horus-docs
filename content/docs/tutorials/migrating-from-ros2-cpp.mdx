...
HORUS C++ FFI adds **~11-21ns** per call (vs ~1-5us DDS serialization in ROS2). Scheduler tick with one node: **~250-550ns** (vs ~10-50us in rclcpp). Throughput: **~1-2.9M ticks/sec**. These figures are hardware-dependent — reproduce them on your own machine from the repo root:
