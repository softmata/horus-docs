...
| QoS | `rclcpp::QoS(10).reliable()` | `.budget(5_ms).on_miss(Miss::Skip)` |
...
See [C++ Performance](/cpp/performance) for the full measurement table.

## Next Steps

- [Tutorial 1: IMU Sensor Node (C++)](/tutorials/01-sensor-node-cpp) — the publish/subscribe pattern from scratch
- [Tutorial 2: Motor Controller (C++)](/tutorials/02-motor-controller-cpp) — budgets, `enter_safe_state()` and closed-loop control
- [Tutorial 3: Full Robot System (C++)](/tutorials/03-full-robot-cpp) — six nodes, parameters and transforms in one scheduler
