...
For better performance and cross-language compatibility, use typed messages instead of dicts:

```python
import horus

def tick(node):
    imu = horus.Imu()  # typed message (~50ns vs ~4μs for dicts)
    # ... set fields ...
    node.send("imu.data", imu)

sensor = horus.Node(name="ImuSensor", tick=tick, rate=100, order=0,
