...
print(f"Drivers: {hw.list()}")  # ["arm", "imu", "lidar"]

if hw.has("arm"):
    arm = hw.dynamixel("arm")
    port = arm.get_or("port", "/dev/ttyUSB0")
    servo_ids = arm.get_or("servo_ids", [1, 2, 3])
    print(f"Arm config: {port}, servos: {servo_ids}")
    # Create your own hardware connection using port/baudrate

if hw.has("lidar"):
    lidar = hw.rplidar("lidar")
    scan_mode = lidar.get_or("scan_mode", "standard")
    print(f"LiDAR config: {scan_mode} mode")
```
...
If `horus.toml` has no `[drivers]` section, `drivers.load()` raises an exception. Wrap it for graceful fallback:
