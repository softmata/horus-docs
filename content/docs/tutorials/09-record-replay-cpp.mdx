...
Each tick the recorder snapshots the latest value on every topic your nodes publish or subscribe to, stamped with a microsecond wall-clock timestamp and the tick number.
...
`horus blackbox --last 100` is a separate store: it shows the events the
