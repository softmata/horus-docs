...
recording loaded yet they do nothing and raise no error — you get a full-speed
replay of the whole recording. `set_replay_speed()` gives you no sign of this at
all; `stop_at_tick()` prints `[REPLAY] Will stop at tick N` whether or not the
tick was stored, so that line is not evidence the setting took effect.
`horus record inject` applies them in this same order internally.
</Callout>
...
class Controller(horus.Node):
    def __init__(self):
        super().__init__(
            ...
            rate=100,
            order=10,
            subs=[horus.Sub("lidar.scan", LaserScan)],
            pubs=[horus.Pub("cmd_vel", CmdVel)],
        )
    ...
    def tick(self, info=None):
        ...
        self.send("cmd_vel", CmdVel(linear=linear, angular=0.0))
...
Declare the message type inside `subs=`/`pubs=` rather than declaring the name
there and building a separate `Topic` for it. A bare `subs=["lidar.scan"]`
opens the topic as an untyped `GenericMessage` ring, and a following
`Topic(LaserScan, endpoint="lidar.scan")` on that same name is then rejected —
the ring holds 4364-byte messages and `LaserScan` is 1480, so the constructor
raises before the node is built.
