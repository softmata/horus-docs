...
the client's feedback/result topics. Run the [Action Server](#action-server) from
the next section as a separate binary before running this, or the poll loop has
nothing to receive.
...
int main() {
    ...
    // Requires the "navigate" action server (next section) running elsewhere.
