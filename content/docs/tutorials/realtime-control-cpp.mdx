...
int main() {
    ...
    sched.tick_rate(1000_hz)
         .name("rt_control")
         .blackbox(4)                 // 4 MB flight recorder — nothing is recorded without this
         .prefer_rt();
...
- `horus::blackbox::record()` is a log-buffer write, not a flight-recorder entry — that line comes out of `horus log`. `.blackbox(N)` on the scheduler arms the recorder, and the scheduler fills it by itself with budget violations, deadline misses, node errors and start/stop, each with the measured microseconds
