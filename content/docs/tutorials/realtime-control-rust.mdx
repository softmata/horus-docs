...
with `sudo setcap 'cap_sys_nice=ep' .horus/target/release/<your-binary>` — `horus
build` puts artifacts under `.horus/target/<profile>/`, and `<your-binary>` is
...
scheduler. See [RT Configuration](/advanced/rt-config), or switch to
