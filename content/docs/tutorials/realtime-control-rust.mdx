...
<Callout type="warning" title="There is no `horus::blackbox::record()` in Rust">
...
stops the robot once the process-wide `max_deadline_misses` ceiling — 100 by
default — is crossed.
...
`horus::horus_core::scheduling::BudgetPolicy`, not in the prelude), or
`.on_miss(Miss::Stop)` for the first deadline miss.
</Callout>

<Callout type="warning" title="A debug build will trip the budget immediately">
