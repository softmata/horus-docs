...
The encoder runs BEFORE the controller so the controller always has fresh feedback. `deterministic(true)` is what turns that into a guarantee: it keeps every node on the main tick loop and executes them sequentially in dependency-graph order. Neither node registers a topic during `init()` here, so those steps come from the `.order()` tiers and the encoder really does go first — but the rule is the graph, not the tier number: a program that publishes or subscribes during `init()` gets its order from the topics instead. Without it, `.budget(5_ms)` promotes `MotorController` to an RT node that the scheduler hands to its own executor thread — `.order()` then only sorts nodes *within* an executor, and the encoder/controller interleaving is no longer guaranteed.
