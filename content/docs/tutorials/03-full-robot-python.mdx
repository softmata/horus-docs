...
class MotorDriver(horus.Node):
    def __init__(self):
        ...
        # for it to mean anything — rate= already auto-derives one at 95% of
        # the period (4.75 ms here); a budget only fills the deadline in when
        # there is no rate.
        super().__init__(
    ...
    def tick(self, info=None):
        self.info = info    # redundant here: the base init() already stored it.
        self.ticks += 1     # Only matters if you also override init().
        if self.ticks % 2 != 0:
            return  # 100 Hz
...
| Motors | 100 Hz | 20 | **safe_mode** | Deadline miss (4.75 ms, auto-derived from `rate=200`) → stop motors |
...
or `deadline=` never reaches it — the policy is silently inert. Every node here sets
`rate=200`, so every node already has a deadline; the `budget=` on Motors is a
separate, tighter accounting bound, not what makes its `safe_mode` policy fire.
...
counter (`ticks % 20`), so they share one nominal rate instead of each declaring
its own.

Be aware of what `rate=` costs you, though: a node given a rate is promoted to
the **Rt** execution class and moves onto the RT executor, which gives no
ordering guarantee between RT nodes. So `order=` does *not* describe the data
flow here — what sequences these nodes is the topics they publish and subscribe
to. The Rust version of this tutorial sets the rate once on the scheduler
(`.tick_rate(200_u64.hz())`) and leaves the nodes best-effort, which is why its
nodes really do share one tick sequence.
