...
package-type = "driver"
license = "Apache-2.0"
categories = ["motor", "serial", "driver", "actuator"]

[dependencies]
serialport = { version = "4.5", source = "crates.io" }
```

The `package-type = "driver"` metadata tells the registry this is a driver package, which affects how it shows up in search results.
...
impl Node for SerialMotorDriver {
    ...
    fn tick(&mut self) {
        ...
        let positions = self.read_positions();
        // JointState is a fixed-size POD message: fill it joint by joint
        // (names/positions/velocities/efforts are [_; 16] arrays).
        let mut state = JointState::new();
        for (i, &pos) in positions.iter().enumerate() {
            let _ = state.add_joint(&format!("motor_{}", self.motor_ids[i]), pos, 0.0, 0.0);
        }
        self.state_pub.send(state);

        // Apply commands if any
        if let Some(cmd) = self.cmd_sub.recv() {
            self.write_positions(&cmd.positions[..cmd.joint_count as usize]);
        }
    }

    fn shutdown(&mut self) -> Result<()> {
        hlog!(info, "SerialMotorDriver shutting down — stopping all motors");
...
def tick(node):
    ...
    node.send("motor.joint_state", horus.JointState(
        names=[f"motor_{id}" for id in driver.motor_ids],
        positions=positions,
    ))
    ...
    if cmd:
        driver.write_positions(cmd.positions)
...
fn main() -> Result<()> {
    ...
    for (_name, node) in nodes {
        // .build() registers the node on the scheduler it borrows —
        // it does not return an owned Scheduler, so do not reassign.
        sched.add(node)
            .rate(100_u64.hz())
...
- [ ] `package-type = "driver"` in `horus.toml [package]`
- [ ] `description` explains what hardware it supports
- [ ] `categories` include the hardware name and interface type
- [ ] `license` specified
