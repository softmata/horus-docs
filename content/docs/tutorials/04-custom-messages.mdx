...
impl LogSummary for MotorFeedback {
    fn log_summary(&self) -> String {
        // "MotorFeedback(motor_id=1, rpm=3200.0, current_amps=1.2, temperature_c=45.0)"
        format!(
            "MotorFeedback(motor_id={:?}, rpm={:?}, current_amps={:?}, temperature_c={:?})",
            self.motor_id, self.rpm, self.current_amps, self.temperature_c
        )
    }
}
```
