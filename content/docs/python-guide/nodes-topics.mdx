...
it requires a `horus_py` **source checkout**, `maturin` on `PATH`, and a manual edit to
`horus_py/src/lib.rs` to wire the generated module in. On a pip-installed HORUS,
`build_messages()` raises `RuntimeError` and points you back at dict topics; without the
`lib.rs` edit the generated files are never compiled and the import fails. See
[Custom Messages](/python/api/custom-messages) for the full procedure.
