...
register_message('MotorStatus', 'motor.status', [
    ('motor_id', 'u32'),
    ('velocity', 'f32'),
    ('temperature', 'f32'),
])
...
`register_message()` only queues the definition — `build_messages()` compiles it.
After the build, the type is importable from `horus` like any built-in:

```python
from horus import MotorStatus, Topic

status = MotorStatus(motor_id=1, velocity=3.14, temperature=45.0)
topic = Topic(MotorStatus)  # Bound to "motor.status", zero-copy
topic.send(status)
```
