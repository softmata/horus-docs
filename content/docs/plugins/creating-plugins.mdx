...
Topic backing files live in `/dev/shm/horus_<namespace>/topics/`, where the namespace defaults to the literal `default` and is overridden with `$HORUS_NAMESPACE` &mdash; pass `--shm-dir` to inspect another namespace. Because a topic name may contain `/`, namespaced topics such as `lidar/scan` are stored as subdirectories of `topics/`; the `is_file()` filter above lists only flat topic names, so recurse one level if you need the namespaced ones.
