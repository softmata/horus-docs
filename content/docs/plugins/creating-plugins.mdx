...
horus plugins install horus-mycommand --local
```
...
While zero-config works, you can add metadata for richer plugin listings. Add a `[package.metadata.horus]` section to your `Cargo.toml`:

```toml
[package.metadata.horus]
plugin_type = "cli"
commands = ["mycommand"]
description = "My custom HORUS command"
min_horus_version = "0.1.0"
```

Or add a `plugin` section to `horus.yaml`:
...
- Run `horus plugin verify` to check all plugin integrity
- Reinstall a plugin with `horus plugins install <name>` if verification fails
...
- **[Driver Plugins](/plugins/driver-plugins)** &mdash; Build hardware driver plugins instead
