...
Syscall filtering applies only on x86-64 Linux; the resource limits and file-descriptor closure apply on all Linux architectures. Long-lived (background) plugins get only the file-descriptor closure and `PR_SET_NO_NEW_PRIVS` &mdash; no resource caps and no network restriction.
