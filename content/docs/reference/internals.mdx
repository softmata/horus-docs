...
These types are for **building custom monitoring and discovery tools**. Most users don't need them — the built-in [Monitor](/docs/development/monitor) handles this automatically.

---

## NodePresence

Node presence files written to `/dev/shm/horus/nodes/` for cross-process discovery. The scheduler creates these automatically at startup; monitoring tools read them.
...
- [Monitor Guide](/docs/development/monitor) — Built-in web + TUI monitoring
- [Scheduler API](/docs/rust/api/scheduler) — `ProfileReport` and `status()`
- [Topic API](/docs/rust/api/topic) — `TopicMetrics`
