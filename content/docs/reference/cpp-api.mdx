...
public:
    Controller() : Node("controller") {
        scan_ = subscribe<horus::msg::CmdVel>("lidar.scan");
    ...
    void tick() override {
        ...
        if (!scan) return;
        horus::msg::CmdVel cmd{}; cmd.linear = 0.3f;
        cmd_->send(cmd);
    }
...
private:
    horus::Subscriber<horus::msg::CmdVel>* scan_;
...
auto node = horus::LambdaNode("controller")
    .sub<horus::msg::CmdVel>("lidar.scan")
    .pub<horus::msg::CmdVel>("motor.cmd")
    .on_tick([](horus::LambdaNode& self) {
        auto scan = self.recv<horus::msg::CmdVel>("lidar.scan");
        if (!scan) return;
        self.send<horus::msg::CmdVel>("motor.cmd", horus::msg::CmdVel{0, 0.3f, 0.0f});
    });
...
| `process()` | Drive the server once; call in a loop on the constructing thread |
...
| `set_execute_handler(fn)` | `void(HorusActionGoalHandle* h, const uint8_t* goal, size_t len)` — runs per-goal on its own thread |
...
uint64_t timestamp = 0;  // nanoseconds from your clock source
