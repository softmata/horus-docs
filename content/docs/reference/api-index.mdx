...
// simplified
    use horus::prelude::*;
    use horus_robotics::prelude::{CmdVel, LaserScan};
    ...
    // ── Time ──────────────────────────────────────────
    let now = horus::time::now();            // Current framework time
    let dt = horus::time::dt();              // Time since last tick
    let elapsed = horus::time::elapsed();    // Total runtime
  ...
  <LangTab language="Python">
    ```python
    import horus
    from horus import Node, Scheduler, Topic, CmdVel, LaserScan, run
    ...
    run(my_node, tick_rate=100, rt=True)

    # ── Scheduler API (advanced) ───────────────────────
    sched = Scheduler(tick_rate=100, rt=True, deterministic=True)
    sched.add(Node(tick=my_tick, rate=50, budget=200 * horus.us))
    sched.run()
