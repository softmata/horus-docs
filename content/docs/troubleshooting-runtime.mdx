...
2. **Type Mismatch**
   ...
   **Fix**: Ensure both use same type:
   ...
   pub_topic.send(3.14, &mut ctx).ok();
   ...
   **Fix**: Check multiple ticks:
   ...
   impl Node for SubscriberNode {
       fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
           if let Some(msg) = self.topic.recv(&mut ctx) {
               // Process message
           } else {
...
2. **Blocking Operations in tick()**
   ...
   **Fix**: Keep tick() fast:
   ...
   fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
       ...
       self.topic.send(data, &mut ctx).ok();
   }
   ```
...
```rust
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    let msg = self.topic.recv(&mut ctx).unwrap();  // PANIC if no message!
}
```
