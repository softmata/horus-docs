...
The run also prints a `SUMMARY BY MESSAGE TYPE` table with throughput and a `REAL-TIME SUITABILITY ANALYSIS` block. The declared and `size_of` sizes differ for `Imu` (296 vs 304 B) and `JointCommand` (1032 vs 928 B) because of struct padding and layout.
...
- **Complex structs** (Imu with 27-element covariances): 121 ns median — still sub-microsecond
...
- **Deterministic by Default**: sequential execution with reproducible runs (`Scheduler::deterministic()`)
