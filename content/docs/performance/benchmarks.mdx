...
**For most users: No.** This is reference data for performance validation.

**When this page IS useful:**
- Curious about HORUS performance claims (we show our work!)
- Comparing HORUS to other frameworks
- Reporting performance issues and need baseline numbers

**Quick answer:** HORUS is fast. This page proves it with data. Skip unless you're doing performance analysis.
...
```python
from horus import Topic, CmdVel, Pose2D, Imu, Odometry, LaserScan

# Create typed topic (zero-copy shared memory)
topic = Topic(CmdVel)
msg = CmdVel(linear=1.5, angular=0.5)

# Measure send latency
topic.send(msg)  # ~5-10 μs

# Round-trip test
recv_topic = Topic(CmdVel)
topic.send(msg)
received = recv_topic.recv()  # ~10-20 μs round-trip
```
