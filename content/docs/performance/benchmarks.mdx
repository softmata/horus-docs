...
For publication-quality measurements with sustained runs, size sweeps, histograms, and competitor comparison:
...
```bash
# Run full research suite (~30 minutes)
...
| Raw `AtomicU64` store+load | ~11ns | Signaling floor |
| **HORUS DirectChannel** | **~23ns** | +12ns (ring buffer header + dispatch) |
| **HORUS SpscIntra** | **~155ns** | +144ns (cross-thread atomic coordination) |
...
IPC latency scales linearly with message size (measured via `research_latency`):

| Size | DirectChannel | SpscIntra |
|------|--------------|-----------|
| 8B | 23ns | 155ns |
| 64B | 24ns | 161ns |
| 256B | 30ns | 175ns |
...
**Linear scaling**: Latency grows proportionally with message size (memcpy-dominated). No sudden jumps or allocation spikes.
