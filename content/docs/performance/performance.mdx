...
**Where time is spent:**
- **POD path** (fixed-size `Copy` types): a memcpy plus an atomic index update — **~12 ns** one-way when producer and consumer share an executor thread (self-loop), **~19 ns** for a real pub+sub on one thread (i9-14900K, release)
- **Serde path** (types with `Vec`, `String`, etc.): serialization dominates, typically 50-500ns depending on size
- **Cross-core overhead**: the moment producer and consumer run on different cores — whether different threads or different processes — cache-coherence traffic sets the floor at **~220 ns** one-way. This is core-locality physics, not a backend difference.

This is why fixed-size types matter: they skip serialization entirely.

### Optimized Data Structures

HORUS uses carefully optimized memory layouts to minimize latency. Latency is governed by **core locality**, not by which backend is chosen or whether the consumer is in-process — every topic is shared-memory-backed from creation. Co-located on one executor thread, a round trip is **~24 ns** (send+recv) for a self-loop and **~39 ns** for a real pub+sub (**~12–19 ns** one-way, L1-hot). The moment the consumer runs on a different core — thread or process — cache-coherence physics sets a floor of **~220 ns** one-way.
...
### IPC Latency (by core locality)

Latency depends on **where the peers run**, not on which backend HORUS picks. All topics are shared-memory-backed from creation, so the only cliff is same-core vs. different-core:

| Route | One-way | Round trip (send+recv) |
|-------|---------|------------------------|
| Same-thread, self-loop (1 handle) | **~12 ns** | ~24 ns |
| Same-thread, real pub+sub (one executor thread) | **~19 ns** | ~39 ns |
| Cross-thread **or** cross-process (different core) | **~220 ns** | — |

The jump to ~220 ns is cache-coherence physics on a different core — no backend or layout choice moves it. An in-process heap ring would not beat shared memory across cores, so the fastest realistic route is to co-locate the producer and consumer on one executor thread.

HORUS ships four shared-memory backends, selected automatically from topology and message type:

| Backend | Topology |
|---------|----------|
| `SpscShm` | 1 producer : 1 consumer |
| `MpscShm` | N producers : 1 consumer |
| `PodShm` | 1 producer : N consumers (POD broadcast) |
| `FanoutShm` | N producers : M consumers (non-POD broadcast) |

Because all four are shared-memory-backed from topic creation (the creator's PID is stamped at init), a cross-process consumer can join an existing topic with no migration — which is why there is no separate in-process path.
...
| Image.to_numpy (640x480) | **3.0μs** | 14.7μs | 1.5M/s | Zero-copy |

**Key takeaway**: Typed Python topics (`Topic(CmdVel)`) achieve **1.7μs** — fast enough for 30Hz ML inference pipelines. Generic dict topics are 4-60x slower due to serialization. `Image.to_numpy()` gives zero-copy image access at **3.0μs**.
...
|-----------|---------|------------|--------------|
| Shared Memory (same core) | **~24ns** (send+recv) | 100M+ msg/s | Local only |
| Shared Memory (different core) | **~220ns** (one-way) | — | Local only |
...
|----------|---------|------|-----------------------|
| **Ring buffer (1:1 rejects-newest / broadcast drops-oldest)** | Constant memory, no backpressure stalls | Slow subscribers lose messages | Logging, recording, or batch processing that needs every message |
