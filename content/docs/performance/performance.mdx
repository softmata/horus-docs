...
The default scheduler runs at 60 Hz (16ms per tick). Use `SchedulerConfig` to change it:
...
// Custom: 1kHz
let config = SchedulerConfig::builder().rate_hz(1000.0).build();
...
See [Scheduler Configuration](/advanced/scheduler-configuration) for all presets and options.
