...
**Wait-free Link (SPSC)**: 87ns send-only latency - no CAS loops, bounded constant time

**Lock-free Hub (MPMC)**: 313ns send-only latency - CAS-based for multi-producer coordination
...
| Message Type | Size | Hub (send-only) | Link (send-only) | ROS2 DDS | Speedup |
|--------------|------|-------------|--------------|----------|---------|
...
- **Link send/recv**: Small message copying (16B-1KB)
- **Hub operations**: Ring buffer management
