...
|----------|---------|-----|-----|
| 1 pub, 2 sub | PodShm | 198ns | 298ns |
| 1 pub, 8 sub | PodShm | 276ns | 510ns |
...
> **No published throughput table.** `benchmarks/README.md` publishes no measured numbers at all — its result tables were withdrawn. The HORUS repository has no throughput result for a 1 KB or 100 KB payload and none for a many-publisher / many-subscriber topology — no benchmark in it even sends a 100 KB message. The largest payloads anywhere in `benchmarks/` are 4 KB (`topic_probe --bulk 4096`), 1 KB (`ThroughputPayload1K` in `benches/topic_throughput.rs`) and the 1,480-byte `LaserScan` in `robotics_messages_benchmark`.
