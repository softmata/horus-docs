...
The Backend column names the route the benchmark exercised, which it selects explicitly.
It is not what auto-detection would pick for that topology: a real 1-publisher /
many-subscriber POD topic gets `PodShm`, because `SpmcShm`'s consumers compete for
messages rather than each receiving the stream.
