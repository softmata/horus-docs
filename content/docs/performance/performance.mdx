...
**For most users: Not initially.** HORUS is already fast by default.

**When this page IS useful:**
- Your robot needs to respond faster than it currently does
- Profiling shows HORUS as a bottleneck (unlikely)
- Deploying to production and want every microsecond
- Curious about how HORUS achieves its performance

**Quick answer:** Build your robot first with default settings. Performance tune later if needed (you probably won't need to).
...
|----------|--------------|----------|
| **x86_64 (Linux/Windows/macOS)** | AVX2/AVX512 | SSE4.2 → memcpy |
...
|-----------|------------------|---------|
| Memory copy (16-64B) | AVX2 `_mm256_*` | 2-4x |
| Memory copy (64B+) | AVX512 `_mm512_*` | 3-5x |
| Cache prefetch | `_mm_prefetch` | Reduces latency |
| Non-temporal stores | `_mm256_stream_*` | Bypasses cache for large writes |

### Automatic Feature Detection

SIMD features are detected at compile time based on target CPU:
...
- **Topic send/recv**: Small message copying (16B-1KB)
- **Topic operations**: Ring buffer management
- **Message serialization**: Fixed-size struct copying
- **Shared memory operations**: Page-aligned transfers

### SIMD Impact on Latency

Benchmarks on Intel Core i9-13900K (AVX512 enabled):
...
On CPUs without AVX2/AVX512, HORUS automatically falls back to:

1. SSE4.2 operations (if available)
2. Standard `memcpy` (always available)
...
let topic: Topic<GenericMessage> = Topic::new("pose")?;
...
for value in values {
    topic.send(value).ok();  // 100 IPC operations
}
...
topic.send(batched).ok();  // 1 IPC operation
```
...
fn tick(&mut self) {
    ...
    self.process_pub.send(data).ok();  // ~500ns
}
...
let topic: Topic<SensorData> = Topic::new("sensor@192.168.1.100:9870")?;
...
// In node context
// Logging with hlog!
    hlog!(info, "Transport: {}", topic.transport_type());
}
```
...
fn tick(&mut self) {
    // Logging with hlog!
        if ctx.metrics.avg_tick_duration_ms > 1.0 {
            hlog!(warn, "Tick taking too long");
        }
    }
}
```
