...
|----------|---------|-----|-----|
| 1 pub, 1 sub | SpscIntra | 10ns | 19ns |
| 1 pub, N sub | SpmcIntra | 3ns | 10ns |
| N pub, N sub (contended) | MpmcIntra | 100ns | 643ns |
...
**Real message types** (intra-process):
...
The UDP transport sends one datagram per syscall (`send_to`/`recv_from`) on every platform. Configure network endpoints through topic configuration rather than the `Topic::new()` API (which creates local shared memory topics). See [Network Backends](/advanced/network-backends) for details.
