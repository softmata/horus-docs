...
Enable LTO in your `horus.toml` for additional 10-20% speedup:
...
HORUS splices `[rust.profile.release]` into the generated `.horus/Cargo.toml`. Put the
profile there and not in `.horus/Cargo.toml` itself: that file is rewritten from
`horus.toml` on every `horus build` or `horus run`, and the build prints the settings it
just discarded along with the `horus.toml` key each one belongs under. A `horus new`
project has no root `Cargo.toml` to edit at all.
...
Cargo.toml has no slot for rustflags, so HORUS writes this one to a generated
`.cargo/config.toml` that `horus build --release` picks up. An exported `RUSTFLAGS`
...
Topics carrying inline (Pod) messages hold the whole struct in the ring slot, but a POD of 56 bytes or less **whose alignment is 8 or less** is padded to a single 64-byte cache line, so every such type costs the same 64 bytes per slot. Alignment is a hard gate: a small but over-aligned POD (a `u128` field, `#[repr(align(16))]`, an SSE vector) takes the split layout instead — a slot exactly `size_of::<T>()` wide plus an 8-byte sequence entry — so for those, shrinking the message does change the per-slot cost. Shrinking a message below that threshold saves nothing:

```rust
// Inline (Pod) messages: the ring slot holds the whole struct
let cmd: Topic<CmdVel> = Topic::new("cmd_vel")?;       // 64B per slot (16B payload padded to one cache line)
...
**Balance**: For inline Pod messages, footprint tracks message size only above 56 bytes; at or below that every type occupies one 64-byte slot. Pool-backed types (`Image`, `PointCloud`, `DepthImage`, `OccupancyGrid`, `CostMap`) keep only a fixed-size descriptor in the ring, so their slot cost does not grow with the payload.
