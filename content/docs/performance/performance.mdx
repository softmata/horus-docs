...
for value in values {
    topic.send(value, &mut ctx).ok();  // 100 IPC operations
}
...
topic.send(batched, &mut ctx).ok();  // 1 IPC operation
```
...
// In node context
if let Some(ctx) = &ctx {
    ctx.log_info(&format!("Transport: {}", topic.transport_type()));
}
```
...
impl Node for BenchmarkNode {
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        self.pub_topic.send(42.0, &mut ctx).ok();

        // Receive
        if let Some(data) = self.sub_topic.recv(&mut ctx) {
