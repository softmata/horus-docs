...
let max_speed = self.params.get_f64("max_speed", 1.5);
let pid_kp = self.params.get_f64("pid_kp", 1.0);
```
...
self.params.get_f64("max_speed", 1.5);
self.params.get_f64("pid_kp", 1.0);
self.params.get_i32("lidar_rate", 10);
```
...
self.params.get_f64("motion.max_speed", 1.5);
self.params.get_f64("control.pid.kp", 1.0);
```
...
impl VelocityController {
    fn new() -> horus_core::Result<Self> {
        let params = RuntimeParams::init()?;
        let max_speed = params.get_f64("max_speed", 1.5);
        let acceleration = params.get_f64("acceleration_limit", 0.5);
...
impl Node for VelocityController {
    ...
    fn init(&mut self) -> horus_core::Result<()> {
        self.max_speed = self.params.get_f64("max_speed", 1.5);
        self.acceleration = self.params.get_f64("acceleration_limit", 0.5);
...
**Type-specific getters (with default):**
```rust
let speed = self.params.get_f64("max_speed", 1.5);          // f64
let enabled = self.params.get_bool("auto_mode", false);      // bool
let rate = self.params.get_i32("update_rate_hz", 60);        // i32
let name = self.params.get_string("node_name", "default");   // String
```
...
impl Node for AdaptiveController {
    ...
    fn tick(&mut self) {
        ...
        if self.tick_count % 60 == 0 {
            let new_speed = self.params.get_f64("max_speed", 1.5);
...
impl Node for PIDController {
    ...
    fn init(&mut self) -> horus_core::Result<()> {
        self.kp = self.params.get_f64("pid_kp", 1.0);
        self.ki = self.params.get_f64("pid_ki", 0.1);
        self.kd = self.params.get_f64("pid_kd", 0.05);
...
impl Node for AdvancedController {
    ...
    fn init(&mut self) -> horus_core::Result<()> {
        self.enable_obstacle_avoidance = self.params.get_bool("obstacle_avoidance", false);
        self.enable_path_planning = self.params.get_bool("path_planning", false);
        self.enable_localization = self.params.get_bool("localization", true);
        Ok(())
    }
...
impl Node for NetworkNode {
    ...
    fn init(&mut self) -> horus_core::Result<()> {
        let env = self.params.get_string("environment", "development");

        match env.as_str() {
            "production" => {
                self.server_url = self.params.get_string("prod_url", "prod.example.com:8080");
                self.timeout_ms = self.params.get_i32("prod_timeout_ms", 3000) as u64;
            },
            "staging" => {
                self.server_url = self.params.get_string("staging_url", "staging.example.com:8080");
                self.timeout_ms = self.params.get_i32("staging_timeout_ms", 5000) as u64;
            },
            _ => {
                self.server_url = self.params.get_string("dev_url", "localhost:8080");
                self.timeout_ms = self.params.get_i32("dev_timeout_ms", 10000) as u64;
            }
        }
...
impl Node for SensorPublisher {
    ...
    fn init(&mut self) -> horus_core::Result<()> {
        self.publish_rate_hz = self.params.get_i32("publish_rate_hz", 10) as u64;
...
let speed = self.params.get_f64("max_speed", 1.5);
...
```rust
// Good - safe defaults
let emergency_stop = self.params.get_bool("emergency_stop", true);   // Default to safe state
let max_speed = self.params.get_f64("max_speed", 1.0);              // Default to slow

// Bad - unsafe defaults
let emergency_stop = self.params.get_bool("emergency_stop", false);  // Unsafe!
let max_speed = self.params.get_f64("max_speed", 100.0);            // Too fast!
```
...
```rust
use horus_core::{RuntimeParams, ParamMetadata, ValidationRule};
...
- **[Core Concepts](/concepts/core)** - Learn about nodes, topics, and scheduling
