...
**With parameters:**
```rust
// Read at each use, so an in-process set() is picked up immediately
...
- **Live tuning** - another node or thread in the *same process* can `set()` a value and the next `get()` sees it
- **Persistence** - Save/load from YAML files
- **Sharing** - Export/import parameter sets
- **Safety** - Fallback to defaults if missing
- **Validation** - Framework-internal only; clamp your own values in code (see [Validate Parameter Values](#validate-parameter-values))
...
<Callout type="warning" title="A params.yaml at the project root does nothing">
Before it runs anything, `horus run` looks for `params.yaml`, then `config/params.yaml`,
then `.horus/config/params.yaml`, and prints `Loaded N parameters from <path>` for the
first one it finds. That load happens in the CLI process and is thrown away when the
command moves on: nodes are spawned as separate processes, and `horus run` passes them
only `PATH`, `LD_LIBRARY_PATH` and `PYTHONPATH` — no `HORUS_PARAM_*` (only `horus launch`
exports those). A node calls `RuntimeParams::new()`, which reads `.horus/config/params.yaml`
and nothing else.

So a root-level `params.yaml` reports success and changes no value. Move it to
`.horus/config/params.yaml`, or run `horus param load ./params.yaml` to copy it there.
</Callout>
...
### Polling for in-process changes

<Callout type="warning" title="Parameters are loaded once, at startup">
`RuntimeParams::new()` reads defaults, then `.horus/config/params.yaml`, then
`HORUS_PARAM_*`, and that is the only time the file is read. There is no file watcher
and no control topic.

So `horus param set` does **not** reach a running node — it edits the YAML that the
*next* run will load. The monitor cannot change a running node's parameters either.
Restart the node to pick up an edit.

What the pattern below polls for is a `set()` made by something else inside the same
process.
</Callout>

**Check for in-process updates every tick:**
```rust
...
See [Monitor Guide](/development/monitor#api-endpoints) for the `/api/params` REST endpoints.
...
Or tolerate both shapes in code — `get_or` alone will not do this, it just
hands back the default when the stored value is a string:
```rust
...
This records every `set()`, including the defaults that `reset()` replays. Deletions are
not logged: `remove()` — and therefore `horus param delete <key>` — writes no entry, so a
key that vanished from `params.yaml` leaves no trace in the log.
...
- **[CLI Reference](/development/cli-reference#horus-param-parameter-management)** - `horus param` command reference
