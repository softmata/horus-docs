...
| | `horus list [OPTIONS]` | List/search packages (`--global`, `--all`, `--json`) |
...
| **Auth** | `horus auth <cmd>` | Authentication (login, api-key, signing-key, trust-publisher, publishers, logout, whoami) |
...
Options:
  ...
  -e, --enable <LIST>      Enable capabilities (comma-separated)
      ...
      --sim                Enable simulation mode — hardware entries with
                           sim = true are replaced with stubs
      --record <NAME>      Enable recording for this session
      --json               Output build diagnostics as JSON
      --json-diagnostics   Output Cargo diagnostics as JSON
      --no-hooks           Skip pre/post build hooks
  -- <ARGS>                Arguments for your program
```

### Using --sim for Simulation Mode

The `--sim` flag runs your project in simulation mode without physical hardware:

```bash
# Run with simulated drivers
horus run --sim

# Combine with release mode
horus run --sim --release

# Pass arguments to your program
horus run --sim -- --config warehouse.yaml
```

When `--sim` is active:
1. Hardware entries with `sim = true` are replaced with stub nodes
2. The simulator plugin specified in `[robot] simulator` (default: `"sim3d"`) is launched automatically
3. Your node code runs unchanged — the same topics and message types are used

Configure simulation hardware in `horus.toml`:
```toml
[hardware]
front_lidar = { use = "rplidar", port = "/dev/ttyUSB0", sim = true }
imu = { use = "bno055", bus = 1, sim = true }
```

See [Configuration Reference: \[hardware\]](/package-management/configuration#hardware) for full details.
...
// simplified
  ...
  <LangTab language="Python">
    ...
    def control_tick(node):
        ...
        if data is not None:
            node.log_info(f"Received: {data}")

    sensor = horus.Node(name="SensorNode", tick=sensor_tick, rate=10, order=0)
    control = horus.Node(name="ControlNode", tick=control_tick, rate=10, order=1)
    horus.run(sensor, control)
    ```
  </LangTab>
</LanguageTabs>
...
Options:
      --json     Output as JSON
      --full     Run full validation (all checks)
      --health   Run health checks on running system
```
...
# Keys saved under the platform data directory:
#   Linux   ~/.local/share/horus/keys/
#   macOS   ~/Library/Application Support/horus/keys/
#   Windows %LOCALAPPDATA%\horus\keys\
# Public key printed to stdout
```

<Callout type="info">
This key is **your publishing identity**, not a trust anchor. It signs the
packages you publish. It is never used to verify anybody else's packages — for
that, see `horus auth trust-publisher` below.
</Callout>

---

## `horus auth trust-publisher` - Trust a Publisher's Key

**What it does**: Records a publisher's Ed25519 public key so that packages
they sign are verified on install.

**Why it's useful**: A signature is only worth something if it is checked
against the key of whoever was supposed to sign it. Package verification
consults this store, keyed by the package owner (falling back to the package
name).

### Basic Usage

```bash
horus auth trust-publisher <name> <key-or-file>
horus auth publishers            # list what you currently trust
```

### Examples

```bash
# From a file the publisher gave you
horus auth trust-publisher acme ./acme-signing-key.pub

# Or paste the hex key directly (the form `horus auth signing-key` prints)
horus auth trust-publisher acme 3d4f...c1

# Review your trust store
horus auth publishers
```

Keys are stored one per file at `<data-dir>/keys/publishers/<name>.pub`.

**What happens on install**

| Situation | Result |
|-----------|--------|
| Package signed, matching publisher key on file | Verified, install proceeds |
| Package signed, publisher key on file does not match | **Fatal** — install aborts, the package may have been tampered with |
| Package signed, no publisher key on file | Warning: the signature cannot be checked. Install proceeds |
| Package unsigned, publisher key on file | Warning: a signed package was expected |

<Callout type="warning">
Earlier versions verified every package against `signing_key.pub` — the
*installing user's own* public key. That cannot authenticate a publisher, and
it failed in every direction: with no local keypair every signed package was
uninstallable, and with a keypair from anyone else an authentic package was
reported as tampered with. If you previously worked around this by copying a
publisher's key over your own `signing_key.pub`, move it into the
`publishers/` directory and regenerate your own identity.
</Callout>
...
# Trust a publisher's key, so their signed packages are verified on install
horus auth trust-publisher <name> <key-or-file>
horus auth publishers
...
```bash
horus auth signing-key
# Ed25519 key pair saved under the platform data directory
```

**Trust a publisher so their packages are verified**:
```bash
horus auth trust-publisher acme ./acme-signing-key.pub
horus auth publishers
```
...
|-----------|------|-------------|
| `list` | `-v, --verbose` | Show detailed information |
| `list` | `--json` | Output as JSON |
| `echo` | `-n, --count <N>` | Number of messages to print then exit |
| `echo` | `-r, --rate <HZ>` | Throttle display rate |
| `hz` | `-w, --window <N>` | Averaging window size |
| `pub` | `-r, --rate <HZ>` | Publishing rate |
| `pub` | `-n, --count <N>` | Number of messages to publish |
| `bw` | `-w, --window <N>` | Averaging window size |
...
| `list` | `-v, --verbose` | Show detailed information |
| `list` | `--json` | Output as JSON |
| `list` | `-c, --category <CAT>` | Filter by category |
| `kill` | `-f, --force` | Force kill without graceful shutdown |
...
horus param save [path]           # Save parameters to YAML file
...
| `list` | `-v, --verbose` | Show detailed information |
| `list` | `--json` | Output as JSON |
| `get` | `--json` | Output as JSON |
| `reset` | `-f, --force` | Skip confirmation prompt |
...
```bash
# Introspection
horus tf list                    # List all frames (-v, --json)
horus tf echo <source> <target>  # Echo transform (-n, -r, --once, -t)
horus tf tree                    # Show frame tree hierarchy (-o)
horus tf info <frame>            # Detailed frame information
horus tf can <source> <target>   # Check if transform is possible
horus tf hz                      # Monitor frame update rates (-w)
...
| `list` | `-v, --verbose` | Show detailed information |
| `list` | `--json` | Output as JSON |
| `echo` | `-n, --count <N>` | Number of transforms to print then exit |
| `echo` | `-r, --rate <HZ>` | Throttle display rate |
| `echo` | `--once` | Print one transform and exit |
| `echo` | `-t, --timeout <SECS>` | Timeout waiting for transform |
| `tree` | `-o, --output <FILE>` | Write tree to file |
| `hz` | `-w, --window <N>` | Averaging window size |
...
|-----------|------|-------------|
| `list` | `-v, --verbose` | Show detailed information |
| `list` | `-f, --filter <PATTERN>` | Filter by name |
| `list` | `--json` | Output as JSON |
| `info` | `--json` | Output as JSON |
| `hash` | `--json` | Output as JSON |
...
horus msg list
# Output:
# CmdVel (horus_robotics::messages::cmd_vel)
# LaserScan (horus_robotics::messages::sensor)
# Odometry (horus_robotics::messages::sensor)
# Image (horus::memory)
```
...
Options:
  -n, --dry-run                    Show what would launch without actually launching
      ...
      --shutdown-timeout <SECS>    Graceful shutdown timeout in seconds (default: 2)
```
...
```yaml
# robot.yaml
session: warehouse_bot        # Optional: session name for identification
namespace: /robot             # Optional: global namespace prefix for all nodes

env:                          # Optional: global env vars (applied to all nodes)
  ROBOT_CONFIG: /etc/robot/prod.yaml
...
nodes:
  ...
  - name: controller
    command: "horus run src/controller.rs"
    rate_hz: 50
    priority: 1
    depends_on: [sensor_node]   # Waits for sensor_node to start first
    params:
      kp: 1.5
      ki: 0.1
    env:
      # Forwarded to the node process. These are variables your own
      # code reads — horus takes its verbosity from -v/-q, not the
      # environment.
      DETECTOR_PROFILE: fast

  - name: logger
    command: "horus run src/logger.py"
    rate_hz: 10
    priority: 2
    depends_on: [sensor_node, controller]
    start_delay: 1.0            # Wait 1s after dependencies are up
...
|-------|------|----------|---------|-------------|
| `session` | string | No | file stem | Session name for identification |
| `namespace` | string | No | — | Global namespace prefix for all nodes |
| `env` | map | No | `{}` | Environment variables applied to all nodes |
...
| `command` | string | Yes* | — | Shell command to run (*or `package`) |
| `package` | string | Yes* | — | HORUS package name (*or `command`) |
| `args` | list | No | `[]` | Arguments appended to command |
| `priority` | int | No | — | Execution priority (lower = higher) |
| `rate_hz` | int | No | — | Node tick rate in Hz |
| `namespace` | string | No | — | Local namespace prefix |
| `params` | map | No | `{}` | Parameters passed to the node |
| `env` | map | No | `{}` | Node-specific environment variables (overrides global) |
| `depends_on` | list | No | `[]` | Node names that must start before this one |
| `start_delay` | float | No | — | Delay in seconds before starting this node |
| `restart` | string | No | `never` | Restart policy: `never`, `always`, `on-failure` |

### Dependency Ordering

Nodes are launched in **topological order** based on `depends_on`. The launcher detects circular dependencies and rejects them:

```yaml
nodes:
  - name: camera
    command: "horus run src/camera.rs"

  - name: detector
    command: "horus run src/detector.rs"
    depends_on: [camera]          # camera starts first

  - name: planner
    command: "horus run src/planner.rs"
    depends_on: [detector, lidar] # waits for both
```

Diamond dependencies work fine (D depends on B and C, both depend on A — A starts once).

### Namespace Resolution

Namespaces compose hierarchically. Global namespace (from top-level or `--namespace` flag) prefixes the node's local namespace:

| Global | Node namespace | Node name | Result |
|--------|---------------|-----------|--------|
| `/fleet` | `/arm` | `gripper` | `/fleet/arm/gripper` |
| `/fleet` | — | `gripper` | `/fleet/gripper` |
| — | `/arm` | `gripper` | `/arm/gripper` |
| — | — | `gripper` | `gripper` |

### Auto-Set Environment Variables

The launcher automatically sets these for each node process:

| Variable | Value |
|----------|-------|
| `HORUS_NODE_NAME` | Node name |
| `HORUS_NODE_PRIORITY` | Priority (if set) |
| `HORUS_NODE_RATE_HZ` | Rate in Hz (if set) |
| `HORUS_NAMESPACE` | Global namespace (if set) |
| `HORUS_NODE_NAMESPACE` | Node namespace (if set) |
| `HORUS_PARAM_<KEY>` | Each param (key uppercased, dashes become underscores) |

### Lifecycle

1. **Parse** — Validate YAML, check for circular dependencies
2. **Sort** — Topological sort by `depends_on`
3. **Launch** — Start nodes in order, applying `start_delay` per node
4. **Monitor** — All processes run, stdout/stderr inherited to terminal
5. **Shutdown** — Ctrl+C sends SIGTERM, waits `--shutdown-timeout` seconds, then SIGKILL
...
**Multi-robot fleet with same launch file**:
```yaml
# formation.yaml
nodes:
...
```bash
horus launch formation.yaml --dry-run
```

**List nodes without launching**:
```bash
horus launch formation.yaml --list
```
...
horus install driver --target my-workspace  # install into a specific workspace
horus install driver --json            # JSON output
```
...
**Why it's useful**: Enable/disable plugins without uninstalling, verify plugin integrity after updates, and control which project-local plugins are allowed to execute.

<Callout type="warning">
**Project-local plugins must be trusted before they run.** A plugin binary that ships inside a checkout's `.horus/` directory (part of a cloned repository) is **refused execution** unless its content hash is recorded in your out-of-repo trust store. A checksum in `.horus/plugins.lock` is *not* proof of trust — the repository controls that file. Plugins installed globally (`horus install`) are trusted at install time; a project-local plugin installed with `horus install --plugin --target` is auto-trusted. Otherwise, run `horus plugin trust <command>` to allow it. This prevents a cloned repo from silently auto-executing code the moment you run `horus <cmd>`.
</Callout>
...
horus plugin trust <command>         # Trust a project-local plugin so it may execute
horus plugin untrust <command>       # Remove a plugin from the trust store (refuses to execute after)
horus plugin trusted                 # List plugins trusted for execution
```
...
**Trust a project-local plugin so it can run**:
```bash
# A repo you cloned ships .horus/bin/horus-mytool. Running it is refused until trusted:
horus mytool                         # → "Refusing to execute untrusted project-local plugin 'mytool'"
horus plugin trust mytool            # Record its content hash in your trust store
horus mytool                         # Now runs
horus plugin trusted                 # List what you've trusted
horus plugin untrust mytool          # Revoke (it will refuse to execute again)
```
...
| `info` | `--json` | Output as JSON |
| `list` | `--json` | Output as JSON |
| `clean` | `-n, --dry-run` | Preview what would be removed |
| `purge` | `-y, --yes` | Skip confirmation prompt |
...
| Subcommand | Flag | Description |
|-----------|------|-------------|
| `list` | `-l, --long` | Show extended details |
| `list` | `--json` | Output as JSON |
| `info` | `--json` | Output as JSON |
| `delete` | `-f, --force` | Skip confirmation prompt |
| `replay` | `--start-tick <N>` | Start replay from tick N |
| `replay` | `--stop-tick <N>` | Stop replay at tick N |
| `replay` | `--speed <MULT>` | Playback speed multiplier (default: 1.0) |
| `replay` | `--override <EXPR>` | Override node output (format: `node.output=value`, repeatable) |
| `diff` | `-n, --limit <N>` | Limit number of differences shown |
| `export` | `-o, --output <PATH>` | Output file path (required) |
| `export` | `-f, --format <FMT>` | Output format (default: json) |
| `inject` | `-n, --nodes <LIST>` | Node names to inject (comma-separated) |
| `inject` | `--all` | Inject all nodes from recording |
| `inject` | `-s, --script <FILE>` | Script file for injection rules |
| `inject` | `--start-tick <N>` | Start injection from tick N |
| `inject` | `--stop-tick <N>` | Stop injection at tick N |
| `inject` | `--speed <MULT>` | Injection speed multiplier (default: 1.0) |
| `inject` | `--loop` | Loop the injection continuously |
...
Options:
  ...
  -e, --event <TYPE>       Filter by event type (e.g. "DeadlineMiss", "BudgetViolation")
...
#   horus-robotics -> serde (message serialization)
```
...
**Hardware reachability** (checks `[hardware]` entries from horus.toml):
```bash
horus doctor --verbose
# ...
# ✓ Hardware — 3 hardware node(s) checked, some unreachable
#     ✓ 'imu': /dev/i2c-1 found
...
For a detailed walkthrough of how native tool integration works, see **[Native Tool Integration](/development/native-tools)**.
...
horus run --record stutter_debug
# ... reproduce the stutter, then stop with Ctrl-C ...

# Step 6: Replay the session to reproduce
horus record replay stutter_debug
```
...
horus deploy robot@192.168.1.10
```
...
cd robot_alpha
horus install common_messages
horus install lidar_driver
...
horus bench                         # Performance gate (optional)
...
# Debug mode (see what's happening). horus installs its own log bridge in
# place of env_logger, so RUST_LOG has no effect — use the flag.
horus run --verbose

# CI/CD authentication — there is no API-key environment variable;
# write the key into ~/.horus/auth.json instead
printf '{"api_key":"%s"}\n' "$YOUR_CI_SECRET" > ~/.horus/auth.json
```
...
See **[Troubleshooting & Maintenance](/getting-started/troubleshooting)** for complete details.
...
**Having issues?** Check the **[Troubleshooting Guide](/getting-started/troubleshooting)** for solutions to common problems.
...
- [Troubleshooting](/getting-started/troubleshooting) — Solutions to common CLI errors and environment issues
