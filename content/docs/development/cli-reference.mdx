...
Each field carries its own doc comment when the source has one: `show_message`
prints `field.doc` beneath the field (`horus_manager/src/commands/msg.rs:277-281`),
and `--json` emits it as a `doc` key per field. A field whose source has no `///`
line simply prints nothing extra. Use `horus msg hash <type>` on both machines when
you need to confirm two builds agree on a message.
...
> **Note**: `rate_hz` sets the scheduler's tick rate for that process. It is exported as
> `HORUS_NODE_RATE_HZ` and read in `Scheduler::finalize_config`, after the compiled-in
> configuration, so the launch file wins over a `tick_rate()` in the source — which is the
> point of putting it there. A node that declares a faster `.rate()` still raises the tick
> grid, so the launch file sets a floor rather than a ceiling. A value outside
> 0.001–1000000 Hz, or one that does not parse, logs a warning and leaves the rate alone.
>
> `priority` is still inert. It is exported as `HORUS_NODE_PRIORITY` and nothing in the
> workspace reads it, so a launch file cannot set a node's priority; use `.priority()` on
> the Rust builder or `priority=` on the Python `Node`. Both values appear in `--dry-run`
> and `--list` output either way.
