...
and stop being accepted in HORUS 0.4.0 — on `horus run`, `horus build` and `horus test`
the same three letters mean `--release`, `--package` and `--clean`, and nothing errors
when the habit crosses over. Use the long form.
