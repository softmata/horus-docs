...
horus check [path]         # Validate horus.yaml and workspace
...
# Package Management
horus pkg <command>        # Manage packages (install, publish, update, etc.)
horus add <name>           # Add package, driver, or plugin (smart detection)
horus remove <name>        # Remove package, driver, or plugin
horus driver <command>     # Driver management (list, info, search, probe, install)
horus plugin <command>     # Plugin management (list, search, enable, disable, install)
...
horus net <command>        # Network diagnostics (check, ping, trace)
...
**What it does**: Validates `horus.yaml`, source files, and the workspace configuration.
...
```bash
horus check
# Validates horus.yaml, dependencies, and environment
```
...
# List packages
horus pkg list [query]

# Update packages
horus pkg update [package]
...
horus hf echo <source> <target>  # Echo transform between two frames
...
horus hf hz                      # Monitor frame update rates
```
...
**Monitor a transform**:
```bash
horus hf echo laser_frame world
...
## `horus net` - Network Diagnostics

**What it does**: Network connectivity checks, latency measurement, and path tracing.

**Why it's useful**: Debug network communication issues between HORUS nodes on different machines.

### Subcommands

```bash
horus net check              # Full connectivity check (local, external, NAT)
horus net ping <endpoint>    # Measure latency to an endpoint
horus net trace <endpoint>   # Trace network path to endpoint
```

### All Options

```bash
horus net check [OPTIONS]
  -v, --verbose    Show detailed diagnostic output

horus net ping <ENDPOINT> [OPTIONS]
  -c, --count <N>       Number of pings (default: 5)
  -i, --interval <MS>   Interval between pings in ms (default: 1000)

horus net trace <ENDPOINT> [OPTIONS]
...
```bash
horus net check --verbose
```

**Measure latency to another robot**:
```bash
horus net ping 192.168.1.50 --count 10
```

**Trace route to remote machine**:
```bash
horus net trace 192.168.1.100
```
...
## `horus driver` - Driver Management
...
```bash
horus driver list              # List all available drivers (local + registry + plugins)
horus driver info <driver>     # Show driver details
horus driver search <query>    # Search for drivers
horus driver probe             # Probe for connected hardware using plugins
horus driver plugins           # List and manage loaded driver plugins
horus driver install <driver>  # Install a driver package from registry
horus driver remove <driver>   # Remove an installed driver package
```

### All Options

```bash
horus driver list [OPTIONS]
  -c, --category <CAT>    Filter by category (sensor, actuator, bus, input, simulation)
  -r, --registry           Show only registry drivers
  -p, --plugins            Show only loaded plugins
  -m, --mode <MODE>        Driver loading mode (static, dynamic, hybrid)

horus driver search <QUERY> [OPTIONS]
  -b, --bus <TYPE>         Filter by bus type (usb, i2c, spi, serial)

horus driver probe [OPTIONS]
  -p, --plugin <NAME>      Specific plugin to probe
  -b, --backend <NAME>     Specific backend to probe
  -m, --mode <MODE>        Driver loading mode (static, dynamic, hybrid)

horus driver install <DRIVER> [OPTIONS]
  -v, --ver <VER>          Specific version
  -g, --global             Install globally

horus driver remove <DRIVER> [OPTIONS]
  -g, --global             Remove from global scope
```
...
**Find camera drivers**:
```bash
horus driver search camera
```

**Probe for connected hardware**:
```bash
horus driver probe
# Scans I2C, serial, USB for recognized devices
```

**Install a driver**:
```bash
horus driver install horus-rplidar
```
...
horus plugin search <query>     # Search for available plugins
...
horus plugin install <plugin>   # Install a plugin from registry
horus plugin remove <plugin>    # Remove an installed plugin
```
...
```bash
horus plugin search visualizer
horus plugin install horus-visualizer
```
