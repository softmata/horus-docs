...
horus lock                 # Record the resolved dependency versions in horus.lock
...
Options:
  ...
  -l, --lib                Create as a library crate (instead of binary)
      --cargo              Create a cargo-owned project: a root Cargo.toml,
                           not the generated .horus/Cargo.toml
...
The short forms `-r`, `-p` and `-c` are still accepted here, but they are deprecated
and stop being accepted in HORUS 0.4.0 — `-r` means `--release` on `horus run`,
`horus build` and `horus test`, and `-p`/`-c` mean `--package`/`--clean` on `horus run`
and `horus build` only (`horus test` has neither and rejects them outright). On `run`
and `build`, `-r` and `-c` change the build silently rather than erroring, while a bare
`-p` at least fails for want of a value. Use the long form.
...
## `horus eject` - Hand the Build to Cargo

**What it does**: Writes a root `Cargo.toml` from the generated
`.horus/Cargo.toml` and stops owning the build. Plain `cargo build`, `cargo run`
and `cargo test` work from then on, and `horus build`/`run`/`test` build from
your manifest.

**Why it's useful**: A managed project (`horus new`) has no manifest cargo can
find at the project root, so `cargo build` fails with `could not find
Cargo.toml`. Eject is for editors, scripts, CI, and muscle memory that expect a
normal Rust crate.
...
Options:
      --force    Replace an existing root Cargo.toml
```

### What Gets Written

- Every target the generated manifest declared — `src/main.rs`, `src/bin/*`,
  `tests/*`, `examples/*`, `benches/*`, and `[lib]` for a library — with its
  path rewritten relative to the project root.
- The HORUS dependencies, as absolute paths to the source tree HORUS resolved
  when it wrote the file. Moving or upgrading that tree breaks them until you
  repoint them or eject again.
- The two `[patch]` tables the generated manifest carried, so resolution keeps
  working.

What changes after an eject:

- `[rust]` in `horus.toml` is **ignored** — profiles, lints and features belong
  in `Cargo.toml` now. `horus doctor -v` reports the mode as `cargo-native`.
- `.horus/Cargo.toml` is deleted; the root manifest is the only source of truth,
  and HORUS never regenerates it.
- `horus build`/`run`/`test` still work: they detect the root manifest and run
  cargo against it.

An existing root `Cargo.toml` is refused unless `--force`. A workspace ejects
too: the generated virtual manifest becomes the root `Cargo.toml` with its
`members` pointing at the real member directories, and each member manifest
moves into its own directory with its paths rebased. Non-Rust projects have no
Cargo manifest to eject. To start cargo-native instead, use
`horus new my_robot --cargo`.
...
`--sim` swaps a driver for a stub. Which drivers it swaps is decided by the `sim = true`
flag on the `[hardware]` entry itself — nothing else:
...
Naming a driver that is not marked `sim = true` does nothing: the argument list *narrows*
the simulated set, it does not create it. So `horus run --sim motors` above still drives
real motors.

<Callout type="warning" title="`[sim-drivers]` is not consulted">

An older form put the simulated backend in a separate `[sim-drivers]` table. That table
still parses, and is then read by nothing — a project relying on it has **silently lost its
simulation override** and runs its real hardware under `--sim`. `horus check` reports it as
a warning naming the replacement. See
[`[drivers]` and `[sim-drivers]`](/package-management/configuration#drivers-and-sim-drivers-deprecated).

</Callout>

### Using --enable for Capabilities

The `--enable` flag sets Cargo features for one run, without editing `horus.toml`. It takes
a comma-separated list, and it replaces the top-level `enable = [...]` key for that run
rather than adding to it.

```bash
# LAN replication — the one capability HORUS itself implements
horus run --enable net

# Several at once
horus run --enable net,my_own_feature
```

**Capabilities HORUS handles:**

| Capability | Effect |
|------------|--------|
| `net`, `network` | LAN replication — same as `horus run --net`. Applied to the `horus` dependency in the generated manifest, not to your crate |
| `sim`, `simulation` | Accepted and expands to nothing |

Every other name is forwarded to `cargo build --features <name>` against the crate HORUS
generates for **your** code in `.horus`, a few renamed on the way: `gpu` → `cuda`,
`py` → `python`, `opencv` → `opencv-backend`, `io-uring` → `io-uring-net`. Unrecognised
names go through unchanged. Either way, no HORUS crate declares those features, so unless
you declare one yourself under `[rust.features]` the build fails with cargo reporting that
none of the selected packages contains it. In particular `--enable cuda` does not add GPU
support: there is no CUDA capability anywhere in HORUS. **Hardware backends are not
capabilities** — select those with `-d/--drivers`:

```bash
horus run -d camera,lidar,imu --release
```

**Or configure in horus.toml** — `enable` is a top-level key, not a table:
...
`check` inspects the repository, and repairs the gitignored `.horus/Cargo.toml` that
rust-analyzer needs when it is missing or points at a HORUS source tree that has moved;
[`horus doctor`](#horus-doctor-environment-health-check) reads the machine it is on —
toolchains, real-time capability, shared memory. Neither substitutes for the other: a
project can be valid on a machine that cannot build it, and a healthy machine says
nothing about whether the manifest parses. `horus check --health`
runs the machine side without leaving `check`.
...
Lists **installed packages and plugins** — the things `horus install` put on this
machine. It is not the runtime `list` you get from `horus topic list`,
`horus node list`, `horus param list` or `horus msg list`, which ask a running
system what it currently has. If nothing is running, those return nothing while
`horus list` still prints your packages.

| Flag | What it does |
|------|--------------|
| `-g, --global` | Global scope packages only |
| `-a, --all` | Local and global together |
| `--json` | JSON output |
...
Version, description, owner and dependency information for a package or plugin,
...
`-n` is the short form of `--dry-run`, the same as on `horus publish`,
`horus clean`, `horus deploy`, `horus launch` and `horus migrate`.

| Flag | What it does |
|------|--------------|
| `-n, --dry-run` | Report the changes without applying them |
...
Packages the current project and uploads it. Run it from the project directory —
the manifest it reads is the `horus.toml` next to you.

`publish` here means *publish a package*. It is unrelated to the `publish()` call
in the Rust, C++ and Python APIs, which sends one message on a topic; the CLI
equivalent of that is `horus topic pub`.

| Flag | What it does |
|------|--------------|
| `-n, --dry-run` | Validate and package without uploading |
...
Irreversible, and it breaks anyone who depends on that exact version. Prefer
[`horus yank`](#horus-yank-horus-unyank-hide-a-bad-version), which hides a
version from new installs while existing lockfiles keep resolving.
...
**What it does**: Checks the toolchains, system dependencies and real-time readiness of the
machine you are on.

**Why it's useful**: Answers "is this machine set up to build and run HORUS" before you
spend time debugging a build.
...
`--rt` is the one to run on a robot before trusting a control loop: it audits the
real-time configuration and then measures actual jitter and IPC latency, rather than only
reporting what the system claims to support.

### The Installation check

The first check is HORUS itself, and it is the one to read when the CLI behaves as
though it is not the version you installed. It reports:

- **Every `horus` on `PATH`, in `PATH` order**, with what each answers to
  `--version`. Two installs at different versions — typically a `~/.local/bin`
  one and a `~/.cargo/bin` one — shadow each other silently, and `PATH` order
  alone decides which `horus` means.
- **The install record** (`~/.horus/install_manifest.toml`): version, release
  tag, source commit and how it was installed. An older install that predates
  the record reads as "no install record", which is not an error.
- **Whether the two halves agree.** The CLI binary and the cached source tree
  that `horus run` compiles Rust projects against are separate artifacts, and
  the check compares their versions *and* their shared-memory `TOPIC_VERSION`.
  A CLI that cannot read the topics its own libraries write is the failure this
  exists to name — it otherwise shows up as `horus topic list` reporting zero
  messages on a topic that is publishing normally. Fix it with `horus self
  update`, or reinstall pinning `HORUS_VERSION`.
- **Orphaned source caches** under `~/.horus/cache`, with sizes. These are left
  behind by upgrades and are frequently a few hundred megabytes.
- **Whether the package registry is reachable**, so a `horus search` that finds
  nothing is distinguishable from a registry that is down.

Toolchains are graded per language, and each language is graded against the tools the
HORUS commands actually invoke. For C++ that means cmake *and* a compiler (`g++`,
`clang++` or MSVC) *and* a build program (`make` or `ninja`): `horus build` runs
`cmake --build`, and cmake on its own configures a build without being able to perform
one. For Rust it includes `clippy` and `rustfmt`, which `horus lint` and `horus fmt` need
and which a minimal-profile `rustup` does not install.

A language that is missing a required tool is reported as blocked, one missing only an
optional tool as degraded:

```text
! Toolchains — Python ready · Rust missing clippy, rustfmt · C++ needs a C++ compiler, make or ninja
```

Either way it is a warning, not a pass — `horus doctor` exits `1` when any check warns and
`2` when any check fails, so an incomplete language never renders as a green tick next to
a healthy one. Both exit codes are suppressed by `--fix`: `horus doctor --fix` returns `0`
however many checks warned or failed, including when the fixing pass could not install
what was missing. Anything gating on the result has to run a plain `horus doctor` after
the fixing pass. Run `horus doctor -v` to see every tool, its version, and the command that
installs the ones that are missing:

```text
a C++ compiler: not found (g++, clang++ or MSVC — cmake picks one) - install: sudo apt install build-essential
```

There is no per-language filter (`--for cpp` and similar do not exist); `-v` plus the
summary line is the whole surface.
...
**This needs root and may install a kernel package.** Both of the changes it
makes are prompted, and both default to no:

| What it changes | When | Prompt |
|-----------------|------|--------|
| `/etc/security/limits.d/99-horus-rt.conf` — `memlock unlimited` and `rtprio 99`, **scoped to the user that ran the command**, never `*` | Always, unless the file already exists | Asks |
| The distribution's real-time kernel package | Only when the running kernel is not already `PREEMPT_RT` | Asks |

The kernel package is chosen from the distribution in `/etc/os-release`. The
names differ, and `--undo` prints the matching removal command for the same
distribution:

| Distro (`ID=`) | Installs | Removes with |
|----------------|----------|--------------|
| `ubuntu` | `linux-image-realtime` | `sudo apt remove linux-image-realtime` |
| `debian` | `linux-image-rt-amd64` (x86_64), `linux-image-rt-arm64` (aarch64) | `sudo apt remove linux-image-rt-amd64` |
| `fedora` | `kernel-rt kernel-rt-core` | `sudo dnf remove kernel-rt kernel-rt-core` |
| `arch` | `linux-rt linux-rt-headers` | `sudo pacman -R linux-rt linux-rt-headers` |

Anything else prints the four commands above and changes nothing. A kernel
install that fails is reported as a failure — `setup-rt` will not tell you to
reboot into a kernel it did not install.

Declining the kernel prompt aborts the run with a non-zero exit; declining the
limits prompt prints the two lines to add by hand and carries on.

**Non-interactive runs decline.** Without a terminal on stdin — CI, a
provisioning script, a pipe — every prompt answers no and nothing is installed.
Set `HORUS_ASSUME_YES=1` to accept both without asking; `horus setup-rt` has no
`-y` flag of its own.

`--undo` removes only the limits file, and only says it did when the `rm`
succeeded. The kernel package and any `isolcpus` boot parameter have to be
undone by hand — it prints the commands.
...
> **Note**: `horus env` does not snapshot, save, or restore package environments. `horus lock` records the versions resolution picked — a reviewable record, not yet an enforced pin; see [`horus lock`](#horus-lock-record-the-resolved-dependency-versions).
...
horus topic bw <topic>        # Measure bandwidth (bytes/sec; -w/--window N, default 100)
...
# --goal-id must be a UUID — take it from `horus action list` or the send-goal output.
horus action cancel-goal navigate --goal-id 6f1c1a4e-2b90-4b6c-9f4a-6b0f1e0b8a11
...
horus node list
# Output:
#   NAME                     STATUS    HEALTH PRIORITY      RATE    ACTUAL    TICKS   SOURCE
#   ----------------------------------------------------------------------------------------
#   SensorNode              Running   Healthy        50    100 Hz   99.8 Hz    12043    12345
#   ControllerNode          Running   Warning        40     50 Hz   41.2 Hz     6021    12346
#   LoggerNode               Paused         —         0     10 Hz         —     1204    12347
```

`HEALTH` is the node's health grade — `Healthy`, `Warning`, `Error`, `Critical`, or `—`
when no heartbeat has arrived. `RATE` is the rate the node was configured for and `ACTUAL`
is the rate it is measured at, coloured by how far it has fallen behind (green at 90% of
the configured rate or better, yellow above 60%, red below); `—` means nothing has been
measured yet, which is not the same as 0 Hz. `SOURCE` carries the PID for a local node, or
a marker such as `[bridged:…]` / `[horus_net:…]` for one reached through a bridge or the
network. CPU and memory are not in this table — `horus node info <node>` reports those.
...
<Callout type="warning" title="`horus log` shows every HORUS process on this machine">

There is no project scoping. Logs go into one shared-memory ring per **namespace**, at
`/dev/shm/horus_<namespace>/logs`, and the default namespace is `default` for every
process on the box — the same design as ROS 2's `domain_id=0`. So `horus log` in an empty
directory with no project in it still prints whatever ran last:
...
The banner it prints, `(showing local + remote logs)`, means *this machine* plus peers
reached over `horus_net` — not *this project*.

</Callout>

To scope logs to one project, give it a namespace. `HORUS_NAMESPACE` isolates the
shared-memory tree — topics and the log ring live under `/dev/shm/horus_<namespace>/` —
so it has to be set for both the run and the viewer. Parameters are not in that tree:
they are read from `.horus/config/params.yaml` relative to the working directory, and a
namespace does not scope them.
...
Without it, treat the node name in each line as the only indication of which project the
entry came from — and check it before concluding anything from a log during incident
analysis. See [`HORUS_NAMESPACE`](/development/environment-variables) for the full effect
of the setting.
...
**What it does**: Edit the parameter file that nodes read at startup (get, set, list, dump, save, load).

**Why it's useful**: Tune robot behavior without recompiling, persist configurations, and debug parameter values.

<Callout type="warning" title="`horus param set` does not reach a running node">
It writes `.horus/config/params.yaml`. A node reads that file once, in
`RuntimeParams::new()`, and there is no watcher and no control topic — so a process that
is already running keeps the values it started with. Restart the node to pick up a change.
</Callout>
...
horus param reset --force         # Reset all parameters to defaults; --force is required
...
#   KEY                                           VALUE       TYPE
#   ----------------------------------------------------------------
#   acceleration_limit                              0.5      float
#   camera_fps                                       30        int
#   collision_threshold                             0.5      float
#   emergency_stop_distance                         0.3      float
#   lidar_rate                                       10        int
#   max_angular_speed                               1.0      float
#   max_memory_mb                                   512        int
#   max_speed                                       1.0      float
#   pid_kd                                         0.05      float
#   pid_ki                                          0.1      float
#   pid_kp                                          1.0      float
#   sensor_timeout_ms                              1000        int
#   tick_rate                                        30        int
#
# Total: 13 parameter(s)
# Location: Built-in defaults — no .horus/config/params.yaml in this directory.
# Tip: `horus param set <key> <value>` creates it.
```

Keys are flat strings, not node-scoped paths. The list is never empty: the thirteen
built-in defaults are always present, and `params.yaml` and `HORUS_PARAM_*` override
individual keys on top of them rather than replacing the set. The `Location:` line names
`.horus/config/params.yaml` only when that file exists; otherwise it says the values came
...
**Change a controller's gains for the next run**:
```bash
horus param set pid_kp 2.0
horus param set pid_ki 0.2
horus run                              # the new values are read at startup
```
...
Field descriptions are not part of this output — the doc comment appears once,
under `Description:`. Use `horus msg hash <type>` on both machines when you need
to confirm two builds agree on a message.

`horus msg gen` reads `msgs/*.hmsg` and writes the Rust, C++ and Python types into
`.horus/generated/`. See [Custom Messages](/development/custom-messages) for the
`.hmsg` syntax and the CI check.
...
Options:
  -n, --dry-run                Show what would launch without actually launching
      --namespace <NAMESPACE>  Isolate this launch in its own shared-memory tree
      ...
      --shutdown-timeout <N>   Seconds between SIGTERM and SIGKILL
                               (default: 2, capped at 3)
  -h, --help                   Print help
```

`--status` and `--stop` are how you manage a system after it is up: `--status` lists the
running sessions and `--stop <name>` shuts one down.

Shutdown is a fixed three-phase sequence: `GracefulShutdown` is published on every node's
control topic, then a hardcoded 2-second wait, then `SIGTERM` to whatever is still
running, then `min(N, 3)` more seconds, then `SIGKILL`. `--shutdown-timeout` sets only
that last gap, and the value is silently capped at 3 — `--shutdown-timeout 30` still
kills a stuck node about five seconds in, with no message saying the number was reduced.
A node that needs longer must finish its work inside its `GracefulShutdown` handler,
within the 2-second window, rather than by raising this flag. (`horus launch --help`
still describes the flag as the graceful-shutdown wait.)
...
> **Note**: two of these are inert — `priority` and the per-node `namespace`.
>
> `priority` is exported to the child process as `HORUS_NODE_PRIORITY` and nothing in the
> workspace reads it, so a launch file cannot set a node's priority. The value still
> appears in `--dry-run` and `--list` output. Set it with `.priority()` on the Rust
> builder or `priority=` on the Python `Node`.
>
> `rate_hz` **does** work: `finalize_config` reads `HORUS_NODE_RATE_HZ`, after the
> deferred config so a launch file beats a compiled-in `tick_rate()`, and before the tick
> grid is adjusted so a faster node still widens it. A malformed value warns and leaves
> the rate alone.
>
> A **per-node** `namespace` does nothing at all, and `horus launch` now warns when it
> sees one. HORUS has no topic prefixing or remapping; the **file-level** `namespace:`
> works because it names the shared-memory tree, which isolates a whole launch rather than
> prefixing anything, and that granularity is per robot. A per-node value would split that
> node off from its siblings into a tree of its own, so it is not simply "not wired up
> yet". Use the file-level key.
...
Options:
  ...
  -n, --dry-run              Show what would be done without doing it
      --print-service        Write a systemd unit for this deploy to stdout
...
`--run` starts the project over the ssh session that `horus deploy` opened, so it
stops when that session does. Nothing a deploy starts comes back after a power
cycle on its own.

`--print-service` writes a systemd unit for the deploy to stdout — the same host,
directory and command the deploy itself would use. It prints; it does not
install, because where the unit belongs and whether it needs lingering is the
robot owner's decision:

```bash
horus deploy pi@raspberrypi.local --print-service > horus-horus_deploy.service

# As a user service (needs lingering to start without a login):
scp horus-horus_deploy.service pi@raspberrypi.local:~/.config/systemd/user/
ssh pi@raspberrypi.local 'systemctl --user daemon-reload \
  && systemctl --user enable --now horus-horus_deploy.service'
ssh pi@raspberrypi.local 'sudo loginctl enable-linger $USER'

# Or system-wide:
scp horus-horus_deploy.service pi@raspberrypi.local:/tmp/
ssh pi@raspberrypi.local 'sudo mv /tmp/horus-horus_deploy.service /etc/systemd/system/ \
  && sudo systemctl enable --now horus-horus_deploy.service'
```

The unit sets `Restart=on-failure`. It takes one target at a time: each host's
unit names that host, so `--print-service` with several targets is refused rather
than concatenating units onto one stream.
...
**Commit `.horus/deploy.yaml`.** The rest of `.horus/` is build output and is
gitignored, but this file is not — it is the fleet inventory, and `horus deploy`
trusts the hosts and identity paths in it precisely because they arrived through
the checkout and were reviewed like the rest of it. HORUS writes the negation
into `.horus/.gitignore` for you. A target name that is *not* in the inventory is
treated as a bare hostname and warned about, because the sync runs
`rsync --delete` against whatever that name resolves to.

**Python projects install their dependencies on the robot.** Cross-compilation
covers a Rust or C++ artifact, but Python imports need wheels built for the
robot's own architecture, so `horus deploy` runs `pip install --target
.horus/packages` over ssh after the transfer and puts that directory on
`PYTHONPATH` when it runs the entry point. The versions come from `horus.lock`
when the project has one, so a fleet installs what the project recorded rather
than whatever PyPI serves that day. Your local `.horus/venv` is never shipped —
it records host-absolute paths and an interpreter for the wrong architecture.
...
Plugins that ship inside a checkout's `.horus/` directory are unsigned and are
refused at execution time until you trust them on this machine. `horus plugin
trust` records the plugin's content hash in an out-of-repo trust store; cloning
a repository never grants its plugins the right to run. If a plugin refuses to
execute, this is the command you want.
...
1. Walk up from the current directory — at most ten levels — for a `.horus/blackbox/`
   that actually holds a `blackbox.wal` or a `blackbox.json`. This is why it works from
   `src/` and not only from the project root.
2. Failing that, a `.horus/blackbox/` directory in the current directory, even an empty
   one.
...
The first line of output names the directory it read — whether it found events or not —
so check it before drawing conclusions:
...
<Callout type="warning" title="Outside a project you are reading the global store">

Run `horus blackbox` in a directory with no `.horus/blackbox/` above it and the events you
see may come from a different project's run. During incident analysis that is worse than
seeing nothing. Pass `--path .horus/blackbox` to pin it, or `cd` into the project first.
...
**What it does**: writes a dependency into the `horus.toml` next to you. Nothing is
installed system-wide and nothing is executed — the entry is resolved the next time
you build.

`add` and [`install`](#horus-install-install-package-driver-plugin) are the pair
people mix up:

| | Edits `horus.toml`? | Scope |
|---|---|---|
| `horus add <name>` | yes | this project |
| `horus install <name>` | no | the machine (global by default; `-t <workspace>` targets one) |
...
## `horus lock` - Record the Resolved Dependency Versions

**What it does**: resolves the dependency graph and writes `horus.lock`, a record
of the versions that resolution picked.
...
`--check` writes nothing; it exits non-zero when `horus.lock` and what is
actually **installed** disagree — a package installed at a different version
than the lock pins, or installed and not pinned at all — which is the shape a CI
job wants. It does not read `horus.toml`. A pin with nothing installed for it is
not drift: that is every fresh checkout before the first `horus run`, and
failing on it would make the gate fire on a clean clone.

| Flag | What it does |
|------|--------------|
| `--check` | Verify the lockfile is up to date instead of regenerating it |

<Callout type="info" title="What `--check` compares">

`--check` reads the packages actually installed under `.horus/` and compares them
against the `[[package]]` entries, exiting non-zero on any drift: a package pinned
at one version and installed at another, a matching version whose digest differs,
or an installed package the lockfile does not pin at all. Each line names the
package, so the failure says which dependency to look at.

It writes nothing, which is the shape a CI job wants. It previously only checked
that the file parsed, so a lockfile naming versions nobody had installed passed.

`[toolchain]` and `[[system]]` pins are reported but not enforced — nothing
installs or switches toolchains — and `features` records what was active at lock
time rather than constraining the next build.
...
**What it does**: runs an entry from the `[scripts]` table of `horus.toml`, so the
commands a project needs live with the project instead of in someone's shell history.
...
`horus script` (singular) is an alias for the same command. Everything after `--` is
handed to the script untouched. `horus run <name>` runs the same entry: `horus run`
checks the `[scripts]` table before it looks for any file, so a script name shadows a
file or directory called the same thing.

See [Configuration → \[scripts\]](/package-management/configuration) for how to
define them.
...
**What it does**: moves the CLI **and** the cached source tree to the latest
release, together.

```bash
horus self update           # install the latest release
horus self update --check   # report current vs latest, install nothing
```

In order, it:

1. Resolves the newest release from the GitHub releases API
2. Fetches that tag's source and caches it at `~/.horus/cache/horus@<version>`
3. Downloads the platform asset for the **same** tag and verifies it against that
   release's `SHA256SUMS` — a missing or unreadable checksum file is a refusal,
   never a reason to install unverified
4. Replaces the running binary in place, wherever it actually lives
5. Re-runs the installed binary and refuses to record success unless it reports
   the expected version
6. Writes `~/.horus/installed_version` and `~/.horus/install_manifest.toml`

The source half is not optional. `horus run` and `horus build` compile your
projects against `~/.horus/cache/horus@<version>` as a **path** dependency, so a
binary-only update leaves a CLI that cannot build a single Rust project — and,
worse, one that compiles projects against the previous release's shared-memory
headers. The source is fetched *before* the binary is swapped, so a failure
partway leaves the old CLI working.

On a platform with no published asset it compiles that same tag instead of
downloading it (~3-5 min). See
[Prebuilt Binaries](/getting-started/installation#prebuilt-binaries) for which
platforms have one.

| Flag | What it does |
|------|--------------|
| `--check` | Report the current and latest versions; download and install nothing |

`--check` exits **0** whether or not an update is available. A non-zero exit
means the check itself failed — GitHub unreachable, rate-limited, or an
unparseable response — so a CI job can gate on it. Those used to be reported as
"could not determine latest version" on the way to exit 0, indistinguishable
from being up to date.

A build whose version is *ahead* of the latest release (a source build from a
branch) is left alone rather than moved backwards.

### Related

This is the CLI, not your project: `horus update` moves your **dependencies**
forward, `horus self update` moves **horus** forward.

`self update` only goes to the newest release. To install a **specific** version,
or to roll back, re-run the installer with the tag pinned:

```bash
curl -fsSL https://github.com/softmata/horus/raw/main/install.sh | HORUS_VERSION=v0.4.0 bash
```

See [Upgrading HORUS](/getting-started/installation#upgrading-horus).
...
`install.sh` already does this for you; the command exists for package maintainers
and for anyone who built from source. The page is rendered from the same clap
definition the binary parses arguments with, so it cannot describe a command this
...
# Record what resolved, for the diff and for `horus lock --check` in CI
horus lock
...
```bash
# Install, or reinstall a specific release (both the binary and the cached source)
curl -fsSL https://github.com/softmata/horus/raw/main/install.sh | bash
curl -fsSL https://github.com/softmata/horus/raw/main/install.sh | HORUS_VERSION=v0.4.0 bash

# Remove HORUS from this machine (prompts first; --dry-run lists, --yes is unattended)
curl -fsSL https://github.com/softmata/horus/raw/main/uninstall.sh | bash
```

Windows has native PowerShell equivalents, `install.ps1` and `uninstall.ps1`.
To move to the latest release once HORUS is installed, use `horus self update` —
it is the same job with one command.
