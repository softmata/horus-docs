...
> package, not from the built-in types that `horus msg list` enumerates — that
> command only reads HORUS's own `horus_types`, so these three will not appear in
> its output. The built-in stand-in for `CmdVel` is `Twist` (`linear: [f64; 3]`,
> `angular: [f64; 3]`, `timestamp_ns: u64`).
