...
An existing root `Cargo.toml` is refused unless `--force`. Workspace projects
already build from a generated root workspace manifest, and non-Rust projects
have no Cargo manifest to eject. To start cargo-native instead, use
