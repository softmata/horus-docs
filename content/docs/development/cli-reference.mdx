...
<Callout type="warning">
**Project-local plugins must be trusted before they run.** A plugin binary that ships inside a checkout's `.horus/` directory (part of a cloned repository) is **refused execution** unless its content hash is recorded in your out-of-repo trust store. A checksum in `.horus/plugins.lock` is *not* proof of trust — the repository controls that file. Plugins installed globally (`horus install`) are trusted at install time; a project-local plugin installed with `horus install --local` is auto-trusted. Otherwise, run `horus plugin trust <command>` to allow it. This prevents a cloned repo from silently auto-executing code the moment you run `horus <cmd>`.
</Callout>
