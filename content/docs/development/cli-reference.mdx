...
> **Note**: three of these are inert — `priority`, `rate_hz` and the per-node `namespace`.
>
> `priority` and `rate_hz` are each exported to the child process — as
> `HORUS_NODE_PRIORITY` and `HORUS_NODE_RATE_HZ` — and nothing in the workspace reads
> either one, so a launch file cannot set a node's priority or its tick rate. Both values
> still appear in `--dry-run` and `--list` output. Set priority with `.priority()` on the
> Rust builder or `priority=` on the Python `Node`; set the rate with
> `Scheduler::tick_rate()` or `.rate()` on the Rust node builder, or `.rate()` on the
> Python `scheduler.add(...)` chain.
>
