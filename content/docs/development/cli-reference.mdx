...
**What it does**: resolves the dependency graph and writes `horus.lock`, so the same
source builds the same way on the robot as it did on your laptop.
...
forward, `horus self update` moves **horus** forward. If HORUS was installed from a
package manager, update it there instead.
