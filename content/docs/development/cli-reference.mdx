...
<Callout type="warning" title="The pins are recorded, not yet enforced">

`horus.lock` is not yet a reproducibility guarantee. The `[[package]]` entries
are written by the resolver, but no install path reads them back: the pip,
crates.io and registry installers still take their versions from `horus.toml`
and fall back to `latest`. So checking out a committed `horus.lock` on a clean
machine can install different versions than the ones recorded — and the next
`horus lock` on that machine overwrites the committed pin with whatever `latest`
resolved to there.

`checksum` is written too (always empty today) and never compared against a
download. The one field that *is* consulted is `config_hash`, which only decides
whether resolution can be skipped.

Until the resolver applies the pins before installing, treat `horus.lock` as a
record of what **was** resolved, not a guarantee of what **will** be. For a
dependency that must not move, write an exact version in `horus.toml`.
