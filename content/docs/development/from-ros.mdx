...
| Feature | ROS | HORUS |
|---------|-----|-------|
| **Latency** | ~100μs (DDS) | ~1μs (shared memory) |
...
|---------|------------------|----------------|
| **Simple Relay** | 1 sub → 1 pub, no processing | Direct message forwarding |
...
| **Aggregator** | Multiple subs → single pub | Message merging |
...
**Matched patterns → built-in nodes:**
...
Conversion Plan:
  ...
  │   │   ├── lib.rs
  │   │   ├── nodes/
  │   │   │   ├── motor_controller.rs (pattern: PID Controller)
  │   │   │   ├── sensor_fusion.rs (pattern: Aggregator)
  │   │   │   ├── navigation.rs (pattern: State Machine)
  │   │   │   ├── safety_monitor.rs (pattern: Safety Monitor)
  │   │   │   └── camera_processor.rs (pattern: Sensor Processor)
  │   │   └── messages/
  │   │       ├── robot_state.rs
  │   │       ├── sensor_data.rs
  │   │       └── command.rs
...
Recommendations:
  • motor_controller.rs → Consider using PidNode<f64> built-in
  • camera_processor.rs → Consider using CameraNode<D,P> built-in

Potential Issues:
  ⚠ navigation.rs: Complex state machine (15 states), review generated code
  ⚠ sensor_fusion.rs: Uses 6 message types, verify all are available
```
...
///
/// Subscriptions:
///   - /scan (LaserScan) → checks for obstacles
///
/// Publications:
...
```
Analyzing: state_machine_node.cpp
...
2. **Review recommendations:**
   Look for `// HORUS Built-in Recommendation` comments and consider using built-ins.
...
The converter has been tested against 337 ROS packages with **100% compilation success**. However, compilation success doesn't mean the code is production-ready. Here's what to expect:

### What Converts Well

| Package Type | Confidence | Notes |
|--------------|------------|-------|
| **Simple relay nodes** | 95% | Subscribe → transform → publish |
| **Sensor drivers** | 90% | Standard message types, hardware abstraction |
| **Motor controllers** | 85% | PID loops, velocity commands |
| **Message definitions** | 95% | All standard ROS message types |
| **Service definitions** | 90% | Request/response patterns |
| **Launch parameters** | 80% | Extracted from launch files and YAML |

### What Needs Manual Review

| Package Type | Confidence | What to Check |
|--------------|------------|---------------|
| **State machines** | 60% | Complex state transitions, verify logic |
| **Sensor fusion** | 70% | Multi-input aggregation, timing |
| **Navigation stacks** | 50% | Heavy ROS-specific dependencies (tf2, costmap) |
| **Computer vision** | 40% | OpenCV integration, image pipelines |
| **Action servers** | 60% | Long-running task management |

### Limitations

The converter **cannot** automatically handle:

- **tf2 transforms** - HORUS uses a different transform system
- **actionlib state machines** - Need manual adaptation to HORUS patterns
- **ROS parameter server** - Converted to local config, no dynamic reconfigure
- **Plugin-based architectures** - pluginlib/nodelet patterns need restructuring
- **Hard-coded ROS paths** - References to `/opt/ros/...` won't work

### Recommended Workflow

```
1. Run with --analyze-only first    → Understand what you're getting
2. Convert and cargo check          → Verify it compiles
3. Review tick() implementations    → Logic may need adjustment
4. Test with mock data              → Verify behavior matches
5. Profile performance              → Confirm latency improvements
```

### Production Readiness Checklist

Before deploying converted code:

- [ ] All `tick()` functions reviewed for correct logic
- [ ] Topic names verified against your system
- [ ] Parameters validated against original launch files
- [ ] Unit tests passing (or added for critical paths)
- [ ] Integration tested with real hardware
- [ ] Performance benchmarked against ROS baseline

### When to Use the Bridge Instead

Consider using the [ROS2 Bridge](/development/ros2-bridge) instead of converting when:

- You need to run ROS and HORUS nodes together
- The package has deep ROS dependencies (tf2, navigation2)
- You're prototyping and want quick interop
- The package will continue to receive ROS updates
