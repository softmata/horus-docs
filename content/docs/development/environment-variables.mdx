...
|----------|---------|--------|
| `HORUS_LOG_FILE` | unset | Exactly `1` or `true` starts the file drain. Without it nothing is written to disk, and `HORUS_LOG_DIR`, `HORUS_LOG_MAX_SIZE` and `HORUS_LOG_MAX_FILES` have no effect — they are read past the `enabled` gate in `start_file_drain` (`log_buffer.rs:617`). It does **not** gate the whole family: `HORUS_LOG_BUFFER_SIZE` is read by `max_log_entries` when the shared buffer is created (`log_buffer.rs:37`) and `HORUS_LOG_LEVEL` on the first `hlog!` (`hlog.rs:86`), so both take effect with `HORUS_LOG_FILE` unset |
...
| `HORUS_RECORD_MAX_MB` | `512` | Caps the in-RAM recording buffer for `horus run --record`, in megabytes. `0` means unlimited — the recorder then keeps every tick for the life of the run, which is what `--record` used to do by construction |
| `HORUS_RECORD_INTERVAL` | `1` | Record every Nth tick rather than all of them. Values below 1 are clamped to 1, so a session always records something |
| `HORUS_SEND_RETRY_BUDGET_US` | `200` | Wall-clock a lossy `send()` may spend in its retry cold path before giving up, in microseconds. Well above the ~1.4 µs four yields cost on an idle machine, so it only bites once yielding has become expensive; `0` means never yield |
| `HORUS_RT_IDLE_LATENCY_US` | derived from the fastest RT chain | The cpuidle exit latency the guard spin can still absorb, in microseconds. Left unset, it is computed from the shortest tick period, so a 4 kHz chain gets a tighter budget than a 1 kHz one. `0` is accepted but holds every CPU in `POLL` and logs that it has done so — lowest latency, highest power |
| `HORUS_RT_IDLE_LATENCY_SCOPE` | global | Where that budget is applied. The default writes `/dev/cpu_dma_latency`, which the kernel releases the moment the fd closes — including on `SIGKILL`, so it cannot leak. Exactly `percpu` uses the per-CPU sysfs path instead, which lets non-RT cores keep their power savings on a shared box but **survives the process**, so it must be opted into |
| `HORUS_WIN_REALTIME_CLASS` | unset | Windows only. Exactly `1` opts the process back into `REALTIME_PRIORITY_CLASS`. Attempted once per process however many RT threads start, and every thread learns whether it worked |
...
|----------|--------|-------|
| `HORUS_NODE_RATE_HZ` | `horus launch`, when a node declares `rate_hz` | The scheduler's tick rate for that process. Read in `Scheduler::finalize_config` (`scheduler/mod.rs:2894`) and applied *after* the deferred config, so the launch file wins over a compiled-in `tick_rate()` — putting it in the launch file is the point. It is applied *before* the node-rate sweep, so a node that declares a faster `.rate()` still bumps the grid up rather than being starved. Values outside 0.001–1000000 Hz, or unparseable ones, log a warning and leave the tick rate unchanged |
...
| `HORUS_NAMESPACE` | `horus launch`, from the launch file's top-level `namespace`, or from a node's own `namespace:` where it declares one | The shared-memory namespace the launch shares. A node that declares `namespace:` gets that value instead of the file's — the more specific setting wins (`effective_namespace` in `launch.rs` is `node_level.or(file_level)`), so such a node is isolated from its siblings and will not see their topics. Also set by a *test* binary for its children, and only then — the write is confined to a test binary that resolved a `test_`-prefixed namespace and found the variable unset — so a spawned helper or interpreter lands in the same private namespace as the test that spawned it |
