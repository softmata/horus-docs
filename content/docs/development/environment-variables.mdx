...
| `HORUS_REGISTRY_URL` | *(none)* | Package registry. There is no public default while [#173](https://github.com/softmata/horus/issues/173) is open — set this to use a self-hosted registry or an offline mirror. Unset means registry commands report that no registry is configured. |
| `HORUS_PLUGIN_REGISTRY_URL` | *(none)* | Plugin registry. Same as above. |
...
The installer knobs above are read by `install.sh` and by `install.ps1`, which
takes the same six names. They are read from the **installer's** environment, so
with a pipe they belong on the right-hand side —
`HORUS_VERSION=v0.4.0 curl ... | bash` puts the variable in curl's environment,
...
| `HORUS_VERSION` | every CLI plugin | The `horus` version that launched the plugin, for compatibility checks. The same name is a version pin when the **installer** reads it — see the warning above |
