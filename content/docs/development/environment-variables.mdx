...
| `HORUS_NET_HEARTBEAT_MS` | `50` (`200` under WSL2) | How often peers announce themselves |
...
| `HORUS_NET_DISCOVERY_MISSES` | `3` (`6` under WSL2) | Missed announcements before a peer is considered gone |
...
`HORUS_ENABLE` (or `[enable]` in `horus.toml`) itself and adds `net` to the
`horus` dependency in the generated manifest
