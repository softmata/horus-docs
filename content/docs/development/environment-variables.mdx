...
| `HORUS_RECORD_MAX_MB` | `512` | Ceiling on the in-RAM recording buffer for a `--record` session, in megabytes. `0` means unlimited, which is what `--record` used to get by construction — a debugging session recorded every tick, forever, into memory. The buffer is a ring: at the cap the oldest snapshot is evicted, so a long run keeps the most recent 512 MB rather than failing. A value that does not parse falls back to the default |
| `HORUS_RECORD_INTERVAL` | `1` | Record one tick in every N. `1` records every tick, which is the full-rate default a `--record` session gets; `10` at 1 kHz records 100 snapshots a second. Floored at 1, so `0` does not mean "never". A value that does not parse falls back to the default |
...
| `HORUS_RT_IDLE_LATENCY_US` | derived from the tick rate | Microseconds of CPU idle-state exit latency the run will tolerate, published to the kernel through `/dev/cpu_dma_latency`. Nothing else in the stack bounds this: a core that drops into a deep C-state takes tens to hundreds of microseconds to come back, which lands on the tick as jitter no scheduling policy can remove. The default tracks the spin guard, so the deepest state the governor may enter is one the guard can still absorb. `0` is accepted but warned about — it pins every core in POLL, which is the lowest-latency and highest-power setting and on a battery robot is a thermal and runtime cost, not a free win |
| `HORUS_RT_IDLE_LATENCY_SCOPE` | `global` | Where the idle-latency bound above applies. The default holds `/dev/cpu_dma_latency` open for the whole machine; the kernel drops the constraint the instant the fd closes, including on `SIGKILL`, so it cannot outlive the process. `percpu` writes the bound into each RT core's `pm_qos_resume_latency_us` instead, leaving the other cores their power savings on a shared box — but sysfs has no owning fd, so that value **survives the process** and is restored only on a clean exit. Opt in deliberately. Any other value, or `percpu` with no RT cores pinned, keeps `global` |
| `HORUS_SEND_RETRY_BUDGET_US` | `200` | Wall-clock a lossy `Topic::send` may spend retrying a full ring before it drops the message, in microseconds. The cold path spins and then yields, and `yield_now` is a request with no upper bound on when the thread comes back: four unconditional yields on a full ring measured 26 ms mean with 32 competing threads and 110 ms with 128. `0` means never yield — one attempt, then drop. Threads the kernel actually granted a real-time policy use `0` regardless of this setting, because under `SCHED_FIFO` a yield returns only once every same-priority peer has blocked |
| `HORUS_WIN_REALTIME_CLASS` | unset | **Windows only.** Exactly `1` puts the process in `REALTIME_PRIORITY_CLASS`. Off by default because it is a *process* attribute: it moves every thread — helpers, the log drain, the tokio blocking pool, threads that already existed — into the 16-31 band above most driver threads, and no per-thread call brings one back out, since even `THREAD_PRIORITY_IDLE` inside that class is base 16. The default instead gives the tick thread `THREAD_PRIORITY_TIME_CRITICAL` inside the inherited class, which preempts every ordinary thread on the box while leaving the kernel's own alone. Set it on a machine genuinely dedicated to the robot |
...
| `HORUS_NODE_RATE_HZ` | set by `horus launch` | A launch file's per-node `rate_hz`. Read by `finalize_config`, **after** the deferred config so the launch file beats a compiled-in `tick_rate()`, and before the tick grid is adjusted so a faster node still widens it. A malformed value warns and leaves the rate alone rather than refusing to start a node whose environment the operator cannot see. Read, not written, by hand |
| `HORUS_DRIVER_NAME` | set by HORUS | The `[hardware.<name>]` key, injected into every exec driver — the same name the scheduler uses and the driver logs under. Read, not written, by hand |
...
| `HORUS_REGISTRY_URL` | unset | Package registry base URL. There is **no compiled-in default** — `DEFAULT_REGISTRY_URL` is `None` (`horus_manager/src/config.rs:22`), on purpose. With nothing set, registry-backed commands print "no package registry is configured" and name this variable instead of attempting a request to a host that is not serving (see [horus#173](https://github.com/softmata/horus/issues/173)). Point it at a self-hosted registry or an offline mirror to enable them. A whitespace-only value counts as unset |
| `HORUS_PLUGIN_REGISTRY_URL` | unset | Plugin registry base URL. Same shape: `DEFAULT_PLUGIN_REGISTRY_URL` is `None` (`config.rs:28`), and plugin commands report the same "not configured" message until you set this |
...
There is no `HORUS_NET` variable. There was one, read only by `horus doctor` to
decide its own output, so setting it changed nothing about a build except what
`doctor` then claimed — it has been retired, and `doctor` now reports the
capability the build would actually get.

What enables networking is the `net` capability: `horus run --net` appends it to
the capability list, which is written to `HORUS_ENABLE`, and `enable = ["net"]`
in `horus.toml` does the same thing persistently. The `net` capability
deliberately contributes no Cargo feature to your own crate — `enable_to_features` returns an empty list for it,
...
Both of those require `HORUS_PREFIX` to be **in the environment that runs
`horus`** — the CLI reads it from there and from nowhere else, because nothing
outside a prefix tree records that the prefix exists. The installer exports it
into your shell rc alongside `PATH`, so a normal install is already set up; a
shell that predates the install, or a service unit with a scrubbed environment,
...
Without it the cache root falls back to `~/.horus/cache`, which is deduped away
as the legacy root — so the prefix cache is never searched — and the version gate
resolves a state root holding none of that install's files. That is why the
installer used to tell prefix users to `export HORUS_SOURCE=…` instead, and to
warn that the version gate was inactive: both were true for the shell it left
behind. Exporting `HORUS_PREFIX` is what makes them not.
...
`HORUS_RECORD_SESSION` from `horus run --record`; `HORUS_SIM_MODE` and
`HORUS_SIM_TARGETS` from `--sim`; and `HORUS_ASSUME_YES` from `horus new --yes`.
A child inherits all of them. (`--net` sets nothing of its own — it appends
`net` to the capability list, which is what `HORUS_ENABLE` carries.)
...
|----------|--------|--------|
| `HORUS_NODE_PRIORITY` | `horus launch`, when a node declares `priority` | Nothing reads it. The source says so at the `HORUS_NODE_PRIORITY` assignment in `horus_manager/src/commands/launch.rs`: kept only so a configured value leaves a trace |
