...
| `Backend { backend, message }` | Hardware backend errors (named fields) |
...
// Backend error with context (takes backend name + message)
...
// Driver error
let err = Error::driver("Failed to load serial driver");
...
let err = Error::internal("Unexpected state reached");
```
...
| `std::str::ParseBoolError` | `Error::ParseError` |
