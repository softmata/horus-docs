...
- **HORUS-native** - Direct Topic integration, no middleware
...
flowchart TB
    subgraph UI["User Interface"]
        CLI["CLI<br/>horus sim3d"]
        GUI["GUI (egui)<br/>Debug panels"]
        PY["Python (Gym)<br/>RL training"]
    end
    subgraph CORE["Simulation Core (Bevy ECS)"]
        PHYS["Physics<br/>(Rapier3D)"]
        SENS["Sensors<br/>Simulation"]
        TF["TF System<br/>(frames)"]
        subgraph BRIDGE["HORUS Topic Bridge"]
            B1["Publish sensor data"]
            B2["Subscribe to cmd_vel"]
            B3["Publish /tf transforms"]
        end
        PHYS --> BRIDGE
        SENS --> BRIDGE
        TF --> BRIDGE
    ...
    GUI --> CORE
    PY --> CORE
    CORE --> VISUAL["Visual Rendering<br/>(60 FPS)"]
    CORE --> HEADLESS["Headless RL Mode<br/>(100K+ steps/sec)"]
`}
  caption="sim3d architecture: Bevy ECS simulation core with HORUS integration"
/>
...
```rust
use horus::prelude::*; // Provides {ImuNode, ImuBackend, GpsNode, GpsBackend, LidarNode, LidarBackend}
...
let cmd_pub = Topic::CmdVel>::new("cmd_vel")?;
let scan_sub = Topic::LaserScan>::new("scan")?;
