...
**For most users: Eventually, maybe.** AI/vision is common but not required.

**When this page IS useful:**
- Adding camera-based object detection or tracking
- Using ML models for decision making
- Integrating LLMs (like GPT) for voice commands

**Quick answer:** Build your basic robot first (sensors, motors, control). Add AI/vision when you need perception or intelligent behavior.
...
```
Camera Node ──Topic──> Vision Node ──Topic──> Control Node
...
```rust
use horus::prelude::*; // Provides {Scheduler, CameraNode, Result}
...
pub struct VisionProcessorNode {
    camera_sub: Topic<ImageFrame>,
    detections_pub: Topic<String>,
}

impl VisionProcessorNode {
    pub fn new() -> Result<Self> {
        Ok(Self {
            camera_sub: Topic::new("camera.raw")?,
            detections_pub: Topic::new("vision.detections")?,
        })
    }
...
pub struct YOLOv8Node {
    camera_sub: Topic<ImageFrame>,
    detections_pub: Topic<Vec<Detection>>,
    session: Session,
}
        ...
        if let Some(frame) = self.camera_sub.recv() {
            if let Some(input) = self.preprocess_image(&frame.data, frame.width, frame.height) {
                if let Some(detections) = self.run_inference(input) {
                    self.detections_pub.send(detections).ok();
                }
            }
        }
    }
...
impl TFLiteClassifierNode {
    pub fn new(model_path: &str, labels_path: &str) -> Result<Self> {
        ...
        Ok(Self {
            camera_sub: Topic::new("camera.raw")?,
            class_pub: Topic::new("vision.classification")?,
            interpreter,
            labels,
        })
    }
...
impl TractInferenceNode {
    pub fn new(model_path: &str) -> Result<Self> {
        ...
        Ok(Self {
            camera_sub: Topic::new("camera.raw")?,
            output_pub: Topic::new("vision.output")?,
            model,
        })
    }
...
pub struct LLMCommandNode {
    text_sub: Topic<String>,  // Natural language commands
    cmd_pub: Topic<String>,   // Structured robot commands
...
impl LLMCommandNode {
    pub fn new(model_path: &str) -> Result<Self> {
        ...
        Ok(Self {
            text_sub: Topic::new("speech.text")?,
            cmd_pub: Topic::new("robot.commands")?,
            context,
        })
    }
...
impl Node for LLMCommandNode {
    ...
    fn tick(&mut self) {
        if let Some(text) = self.text_sub.recv() {
            if let Some(command) = self.generate_command(&text) {
                self.cmd_pub.send(command).ok();
            }
        }
    }
}
...
pub struct CandleLLMNode {
    text_sub: Topic<String>,
    response_pub: Topic<String>,
    model: Llama,
    tokenizer: Tokenizer,
    device: Device,
}
...
impl CandleLLMNode {
    pub fn new(model_path: &str, tokenizer_path: &str) -> Result<Self> {
        ...
        Ok(Self {
            text_sub: Topic::new("user.input")?,
            response_pub: Topic::new("llm.response")?,
            model,
            tokenizer,
            device,
        })
...
impl Node for CandleLLMNode {
    ...
    fn tick(&mut self) {
        if let Some(input) = self.text_sub.recv() {
            if let Some(response) = self.generate(&input, 100) {
                self.response_pub.send(response).ok();
            }
        }
    }
}
...
pub struct GPT4PlannerNode {
    goal_sub: Topic<String>,
    plan_pub: Topic<String>,
    client: Client,
    api_key: String,
...
impl GPT4PlannerNode {
    pub fn new(api_key: String) -> Result<Self> {
        ...
        Ok(Self {
            goal_sub: Topic::new("user.goal")?,
            plan_pub: Topic::new("robot.plan")?,
            client: Client::new(),
            api_key,
...
pub struct ClaudeReasoningNode {
    sensor_sub: Topic<String>,  // Sensor readings as JSON
    decision_pub: Topic<String>,
    client: Client,
    api_key: String,
}
...
impl ClaudeReasoningNode {
    pub fn new(api_key: String) -> Result<Self> {
        Ok(Self {
            sensor_sub: Topic::new("sensors.summary")?,
            decision_pub: Topic::new("robot.decision")?,
            client: Client::new(),
            api_key,
        })
    }
...
impl Node for ClaudeReasoningNode {
    ...
    fn tick(&mut self) {
        if let Some(sensors) = self.sensor_sub.recv() {
            if let Some(decision) = self.reason(&sensors) {
                self.decision_pub.send(decision).ok();
            }
        }
    }
}
...
impl SceneUnderstandingNode {
    pub fn new(api_key: String) -> Result<Self> {
        Ok(Self {
            camera_sub: Topic::new("camera.raw")?,
            description_pub: Topic::new("scene.description")?,
            client: Client::new(),
            api_key,
        })
    }
        ...
        if let Some(frame) = self.camera_sub.recv() {
            ...
            if frame.stamp_nanos % 30 == 0 {
                if let Some(description) = self.understand_scene(&frame.data) {
                    self.description_pub.send(description).ok();
                }
            }
        }
    }
...
```rust
use horus::prelude::*; // Provides {Scheduler, DifferentialDriveNode, Result}
...
fn main() -> Result<()> {
    ...
    // Add nodes
    scheduler.add(speech_node).order(0).done();
    scheduler.add(llm_node).order(1).done();
    scheduler.add(parser_node).order(2).done();
    scheduler.add(drive_node).order(3).done();
...
pub struct CommandParserNode {
    llm_sub: Topic<String>,
    cmd_pub: Topic<(f32, f32)>,
}

impl CommandParserNode {
    pub fn new() -> Result<Self> {
        Ok(Self {
            llm_sub: Topic::new("robot.plan")?,
            cmd_pub: Topic::new("motor.cmd")?,
        })
    }
}
...
impl Node for CommandParserNode {
    ...
    fn tick(&mut self) {
        if let Some(json_str) = self.llm_sub.recv() {
            // Parse JSON from LLM
...
spawn(async move {
    ...
    response_topic.send(response, None).ok();
});
```
...
impl PythonMLNode {
    pub fn new() -> Result<Self> {
        Ok(Self {
            camera_sub: Topic::new("camera.raw")?,
            detections_pub: Topic::new("ml.detections")?,
            frame_count: 0,
        })
    }
...
impl OpenAIVisionNode {
    pub fn new(api_key: String) -> Result<Self> {
        Ok(Self {
            camera_sub: Topic::new("camera.raw")?,
            description_pub: Topic::new("vision.description")?,
            client: Client::new(),
            api_key,
        })
    }
...
```rust
use horus::prelude::*; // Provides {Topic, CameraNode, DifferentialDriveNode, Result}

// Simple vision-based follower
pub struct ObjectFollowerNode {
    vision_sub: Topic<String>,
    cmd_pub: Topic<(f32, f32)>,
}
...
impl ObjectFollowerNode {
    pub fn new() -> Result<Self> {
        Ok(Self {
            vision_sub: Topic::new("vision.detections")?,
            cmd_pub: Topic::new("motor.cmd")?,
        })
    }
}
...
impl Node for ObjectFollowerNode {
    ...
    fn tick(&mut self) {
        if let Some(detection) = self.vision_sub.recv() {
...
fn main() -> Result<()> {
    ...
    scheduler.add(camera).order(0).done();     // Highest priority
    scheduler.add(vision).order(1).done();     // Process images
    scheduler.add(follower).order(2).done();   // Make decisions
    scheduler.add(drive).order(3).done();      // Control motors
...
Decision:    ~1μs   (HORUS Topic communication)
Control:     ~1μs   (HORUS Topic communication)
...
while let Ok(data) = rx.recv() {
    ...
    result_topic.send(result, None).ok();
}
```
...
if let Some(result) = try_inference(&data) {
    control_pub.send(result).ok();
} else {
    // Fallback behavior
    control_pub.send(default_safe_command()).ok();
}
```
