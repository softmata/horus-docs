...
HORUS supports local AI inference through two main approaches (cloud APIs, covered further
down, are a third path for higher-level reasoning):
...
flowchart LR
    S["<b>Sensor Node</b><br/>capture ~1-16ms"] -->|"Topic ~85ns"| M["<b>ML Node</b><br/>inference ~5-50ms"]
    M -->|"Topic ~85ns"| C["<b>Control Node</b><br/>~1μs of work per tick,<br/>at its own rate (e.g. 100 Hz)"]
`}
...
~920,000 Python ints, ~1.7MB packed — is rejected outright: `ValueError: 'data'
out of range: expected [0..4096]`, raised on every tick. Declare it as `horus.Tensor`
...
[dependencies]
horus = "0.4"
ort = { version = "2.0", source = "crates.io" }
ndarray = { version = "0.15", source = "crates.io" }
```
...
[dependencies]
horus = "0.4"
tract-onnx = { version = "0.21", source = "crates.io" }
```
