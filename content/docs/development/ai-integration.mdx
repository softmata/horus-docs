...
- Custom inference with `horus_ai` tensor system
- 1-50ms typical inference latency

### Architecture Pattern

```
Sensor Node ──Topic──> ML Node ──Topic──> Control Node
...
## horus_ai: Tensor System

HORUS includes the `horus_ai` crate, a tensor system with DLPack interoperability for zero-copy tensor sharing between frameworks.
...
The `horus_ai` crate provides:
- **Tensor types** for ML data representation
- **DLPack interop** for zero-copy sharing with PyTorch, TensorFlow, JAX, etc.
- **Device abstraction** (CPU/GPU)
- **ML utilities** for common operations

This is useful when you need to pass tensor data between HORUS nodes and external ML frameworks without copying.
...
```python
from horus import Node, Scheduler, CameraNode
import numpy as np

# Camera capture (auto-detects hardware, falls back to simulation)
cam = CameraNode(device=0, width=640, height=480, prefix="cam")

# ML processing node
def ml_tick(node):
    if node.has_msg("cam.frame"):
        frame = node.get("cam.frame")
...
| **Custom** | `horus_ai` | Tensor/DLPack interop | None |
...
class AsyncMLNode(AsyncNode):
    async def setup(self):
        self.input = AsyncTopic("cam.frame", dict)
        self.output = AsyncTopic("detections", dict)
...
- [Hardware Nodes](/python/library/python-hardware-nodes) - Camera and sensor capture
- [Async Nodes](/python/api/async-nodes) - Non-blocking inference patterns
