...
fn main() -> Result<()> {
    ...
    // 4. Motor control (user-defined node)
    let drive_node = DriveNode::new()?;
    ...
    fn tick(&mut self) {
        if let Some(json_str) = self.llm_sub.recv() {
            ...
            if let Ok(cmd) = serde_json::from_str::<serde_json::Value>(&json_str) {
                ...
                self.cmd_pub.send((linear, angular)).ok();
            }
        }
    }
}
...
spawn(async move {
    ...
    response_topic.send(response).ok();
});
```
...
```rust
use horus::prelude::*; // Provides {Topic, CameraNode, Result}
