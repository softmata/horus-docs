...
// simplified
  ...
  <LangTab language="Python">
    ...
    HORUS ships no ML wrapper library — you use ONNX Runtime, PyTorch, TensorFlow, or OpenCV
    directly inside a node's `tick`, exactly as shown above. The interop is provided by the
    message types themselves, which expose zero-copy NumPy views:

    ```python
    img = node.recv("cam.image_raw")

    np_arr = img.to_numpy()                    # zero-copy NumPy view (~3μs)
    tensor = torch.from_numpy(img.to_numpy())  # PyTorch
    outputs = session.run(None, {input_name: np_arr[np.newaxis]})  # ONNX Runtime
    ```

    `to_numpy()` / `from_numpy()` are available on `Image`, `DepthImage`, and `PointCloud`.

    See [ML Integration](/python/library/ml-utilities) for the full interop matrix.
  </LangTab>
</LanguageTabs>
...
<LanguageTabs>
  <LangTab language="Rust">
Add the HTTP client to your project first:
