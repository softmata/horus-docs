...
**For most users: No.** Only needed if you have existing ROS2 systems.

**When this page IS useful:**
- Integrating with existing ROS2 robots or sensors
- Using ROS2 packages (like Nav2) with HORUS
- Gradual migration from ROS2 to HORUS

**Quick answer:** If you're new to robotics or starting fresh, skip this. Build everything in HORUS. If you have ROS2 code you want to keep, the bridge lets them communicate.

</Callout>

The HORUS ROS2 Bridge enables **runtime interoperability** between HORUS and ROS2 systems using Zenoh as the transport layer. The bridge allows existing ROS2 nodes to coexist and communicate with HORUS nodes at runtime without any code conversion.
...
`}
  caption="ROS2 Bridge architecture: Zenoh-based interoperability"
/>
...
let odom = Topic::new("robot.odom")?;  // Receives from ROS2 /robot/odom
let cmd = Topic::new("cmd_vel")?;            // Publishes to ROS2 /cmd_vel
```
...
```rust
use horus::prelude::*; // Provides Topic

// Call a bridged ROS2 service
let client = Topic::service_client("get_map")?;
...
use horus::prelude::*; // Provides {Topic, Node, NodeInfo}
