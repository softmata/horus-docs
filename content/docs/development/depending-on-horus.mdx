...
| `[[bin]]`, `[lib]`, `[[test]]`, `[[example]]`, `[[bench]]` for the project's targets | `cargo_gen.rs`, `write_layout_targets` | Cargo discovers targets relative to the manifest's directory, which is `.horus/` — so `tests/`, `examples/`, `benches/` and `src/bin/` have to be declared or they compile to nothing. Top-level `*.rs` files are targets, `dir/name/main.rs` is a target named `name`, and nested modules (`tests/common/mod.rs`) are helpers, matching cargo |
...
## Managed, or cargo-native

A HORUS project is in one of two modes, and which one it is decides whether a
plain `cargo` command works.

| | **Managed** (what `horus new` writes) | **Cargo-native** |
|---|---|---|
| Manifest | `horus.toml`; `.horus/Cargo.toml` is generated and gitignored | root `Cargo.toml` |
| Plain `cargo build` / `run` / `test` | fails — no manifest at the project root | works normally |
| `horus build` / `run` / `test` | generate the manifest, run cargo in `.horus/` | run cargo against your manifest |
| `[rust]` in `horus.toml` | applies (profile, lints, features, rustflags) | **ignored** |
| `horus add` | writes to `horus.toml` | writes to `Cargo.toml` |

`horus doctor -v` prints which mode the project is in.

There are three ways to get a cargo-native project:

- `horus new my_robot --cargo` — start that way.
- `horus eject` — convert a managed project: it writes the root `Cargo.toml`
  from the generated one, rewrites every relative path (targets and local path
  dependencies), deletes `.horus/Cargo.toml` so there is only one source of
  truth, and refuses to replace an existing root manifest without `--force`.
- Write the manifest yourself, as the rest of this page describes — the right
  route for adding HORUS to a crate you already have.

Once a root `Cargo.toml` exists, `horus build`/`run`/`test` build from it and
never regenerate `.horus/Cargo.toml`; artefacts still land in `.horus/target`
because every cargo spawn gets `CARGO_TARGET_DIR` set. The two `[patch]` tables
below are still required — they belong in the root manifest now, not in
`horus.toml`.

In managed mode, shell integration is the other way to keep using cargo: the
installer runs `horus env --init`, which shadows `cargo` with a function that
routes it through `horus cargo` inside HORUS projects and to the real binary
everywhere else. Without that function installed, a bare `cargo build` in a
managed project fails with `could not find Cargo.toml`, and the command to use
is `horus build`.
