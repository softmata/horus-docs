...
fn test_single_tick_behavior() {
    let mut scheduler = Scheduler::new();
    scheduler.add(SensorNode::new().unwrap()).order(0).build();
    scheduler.add(ControlNode::new().unwrap()).order(1).build();
...
fn test_selective_tick() {
    ...
    scheduler.add(SensorNode::new().unwrap()).order(0).build();
    scheduler.add(ControlNode::new().unwrap()).order(1).build();
    scheduler.add(LoggerNode::new().unwrap()).order(2).build();
...
fn test_replay_crash_scenario() {
    ...
    scheduler.add(DiagnosticNode::new().unwrap()).order(2).build();
