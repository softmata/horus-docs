...
fn main() -> Result<()> {
    ...
    scheduler.add(TemperatureSensor::new()?).order(0).done();
    scheduler.run()?;
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    scheduler.add(SubscriberNode::new(received)?).order(1).done();
    scheduler.run()?;
    Ok(())
}
...
mod tests {
    ...
    fn test_pubsub_communication() {
        ...
        // Small delay so the publisher's tick can run — a send() is visible to
        // other processes the instant it completes, so this is about scheduling,
        // not propagation
...
**Why single-threaded (`--test-threads 1`)?**
- Prevents tests from running in parallel — `horus test` already defaults to one thread, so the flag just makes that explicit
...
The positional filter is passed straight through to `cargo test`, so only tests
whose full name contains `test_pubsub_communication` run — `test_multiple_messages`
is filtered out. Drop the filter (`horus test --test-threads 1`) to run both.

## Testing Business Logic in Isolation

Test node logic without exercising any Topic traffic.
...
fn main() -> Result<()> {
    ...
    let mut scheduler = Scheduler::new();
    scheduler.add(GeneratorNode::new()?).order(0).done();
    scheduler.add(DoublerNode::new()?).order(1).done();
    scheduler.add(CollectorNode::new(collected)?).order(2).done();
...
Give the publisher's tick a chance to run:
...
`horus test` defaults to single-threaded execution to prevent shared memory conflicts:

```bash
horus test    # Already single-threaded by default
```

If you override that with `--parallel`, make sure each test uses unique topic names.
...
#[test]
fn test_no_messages() {
    // SubscriberNode::new takes the shared buffer it pushes into
    let received = Arc::new(Mutex::new(Vec::new()));
    let mut node = SubscriberNode::new(Arc::clone(&received)).unwrap();

    node.tick();  // Should handle an empty topic gracefully
...
fn test_invalid_data() {
    ...
    // process_temperature is infallible — it has no Err to return, so an
    // edge-case test asserts on the value it produces
    assert!(processor.process_temperature(f32::NAN).is_nan());
}
```
...
# Verbose output (no short -v form on this subcommand)
horus test --verbose
...
A `tests/` directory at the project root works too: `horus test` declares every
top-level `tests/*.rs` (and `examples/`, `benches/`, `src/bin/`) as a cargo
target in the generated manifest, so a normal integration-test layout runs
without a root `Cargo.toml`. Nested modules like `tests/common/mod.rs` stay
helpers, as they do in cargo. In a cargo-native project (`horus eject`, or
`horus new --cargo`) plain `cargo test` sees the same files.
...
fn test_system_runs_for_one_second() {
    let mut scheduler = Scheduler::new();
    scheduler.add(SensorNode::new().unwrap()).order(0).done();
    scheduler.add(ControlNode::new().unwrap()).order(1).done();
...
```bash
# Record while running — the value is the session name (required)
horus run --record my_session
```

Recordings are saved to the platform data directory — `~/.local/share/horus/recordings/<session>/` on Linux (`$XDG_DATA_HOME/horus/recordings` if set; `~/Library/Application Support/horus/recordings` on macOS, `%LOCALAPPDATA%\horus\data\recordings` on Windows) — in binary format (`.horus` files). A project-local `.horus/recordings/` directory in the current working directory is also searched. Use `horus record list` to see sessions without knowing the path. Each node gets its own recording file:

```
~/.local/share/horus/recordings/my_session/
...
fn test_replay_crash_scenario() {
    ...
    // `add_replay` opens the path verbatim — `~` is NOT expanded, so pass a real path.
    let recording = PathBuf::from(std::env::var("HOME").unwrap())
        .join(".local/share/horus/recordings/crash/motor_node@abc123.horus");
    ...
    scheduler.add(DiagnosticNode::new().unwrap()).order(2).done();
...
# Show file sizes and tick counts
horus record list --long

# Recordings are unlimited in size by default — delete old ones to free space
horus record delete <session>
horus record clean --older-than 7
```

Note: a recording file larger than 100 MB is rejected when it is *loaded* for
replay (a sanity check against corrupted files), but nothing caps a recording
...
**Cause:** the publisher's tick has not run yet
...
- **[Core Concepts](/concepts/core-concepts-nodes)** - Understand node lifecycle
- **[Monitor](/development/monitor)** - Debug with visual monitoring
- **[CLI Reference](/development/cli-reference#horus-test-run-tests)** - Full `horus test` command reference
- **[Deterministic Execution](/advanced/deterministic-execution)** - Deterministic scheduling for reproducible tests
