...
impl Node for TemperatureSensor {
    ...
    fn tick(&mut self) {
        ...
        // Publish temperature
        self.temp_pub.send(self.reading).ok();
    }
...
Test nodes without real Topic connections.

### Example: Mock Topic for Testing
...
impl Node for TemperatureProcessor {
    ...
    fn tick(&mut self) {
        if let Some(celsius) = self.input_sub.recv() {
            ...
            self.output_pub.send(fahrenheit).ok();
        }
    }
}
...
```rust
// Use real Topics in tests (they're lightweight)
#[test]
...
impl Node for GeneratorNode {
    ...
    fn tick(&mut self) {
        self.counter += 1;
        self.output_pub.send(self.counter).ok();
    }
}
