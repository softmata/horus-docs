...
# The install directory is not one horus monitor searches — add it to PATH
export PATH="$HOME/.config/horus/bin:$PATH"
```

`horus monitor` is only a shim: it locates the `horus-monitor` binary (first `~/.horus/bin/horus-monitor`, then `PATH`) and forwards every argument to it. A global plugin install reaches neither on its own — it symlinks the binary to `~/.config/horus/bin/horus-monitor` (`$XDG_CONFIG_HOME/horus/bin`, or `~/Library/Application Support/horus/bin` on macOS), and nothing puts that directory on `PATH`. Without the export above, a successful install is still followed by `Note: The monitor plugin is not installed.` and a non-zero exit — no server and no browser.
