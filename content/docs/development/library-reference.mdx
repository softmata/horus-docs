...
- **HFrame** - Coordinate frame transformations for robotics
...
**Import:**
```rust
use horus::prelude::*; // Provides {CmdVel, LaserScan, ImuNode, ...}
```
...
**Size:** 16 bytes
**Latency (round-trip):** ~85ns (Topic SPSC) / ~500ns (Topic MPMC)
...
**Size:** 1.5 KB
**Latency (round-trip):** ~900ns (Topic SPSC) / ~2.2µs (Topic MPMC)

**Usage:**
```rust
if let Some(scan) = lidar_topic.recv() {
    for (i, range) in scan.ranges.iter().enumerate() {
        if *range < 0.5 {  // Obstacle within 0.5m
            hlog!(warn, "Obstacle detected!");
        }
    }
}
```
...
**Size:** 304 bytes
**Latency (round-trip):** ~400ns (Topic SPSC) / ~940ns (Topic MPMC)
...
**Usage:**
```rust
if let Some(input) = keyboard_topic.recv() {
...
The library includes HFrame for robotics coordinate frame management:

```rust
use horus::prelude::*;

// Create transforms between frames
let robot_to_sensor = HFrame::from_xyz_rpy(0.1, 0.0, 0.2, 0.0, 0.0, 0.0);

// Transform points between frames
let point_in_robot = robot_to_sensor.transform_point(&point_in_sensor);
```
...
See [Hardware Discovery](/development/hardware-discovery) for hardware configuration details.
