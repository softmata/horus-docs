...
<Callout type="warning" title="Dashes do not survive, and `HORUS_DRIVER_NAME` is not set">
...
Separately: [Environment Variables](/development/environment-variables) lists
...
| Python, via `horus.drivers.load()` | Instantiates a class registered with `horus.drivers.register_driver` | **No subprocess.** The Python path calls `load_config_entries`, which parses entries into `(name, use_name, params, simulated)` with no `exec:` dispatch; an `exec:` entry comes back as a bare `NodeParams` object |
...
Simulation reaches both paths. `load_config_entries` applies the same predicate
`load_from` does — `sim` (or its alias `simulated`) on the entry, `HORUS_SIM_MODE`
on the process, and `HORUS_SIM_TARGETS` as the filter — and reports the verdict as
the fourth element of each tuple. Where Rust substitutes a `SimStubNode`, Python
returns a `SimStub`: it never calls your driver class, so nothing opens a port or
enables a motor, and it still carries the entry's params as `stub.params`.
