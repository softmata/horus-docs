...
"No node types are registered. Call `register_driver!()` first, or depend on a
crate that registers its driver types." (`drivers/mod.rs:286`) — the same macro
this page uses, so there is nothing to translate.
