...
**4. For messages you cannot afford to lose, use `send_blocking()`:**
```rust
...
A `#[repr(C)] #[derive(Clone, Copy)]` POD message bypasses serialization entirely and needs no serde derives. If you keep a serde message with a large fixed array, note that serde only derives for arrays up to length 32 — longer ones need `serde_arrays` (add `serde_arrays = "0.2"` to your dependencies):
```rust
