...
HORUS ships two utility scripts, `install.sh` and `uninstall.sh`. This page covers updating, recovering from a broken installation, and debugging HORUS applications at runtime.
...
rm -rf ~/.cache/horus    # installer's XDG source cache (horus@<version>)
...
#   - /usr/local/horus
#
# It also falls back to the installer's source cache:
#   - ~/.cache/horus/horus@<version>
#   - ~/.horus/cache/horus@<version>
...
2. **Type Mismatch** (not silent — it fails loudly)
   - Opening the same topic name with two different message types is rejected
     at construction, so you get an error rather than zero messages:
     ```
     Type mismatch on topic 'data': existing type 'f32', attempted 'f64'.
     ```
   ...
   **Fix**: Ensure both use same type:
   ...
   // Subscriber (WRONG TYPE) — Topic::new returns Err, recv() is never reached
   let sub_topic: Topic<f64> = Topic::new("data")?;  // f64 != f32

   // CORRECT:
   let sub_topic: Topic<f32> = Topic::new("data")?;  // Same type
   ```
   If you see that error, either use the same type on both sides or pick
...
**Cause**: `send()` is lossy. The usual cause is a full ring buffer — the subscriber isn't draining it fast enough. When the ring is full, `send()` retries briefly (one immediate retry, then 64 spins and 4 yields) and then drops the message, incrementing the topic's `send_failures` counter.
...
**1. Make Sure the Subscriber Keeps Up:**
- Check the topic's send failure count in the monitor.
- Make sure the subscribing node ticks often enough to drain the ring buffer.
...
```rust
// BAD: Unbounded size — the payload grows with the data
...
// GOOD: Fixed size
// serde only derives for arrays up to length 32, so longer arrays
// need `serde_arrays` (add `serde_arrays = "0.2"` to your dependencies).
#[derive(Clone, Serialize, Deserialize)]
pub struct LargeMessage {
...
Alternatively, make the struct a `#[repr(C)] #[derive(Clone, Copy)]` POD type — POD messages bypass serialization entirely and need no serde derives.

**4. Use Monitor to Check:**
```bash
