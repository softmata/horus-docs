...
thread 'main' panicked at 'Topic::new() failed'
```
...
// Bad (will fail):
let topic = Topic::new("sensor data")?;  // Space
let topic = Topic::new("temp@sensor")?;  // Special char
```
...
fn tick(&mut self) {
    let data = 42.0;
    match self.pub_topic.send(data, ) {
        Ok(()) => {
        hlog!(info, "Published successfully");
        }
        Err(_) => {
        hlog!(error, "Publish failed!");
        }
    }
}
```
...
self.pub_topic.send(Arc::clone(&data)).ok();
```
