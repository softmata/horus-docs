...
**Symptom:**
```
Error: Failed to create shared memory region for topic "sensor_data"
```

Or, if your code uses `.unwrap()`:
```
...
let topic = Topic::new("camera/front/raw")?;  // Slashes create subdirectories
```
...
```rust
// Publisher
let pub_topic = Topic::new("temperature")?;  // Note: "temperature"

// Subscriber (wrong - typo!)
let sub_topic = Topic::new("temperatur")?;   // Missing 'e'

// Fixed:
let sub_topic = Topic::new("temperature")?;  // Now matches!
```
...
// Publisher (order 0 - runs first)
scheduler.add(publisher).order(0).done();

// Subscriber (order 1 - runs after)
scheduler.add(subscriber).order(1).done();
```
...
**Symptom:**
Publisher sends messages but subscriber never receives them, and no error is reported.

**Cause:**
- For serialized (non-POD) messages: the serialized data exceeds the slot size
- Ring buffer is full and all retry attempts failed
- `send()` is lossy — it retries briefly then drops the message silently
...
```rust
// Large fixed-size arrays increase shared memory usage
struct SensorData {
    readings: [f32; 100],  // 400 bytes — reasonable
}
```
