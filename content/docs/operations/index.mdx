...
<LanguageTabs>
  ...
  <LangTab language="Python">
    ...
    sched = horus.Scheduler(tick_rate=100, blackbox_mb=64)
    ```
  </LangTab>
</LanguageTabs>
...
# Export for analysis
horus record export session1 --format json
```
...
<LanguageTabs>
  <LangTab language="Rust">
    ...
    6. **Monitor** — `horus monitor` from your development machine
  </LangTab>
  <LangTab language="Python">
    1. **Configure scheduler** — `horus.Scheduler(tick_rate=100, watchdog_ms=500)`
    2. **Run tests** — `horus test`
    3. **Check health** — `horus doctor`
    4. **Set up venv on robot** — `python3 -m venv ~/horus_venv && pip install horus-robotics`
    5. **Deploy** — `horus deploy <target> --run`
    6. **Monitor** — `horus monitor` from your development machine
  </LangTab>
</LanguageTabs>
