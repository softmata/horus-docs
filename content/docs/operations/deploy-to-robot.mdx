...
<LanguageTabs>
  ...
  <LangTab language="Python">
    For Python projects, the build step is skipped (no compilation needed). The source files are synced directly:
    ...
    Step 1: Skipping build (Python project)
    ...
    The robot needs Python 3 and the HORUS Python package installed:
    ...
    For reproducible Python deploys, use a virtual environment on the robot:

    ```bash
    # On the robot (one-time setup)
    python3 -m venv ~/horus_venv
    source ~/horus_venv/bin/activate
    pip install horus-robotics
    ```
  </LangTab>
</LanguageTabs>
...
<LanguageTabs>
  ...
  <LangTab language="Python">
    ...
    User=pi
    WorkingDirectory=/home/pi/horus_deploy
    Environment=PATH=/home/pi/horus_venv/bin:/usr/bin
    ExecStart=/home/pi/horus_venv/bin/python3 src/main.py
    Restart=on-failure
    RestartSec=5
    ...
    Note the `Environment=PATH` line points to your virtual environment so the correct Python and packages are used.
  </LangTab>
</LanguageTabs>
...
For containerized robots or fleet management, deploy as a Docker image:

### Multi-Stage Dockerfile (Rust)

```dockerfile
# Stage 1: Build on x86_64 (cross-compile for ARM if needed)
...
# Cross-compile for ARM64 (adjust for your robot's arch)
...
COPY --from=builder /app/target/aarch64-unknown-linux-gnu/release/my_robot /usr/local/bin/
COPY horus.toml /app/
...
| `--device /dev/gpiochip0` | GPIO access |
| `--cap-add SYS_NICE` | RT scheduling (SCHED_FIFO) |
| `--privileged` | Full hardware access (use only if needed) |
| `--ipc=host` | Share host IPC namespace (for cross-container SHM topics) |
| `-v /dev/shm:/dev/shm` | Share host shared memory (alternative to `--ipc=host`) |

### Docker Compose (Multi-Node Robot)
...
services:
  sensor:
    ...
    cap_add:
      - SYS_NICE
    ipc: host  # Share SHM for zero-copy topics
...
> **SHM sharing**: For nodes in separate containers to communicate via zero-copy shared memory, use `ipc: host` or mount `/dev/shm`. Without this, topics fall back to network transport.

<Callout type="warning">
**Run the containers as the same UID.** HORUS refuses to use a shared-memory
directory owned by a different user — a foreign-owned directory on the IPC path
is how one account gets another to read and write its topics. Containers
sharing `/dev/shm` therefore need a matching `user:` (or all need to be root),
or the second one to start will fail rather than silently attach to a directory
it does not own.
</Callout>
