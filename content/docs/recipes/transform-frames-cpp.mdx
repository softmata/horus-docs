...
The C++ `TransformFrame` is a per-process TF tree — use `register_frame`,
`update`, and `lookup` as shown above. (A topic-based transform message for
cross-process sharing is not part of the current C++ API.)
