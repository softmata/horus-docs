...
# The Rust example defines messages with the `message!` macro, which derives
# serde — required by typed `Topic<T>`. Not needed for the Python/C++ variants.
[dependencies]
...
// simplified
    ...
    // Pose data to log — message! gives repr(C), Copy, Default, serde, LogSummary
    message! {
        #[fixed]
        FusedPose {
            x: f32,
    ...
    impl Node for LoggerNode {
        ...
        fn init(&mut self) -> Result<()> {
            ...
            // `?` auto-converts std::io::Error into horus Error (Error: From<io::Error>).
