...
flowchart LR
    subgraph HORUS["horus process — one Scheduler"]
        direction TB
        C["your control nodes<br/>MotorCtrl 1kHz, Planner 10Hz, EStop 100Hz"]
        T["horus topics<br/>cmd_vel, imu.raw"]
        B["BridgeOut<br/>.order(90), 50Hz"]
        C --> T --> B
    end

    subgraph ROSSIDE["ROS2-side process — horus Python + rclpy"]
        I["BridgeIn<br/>50Hz"]
    end

    subgraph ROS2["ROS2 graph"]
        R["RViz, Nav2, ..."]
    end

    B -->|"shared memory<br/>bridge.out (BridgePacket)"| I
    I -->|"DDS<br/>/cmd_vel (Twist)"| R
`}
  caption="BridgeOut runs in the same scheduler as your control nodes and publishes one packet topic; a second process reads it and republishes over DDS"
/>
...
<Callout type="warning" title="A custom Rust struct does not cross into Python by itself">
The Rust side above publishes `Topic<BridgePacket>`, a struct defined in that file.
A Python subscriber given a bare topic name resolves the type by looking the ring's
recorded type name up in a table of built-in messages (`_detect_topic_type`), and
`BridgePacket` is not in it — so Python falls back to decoding the slot as MessagePack
while the Rust side wrote packed struct bytes.

Two ways to make the sketch below real: put a built-in message on the bridge topic
(`CmdVel`, `Twist`, `Imu` and the rest of the standard set all resolve), or generate a
Python counterpart for your struct with [`horus.msggen`](/python/api/custom-messages).

Either way an untyped topic hands you a **dict**, not an object — `packet["linear"]`,
not `packet.linear`.
</Callout>
