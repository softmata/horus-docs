...
The same drive node in Python, using `horus.Node` with the callback API:
...
- Python uses `node.recv("topic")` and `node.send("topic", data)` instead of typed `Topic<T>` fields
- No `#[repr(C)]` — Python topics use `GenericMessage` serialization (slower but flexible)
- `node.state` is a dict you can use for persistent state across ticks
- `run()` is the one-liner entry point — no manual `Scheduler` construction needed
