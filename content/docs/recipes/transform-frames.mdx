...
// simplified
  ...
  <LangTab language="Python">
    ```python
    import horus
    from horus import Node, TransformFrame, run
    import math
    ...
    def publisher_init(node):
        """Register the frame hierarchy."""
        tf.add_frame("world")
        tf.add_frame("base_link", parent="world")
        tf.add_frame("camera_link", parent="base_link",
                      static_transform={"xyz": [0.1, 0.0, 0.3], "rpy": [0.0, 0.26, 0.0]})
        tf.add_frame("lidar_link", parent="base_link",
                      static_transform={"xyz": [0.0, 0.0, 0.4]})
        tf.add_frame("arm_link", parent="base_link")
        tf.add_frame("end_effector", parent="arm_link")
        tf.print_tree()
    ...
    def publisher_tick(node):
        ...
        tf.update_transform("base_link", xyz=[base_x, base_y, 0.0], yaw=base_yaw)

        # Update arm_link: joint sweeps back and forth
        arm_angle = 0.8 * math.sin(t * 0.5)
        tf.update_transform("arm_link", xyz=[0.15, 0.0, 0.2], yaw=arm_angle)

        # Update end_effector: extends from arm tip
        tf.update_transform("end_effector", xyz=[0.3, 0.0, 0.0])
    ...
    def user_tick(node):
        ...
        if s["tick_count"] % 100 == 0:
            ee = tf.lookup("end_effector", "world")
            if ee is not None:
