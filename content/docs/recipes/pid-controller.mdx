...
The same PID controller in Python, using `horus.Node`:
...
def pid_tick(node):
    ...
    # Read inputs
    if node.has_msg("pid.setpoint"):
        msg = node.recv("pid.setpoint")
        s["target"] = msg["target"]

    if node.has_msg("pid.measurement"):
        msg = node.recv("pid.measurement")
        s["measured"] = msg["value"]
