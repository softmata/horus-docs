...
See [Basic Examples — Camera Image Pipeline](/rust/examples/basic-examples#section-6-camera-image-pipeline) for complete send/recv examples.
...
### Publisher keep-alive

`send_handle()` — and the `Image`, `PointCloud` and `DepthImage` sends — takes an extra pool reference and holds it for as long as the descriptor can still be reached in the ring: one reference per ring slot, released when a later publish pushes it out of that queue or the `Topic` drops. That is what makes an unread frame safe to read. Without it, publishing the next frame would free the backing of a frame still sitting in the ring, and the subscriber's `recv_handle()` would return `None` while `pending_count()` was non-zero — with nothing counting the loss, since the ring itself dropped nothing.

The cost is that a publisher pins up to `ring_capacity` pool slots on top of any handles your code holds: 32 for a default `Topic<Tensor>`, where the capacity comes from a 4 KB page divided by the 168-byte descriptor, rounded up to a power of two.
...
let dtype = tensor.dtype();         // TensorDtype (accessor — the field is private)
...
7. **Watch both pool limits**: `stats()` reports `allocated_slots` and `used_bytes` — which one a leaking pipeline exhausts first depends on tensor size. With the 1GB/1024-slot default, leaked 1080p RGB frames run the data region out after ~170 allocations (173 slots used); small tensors hit `max_slots` first. Size for the publisher's keep-alive slots too: each sending `Topic` pins up to `ring_capacity` more (32 by default), so a pool has to cover the handles you hold plus that per-publisher reserve
