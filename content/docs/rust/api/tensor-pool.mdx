...
```text
Allocation failed: device mismatch: requested cuda:0 but pool backend
targets cpu (backend: mmap). Use a pool created with the matching backend,
or pass Device::cpu() to alloc().
```
...
| Operation | Latency |
|-----------|---------|
| Slot alloc + release, 64 B | ~1.3 µs |
...
7. **Watch `allocated_slots`**: `max_slots`, not `pool_size`, is usually the limit a leaking pipeline hits first
