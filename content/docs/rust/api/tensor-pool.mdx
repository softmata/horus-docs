...
// Send via topic — zero-copy, only 168-byte descriptor travels
...
# Use numpy arrays directly with ML models
import numpy as np
from horus.ai import Model

arr = np.array([1.0, 2.0, 3.0], dtype=np.float32)
mat = np.array([[1, 2], [3, 4]], dtype=np.float32)

# Load and run model inference (returns np.ndarray)
model = Model.load("model.onnx", device="cuda:0")
