...
`HorusTensor` is a 232-byte `Pod` descriptor that acts as a lightweight handle to tensor data in shared memory. Only the descriptor travels through the ring buffer — the actual data stays in-place.
...
let size = tensor.size();           // Total size in bytes
