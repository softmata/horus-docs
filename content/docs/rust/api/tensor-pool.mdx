...
HORUS includes CUDA availability detection via `cuda_available()` and `cuda_device_count()`. Tensors can target GPU devices using `Device::cuda(N)`:
...
if cuda_available() {
    println!("CUDA devices: {}", cuda_device_count());
}

// Create a tensor descriptor targeting GPU
let mut tensor = Tensor::default();
tensor.set_device(Device::cuda(0));
```

GPU tensor pool management is exposed to Python via `horus.cuda_is_available()` and the `horus.ai` module. See the Python API docs for details.
