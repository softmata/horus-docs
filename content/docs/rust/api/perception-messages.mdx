...
**PointField** uses `TensorDtype` (available via prelude) for its `datatype` field. See [TensorDtype values](/rust/api/ml-messages#tensordata) for the full list.
...
let mut depth_u16 = DepthImage::new(480, 640, TensorDtype::U16)?;
