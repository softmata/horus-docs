...
- **Pod (zero-copy) types**: `PointField`, `PlaneDetection`, `PlaneArray`, `PointXYZ`, `Landmark`, `TrackedObject`, `SegmentationMask` — fixed-size, suitable for shared memory transport with zero serialization overhead

Every direct message type here also derives `Serialize`/`Deserialize`, because the topic
layer requires those bounds. That does not put them on a serialization path: HORUS picks
the POD transport from the type's layout (no `Drop`, size > 1), so a type deriving serde
still travels as a raw byte copy.
...
**PointField** uses `TensorDtype` (available via prelude) for its `datatype` field. See [TensorDtype](/rust/api/tensor-messages#tensordtype) for the full list.

## Pod Point Types (Zero-Copy)

Fixed-size element types used inside `PointCloud` buffers for zero-copy shared memory
transport. There is no separate POD topic type — IPC always goes through `Topic<T>`,
and POD-ness is a property of the message type itself.

The extension methods shown below (`distance`, `distance_to`, `from_xyz`, `rgb_packed`,
`xyz`) come from the `PointXYZExt` / `PointXYZRGBExt` / `PointXYZIExt` traits, which are
not in the prelude — import the trait you need explicitly, as each example shows.

### PointXYZ

Basic 3D point (12 bytes, `#[repr(C)]`).

```rust
use horus::prelude::*;
use horus::types::point::PointXYZExt;
...
```rust
use horus::prelude::*;
use horus::types::point::PointXYZRGBExt;
...
```rust
use horus::prelude::*;
use horus::types::point::PointXYZIExt;
...
## Transmitting Point Clouds

There is no separate point-cloud header type. Framing metadata (frame id, timestamp,
point count, fields per point) lives on [`PointCloud`](#pointcloud) itself, which is what
you send over a topic. Build one from coordinate arrays with the `from_xyz()` /
`from_xyzi()` / `from_xyzrgb()` constructors:
...
let mut cloud = PointCloud::from_xyz(&points)?;
cloud.set_frame_id("lidar_front").set_timestamp_ns(1234567890);

println!("Points: {}, bytes: {}", cloud.point_count(), cloud.data().len());

// LiDAR returns: [x, y, z, intensity]
let returns = vec![[1.0f32, 2.0, 3.0, 0.8]];
let intensity_cloud = PointCloud::from_xyzi(&returns)?;

// Colored points: [x, y, z, r, g, b]
let colored = vec![[1.0f32, 2.0, 3.0, 255.0, 0.0, 0.0]];
let rgb_cloud = PointCloud::from_xyzrgb(&colored)?;
```

| Constructor | Fields per point | Element layout |
|-------------|------------------|----------------|
| `PointCloud::from_xyz(&[[f32; 3]])` | 3 | `[x, y, z]` |
| `PointCloud::from_xyzi(&[[f32; 4]])` | 4 | `[x, y, z, intensity]` |
| `PointCloud::from_xyzrgb(&[[f32; 6]])` | 6 | `[x, y, z, r, g, b]` |
...
// Create F32 depth image (meters), 640x480
...
// Get statistics (min, max, mean in meters); None if no valid pixels
...
DepthImage is an RAII type — fields are private, accessed through methods. Mutation methods return `&mut Self` (or `Result<&mut Self>`, for `set_depth`) for chaining.
...
| `set_depth(x, y, value)` | `Result<&mut Self>` | Set depth in meters at pixel; errors for U16 images when the value is outside `u16` millimeter range (> 65.535m at default scale) |
| `get_depth_u16(x, y)` | `Option<u16>` | Get raw U16 value (millimeter format only) |
| `depth_statistics()` | `Option<(f32, f32, f32)>` | Get (min, max, mean) of valid depths in meters; `None` if no valid pixels |
...
Multi-object tracking result with a constant-velocity prediction. Pod type (96 bytes).
...
| `predicted_bbox` | `BoundingBox2D` | Predicted next position — `bbox + velocity`, refreshed only by `mark_missed()`. No filter is applied |
