...
|--------|---------|-------------|
| `new(num_points, fields_per_point, dtype)` | `HorusResult<PointCloud>` | Create point cloud (allocates from global pool) |
...
let (min, max, mean) = depth.depth_statistics();
...
| Method | Returns | Description |
|--------|---------|-------------|
| `new(height, width, dtype)` | `HorusResult<DepthImage>` | Create depth image (F32 or U16) |
...
| `depth_statistics()` | `(f32, f32, f32)` | Get (min, max, mean) of valid depths in meters |
