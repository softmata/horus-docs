...
2D velocity command for differential-drive robots. Use `Twist` for full 6-DOF motion;
`From` converts both ways, keeping only `linear[0]` and `angular[2]`.

```rust
pub struct CmdVel {
    pub timestamp_ns: u64,
    pub linear: f32,   // m/s forward
    pub angular: f32,  // rad/s yaw
}
```
...
| Perception | 3/5 | `PointCloud` (pool-backed RAII), `DepthImage` (pool-backed RAII) |
