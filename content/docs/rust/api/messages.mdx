...
Note: For coordinate frame management (lookups, chains, interpolation), see [HFrame](/concepts/hframe) which uses its own `Transform` type.
...
Most HORUS message types are **POD** (Plain Old Data) — they use `#[repr(C)]` layout and implement the `PodMessage` trait, enabling zero-copy shared memory transport at ~50ns latency via the `PodShm` backend.
