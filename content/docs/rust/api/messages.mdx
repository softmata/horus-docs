...
| Vision | 0/5 | `Image` (pool-backed RAII), `CompressedImage` (Vec), `CameraInfo`, `RegionOfInterest`, `StereoInfo` (repr(Rust), no `PodMessage`) |
| Perception | 1/4 | `PointCloud` (pool-backed RAII), `DepthImage` (pool-backed RAII), `PlaneDetection` (repr(Rust)) |
...
| `GenericMessage` | ~4.3 KB (4364 bytes) | No — MessagePack over the serde backend |
...
| [ML Messages](/rust/api/ml-messages) | TensorData, Predictions, Classification, LLM integration |
