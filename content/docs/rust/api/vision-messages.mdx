...
```rust
// simplified
use horus_robotics::messages::vision::StereoInfo;
use horus::prelude::*;
...
> **Note:** `StereoInfo` is not included in the convenience re-exports. Import it directly from `horus_robotics::messages::vision::StereoInfo`.
...
2D bounding box for object detection. Fixed-size, suitable for zero-copy shared memory transport.
...
2D object detection result. Fixed-size (72 bytes), suitable for zero-copy shared memory transport.
...
3D object detection from point clouds or depth-aware models. Fixed-size (104 bytes) with velocity tracking.
...
**BoundingBox3D methods:**

| Method | Returns | Description |
|--------|---------|-------------|
| `new(cx, cy, cz, length, width, height, yaw)` | `BoundingBox3D` | Create with yaw only |
| `with_rotation(center, dimensions, rotation)` | `BoundingBox3D` | Create with full roll/pitch/yaw |
| `volume()` | `f32` | Box volume in m³ |
