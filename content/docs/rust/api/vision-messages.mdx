...
if image.is_valid() {
    ...
    println!("Expected size: {} bytes", image.expected_size());
}
...
| `step` | `u32` | Bytes per row (may include padding) |
...
| Method | Returns | Description |
|--------|---------|-------------|
| `new(width, height, encoding, data)` | `Image` | Create image (auto-sets step and timestamp) |
| `with_frame_id(id)` | `Image` | Set frame ID from string (builder) |
| `is_valid()` | `bool` | Validate dimensions and data consistency |
| `expected_size()` | `usize` | Expected data size in bytes (`step * height`) |
| `get_pixel(x, y)` | `Option<&[u8]>` | Get pixel bytes at coordinates |
| `roi(x, y, width, height)` | `Option<Image>` | Extract a cropped region |
...
| `get_class_name()` | `&str` | Get class name as string |
