...
For ML inference pipelines, use the zero-copy [Tensor](/rust/api/tensor) and [TensorPool](/rust/api/tensor-pool) APIs to transport model inputs and outputs between nodes. For structured inference results, use the detection types:
...
Segmentation mask header (64 bytes). The actual pixel data follows the header in shared memory -- each pixel is a `u8` class/instance ID.
