...
`TensorHandle` is not in the prelude — import it from `horus::memory`. See [TensorPool API](/rust/api/tensor-pool) for allocation and device placement, and [Supported Data Types](/rust/api/tensor-pool#supported-data-types) for the full `TensorDtype` table.
