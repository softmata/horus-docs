...
```rust
use horus_ai::MlDetection;

let detection = MlDetection {
...
```rust
use horus_ai::{MlDetectionArray, MlDetection};
...
| Field | Type | Description |
|-------|------|-------------|
| `detections` | `Vec<MlDetection>` | List of detections |
...
```rust
use horus_ai::PoseKeypoint;

let nose = PoseKeypoint {
    x: 320.0,
    y: 100.0,
...
```rust
use horus_ai::{Pose, PoseKeypoint};

let pose = Pose {
    keypoints: vec![
        PoseKeypoint { x: 320.0, y: 100.0, z: None, confidence: 0.98, name: "nose".into() },
        PoseKeypoint { x: 300.0, y: 200.0, z: None, confidence: 0.95, name: "left_shoulder".into() },
        PoseKeypoint { x: 340.0, y: 200.0, z: None, confidence: 0.96, name: "right_shoulder".into() },
...
| Field | Type | Description |
|-------|------|-------------|
| `keypoints` | `Vec<PoseKeypoint>` | Detected keypoints (17-33 typically) |
...
```rust
use horus_ai::{ModelInfo, ModelFormat};
...
```rust
use horus_ai::{LLMRequest, ChatMessage};
...
```rust
use horus_ai::{TrajectoryPoint, Tensor, DataType};
...
```rust
use horus_ai::{DeploymentConfig, ModelFormat};
...
```rust
use horus::prelude::*;
use horus_ai::{MlDetection, MlDetectionArray, InferenceMetrics};

struct ObjectDetectionNode {
    image_sub: Topic<Image>,
    detection_pub: Topic<MlDetectionArray>,
...
impl Node for ObjectDetectionNode {
    ...
    fn tick(&mut self) {
        if let Some(image) = self.image_sub.recv() {
            ...
            // Publish detections
            let detection_msg = MlDetectionArray {
                detections,
                image_width: image.width,
                image_height: image.height,
                timestamp_ns: image.timestamp,
            };
            self.detection_pub.send(detection_msg);
...
impl ObjectDetectionNode {
    fn run_inference(&self, _image: &Image) -> Vec<MlDetection> {
