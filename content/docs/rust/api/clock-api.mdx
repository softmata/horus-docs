...
- Time advances by exactly one tick period — `1/tick_rate` — **per tick**, however many
  dependency-graph steps that tick decomposes into. Adding a stage to a pipeline does not
  change the rate of the virtual clock
