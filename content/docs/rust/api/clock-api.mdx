...
When using `Scheduler::replay_from(path)` or `.add_replay(path, priority)` — both take a `PathBuf` and return a `HorusResult`:

- `Scheduler::replay_from` steps time through an evenly-spaced tick clock derived from the `.horus` recording's average tick period (`started_at..ended_at` / `total_ticks`)
...
scheduler.add_replay(PathBuf::from("sensor_recording.horus"), 0)
    .expect("failed to load recording");              // replay sensor data
