...
// Ultra-fast ~50ns transfer using PodTopic
let twist_topic: PodTopic<Twist> = PodTopic::producer("velocity_cmd")?;
let pose_topic: PodTopic<Pose2D> = PodTopic::consumer("robot_pose")?;

// Send without serialization overhead
twist_topic.send(Twist::new_2d(0.5, 0.1)).ok();
...
impl Node for DifferentialDriveController {
    ...
    fn tick(&mut self) {
        ...
        let pose = match self.pose_sub.try_recv() {
            Some(p) => p,
            None => return,
        };

        let goal = match self.goal_sub.try_recv() {
            Some(g) => g,
            None => return,
        };
