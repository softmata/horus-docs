...
Full 3D pose: a position and an orientation quaternion.
...
| Field | Type | Description |
|-------|------|-------------|
| `position` | `Point3` | Position in metres |
| `orientation` | `Quaternion` | Orientation |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |

## PoseStamped

A `Pose3D` tagged with the frame it is expressed in.

```rust
use horus::prelude::*;

let stamped = PoseStamped::new(pose);
```
...
| Field | Type | Description |
|-------|------|-------------|
| `pose` | `Pose3D` | The pose |
| `frame_id` | `[u8; 32]` | Reference frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |

## PoseWithCovariance

A pose plus its 6x6 covariance — the shape a localisation estimate takes.

```rust
use horus::prelude::*;

let estimate = PoseWithCovariance::new(pose);
let sigma = estimate.position_variance();     // [x, y, z]
let ang = estimate.orientation_variance();    // [roll, pitch, yaw]
```
...
|-------|------|-------------|
| `pose` | `Pose3D` | The pose |
| `covariance` | `[f64; 36]` | Row-major 6x6, ordered x, y, z, roll, pitch, yaw |
| `frame_id` | `[u8; 32]` | Reference frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
| Method | Description |
|--------|-------------|
| `new(pose)` | Zero covariance |
| `position_variance()` | The x, y, z diagonal |
| `orientation_variance()` | The roll, pitch, yaw diagonal |

## TwistWithCovariance

A velocity with its 6x6 covariance.

```rust
use horus::prelude::*;

let vel = TwistWithCovariance::new(twist);
let lin = vel.linear_variance();
let ang = vel.angular_variance();
```
...
| Method | Description |
|--------|-------------|
| `new(twist)` | Zero covariance |
| `linear_variance()` / `angular_variance()` | The two diagonals |
...
| Field | Type | Description |
|-------|------|-------------|
| `linear` | `[f64; 3]` | Linear acceleration, m/s^2 |
| `angular` | `[f64; 3]` | Angular acceleration, rad/s^2 |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |

## AccelStamped

An `Accel` tagged with its frame.

**Fields:**

| Field | Type | Description |
|-------|------|-------------|
| `accel` | `Accel` | The acceleration |
| `frame_id` | `[u8; 32]` | Reference frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
- [PodMessage](/concepts/message-types#pod-custom-message-zero-copy) - Zero-copy transfer
