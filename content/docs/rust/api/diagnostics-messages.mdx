...
```rust
// simplified
use horus_types::diagnostics::NodeStateMsg;
...
```rust
// simplified
use horus::prelude::*; // Provides HealthStatus (from horus_core::core, re-exported via prelude)
...
use horus_types::diagnostics::NodeStateMsg; // POD version (distinct from core NodeState)
...
let status = DiagnosticStatus::new(StatusLevel::Warn, 0, "Battery low: 15%");
```
