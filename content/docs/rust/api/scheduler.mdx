...
**Parameters:**
- `policy: Miss` — `Miss::Warn`, `Miss::Skip`, `Miss::SafeMode`, or `Miss::Stop`
...
> **Advanced:** Most users should use the per-node `.watchdog()` builder method instead. `add_critical_node` is for post-registration configuration.

#### `apply_config(config) -> &mut Self`

> **Advanced:** Apply a `SchedulerConfig` struct to the scheduler. Most users should use the builder methods (`.watchdog()`, `.blackbox()`, `.tick_rate()`) instead.
