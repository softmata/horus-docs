...
**Errors:** `ValidationError::Other` if value exceeds U16 range for millimeter images.
...
| `TensorDtype::U16` | millimeters | `false` | `1.0` | RealSense, Kinect, stereo |
...
depth = horus.DepthImage.from_numpy(t.numpy())
...
# Convert to numpy, then bridge into torch/jax
arr = depth.to_numpy()
t = torch.from_numpy(depth.to_numpy())
j = jnp.asarray(depth.to_numpy())
