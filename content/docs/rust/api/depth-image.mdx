...
Pool-backed RAII depth image with zero-copy shared memory transport. DepthImage allocates from a global tensor pool and supports two depth formats: F32 (meters) and U16 (millimeters). All depth access methods work in meters regardless of the underlying format -- U16 values are automatically converted.
...
let mut depth_mm = DepthImage::new(640, 480, TensorDtype::U16)?;
```
...
let depth_u16 = DepthImage::new(640, 480, TensorDtype::U16)?;
...
impl Node for RgbdNode {
    ...
    fn tick(&mut self) {
        ...
        if let Ok(mut cloud) = PointCloud::new(w * h, 3, TensorDtype::F32) {
...
|--------|---------|-------------|
| `new(width, height, dtype)` | `HorusResult<Self>` | Create depth image (F32 or U16) from global pool |
| `get_depth(x, y)` | `Option<f32>` | Depth in meters at pixel (auto-converts U16) |
| `set_depth(x, y, value)` | `HorusResult<&mut Self>` | Set depth in meters at pixel |
...
- [TensorPool API](/rust/api/tensor-pool) -- Underlying pool management
