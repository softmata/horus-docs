...
Positions, velocities and efforts for up to 16 named joints, in one fixed-size message.

```rust
use horus::prelude::*;

let mut state = JointState::new();
state.add_joint("shoulder", 0.5, 0.1, 2.4)?;
state.add_joint("elbow", -1.2, 0.0, 1.1)?;

// Look a joint up by name
if let Some(pos) = state.position("elbow") {
    println!("elbow at {:.2} rad", pos);
}
```
...
| Field | Type | Description |
|-------|------|-------------|
| `names` | `[[u8; 32]; 16]` | Joint names, fixed 32 bytes each |
| `joint_count` | `u8` | How many of the 16 slots are in use |
| `positions` | `[f64; 16]` | Joint positions (radians or metres) |
| `velocities` | `[f64; 16]` | Joint velocities |
| `efforts` | `[f64; 16]` | Joint efforts (Nm or N) |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
| Method | Description |
|--------|-------------|
| `new()` | Empty state with no joints |
| `add_joint(name, position, velocity, effort)` | Append a joint; `Err` once 16 are used |
| `joint_name(index)` | Name at an index, or `None` |
| `position(name)` / `velocity(name)` / `effort(name)` | Look up by name, or `None` |

## MagneticField

Three-axis magnetometer reading with its covariance.

```rust
use horus::prelude::*;

let mag = MagneticField::new([21.5e-6, -3.2e-6, 43.1e-6]);  // tesla
```
...
| Field | Type | Description |
|-------|------|-------------|
| `magnetic_field` | `[f64; 3]` | Field vector in tesla (x, y, z) |
| `magnetic_field_covariance` | `[f64; 9]` | Row-major 3x3 covariance |
| `frame_id` | `[u8; 32]` | Sensor frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |

## Temperature

A single temperature reading with variance.

```rust
use horus::prelude::*;

let temp = Temperature::new(21.5);  // degrees Celsius
```
...
| Field | Type | Description |
|-------|------|-------------|
| `temperature` | `f64` | Temperature in degrees Celsius |
| `variance` | `f64` | Variance; `0.0` when unknown |
| `frame_id` | `[u8; 32]` | Sensor frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |

## FluidPressure

Absolute pressure, for barometers and depth sensors.

```rust
use horus::prelude::*;

let baro = FluidPressure::new(101_325.0);  // pascals
```
...
| Field | Type | Description |
|-------|------|-------------|
| `fluid_pressure` | `f64` | Pressure in pascals |
| `variance` | `f64` | Variance; `0.0` when unknown |
| `frame_id` | `[u8; 32]` | Sensor frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
let light = Illuminance::new(320.0);  // lux
```
...
| Field | Type | Description |
|-------|------|-------------|
| `illuminance` | `f64` | Illuminance in lux |
| `variance` | `f64` | Variance; `0.0` when unknown |
| `frame_id` | `[u8; 32]` | Sensor frame |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
