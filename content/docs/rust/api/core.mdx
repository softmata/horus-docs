...
It does bypass the logging path, but it also skips two things `recv()` does: it does not
auto-register the caller as a subscriber, so the node never appears as a subscriber in the
topic and node registries; and it omits the empty-receive migration-epoch check, so a
`try_recv`-only subscriber can silently never see a cross-process publisher it should have
migrated to.
...
Runs all nodes sequentially on the main thread in registration order — no thread pools, no watcher threads, no executors. Intended for simulation and testing with `tick_once()`.
