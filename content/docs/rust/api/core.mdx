...
```rust
pub fn rt_scheduling(self, enabled: bool) -> Self
```

Enables real-time scheduling features (SCHED_FIFO, mlockall, CPU affinity). Features that cannot be applied are recorded as degradations, not errors.

**Example:**
```rust
let scheduler = Scheduler::new().rt_scheduling(true);
```
...
**Example:**
```rust
let scheduler = Scheduler::new().with_blackbox(16); // 16 MB flight recorder
```
...
```rust
pub fn safety_monitor(self, enabled: bool) -> Self
```

Enables the safety monitor (watchdog / WCET enforcement).

**Example:**
```rust
let scheduler = Scheduler::new().safety_monitor(true);
```
...
let mut scheduler = Scheduler::new()
    ...
    .rt_scheduling(true)       // RT priority, mlockall, CPU affinity
    .with_blackbox(16)         // Flight recorder
    .safety_monitor(true);     // Watchdog / WCET enforcement
```
