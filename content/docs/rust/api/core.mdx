...
Sends a message to the topic. Non-blocking, ~200ns latency. Zero-overhead (no metrics).
...
**Variants:**
- `send_tracked(&self, msg: T)` - Same but updates internal metrics
- `send_logged(&self, msg: T)` - Same but logs message summary (requires `T: LogSummary`)
...
Receives the latest message from the topic. Non-blocking. Zero-overhead (no metrics).
...
**Variants:**
- `recv_tracked(&self)` - Same but updates internal metrics
- `recv_logged(&self)` - Same but logs message summary (requires `T: LogSummary`)
