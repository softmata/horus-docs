...
Runs all nodes sequentially on the main thread — no thread pools, no watcher threads, no executors. The sequence is the topological order of the topic dependency graph, so producers run before their consumers; `.order()` tiers are only a fallback, used when no node has registered any topic. Intended for simulation and testing with `tick_once()`.
...
Arms a watchdog for every **RT** node — anything with `.rate()`, `.budget()` or
`.deadline()` — and marks it critical. The safety monitor exists on every scheduler
regardless of configuration, so emergency stop always works; `.watchdog()` is what arms
the frozen-node detector.

The watchdog escalates by timeout multiple: at 1x the node is marked Warning, at 2x
Unhealthy and skipped, at 3x isolated with `enter_safe_state()` called on it — and if that
call panics the node did not reach a safe state, so the scheduler latches an emergency
stop. Sustained **deadline misses** drive a separate ladder counted in consecutive misses:
warn at 3, halve the node's rate at 5, isolate and enter safe state at 10, and at the
`.max_deadline_misses(n)` ceiling (default 100) kill the node — `shutdown()` and permanent
removal from execution — and latch an emergency stop. The ceiling is the ladder's last
rung, so setting `n` moves it.

<Callout type="warning" title="Best-effort nodes are not covered by this">
A scheduler-level `.watchdog()` registers RT nodes only. A node without an execution
class is watched **only if it carries its own** `.watchdog(timeout)` on the node builder.

That is deliberate — a best-effort node is allowed to take as long as it likes, and
firing a watchdog at it would be wrong — but it means configuring a watchdog on the
scheduler and adding best-effort nodes gives you partial coverage. The scheduler logs
which nodes are uncovered rather than leaving you to infer it.
</Callout>
...
Sets the ceiling on **consecutive** deadline misses by a single node before the safety monitor latches an emergency stop. Default `100`. The count is per node and is reset by any tick that **meets its deadline** — not by one that merely stayed inside its budget, which is a different measurement: a node released late misses its deadline having executed in a fraction of its budget. So scattered misses that each recover do not accumulate toward a stop — a sustained run of them does. Only meaningful for nodes that set `.rate()`, since a node without a rate has no deadline to miss. `n` is clamped to a minimum of `1`, so passing `0` does not disable the stop — it makes the first miss fatal.

**Example:**
```rust
let scheduler = Scheduler::new()
    .max_deadline_misses(10)   // stop after 10 consecutive misses, not 100
...
Turns non-emergency logging from the executor threads on or off — budget warnings, pre/post-condition failures, startup and shutdown notices. Default `true`. Emergency-stop messages print either way, so `.verbose(false)` cannot hide a latched stop.
...
```rust
pub fn with_params(self, params: RuntimeParams) -> Self
```

Attaches a [`RuntimeParams`](/development/parameters) store to the scheduler. Its `get()` and `set()` work immediately and the store is shared with the nodes.

**Not yet wired:** attaching a store does not route `RuntimeParams::set()` into nodes — the scheduler does not call `Node::on_parameter_change()`, and there is no automatic rollback on reject. To react to a change today, register a callback with `RuntimeParams::on_change()`.
...
Records everything: node inputs, outputs and timing, on every tick, zstd-compressed, with no size cap, into `~/.horus/recordings` under an auto-generated session name.
...
```rust
pub fn telemetry(self, endpoint: &str) -> Self
```

Exports node timing, deadline misses and budget violations to `endpoint`. UDP and file URIs are supported. Can also be set with the `HORUS_TELEMETRY_ENDPOINT` environment variable; the builder call takes precedence.
...
Turns network replication on or off. It is **on by default**, so this is the opt-out. The builder call overrides `HORUS_NET_ENABLED` in either direction — setting the environment variable will not re-enable a scheduler built with `.network(false)`, and it will not disable one built with `.network(true)`.
...
Executes exactly one tick cycle, then returns. It does not loop and does not
sleep, so the caller owns the timing — that is what lets an external physics step drive
the schedule instead of a wall clock. Lazily initialises on the first call: deferred
config is applied and every node's `init()` runs there, not at `build()`.

`tick_once()` starts no executor threads. The execution-class partition runs only under
`run()`, so on this path RT, compute, event and async-I/O nodes all tick in-line and
sequentially on the calling thread, whatever class they declared.

It also does not tick every node on every call. Per-node `.rate()` limiting applies here
too, so a node slower than the drive cadence is skipped on most calls — a `.rate(100hz)`
node driven at 1 kHz runs on roughly one call in ten. `.deterministic(true)` turns the
rate gate off, and then every node runs on every call.

**Returns:** `Result<()>` — `HorusError::InvalidInput` for a duplicate node name,
`HorusError::Resource` when `require_rt()` was set and real-time could not be acquired,
and `HorusError::Internal` when the tick did not complete (a node panic whose failure
policy said stop, an RT budget or deadline escalation, or the safety monitor latching).
Treat every `Internal` as fatal and stop the loop: those causes share one variant, and the
message is diagnostic text rather than something to match on.

**Example:**
```rust
let mut scheduler = Scheduler::new().tick_rate(100_u64.hz());
// ... add nodes and build, then drive the loop yourself
for _ in 0..1000 {
    scheduler.tick_once()?;
}
```
...
impl NodeMetrics {
    ...
    pub fn messages_sent(&self) -> u64;      // always 0 — never written (NodeMetrics)
    pub fn messages_received(&self) -> u64;  // always 0 — never written (NodeMetrics)
...
impl TopicMetrics {
    pub fn messages_sent(&self) -> u64;      // this handle's sends
    pub fn messages_received(&self) -> u64;  // this handle's recv() deliveries
    pub fn send_failures(&self) -> u64;
    pub fn recv_failures(&self) -> u64;      // MockTopic only
}
```

These are **per handle**. `messages_sent` counts every `send()`, `try_send()` and
`send_blocking()` on this `Topic`; `messages_received` counts the messages its `recv()`
handed back — `try_recv()` is the low-level path and is not counted. Both used to move
only on the `#[cold]` content-logging path, which runs while a topic's verbose flag is
set from the `horus monitor` TUI, so in a normal run they read `0` however much traffic
the topic carried.

`recv_failures` is the odd one out: only [`MockTopic`](#mocktopic) ever makes it
non-zero, from its `recv_fail_after` / `recv_fail_every_n` fault injection. The real
transport never increments it, deliberately — a `recv()` that returns `None` is an empty
ring, not a failure. For real consumer-side loss, where a publisher lapped a subscriber,
use `Topic::missed_count()`.

`send_failures` is exposed as `Topic::dropped_count()`. The **per topic** counter, shared
by every handle, is `messages_total` in the shared-memory header — that is what
`horus topic list` and `horus topic hz` report.
...
An in-memory stand-in for `Topic<T>` with configurable fault injection. It backs
its buffer with a `Mutex<VecDeque<T>>` rather than shared memory, so a test can
exercise the failure branches a real topic almost never takes.

It is the only thing in the codebase that makes `recv_failures` non-zero.
...
`MockTopic::simple(name)` is the same thing with `MockTopicConfig::default()`,
which injects no failures at all.

| `MockTopicConfig` field | Effect |
|---|---|
| `capacity` | Buffer bound; `0` means unlimited |
| `send_fail_every_n` | Drop every N-th `send`. `None` never fails |
| `recv_fail_every_n` | Return `None` from every N-th `recv` |
| `send_fail_after` | Drop every `send` after this many total sends |
| `recv_fail_after` | Return `None` from every `recv` after this many total recvs |
| `send_blocking_always_timeout` | `send_blocking` always returns `Timeout` |

Note the import path: this is the one type on this page that the `horus` facade
does **not** re-export, so `use horus::prelude::*` will not reach it. A test
that needs it depends on `horus_core` directly.

---

## Performance

Every topic is shared-memory backed, so the figures below are cross-process numbers — there is no separate intra-process transport to select. One handle that both sends and receives a POD message is the exception: it takes a shorter inlined path inside the same SHM backend, which is the 20 ns same-thread figure in the [benchmarks](/performance/benchmarks).
