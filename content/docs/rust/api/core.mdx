...
Runs all nodes sequentially on the main thread in `.order()` order (ties broken by registration order) — no thread pools, no watcher threads, no executors. Intended for simulation and testing with `tick_once()`.
...
`.deadline()` — and marks it critical. Setting it auto-creates the safety monitor: if a
watched node does not tick within the timeout, the watchdog triggers graduated
degradation (warn → reduce rate → isolate → safe state).
