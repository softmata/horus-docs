...
```rust
pub fn new_with_capacity(topic_name: &str, capacity: usize) -> Result<Self>
```
...
**Example:**
```rust
let topic: Topic<Image> = Topic::new_with_capacity("camera.image", 16)?;
```
...
```rust
pub fn from_config(topic_name: &str) -> Result<Self>
```

Creates a Topic from configuration file (`horus.yaml`).
...
```rust
let topic: Topic<Image> = Topic::from_config("camera")?;
```
