...
warn at 3, halve the node's rate at 5, isolate and enter safe state at 10, kill at 20
(`shutdown()` and permanent removal from execution).
...
Sets the ceiling on **consecutive** deadline misses by a single node before the safety monitor latches an emergency stop. Default `100`. The count is per node and resets on any tick that meets its budget, so scattered misses that each recover do not accumulate toward a stop — a sustained run of them does. Only meaningful for nodes that set `.rate()`, since a node without a rate has no deadline to miss. `n` is clamped to a minimum of `1`, so passing `0` does not disable the stop — it makes the first miss fatal.
...
`DurationExt` — the trait behind `.us()`, `.ms()` and `.hz()` — and the `Frequency` type that `.hz()` returns both come from the prelude. Their signatures are under [DurationExt and Frequency](#durationext-and-frequency).
...
Every rate and duration in this API is built with these two, so their signatures are
given in full rather than left to be inferred from call sites.

Both are re-exported by `horus::prelude`
(`pub use horus_core::core::{DurationExt, Frequency, Miss, RtStats};`), so
`use horus::prelude::*;` is enough.
...
Each method takes `self` **by value**, so it is called on a literal or on any
expression of an implementing type — including a `const`.

**Implemented for `u64`, `f64` and `i32`.** That is why both spellings you will see
in these docs compile:

```rust
use horus::prelude::*;

fn main() {
    // All three compile. This block is compiled by `npm run verify:rust`, so the
    // claim is checked rather than asserted.
    let _suffixed = Scheduler::new().tick_rate(1000_u64.hz());  // u64 impl
    let _inferred = Scheduler::new().tick_rate(100.hz());       // bare literal infers i32
    let _fractional = Scheduler::new().tick_rate(2.5.hz());     // f64 impl

    // The same three receivers work for durations.
    let _budget: std::time::Duration = 500_u64.us();
    let _period: std::time::Duration = 20.ms();
    let _secs: std::time::Duration = 1.5.secs();

    // `self` is taken by value, so a named const or a variable works too —
    // the receiver does not have to be a literal.
    const RATE_HZ: u64 = 250;
    let hz = RATE_HZ;
    let _from_const = Scheduler::new().tick_rate(RATE_HZ.hz());
    let _from_var = Scheduler::new().tick_rate(hz.hz());
}
```

There is no `u32`, `usize` or `i64` impl. `1000u32.hz()` does not compile; annotate
or suffix the literal instead.
...
`Frequency` is constructed through `.hz()` — the tuple field is private, so
`Frequency(100.0)` will not compile from your crate.

`budget_default` and `deadline_default` are where a node's implied budget and deadline
come from when you set a rate and neither: 80% and 95% of the period.
