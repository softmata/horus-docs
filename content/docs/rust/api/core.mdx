...
`Scheduler::new()` returns a builder. Chain methods to configure global behavior before adding nodes:
...
Applies a `SchedulerConfig` for global scheduler settings. Use this to configure production behavior instead of the removed preset constructors.

**Example:**
```rust
let config = SchedulerConfig {
    enable_rt: true,           // Request RT scheduling (best-effort)
    enable_blackbox: true,     // Enable flight recorder
    lock_memory: true,         // Lock pages (prevents page faults)
    ..Default::default()
};
...
Global configuration struct for scheduler behavior. Replaces the old preset constructors with explicit, composable settings.

```rust
pub struct SchedulerConfig {
    pub enable_rt: bool,           // Request SCHED_FIFO for RT nodes (default: false)
    pub enable_blackbox: bool,     // Enable BlackBox flight recorder (default: false)
    pub lock_memory: bool,         // mlockall to prevent page faults (default: false)
    pub watchdog_timeout_ms: u64,  // Watchdog timeout in ms, 0 = disabled (default: 0)
    pub parallel: bool,            // Enable parallel execution (default: false)
    pub deterministic_clock: bool, // Use deterministic clock (default: false)
}
```
...
| `.rt()` | Mark as **real-time** execution class (pinned thread, FIFO scheduling) |
...
|-------|--------|-------------|----------|
| Real-time | `.rt()` | Pinned thread, SCHED_FIFO | Safety monitors, motor control |
...
If no execution class is specified, the node runs in the default scheduler loop (inline, sequential).
...
scheduler.add(path_planner).order(10).compute().done();
scheduler.add(data_processor).order(20).on("sensor_data").done();
scheduler.add(cloud_uploader).order(100).async_io().rate_hz(1.0).done();
...
fn main() -> Result<()> {
    ...
    scheduler.add(PathPlanner::new()?).order(10).compute().done();
    scheduler.add(SensorReader::new()?).order(50).rate_hz(1000.0).done();
    scheduler.add(CloudSync::new()?).order(100).async_io().rate_hz(1.0).done();
...
Unified error type for all HORUS operations. The prelude exports `Error` and `Result<T>` as short aliases (the long names `HorusError` and `HorusResult<T>` still work for backward compatibility).
...
Type alias for Results using `Error`. The prelude exports `Result<T>` as the short alias (the long name `HorusResult<T>` still works for backward compatibility).

```rust
use horus::prelude::*;

pub type Result<T> = std::result::Result<T, Error>;
// HorusResult<T> is also available as a backward-compatible alias
```
...
Built-in implementations exist for `GenericMessage`, `HorusTensor`, `HealthStatus`, and all standard message types.
