...
allocated from the global pool, not in a `Vec` — what travels on a topic is a
fixed-size 320-byte Pod descriptor (dimensions, resolution, origin, frame, timestamp)
holding a handle to that tensor, copied byte-for-byte into the ring with no
serialization step.
