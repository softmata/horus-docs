...
**Re-exported types** (available via `use horus::prelude::*`): `Goal`, `Path`, `PathPlan`, `OccupancyGrid`, `CostMap`.
...
| `get_waypoints()` | Get slice of valid waypoints |
...
| `with_frame_id(frame_id)` | Set coordinate frame (builder pattern) |
...
| `get_cost(x, y)` | Get cost at world coordinates (returns lethal for out-of-bounds) |
...
```rust
use horus::prelude::*;
use horus_library::messages::navigation::{Goal, GoalResult, GoalStatus, Waypoint};
...
impl NavigationNode {
    fn plan_path(&self, _goal: &Goal, _map: &OccupancyGrid) -> Path {
