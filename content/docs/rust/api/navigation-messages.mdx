...
impl Node for NavigationNode {
    ...
    fn tick(&mut self) {
        ...
        if let Some(goal) = self.goal_sub.recv() {
            ...
            // Plan path to goal
            if let Some(map) = self.map_sub.try_recv() {
                let path = self.plan_path(&goal, &map);
                self.path_pub.send(path.clone()).ok();
        ...
        if let (Some(goal), Some(odom)) = (&self.current_goal, self.odom_sub.recv()) {
            ...
            if goal.is_reached(&current_pose) {
                ...
                self.result_pub.send(result).ok();
...
- [Sensor Messages](/rust/api/sensor-messages) - LIDAR and sensor data
