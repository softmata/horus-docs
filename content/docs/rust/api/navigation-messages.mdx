...
`GoalStatus` from the prelude is the *action* enum, not the navigation one. Reach the
navigation enum through its module instead: `use horus::prelude::navigation::GoalStatus;`
...
| `status` | `u8` | Current status (use `GoalStatus as u8` to set directly) |
