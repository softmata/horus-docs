...
`HorusError` is a `#[non_exhaustive]` enum with 15 variants — 13 domain variants (below) plus the `Internal` and `Contextual` framework variants (see [Internal & Contextual Variants](#internal--contextual-variants)). Each wraps a domain-specific sub-error:
...
| `params.set("key", val)` | `ValidationError::Other` | Value outside configured min/max (or key read-only) |
