...
impl Node for FleetManagerNode {
    ...
    fn tick(&mut self) {
        // Update robot states
        while let Some(state) = self.robot_state_sub.try_recv() {
            self.fleet.update_robot(state).ok();
        }

        // Process new task requests
        while let Some(task) = self.task_request_sub.try_recv() {
            match self.fleet.coordination_mode {
                CoordinationMode::Centralized => {
                    ...
                    if let Some(robot) = self.find_best_robot(&task) {
                        ...
                        self.task_pub.send(assigned_task).ok();
                    }
                }
        ...
        while let Some(bid) = self.bid_sub.try_recv() {
        ...
        self.fleet_pub.send(self.fleet.clone()).ok();
    }
}
