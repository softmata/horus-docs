...
**Re-exported types** (available via `use horus::prelude::*`): `RobotState`, `FleetStatus`, `TaskAssignment`, `FormationControl`.

**Non-re-exported types** (require direct import): `RobotType`, `RobotCapability`, `CoordinationMode`, `TaskType`, `TaskStatus`, `FormationType`, `AuctionBid`, `BidStatus` — import from `horus_library::messages::coordination::*`.
...
```rust
use horus::prelude::*;
use horus_library::messages::coordination::{RobotType, RobotCapability};
...
| `status` | `u8` | Operational status (use `StatusLevel as u8` to set) |
...
| `robot_type` | `u8` | Robot classification (use `RobotType as u8` to set) |
...
```rust
use horus::prelude::*;
use horus_library::messages::coordination::{RobotType, CoordinationMode};

// Create fleet
let mut fleet = FleetStatus::new("warehouse_fleet");
fleet.coordination_mode = CoordinationMode::Centralized as u8;
...
| `coordination_mode` | `u8` | Fleet coordination mode (use `CoordinationMode as u8` to set) |
| `coordinator_id` | `[u8; 32]` | Central coordinator ID |
| `emergency_active` | `u8` | Fleet-wide emergency (0 = off, 1 = on) |
...
```rust
use horus::prelude::*;
use horus_library::messages::coordination::{TaskType, TaskStatus, RobotCapability};
...
| `task_type` | `u8` | Task category (use `TaskType as u8` to set) |
...
| `status` | `u8` | Current task status (use `TaskStatus as u8` to set) |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
```rust
use horus::prelude::*;
use horus_library::messages::coordination::FormationType;
...
formation.formation_type = FormationType::Grid as u8;
...
| Field | Type | Description |
|-------|------|-------------|
| `formation_type` | `u8` | Formation pattern (use `FormationType as u8` to set) |
...
| `enabled` | `u8` | Formation control active (0 = off, 1 = on) |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
> **Note:** `AuctionBid` and `BidStatus` are not re-exported in the prelude. Import directly: `use horus_library::messages::coordination::{AuctionBid, BidStatus};`

```rust
use horus_library::messages::coordination::{AuctionBid, BidStatus};
...
bid.status = BidStatus::Won as u8;
```
...
| `status` | `u8` | Bid status (use `BidStatus as u8` to set) |
| `timestamp_ns` | `u64` | Nanoseconds since epoch |
...
use horus_library::messages::coordination::{
    TaskType, TaskStatus, AuctionBid, BidStatus, CoordinationMode,
};

struct FleetManagerNode {
    fleet: FleetStatus,
    robot_state_sub: Topic<RobotState>,
    task_request_sub: Topic<TaskAssignment>,
    bid_sub: Topic<AuctionBid>,
    task_pub: Topic<TaskAssignment>,
    fleet_pub: Topic<FleetStatus>,
...
impl Node for FleetManagerNode {
    ...
    fn tick(&mut self) {
        // Update robot states
        while let Some(state) = self.robot_state_sub.recv() {
            self.fleet.update_robot(state).ok();
        }

        // Process new task requests
        while let Some(task) = self.task_request_sub.recv() {
            let mode = self.fleet.coordination_mode;
            if mode == CoordinationMode::Centralized as u8 {
                ...
                if let Some(robot) = self.find_best_robot(&task) {
                    ...
                    self.task_pub.send(assigned_task);
                }
            } else if mode == CoordinationMode::MarketBased as u8 {
                // Start auction
        ...
        while let Some(bid) = self.bid_sub.recv() {
        ...
        self.pending_auctions.retain(|(task_id, bids)| {
            if bids.len() >= self.fleet.robot_count as usize {
                ...
                {
                    let robot_str = Self::robot_id_str(&winner.robot_id);
                    let task = TaskAssignment::new(*task_id, &robot_str, TaskType::Navigation);
                    self.task_pub.send(task);
                }
        ...
        self.fleet_pub.send(self.fleet);
    }
}
...
- [Control Messages](/rust/api/control-messages) - Motor and velocity commands
