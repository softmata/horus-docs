...
tick {
    ...
    //   - point-to-point (1 pub / 1 sub, SpscShm): while a subscriber is
    //     draining, the NEW message is dropped. If nothing is draining -- no
    //     subscriber, a dead subscriber, or a tail that has not moved for the
    //     lease timeout -- send() instead retires the OLDEST slot and takes it,
    //     so the ring keeps the most recent `capacity` messages rather than
    //     freezing on the first ones. Both losses count in
    //     self.critical.dropped_count().
    ...
    // try_send() and send_blocking() never reclaim: they report a full ring
    // instead. send_blocking(msg, timeout) only applies backpressure on a
    // point-to-point ring. On a broadcast ring it returns
    // SendBlockingError::NoBackpressure before its first send attempt and the
    // message is discarded, so reaching for it on a topic that gains a second
    // subscriber turns partial loss into total loss. Assert
    // self.critical.provides_backpressure() AFTER the first send -- an
    // unresolved backend answers false -- to see which ring you actually got.
