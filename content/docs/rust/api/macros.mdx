...
Called once before the first tick. The block must return `Ok(())` on success (it generates `fn init(&mut self) -> HorusResult<()>`).
...
Called once when the scheduler stops. Must return `Ok(())` on success (generates `fn shutdown(&mut self) -> HorusResult<()>`).
