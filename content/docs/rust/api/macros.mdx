...
tick() {
    ...
    self.status.send("ok".to_string()).ok();

    // Or handle errors explicitly
    if let Err(msg) = self.critical.send(data) {
        hlog!(error, "Failed to send critical data");
    }
}
```
