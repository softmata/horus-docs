...
// simplified
tick {
    // send() never returns an error (but may drop the message if the buffer stays full)
    self.status.send("ok".to_string());

    // plain send() drops the newly-sent message after a brief spin+yield retry
    // if the buffer is full — use send_blocking() when delivery must be guaranteed
...
use horus::prelude::*;
use horus_robotics::prelude::*;
use horus_core::topics;  // `topics!` is exported at the horus_core crate root, not in the prelude
...
use horus::register_driver;  // not in the prelude — import it explicitly
...
// simplified
fn main() -> Result<()> {
    // hardware::load() returns Vec<(String, Box<dyn Node>)> — destructure the tuple.
