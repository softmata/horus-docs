...
2D velocity command for differential-drive and unicycle robots: forward speed and yaw
rate only. Use `Twist` when you need all six degrees of freedom (drones, manipulators).
16 bytes.

```rust
use horus::prelude::*;

// 1.0 m/s forward, 0.5 rad/s left turn, timestamped now
let cmd = CmdVel::new(1.0, 0.5);

// Stop
let stop = CmdVel::zero();

// Replay or simulation: supply the timestamp yourself
let replayed = CmdVel::with_timestamp(1.0, 0.5, 1_234_567_890);

// Conversions both ways, via From
...
| `linear` | `f32` | Forward velocity (m/s) |
| `angular` | `f32` | Yaw rate (rad/s) |

<Callout type="warning">
The `Twist` conversions are lossy. `Twist` to `CmdVel` keeps only `linear[0]` and
`angular[2]` and narrows `f64` to `f32`; `CmdVel` to `Twist` zeroes the other four
components. Round-tripping a 6-DOF command through `CmdVel` silently drops strafe,
vertical motion and roll/pitch.
</Callout>
