...
| `max_velocity` | `f64` | rad/s | Velocity limit in position mode. The `velocity`/`position`/`stop` constructors set this to `f64::INFINITY` (no limit) |
| `max_acceleration` | `f64` | rad/s² | Acceleration limit. The convenience constructors set this to `f64::INFINITY` (no limit) |
