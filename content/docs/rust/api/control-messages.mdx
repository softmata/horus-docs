...
impl Node for MotorDriverNode {
    ...
    fn tick(&mut self) {
        if let Some(cmd) = self.cmd_sub.recv() {
            ...
            // Send PWM commands
            self.left_pwm_pub.send(PwmCommand::new(0, left_duty)).ok();
            self.right_pwm_pub.send(PwmCommand::new(1, right_duty)).ok();
        }
    }
}
```
...
- [I/O Messages](/rust/api/io-messages) - Low-level I/O protocols
