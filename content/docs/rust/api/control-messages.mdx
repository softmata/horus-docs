...
HORUS provides comprehensive message types for controlling motors, servos, and multi-joint robotic systems. All control messages are **POD** (Plain Old Data) types with `#[repr(C)]` layout, enabling zero-copy shared memory transport via the `PodShm` backend at ~50ns latency.
...
| `enable` | `u8` | Output enabled (1=enabled, 0=disabled) |
| `brake_mode` | `u8` | Brake when stopped (1=brake, 0=coast) |
...
struct MotorDriverNode {
    ...
    left_pwm_pub: Topic<PwmCommand>,
    right_pwm_pub: Topic<PwmCommand>,
    wheel_base: f64,
    wheel_radius: f64,
    max_speed: f64,
}
...
impl Node for MotorDriverNode {
    ...
    fn tick(&mut self) {
        if let Some(cmd) = self.cmd_sub.recv() {
            ...
            // Send PWM commands
            self.left_pwm_pub.send(PwmCommand::new(0, left_duty));
            self.right_pwm_pub.send(PwmCommand::new(1, right_duty));
        }
    }
}
```
...
- [Hardware API](/rust/api/hardware) - Hardware abstraction layer
