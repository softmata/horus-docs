...
| Error | Condition |
|-------|-----------|
| `ServiceError::Timeout` | No response within timeout (also how a missing server manifests) |
| `ServiceError::ServiceFailed(msg)` | Server handler returned `Err(msg)` |
| `ServiceError::NoServer` | Not produced by the current transport — a missing server surfaces as `Timeout` |
| `ServiceError::Transport(msg)` | Not produced by the current shared-memory transport |
...
- `Err(ServiceError)` — a non-timeout error (in practice `ServiceFailed`)
    ...
    .on_request(move |req| {
        match poses.get(&req.robot_name) {
            Some(&(x, y, theta)) => Ok(GetRobotPoseResponse {
                x, y, theta, timestamp_ns: horus::time::now().as_nanos(),
            }),
...
> **Note**: The current shared-memory transport only produces `Timeout` and `ServiceFailed`. `NoServer` and `Transport` are defined in the enum but are not returned by `call()` — a missing server surfaces as `Timeout`.
