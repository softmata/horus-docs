...
**Re-exported types** (available via `use horus::prelude::*`): `WrenchStamped`, `TactileArray`, `ImpedanceParameters`, `ForceCommand`.
...
Pressure/force array from tactile sensors (fingertip sensors, skin patches). Implements `PodMessage` for zero-copy transfer.
...
// Set and get sensor readings
tactile.set_sensor(0, 1.5);
tactile.set_sensor(1, 2.0);
if let Some(reading) = tactile.get_sensor(0) {
    println!("Sensor 0: {:.1}", reading);
}
...
| Method | Description |
|--------|-------------|
| `new(sensor_count, arrangement)` | Create a new tactile array |
| `get_active_sensors()` | Get slice of active sensor readings |
| `set_sensor(index, value)` | Set sensor reading (returns `bool`) |
| `get_sensor(index)` | Get sensor reading as `Option<f32>` |
| `total_force()` | Calculate total force from all sensors |
| `center_of_pressure()` | Calculate center of pressure (grid only) |
| `detect_contact(threshold)` | Check if any sensor exceeds threshold |
| `contact_pattern(threshold)` | Get contact pattern as `Vec<bool>` |
