...
// The summary fields are carried, not computed: set them the way a driver would
// before anything reads them back.
tactile.total_force = [0.0, 0.0, 3.5];
tactile.center_of_pressure = [0.0, 0.07];
...
`total_force`, `center_of_pressure` and `in_contact` are carried, not computed. `new()`
and `set_force()` write only `forces` and leave the three summary fields at zero and
`false` — the publishing driver is what fills them. Reading them off an array you built
locally gives you those defaults, not a summary of the taxels you just wrote.
