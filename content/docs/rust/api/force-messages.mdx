...
**Available via** `use horus_robotics::prelude::*`: `WrenchStamped`, `ImpedanceParameters`, `ForceCommand`, `ContactInfo`, `ContactState`, `HapticFeedback`, `TactileArray` — all robotics force types are re-exported through the prelude glob.
...
> **Note:** `ContactInfo` and `ContactState` are available via `use horus_robotics::prelude::*;`.
...
> **Note:** `HapticFeedback` is available via `use horus_robotics::prelude::*;`.
...
> **Note:** `TactileArray` uses `Vec<f32>` internally, so it is **not** zero-copy POD — it goes through the serde serialization path. It is available via `use horus_robotics::prelude::*;`.
