...
**Convenience re-exports** (available via `use horus_robotics::prelude::*`): `WrenchStamped`, `ImpedanceParameters`, `ForceCommand`.

**Non-re-exported types** (require direct import): `ContactInfo`, `ContactState`, `HapticFeedback`, `TactileArray` — import from `horus_robotics::messages::force::*`.
...
> **Note:** `ContactInfo` and `ContactState` are not re-exported in the prelude. Import directly: `use horus_robotics::messages::force::{ContactInfo, ContactState};`

```rust
// simplified
use horus_robotics::messages::force::{ContactInfo, ContactState};
...
> **Note:** `HapticFeedback` is not re-exported in the prelude. Import directly: `use horus_robotics::messages::force::HapticFeedback;`

```rust
// simplified
use horus::prelude::*;
use horus_robotics::messages::force::HapticFeedback;
...
// simplified
use horus::prelude::*;
use horus_robotics::messages::force::ContactState;
...
> **Note:** `TactileArray` uses `Vec<f32>` internally, so it is **not** zero-copy POD — it goes through the serde serialization path. Import directly: `use horus_robotics::messages::force::TactileArray;`

```rust
// simplified
use horus_robotics::messages::force::TactileArray;
