...
Pool-backed RAII image type with zero-copy shared memory transport. `Image` allocates from a global tensor pool automatically — you never manage memory directly. Only a 168-byte descriptor travels through topics; the pixel data stays in shared memory at ~50ns IPC latency.
...
| `is_cuda()` | `bool` | Whether the image data resides on a CUDA GPU |
...
- [TensorPool API](/rust/api/tensor-pool) — Advanced pool management and GPU support
