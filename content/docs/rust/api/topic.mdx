...
|-------------|---------|-------------|
| `Topic::<T>::new(name)` | `HorusResult<Topic<T>>` | Creates with default capacity (4 slots) and auto-sized slots |
...
| `mode()` | `BackendMode` | Current backend (InProcess, SharedMemory, etc.) |
| `is_same_process()` | `bool` | Whether all pub/sub are in the same process |
| `is_same_thread()` | `bool` | Whether all pub/sub are on the same thread |
| `backend_name()` | `&'static str` | Human-readable backend name |
...
|------|------|----------|-------------|
| `name` | `impl Into<String>` | yes | Topic identifier. Case-sensitive. Use `/`-delimited hierarchical names: `"sensors/imu"`, `"cmd_vel"`. Two Topic instances with the same name and type `T` connect automatically. |
...
- Default capacity: 4 slots (ring buffer)
...
`BackendMode` — the active transport. Auto-selected based on publisher/subscriber location.

---

### `is_same_process()`

Checks whether all publishers and subscribers are in the same process.
...
Checks whether all publishers and subscribers are on the same thread.
...
`&'static str` — e.g., `"DirectChannel"`, `"SharedMemory"`.
...
`ConnectionState` — whether the topic is connected, waiting, or disconnected.
...
Transport backend, auto-selected by the topic.

| Variant | Description |
|---------|-------------|
| `InProcess` | In-process ring buffer (same process, ~3ns) |
| `SharedMemory` | Cross-process shared memory (~150ns) |
...
| Variant | Description |
|---------|-------------|
| `Connected` | Publisher and subscriber are actively communicating |
| `Waiting` | Topic created but no peer connected yet |
| `Disconnected` | Peer was connected but is now gone |
...
- [Tensor API](/rust/api/tensor) — Tensor descriptor and DLPack
