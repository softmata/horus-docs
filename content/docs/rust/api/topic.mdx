...
Topics are how nodes communicate in HORUS. `Topic<T>` provides typed publish/subscribe channels backed by shared memory for zero-copy IPC. Two Topic instances with the same name and type connect automatically — one publishes, the other subscribes. Every topic is shared-memory-backed from creation, so a cross-process consumer can always join later with no migration. HORUS picks the matching SHM backend automatically from the pub/sub pattern — `SpscShm` (1:1), `MpscShm` (N:1), `PodShm` (1:N POD broadcast), or `FanoutShm` (N:M / non-POD broadcast) — there is no separate in-process path.
...
|------|------|----------|-------------|
| `name` | `impl Into<String>` | yes | Topic identifier. Case-sensitive. Use dot-delimited hierarchical names: `"sensors.imu"`, `"cmd_vel"`. Two Topic instances with the same name and type `T` connect automatically. **Note:** Dots are preferred over slashes for macOS compatibility (`shm_open` does not allow `/` in names). Names that are absolute paths or contain `..` are **rejected** — the name becomes part of a shared-memory path, so a traversal there would place the region outside the HORUS namespace. |
...
- Backend auto-selection: HORUS picks the shared-memory backend from the topic's pub/sub pattern — `SpscShm` (1:1), `MpscShm` (N:1), `PodShm` (1:N POD broadcast), or `FanoutShm` (N:M / non-POD broadcast). Every backend is SHM-backed from creation, so a cross-process subscriber can join later with no migration.
...
- Use `dropped_count()` to detect drops
- Notifies any event-driven nodes (`.on("topic")`) that new data arrived
- Latency: every topic is SHM-backed from creation (so a cross-process consumer can join later), so `send()` always writes to the shared-memory ring. The cost floor is **core locality**, not intra-vs-SHM: with publisher and subscriber on the same executor thread (same core, L1-hot) a send+recv round trip is ~39 ns (~19 ns one-way); when the consumer runs on a different core — whether another thread or another process — it is ~220 ns one-way, which is cache-coherence physics that no backend or layout choice can avoid.
...
// Fire-and-forget (non-blocking; drops the new message if the buffer is full)
...
- Latency: `recv()` reads from the shared-memory ring in all cases. Same-core (publisher and subscriber on one executor thread) is L1-hot — ~19 ns one-way; a consumer on a different core, whether another thread or another process, is ~220 ns one-way (cache-coherence physics)
...
`Option<T>` — same values as `recv()`, but `try_recv()` skips the same-thread fast path plus the logging/recording hooks that `recv()` runs. Prefer `recv()` in application code.
...
`true` if all participants are in the same process. Note: every topic is SHM-backed even when same-process (so a cross-process consumer can join later), so being same-process does **not** change the backend — what actually moves latency is whether the participants share a core (see the `send()`/`recv()` latency notes).
...
`true` if all publishers and subscribers are on the same thread. (Every topic stays SHM-backed; same-thread participants share a core and hit the L1-hot fast path — ~12–19 ns one-way — but the backend is still shared memory.)
