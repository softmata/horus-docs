...
- Latency: real topics are SHM-backed from creation (so a cross-process consumer can join later), so `send()` uses the shared-memory ring even same-process — tens of nanoseconds, and faster same-process than cross-process (~150ns typical). The ~3ns in-process heap-ring path applies only to in-memory-only topics, which normal usage never creates.
...
- Latency: real topics read from the shared-memory ring even same-process (tens of nanoseconds); the ~3ns in-process heap-ring path applies only to in-memory-only topics
...
`true` if all participants are in the same process. Note: real topics are SHM-backed even when same-process (so a cross-process consumer can join later), so this does **not** imply the ~3ns in-process heap ring — that path is used only by in-memory-only topics.
...
`true` if all publishers and subscribers are on the same thread. (Real topics stay SHM-backed; the atomic-free same-thread heap path is used only by in-memory-only topics.)
