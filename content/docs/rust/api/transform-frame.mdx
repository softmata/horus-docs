...
`TransformFrame` manages coordinate frame trees for real-time robotics. All lookups are lock-free and wait-free. Register frames with `FrameBuilder`, query transforms with `TransformQuery`, and update dynamic frames at sensor rates.
...
| `.build()` | `HorusResult<FrameId>` | Register and return frame ID |
...
| Method | Returns | Description |
|--------|---------|-------------|
| `update_transform(name, transform, timestamp_ns)` | `HorusResult<()>` | Update by name (validates transform) |
| `update_transform_by_id(id, transform, timestamp_ns)` | `HorusResult<()>` | Update by ID (fastest, lock-free) |
| `set_static_transform(name, transform)` | `HorusResult<()>` | Update a static frame's offset |
...
| Method | Returns | Description |
|--------|---------|-------------|
| `lookup()` | `HorusResult<Transform>` | Latest transform |
| `at(timestamp_ns)` | `HorusResult<Transform>` | At timestamp with interpolation |
| `at_strict(timestamp_ns)` | `HorusResult<Transform>` | Error if extrapolation needed |
| `at_with_tolerance(timestamp_ns, tolerance_ns)` | `HorusResult<Transform>` | Error if gap exceeds tolerance |
| `point(xyz)` | `HorusResult<[f64; 3]>` | Transform a 3D point |
| `vector(xyz)` | `HorusResult<[f64; 3]>` | Transform a vector (rotation only) |
...
| `chain()` | `HorusResult<Vec<String>>` | Frame chain from source to destination |
| `wait(timeout)` | `HorusResult<Transform>` | Block until available (feature `wait`) |
| `wait_at(timestamp_ns, timeout)` | `HorusResult<Transform>` | Block until timestamp data available (feature `wait`) |
| `wait_async(timeout)` | `HorusResult<Transform>` | Async wait (feature `async-wait`) |
| `wait_at_async(timestamp_ns, timeout)` | `HorusResult<Transform>` | Async wait at timestamp (feature `async-wait`) |
...
| Method | Returns | Description |
|--------|---------|-------------|
| `tf(src, dst)` | `HorusResult<Transform>` | Latest transform src→dst |
| `tf_by_id(src_id, dst_id)` | `Option<Transform>` | By ID (fastest, lock-free) |
| `tf_at(src, dst, timestamp_ns)` | `HorusResult<Transform>` | At timestamp with interpolation |
| `tf_at_strict(src, dst, timestamp_ns)` | `HorusResult<Transform>` | Error if extrapolation needed |
| `tf_at_with_tolerance(src, dst, ts, tol)` | `HorusResult<Transform>` | Error if gap exceeds tolerance |
...
| `transform_point(src, dst, point)` | `HorusResult<[f64; 3]>` | One-shot point transform |
| `transform_vector(src, dst, vector)` | `HorusResult<[f64; 3]>` | Rotation-only vector transform |
