...
> **Rust**: `TransformFrame` lives in the `horus_tf` crate, which is not a
> dependency of a `horus new -r` project. Add it first —
> `horus add horus_tf` — then `use horus_tf::prelude::*;`
...
|--------|---------|-------------|
| `TransformFrame::new()` | `Self` | Create a transform frame tree with the default config |
| `TransformFrame::with_config(config)` | `Self` | Create a transform frame tree with a custom config |
| `.register_frame(name, parent)` | `Result<FrameId>` | Add a frame to the tree |
...
| `.is_stale_now(frame, max_age_ns)` | `bool` | Check if a frame's data is older than `max_age_ns` (wall clock); `.is_stale(frame, max_age_ns, now_ns)` takes an explicit time |
| `.has_frame(name)` | `bool` | Check if a frame is registered |
| `.stats()` | `TransformFrameStats` | Get tree statistics and health |
