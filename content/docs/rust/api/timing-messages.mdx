...
**Re-exported types** (available via `use horus::prelude::*`): `TimeSync`, `ScheduledEvent`, `Timeline`, `ClockStats`.

**Non-re-exported types** (require direct import): `SyncQuality`, `EventType`, `EventStatus`, `TimelineStatus` — import from `horus_library::messages::timing::*`.
...
| `sync_quality` | `u8` | Sync quality (use `SyncQuality as u8` to set) |
...
| `event_type` | `u8` | Event type (use `EventType as u8` to set) |
...
| `status` | `u8` | Event status (use `EventStatus as u8` to set) |
...
| `status` | `u8` | Timeline status (use `TimelineStatus as u8` to set) |
...
| `sync_quality` | `u8` | Current sync quality (use `SyncQuality as u8` to set) |
| `master_id` | `[u8; 32]` | Master clock identifier |
...
struct SynchronizedRobot {
    sync_sub: Topic<TimeSync>,
    event_sub: Topic<ScheduledEvent>,
    clock_stats_pub: Topic<ClockStats>,
    stats: ClockStats,
    timeline: Timeline,
}
...
impl Node for SynchronizedRobot {
    ...
    fn tick(&mut self) {
        ...
        if let Some(mut sync) = self.sync_sub.recv() {
            sync.update_receive_time();
        ...
        if let Some(event) = self.event_sub.recv() {
        ...
        self.clock_stats_pub.send(self.stats);
    }
}

impl SynchronizedRobot {
    fn execute_event(&mut self, idx: usize) {
