...
HORUS provides a pool-backed RAII `PointCloud` type for LiDAR and 3D sensing workloads. Point cloud data lives in shared memory and is transported zero-copy between nodes — only a 168-byte descriptor travels through the ring buffer.
...
Only the 168-byte descriptor goes through the ring buffer. The point data stays in shared memory — true zero-copy IPC.
...
sub = horus.Topic("lidar/points", horus.PointCloud)
pub = horus.Topic("lidar/filtered", horus.PointCloud)
