...
|-----------|-------------|
| `PointCloud::from_xyz(&[[f32; 3]]) -> Result<Self>` | Create from XYZ coordinate arrays **(recommended)** |
| `PointCloud::from_xyzi(&[[f32; 4]]) -> Result<Self>` | Create from XYZI arrays (with intensity) **(recommended)** |
| `PointCloud::from_xyzrgb(&[[f32; 6]]) -> Result<Self>` | Create from XYZRGB arrays **(recommended)** |
| `PointCloud::new(num_points, fields_per_point, dtype)` | Low-level constructor (internal, prefer `from_*` methods) |
