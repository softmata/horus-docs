...
let mut cloud_rgb = PointCloud::from_xyzrgb(&points)?;  // 20_000 points, 6 fields (RGB)
```
...
if let Some(point_bytes) = cloud.point_at(0) {
    // Reinterpret the raw bytes as f32 (XYZ cloud: 12 bytes = 3 x f32)
    let x = f32::from_le_bytes(point_bytes[0..4].try_into().unwrap());
    let y = f32::from_le_bytes(point_bytes[4..8].try_into().unwrap());
    let z = f32::from_le_bytes(point_bytes[8..12].try_into().unwrap());
    println!("Point 0: x={}, y={}, z={}", x, y, z);
}
        ...
        if let Some(raw) = self.raw_sub.recv() {
            if let Some(points) = raw.extract_xyz() {
                ...
                if let Ok(mut filtered) = PointCloud::new(
                    ...
                    let bytes: &[u8] = bytemuck::cast_slice(&non_ground);
                    filtered.copy_from(bytes)
...
|------|------|----------|-------------|
| `points` | `&[[f32; 3]]` | yes | Array of [x, y, z] coordinates in meters. Right-hand, Z-up convention. |

**Returns**

`HorusResult<PointCloud>` — `Err(MemoryError::PoolExhausted)` if tensor pool is full.
...
cloud = horus.PointCloud.from_numpy(tensor.numpy())
```
...
# Convert to PyTorch tensor (via NumPy)
tensor = torch.from_numpy(cloud.to_numpy()) # shape: (N, fields)

# Convert to JAX array (via NumPy)
jax_arr = jnp.asarray(cloud.to_numpy())  # shape: (N, fields)
```
...
- [Perception Messages](/rust/api/perception-messages) - PointField, PointCloud, DepthImage, landmarks, tracking
- [Tensor Messages](/rust/api/tensor-messages) - TensorDtype, Device, auto-managed tensor pools
- [Tensor API](/rust/api/tensor) - Advanced pool management
...
- [Python PointCloud](/python/api/pointcloud) — Python PointCloud API
