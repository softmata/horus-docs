...
A fixed-size binary message for sending arbitrary data between nodes, including across language boundaries (Rust to Python, Python to Rust). Uses MessagePack serialization under the hood.
...
Use `GenericMessage` when standard message types (`Twist`, `Pose2D`, `LaserScan`, etc.) do not cover your data. Common scenarios:

- Sending structured data from a Rust node to a Python node (or vice versa)
- Prototyping a custom message before defining a dedicated type
- Bridging with external systems that produce arbitrary payloads
...
| `to_value::<T>()` | `HorusResult<T>` | Deserialize payload from MessagePack into `T` |
...
topic = horus.Topic("controller.params", horus.GenericMessage)
...
The 4KB limit keeps `GenericMessage` as a fixed-size POD type, safe for shared memory ring buffers. For larger payloads, use `Topic<Tensor>` with [TensorPool](/rust/api/tensor-pool) instead.

## See Also

- [Standard Messages](/rust/api/messages) -- All built-in message types
- [TensorPool API](/rust/api/tensor-pool) -- For payloads larger than 4KB
