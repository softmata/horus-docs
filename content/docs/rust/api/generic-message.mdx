...
- Performance-critical paths — GenericMessage uses MessagePack serialization (~4us overhead)
...
# A bare-string topic carries a generic MessagePack dict.
topic = Topic("external.detections")
data = topic.recv()  # plain dict (already deserialized) or None: {"class": "box", ...}
```
...
# A bare-string topic carries a generic MessagePack dict. On the Python side
# recv() returns a plain dict directly (already deserialized) — there is no
# GenericMessage wrapper class and no .to_dict() call.
topic = horus.Topic("controller.params")
...
The Rust sender's `metadata` label is not surfaced on the Python side — a generic
topic yields only the payload dict. Put any dispatch key (e.g. `"kind": "pid_gains"`)
inside the dict itself if the Python receiver needs to branch on it.
