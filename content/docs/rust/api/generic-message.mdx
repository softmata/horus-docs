...
The 4KB limit keeps `GenericMessage` as a fixed-size type suitable for shared memory transport. For larger payloads, use `Topic<Tensor>` with [TensorPool](/rust/api/tensor) instead.
