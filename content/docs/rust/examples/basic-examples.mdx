...
impl Node for TeleopNode {
    ...
    fn tick(&mut self) {
        ...
        // In real code, subscribe to a Topic<KeyboardInput> (already in horus::prelude)
...
fn main() -> Result<()> {
    ...
    // Obstacle detector ticks right after the lidar, in the critical band (order 1)
...
- Safety nodes belong in the low `order` band (0-9), so they tick early every cycle
...
impl Node for CameraSender {
    ...
    fn tick(&mut self) {
        // Create a 640x480 RGB8 image in shared memory
    ...
    fn tick(&mut self) {
        if let Some(img) = self.topic.recv() {
            ...
            hlog!(debug, "Received {}x{} {:?} image ({} bytes)",
                img.width(), img.height(), img.encoding(), img.nbytes());
        }
    }
...
- `topic.send(&img)` sends the 224-byte descriptor; pixel data stays in shared memory
...
impl Node for PointCloudSender {
    ...
    fn tick(&mut self) {
        // Build 1000 XYZ points — a sphere of radius 1.0
        ...
        // Copies the points into shared memory as 1000 x 3 float32
...
**Key Concepts**:
- `PointCloud::from_xyz(&points)` copies `[f32; 3]` points into shared memory (also `from_xyzi`, `from_xyzrgb`)
- `PointCloud::new(num_points, fields_per_point, dtype)` for other layouts — 3=XYZ, 4=XYZI, 6=XYZRGB
