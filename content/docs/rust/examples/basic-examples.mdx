...
fn main() -> Result<()> {
    let mut scheduler = Scheduler::new();
    scheduler.add(ProcessorNode::new()?).order(0).done();
    scheduler.run()?;
    Ok(())
}
```
...
fn main() -> Result<()> {
    ...
    scheduler.add(TeleopNode::new()?).order(0).done();
    scheduler.add(MotorDriverNode::new()?).order(1).done();
...
fn main() -> Result<()> {
    ...
    scheduler.add(ObstacleDetector::new(0.5)?).order(1).done();
...
impl Node for TestSystem {
    ...
    fn tick(&mut self) {
        ...
        self.feedback_pub.send(self.position).ok();
    }
...
impl Node for SetpointNode {
    ...
    fn tick(&mut self) {
        // Target position
        let setpoint = 10.0;
        self.setpoint_pub.send(setpoint).ok();
    }
...
fn main() -> Result<()> {
    ...
    scheduler.add(TestSystem::new()?).order(1).done();

    // PID controller (Kp=0.5, Ki=0.01, Kd=0.1)
    scheduler.add(PIDController::new(0.5, 0.01, 0.1)?).order(2).done();
...
impl Node for FilterNode {
    ...
    fn tick(&mut self) {
        if let Some(raw) = self.raw_sub.recv() {
            ...
            self.filtered_pub.send(self.filtered_value).ok();
...
fn main() -> Result<()> {
    ...
    scheduler.add(SensorNode::new()?).order(0).done();
    scheduler.add(FilterNode::new(0.2)?).order(1).done();
    scheduler.add(ControllerNode::new(50.0)?).order(2).done();
