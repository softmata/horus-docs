...
impl Node for PointCloudSender {
    ...
    fn tick(&mut self) {
        // NOTE: PointCloud::new() allocates from the shared memory pool — zero-copy transport
        let mut pc = PointCloud::from_xyz(\&points)  // 1000 points, 3 fields
            .expect("failed to allocate point cloud");

        // Fill with sample data — a sphere of radius 1.0
        let floats = pc.data_mut_as::<f32>();
        for i in 0..1000 {
