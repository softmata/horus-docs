...
**Key Concepts**:
- `Image::new(height, width, encoding)` allocates from a global shared memory pool
- `topic.send(&img)` sends the 168-byte descriptor; pixel data stays in shared memory
