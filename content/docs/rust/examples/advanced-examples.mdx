...
impl Node for StateMachineNode {
    ...
    fn tick(&mut self) {
        ...
        self.state = match self.state {
            ...
            RobotState::Moving => {
                if obstacle {
                    self.cmd_pub.send(CmdVel::zero()).ok();  // Stop
                    RobotState::ObstacleDetected
                } else {
                    self.cmd_pub.send(CmdVel::new(1.0, 0.0)).ok();  // Forward
                    RobotState::Moving
                }
            }
            ...
            RobotState::Rotating => {
                self.cmd_pub.send(CmdVel::new(0.0, 0.5)).ok();  // Rotate
...
impl Node for EmergencyStopNode {
    ...
    fn tick(&mut self) {
        ...
        if should_stop != self.estop_active {
            self.estop_pub.send(should_stop).ok();
