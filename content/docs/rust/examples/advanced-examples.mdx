...
- Lower number = higher priority. Bands: 0-9 critical real-time (motor control, safety),
  10-49 high, 50-99 normal, 100-199 low (logging, diagnostics), 200+ background
...
<Callout type="warning" title="Both sides must name the same message type">
A topic header records the size of its message type, and the second process to open
a topic is rejected when that size disagrees. A bare string in the Python
`subs`/`pubs` list opens a `GenericMessage` topic, which no `Topic&lt;f32&gt;` on the Rust
side can pair with — so both languages name a shared message type here (`Temperature`
and `MotorCommand`) rather than a primitive.
</Callout>
...
impl Node for TempSensor {
    ...
    fn tick(&mut self) {
        ...
        self.temp_pub.send(Temperature::new(temp as f64));
...
def controller_tick(node):
    ...
    if reading is not None:
        status = "HOT" if reading.temperature > 22.0 else "NORMAL"
        print(f"Python controller: {reading.temperature:.2f}°C - {status}")

        # Send command back to Rust actuator
        target = 1.0 if reading.temperature > 22.0 else 0.0
        node.send("actuator_cmd", horus.MotorCommand(target=target))

node = horus.Node(
    name="PyController",
    subs=[horus.Sub("temperature", horus.Temperature)],
    pubs=[horus.Pub("actuator_cmd", horus.MotorCommand)],
    tick=controller_tick,
    rate=10
)
...
impl Node for Actuator {
    ...
    fn tick(&mut self) {
        if let Some(cmd) = self.cmd_sub.recv() {
            let action = if cmd.target > 0.5 { "COOLING" } else { "IDLE" };
...
Measured with `all_paths_latency` on an Intel Core i7-10750H — RDTSC timing, 100K samples
per scenario, calibrated overhead subtraction. Reproduce with
`cargo run --release --bin all_paths_latency`; see [Benchmarks](/performance/benchmarks)
for the full percentile distribution and per-topology backend selection.
