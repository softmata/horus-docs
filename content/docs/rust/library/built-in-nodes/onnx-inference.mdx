...
let predictions_hub = Topic::Predictions>::new("ml.predictions")?;

if let Some(preds) = predictions_topic.recv(&mut None) {
...
let input_pub = Topic::Tensor>::new("ml.input.tensor")?;
let output_sub = Topic::Tensor>::new("ml.output.tensor")?;
