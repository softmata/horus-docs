...
```rust
let frame_hub = Topic::CameraFrame>::new("camera.frame")?;
let objects_hub = Topic::DetectedObjects>::new("vision.objects")?;

if let Some(frame) = frame_topic.recv(&mut None) {
    // Process frame...
}

if let Some(objects) = objects_topic.recv(&mut None) {
...
let blobs_hub = Topic::BlobList>::new("vision.blobs")?;
if let Some(blobs) = blobs_topic.recv(&mut None) {
...
let markers_hub = Topic::ArucoMarkers>::new("vision.markers")?;
if let Some(markers) = markers_topic.recv(&mut None) {
