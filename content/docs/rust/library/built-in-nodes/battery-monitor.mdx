...
let topic = Topic::BatteryData>::new("battery.data")?;

if let Some(data) = topic.recv(&mut None) {
...
```rust
// Subscribe to alerts
let alert_hub = Topic::BatteryAlert>::new("battery.alert")?;

if let Some(alert) = alert_topic.recv(&mut None) {
...
while let Some(data) = topic.recv(&mut None) {
...
fn main() -> Result<()> {
    ...
    let monitor_node = node! {
        ...
        tick: |ctx| {
            let data_hub = Topic::BatteryData>::new("battery.data")?;
            let alert_hub = Topic::BatteryAlert>::new("battery.alert")?;

            // Display battery status
            if let Some(data) = data_topic.recv(&mut None) {
                ctx.log_info(&format!(
            ...
            // Handle alerts
            while let Some(alert) = alert_topic.recv(&mut None) {
                match alert.alert_type {
                    AlertType::LOW_VOLTAGE => {
