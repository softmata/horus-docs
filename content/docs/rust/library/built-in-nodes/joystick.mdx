...
```rust
let topic = Topic::JoystickState>::new("joystick.state")?;

if let Some(state) = topic.recv(&mut None) {
    // Read analog sticks
...
let joy_hub = Topic::JoystickState>::new("joystick.state")?;
let cmd_topic = Topic::TwistCommand>::new("cmd_vel")?;

if let Some(joy) = joy_topic.recv(&mut None) {
    cmd_topic.send(TwistCommand {
