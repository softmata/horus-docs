...
```rust
let topic = Topic::KeyEvent>::new("keyboard.event")?;

if let Some(event) = topic.recv(&mut None) {
    if event.pressed {
        match event.key {
...
```rust
let key_hub = Topic::KeyEvent>::new("keyboard.event")?;
let cmd_topic = Topic::TwistCommand>::new("cmd_vel")?;

let mut linear = 0.0;
let mut angular = 0.0;

while let Some(event) = key_topic.recv(&mut None) {
    if event.pressed {
        match event.key {
            Key::W => linear = 0.5,
            Key::S => linear = -0.5,
    ...
    cmd_topic.send(TwistCommand { linear_x: linear, angular_z: angular }, &mut None).ok();
}
```
