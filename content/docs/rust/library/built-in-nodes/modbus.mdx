...
// Send request
let topic = Topic::ModbusRequest>::new("modbus.request")?;
topic.send(request, &mut None).ok();

// Receive response
let resp_hub = Topic::ModbusResponse>::new("modbus.response")?;
if let Some(response) = resp_topic.recv(&mut None) {
    if response.success {
...
for slave_id in 1..=3 {
    ...
    if let Some(resp) = resp_topic.recv(Some(Duration::from_millis(500))) {
...
fn main() -> Result<()> {
    ...
    let scada_node = node! {
        name: "scada_poller",
        tick: |ctx| {
            let req_hub = Topic::ModbusRequest>::new("modbus.request")?;
            let resp_hub = Topic::ModbusResponse>::new("modbus.response")?;
            ...
            req_topic.send(request, &mut None).ok();
