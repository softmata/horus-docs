...
let depth_hub = Topic::DepthImage>::new("depth_camera.depth.image")?;
if let Some(depth) = depth_topic.recv(&mut ctx) {
    println!("Depth image: {}x{}", depth.width, depth.height);
}

// Subscribe to point cloud:
let pc_hub = Topic::PointCloud>::new("depth_camera.pointcloud")?;
if let Some(pc) = pc_topic.recv(&mut ctx) {
...
impl ObstacleDetector {
    fn new(min_distance: f32) -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Self {
            depth_sub: Topic::new("depth_camera.depth.image")?,
            min_distance,
        })
    }
}
