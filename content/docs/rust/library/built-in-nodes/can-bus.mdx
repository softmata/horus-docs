...
// Publish to TX topic
let tx_hub = Topic::CanFrame>::new("can.can0.tx")?;
tx_topic.send(frame, &mut None).ok();
```
...
let rx_hub = Topic::CanFrame>::new("can.can0.rx")?;

if let Some(frame) = rx_topic.recv(&mut None) {
...
let error_hub = Topic::CanError>::new("can.can0.error")?;

if let Some(error) = error_topic.recv(&mut None) {
...
fn main() -> Result<()> {
    ...
    let tx_node = node! {
        ...
        tick: |ctx| {
            ...
            let topic = Topic::CanFrame>::new("can.can0.tx")?;
            topic.send(frame, &mut None).ok();
            Ok(())
        }
    };
    ...
    let rx_node = node! {
        name: "can_rx",
        tick: |ctx| {
            let topic = Topic::CanFrame>::new("can.can0.rx")?;
            while let Some(frame) = topic.recv(&mut None) {
                ctx.log_info(&format!(
