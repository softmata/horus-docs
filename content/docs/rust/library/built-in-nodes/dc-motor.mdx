...
let topic = Topic::DcMotorCommand>::new("dc_motor.command")?;
topic.send(cmd, &mut None).ok();
...
// Drive forward
topic.send(DcMotorCommand { motor_id: 0, speed: 70, ..Default::default() }, None)?;
topic.send(DcMotorCommand { motor_id: 1, speed: 70, ..Default::default() }, None)?;

// Turn right (left motor faster)
topic.send(DcMotorCommand { motor_id: 0, speed: 80, ..Default::default() }, None)?;
topic.send(DcMotorCommand { motor_id: 1, speed: 40, ..Default::default() }, None)?;
```
...
fn main() -> Result<()> {
    ...
    let drive_node = node! {
        ...
        tick: |ctx| {
            ...
            topic.send(DcMotorCommand { motor_id: 0, speed: 60, ..Default::default() }, None)?;
            topic.send(DcMotorCommand { motor_id: 1, speed: 60, ..Default::default() }, None)?;
            std::thread::sleep(Duration::from_secs(2));

            // Turn left
            ctx.log_info("Turn left");
            topic.send(DcMotorCommand { motor_id: 0, speed: 30, ..Default::default() }, None)?;
            topic.send(DcMotorCommand { motor_id: 1, speed: 60, ..Default::default() }, None)?;
            std::thread::sleep(Duration::from_secs(1));

            // Reverse
            ctx.log_info("Reverse");
            topic.send(DcMotorCommand { motor_id: 0, speed: -40, ..Default::default() }, None)?;
            topic.send(DcMotorCommand { motor_id: 1, speed: -40, ..Default::default() }, None)?;
            std::thread::sleep(Duration::from_secs(1));
