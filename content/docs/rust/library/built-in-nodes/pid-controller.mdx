...
let setpoint_hub = Topic::SetpointCommand>::new("pid.setpoint")?;
setpoint_topic.send(SetpointCommand::new(0, 100.0), None);  // Target: 100.0

// Send feedback from sensor:
let feedback_hub = Topic::FeedbackValue>::new("pid.feedback")?;
feedback_topic.send(FeedbackValue::new(0, 95.0), None);  // Current: 95.0
```
...
The node handles:
- Topic subscription/publishing (Topic I/O)
