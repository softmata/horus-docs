...
// Send
let topic = Topic::SerialMessage>::new("serial.tx")?;
topic.send(msg, &mut None).ok();
```
...
let topic = Topic::SerialMessage>::new("serial.rx")?;
...
if let Some(resp) = rx_topic.recv(&mut None) {
...
// Read NMEA sentences
let topic = Topic::SerialMessage>::new("serial.rx")?;
while let Some(msg) = topic.recv(&mut None) {
