...
// Publish request
let topic = Topic::I2cMessage>::new("i2c.request")?;
topic.send(request, &mut None).ok();

// Receive response
let response_hub = Topic::I2cMessage>::new("i2c.response")?;
if let Some(response) = response_topic.recv(&mut None) {
    if response.success {
...
if let Some(resp) = response_topic.recv(&mut None) {
