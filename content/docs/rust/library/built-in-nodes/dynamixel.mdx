...
// Send command
let topic = Topic::DynamixelCommand>::new("dynamixel.command")?;
topic.send(cmd, &mut None).ok();

// Read status
let status_hub = Topic::DynamixelStatus>::new("dynamixel.status")?;
if let Some(status) = status_topic.recv(&mut None) {
...
let topic = Topic::DynamixelSyncWrite>::new("dynamixel.sync_write")?;
topic.send(sync_cmd, &mut None).ok();
```
...
let topic = Topic::DynamixelBulkRead>::new("dynamixel.bulk_read")?;
topic.send(bulk_read, &mut None).ok();

// Receive results
let result_hub = Topic::DynamixelBulkReadResult>::new("dynamixel.bulk_result")?;
if let Some(result) = result_topic.recv(&mut None) {
...
fn main() -> Result<()> {
    ...
    let arm_node = node! {
        ...
        tick: |ctx| {
            ...
            let status_hub = Topic::DynamixelStatus>::new("dynamixel.status")?;
            while let Some(status) = status_topic.recv(&mut None) {
                // Check for errors
