...
let topic = Topic::EstopState>::new("estop.state")?;
...
let cmd_topic = Topic::EstopCommand>::new("estop.command")?;
cmd_topic.send(EstopCommand::Trigger, &mut None).ok();

// Reset e-stop
cmd_topic.send(EstopCommand::Reset, &mut None).ok();
```
