...
let topic = Topic::RoboclawCommand>::new("roboclaw.command")?;
...
let topic = Topic::RoboclawMixedCommand>::new("roboclaw.mixed")?;
topic.send(cmd, &mut None).ok();
```
...
fn main() -> Result<()> {
    ...
    let drive_node = node! {
        ...
        tick: |ctx| {
            ...
            topic.send(RoboclawMixedCommand {
                forward_speed: 5000,
                turn_rate: 0,
            }, None)?;

            // Monitor status
            let status_hub = Topic::RoboclawStatus>::new("roboclaw.status")?;
