...
let depth_hub = Topic::DepthImage>::new("vision.depth")?;

if let Some(depth) = depth_topic.recv(&mut None) {
...
let depth_sub = Topic::DepthImage>::new("vision.depth")?;
let pointcloud_pub = Topic::PointCloud2>::new("vision.pointcloud")?;
