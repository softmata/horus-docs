...
// Send command
let topic = Topic::BldcCommand>::new("bldc.command")?;
...
topic.send(arm_cmd, &mut None).ok();
...
let telem_hub = Topic::BldcTelemetry>::new("bldc.telemetry")?;

if let Some(telem) = telem_topic.recv(&mut None) {
...
topic.send(calibrate_cmd, &mut None).ok();
...
fn main() -> Result<()> {
    ...
    let motor_node = node! {
        ...
        tick: |ctx| {
            ...
            // Monitor telemetry
            let telem_hub = Topic::BldcTelemetry>::new("bldc.telemetry")?;
            while let Some(telem) = telem_topic.recv(&mut None) {
