...
```rust
// Read input
let input_hub = Topic::GpioInput>::new("gpio.input")?;
if let Some(input) = input_topic.recv(&mut None) {
...
// Set output
let output_hub = Topic::GpioOutput>::new("gpio.output")?;
output_topic.send(GpioOutput {
    pin: 27,
    value: true,  // LED on
}, None)?;
```
