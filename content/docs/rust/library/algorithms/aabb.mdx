...
```rust
use horus::prelude::*; // Provides AABB
...
flowchart TB
    subgraph AABB["AABB (Axis-Aligned Bounding Box)"]
        direction TB
        SHAPE["Rotated Shape<br/>(actual geometry)"]
        WASTE["Wasted space<br/>= false positives"]
    end
    NOTE["AABB is conservative:<br/>may report collisions<br/>where none exist"]
`}
  caption="AABB conservatively bounds rotated shapes, leading to wasted space"
/>
