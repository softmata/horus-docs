...
flowchart TB
    subgraph STATIC["STATIC MODE"]
        S1["Compiled<br/>Drivers"]
        S2["Zero<br/>Overhead"]
        S1 --> S2
    end
    subgraph DYNAMIC["DYNAMIC MODE"]
        D1[".so/.dylib<br/>Plugins"]
        D2["dlopen()<br/>Load"]
        D1 --> D2
    end
    S2 --> LOADER
    D2 --> LOADER
    LOADER["DriverLoader<br/>Unified API"]
    NOTE["HYBRID MODE: Static + Dynamic combined"]
`}
  caption="Driver loading modes: static, dynamic, and hybrid"
/>
