...
flowchart TB
    subgraph LOADER["DriverLoader"]
        STATIC["Static<br/>Plugins"]
        DYNAMIC["Dynamic<br/>Plugins"]
        HYBRID["Hybrid<br/>Mode"]
    end
    subgraph TRAIT["DriverPlugin Trait"]
        M1["manifest() → PluginManifest"]
        M2["probe() → Vec&lt;ProbeResult&gt;"]
        M3["create() → Box&lt;dyn Any + Send&gt;"]
        M4["health() → PluginHealth"]
    end
    STATIC --> TRAIT
    DYNAMIC --> TRAIT
    HYBRID --> TRAIT
