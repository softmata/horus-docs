...
The simulator, `sim3d`, is **not publicly available**: `softmata/horus-sim3d` is a
private repository, so `horus install horus-sim3d` reports "not found in registry" and the
GitHub link 404s for anyone outside the org. What you lose without it is not uniform:

- The control examples — `differential_drive`, `differential_drive_cpp`, `quadruped`,
  `robot_arm`, `multi_robot` — publish commands and run fully standalone. You see the
  topics and the control loop; there is just nothing rendering the robot.
- `sensor_navigation` and `sensor_navigation_py` subscribe to `lidar.scan` and `imu.data`,
  and the simulator is what publishes them. Those two start and tick but **receive
  nothing**.

If you do have access, start [sim3d](/plugins) in another terminal first to see the robot
move:
